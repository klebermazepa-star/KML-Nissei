...
*******************************************************************************/
{include/i-prgvrs.i ESINT0001-G01 9.99.99.999}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESINT0001-G01 MUT}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button c_descricao bt-ok bt-cancela 
&Scoped-Define DISPLAYED-OBJECTS c_descricao 
...
DEFINE BUTTON bt-ajuda  NO-FOCUS
     LABEL "&Ajuda" 
     SIZE 10 BY 1
     BGCOLOR 8 .
...
DEFINE VARIABLE c_descricao AS CHARACTER FORMAT "x(20)":U 
     LABEL "Fornecedor" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     bt-ajuda AT ROW 4.5 COL 34 NO-TAB-STOP 
     c_descricao AT ROW 2.5 COL 17 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     rt-button AT ROW 4.25 COL 1
     SPACE(0.71) SKIP(0.07)
...
/* SETTINGS FOR BUTTON bt-ajuda IN FRAME d-vapara
   NO-ENABLE                                                            */
ASSIGN 
...
DO:
FIND esp_integracao NO-LOCK
    WHERE esp_integracao.descricao EQ INPUT FRAME {&FRAME-NAME} c_descricao NO-ERROR.

IF NOT AVAILABLE esp_integracao THEN DO:
    {utp/ut-table.i Custom esp_integracao 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(esp_integracao).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_descricao 
      WITH FRAME d-vapara.
  ENABLE rt-button c_descricao bt-ok bt-cancela 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESINT0001-G01" "9.99.99.999"}
