...
DEF INPUT PARAMETER p-retonro-integracao AS MEMPTR NO-UNDO.
...
ASSIGN tabelas = "ide,emit,dest,prod,icms,ipi,pis,cofins,total,transporta,vol,dup,infProt,infAdic,transp,ICMSUFDest,infEventoo,sefaz,nota"  /* Tabelas a serem importadas */
 ...
 IF v-retonro-integracao <> ? THEN do:
    ...
    hXML1:LOAD('MEMPTR':U, v-retonro-integracao , FALSE).
    ...
    IF AVAIL tt-nota-aux THEN do:
       ...
       IF AVAIL nota-fiscal THEN do:
           ...
           ASSIGN c-arq-xml = SESSION:TEMP-DIR + nota-fiscal.cod-estabel + "_" + nota-fiscal.serie + "_" + nota-fiscal.nr-nota-fis + "_" + REPLACE(STRING(TODAY,"99/99/9999"),"/","") + REPLACE(STRING(TIME,"HH:MM:SS"),":","") + ".xml".

            // KML - Correá∆o para fazer cancelamento da forma correta
        IF v-idi-sit-nf-eletro = 6 THEN DO:
        ...
        RELEASE b-nota-fiscal.
          ...
          IF int(tt-nota-aux.situacao) = 8 OR int(tt-nota-aux.situacao) = 3 THEN DO:  // Rejeitada ou criticada
             ...
             ASSIGN lc-aux = BASE64-ENCODE(p-retonro-integracao).
    ...
    v-retonro-integracao  = p-retonro-integracao.
    hXML:LOAD('MEMPTR':U, v-retonro-integracao , FALSE).
    IF c-arq-xml <> "" THEN
    hXML:SAVE('file':U, c-arq-xml).
  ...
  IF vh:NAME = '#cdata-section' THEN DO:
     ...
     IF INDEX(l-xml, '<![CDATA') > 1 THEN do:
        ...
        ASSIGN l-xml = c-xml-aux
               c-arqaux = "c:\temp\" + trim(string(c1-xml-aux)) //SESSION:TEMP-DIRECTOR + trim(string(c1-xml-aux))
