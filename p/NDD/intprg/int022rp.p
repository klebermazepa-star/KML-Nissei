...
DEFINE BUFFER b-int_ndd_retorno FOR int_ndd_retorno.
DEFINE BUFFER b2-int_ndd_retorno FOR int_ndd_retorno.

define temp-table tt-retorno-emissao NO-UNDO
...
define temp-table tt-retorno-cancelamento NO-UNDO
    field idMovto   as int64
    field Id        as char 
    field cStat     as char 
    field xMotivo   as char 
...
define temp-table tt-retorno-inutilizacao NO-UNDO
    field idMovto   as int64
    field Id        as char  
    field tpAmb     as char  
    field verAplic  as char  
...
define temp-table tt-retorno-impressao NO-UNDO
    field idMovto    as int64
    field chave      as char
...
define temp-table tt-retorno-substituicao NO-UNDO
    field idMovto    as int64
...
procedure pi-elimina-tabelas:
   ...
   EMPTY TEMP-TABLE tt-item-doc-est-fat.
   EMPTY TEMP-TABLE tt-retorno-erro.
   EMPTY TEMP-TABLE tt-retorno-substituicao.
...
procedure pi-importa:
    ...
    DEFINE VARIABLE cont-notas AS INTEGER     NO-UNDO.

    for each int_ndd_retorno no-lock where 
        int_ndd_retorno.STATUSNUMBER = 0 /* A PROCESSAR */ and
        ...
        IF cont-notas > 10 THEN NEXT.

        ASSIGN cont-notas = cont-notas + 1.
                ...
                if  nota-fiscal.idi-sit-nf-eletro <= 2   and
                    ...
                    if avail b-nota-fiscal then do:
                        ...
                        for last ret-nf-eletro exclusive-lock where 
                            ...
                            ret-nf-eletro.log-ativo   = yes: end.
...
procedure pi-importa-ecommerce:
    run pi-acompanhar in h-acomp (input "Lendo Retornos").
    ...
    for each int_ndd_retorno no-lock where 
        ...
        int_ndd_retorno.kind <> 9 and
        int_ndd_retorno.kind <> 8 and
        int_ndd_retorno.kind <> 7 AND 
        int_ndd_retorno.serie = "402"   //   Fixado sÇrie do ecommerce para processar com prioridade em relaá∆o ao cupom fiscal loja
        by int_ndd_retorno.id:
...
PROCEDURE pi-marca-processado:
    ...
    for first b2-int_ndd_retorno NO-LOCK
        where b2-int_ndd_retorno.ID = pid:
    end.
    IF AVAIL b2-int_ndd_retorno THEN DO:

        FIND FIRST b-int_ndd_retorno EXCLUSIVE-LOCK 
            WHERE rowid(b-int_ndd_retorno) = ROWID(b2-int_ndd_retorno) NO-WAIT NO-ERROR.

        IF AVAIL b-int_ndd_retorno THEN
            assign b-int_ndd_retorno.STATUSNUMBER = 1.

        release b-int_ndd_retorno.
    END.
