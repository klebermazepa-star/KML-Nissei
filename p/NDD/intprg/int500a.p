...
FOR EACH estabelec NO-LOCK:
    ...
    IF AVAIL es-param-int-nfe-entrada THEN DO:

        IF es-param-int-nfe-entrada.pagina >= 3 THEN
            ASSIGN es-param-int-nfe-entrada.pagina = es-param-int-nfe-entrada.pagina - 3.
    ...
    DO WHILE i-qtd-notas = 100:
        ...
        if OPSYS = "UNIX" then
            COPY-LOB lcResult TO FILE ("/mnt/totvs12/scripts-8080/nota" + string(i-pagina) + ".xml").
