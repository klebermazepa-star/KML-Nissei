...
if opsys = "Unix" then do:
    ...
    /*cCaminhoTMP = session:temp-directory.*/
    cCaminhoTMP = int_ndd_param.caminho.
    if trim(substring(cCaminhoTMP,length(cCaminhoTMP),1)) <> "~\" then
        cCaminhoTMP = cCaminhoTMP + "~\".
    cCaminhoTMP = cCaminhoTMP + "tmp\".
end.

ASSIGN i-job-ndd = 1. /* Produá∆o */
FIND FIRST estabelec WHERE
           estabelec.cod-estabel = "973" NO-LOCK NO-ERROR.
IF AVAIL estabelec THEN DO:
    &IF "{&bf_dis_versao_ems}":U >= "2.07":U &THEN
        ...
        IF estabelec.idi-tip-emis-nf-eletro = 3 THEN
           ASSIGN i-job-ndd = 1. /* Produá∆o */
    &else
        IF INT(SUBSTR(estabelec.char-1,168,1)) = 2 THEN
           ASSIGN i-job-ndd = 2. /* Homologaá∆o */
        IF INT(SUBSTR(estabelec.char-1,168,1)) = 3 THEN
           ASSIGN i-job-ndd = 1. /* Produá∆o */
    &endif.
END.
...
PROCEDURE ConsultarStatusEvento:
  DEFINE INPUT PARAMETER xml AS LONGCHAR NO-UNDO.
  DEFINE OUTPUT PARAMETER ConsultarStatusResult AS LONGCHAR NO-UNDO.
END PROCEDURE.
...
procedure pi-geraHeaderConsultarStatusEventoNDD:
    define input parameter cChave as char no-undo.
    define output parameter cHeader as longchar no-undo.
        ...
        addStack(iId).
            ...
            addStack(iId).
                ...
                addStack(iId).
                    RUN addNode IN hGenXml (getStack(), "chaveacesso", cChave, OUTPUT iId).
                delStack().
                RUN addNode IN hGenXml (getStack(), "statusatual", "1", OUTPUT iId).
                RUN addNode IN hGenXml (getStack(), "codigoevento", "110110", OUTPUT iId).
            delStack().
        delStack().
    delStack().
...
procedure pi-GeraHeaderConsultarColdDFe:
    ...
    DEFINE VARIABLE de-nsu AS DECIMAL   NO-UNDO.

    FOR LAST NDD_ENTRYINTEGRATION EXCLUSIVE-LOCK 
        WHERE NDD_ENTRYINTEGRATION.CNPJDEST = INT64(CCNPJ)
          AND NDD_ENTRYINTEGRATION.NSU <> ?
          AND NDD_ENTRYINTEGRATION.NSU <> 0
        BY NDD_ENTRYINTEGRATION.ENTRYINTEGRATIONID :

        ASSIGN de-nsu = NDD_ENTRYINTEGRATION.NSU.
     //   ASSIGN NDD_ENTRYINTEGRATION.NSU = ?.
    ...
    addStack(iId).
        ...
        RUN addNode IN hGenXml (getStack(), "nsu", STRING(de-nsu), OUTPUT iId).
    delStack().
...
procedure pi-GeraHeaderConsultarColdSincrono:
    ...
    addStack(iId).
        ...
        addStack(iId).
            ...
            delStack().
        delStack().
        RUN addNode IN hGenXml (getStack(), "tiposervico", i-tiposervico-consult-cold, OUTPUT iId).
...
PROCEDURE pi-altera-tiposervico-consult-cold:
    DEFINE INPUT  PARAMETER p-tiposervico-consult-cold AS INTEGER     NO-UNDO.

    ASSIGN i-tiposervico-consult-cold = p-tiposervico-consult-cold.
...
procedure pi-conectaWebServer:
    ...
    /*if i-job-ndd = 2 /* HOMOLOGACAO */ and cColdID <> '418' /* HOMOLOGAÄ«O */ then do:
        run utp/ut-msgs.p(input "show",
                          input 17006,
                          input ("Conex∆o inv†lida!" + "~~" + "Para ambiente de HOMOLOGAÄ«O deve ser alterada conex∆o no programa int560 p/ ID 418")).
        return "ADM-ERROR".
    end.*/
    ...
    hWebService:connect("-WSDL " + cCaminhoNDD + "wsdl/UNICO/"
...
procedure pi-consultarProtocolo:
    ...
    if l-log then do:
        cFile = cCaminhoTMP + c-seg-usuario + "_" + "formsConsultarProtocoloHeader" + "_" + string(today,"99-99-9999") + "_" + replace(string(time,"HH:MM:SS"),':','_') + ".xml".
    ...
    if l-connected then do:
        ...
        if l-log then do:
            cFile = cCaminhoTMP + c-seg-usuario + "_" + "eformsConsultarProtocoloRetorno" + "_" + string(today,"99-99-9999") + "_" + replace(string(time,"HH:MM:SS"),':','_') + ".xml".
...
procedure pi-consultarStatus:
    ...
    define input parameter pkind like int_ndd_envio.kind.

    run pi-desConectaWebServer.

    if pkind <> 6 /* Carta Correcao - Consultar Evento */ then
        ...
        run pi-geraHeaderConsultarStatusEventoNDD (string( /* chNFe */
                                            &if "{&bf_dis_versao_ems}" < "2.07" &then       
                                               trim(substring(nota-fiscal.char-2,3,44))    
                                            &elseif "{&bf_dis_versao_ems}" >= "2.07" &then  
                                               trim(nota-fiscal.cod-chave-aces-nf-eletro)
    ...
    if l-log then do:
        cFile = cCaminhoTMP + c-seg-usuario + "_" + "eformsConsultarStatusHeader" + "_" + string(today,"99-99-9999") + "_" + replace(string(time,"HH:MM:SS"),':','_') + ".xml".
        copy-lob cHeader to FILE cFile.
    end.

    /* envio p/ WebService */
    run pi-conectaWebServer (input if pkind <> 6 then 'WSConsultarStatus.wsdl' else 'WSConsultarStatusEvento.wsdl',
                             input if pkind <> 6 then 'WSConsultarStatus' else 'WSConsultarStatusEvento',
    ...
    if l-connected then do:
        if pkind <> 6 then do:
            ...
            run WSConsultarStatusEventoSoap set hWSPortaSoap on hWebService.
            run ConsultarStatus in hWSPortaSoap(input  cHeader, 
                                                output cReturnValue).
        end.

        if l-log then do:
            cFile = cCaminhoTMP + c-seg-usuario + "_" + "eformsConsultarStatusRetorno" + "_" + string(today,"99-99-9999") + "_" + replace(string(time,"HH:MM:SS"),':','_') + ".xml".
            copy-lob cReturnValue to FILE cFile.
        end.
        ...
        cProtocolo = "".
        cData = "".
        cChaveAcesso = "". /* sequencial do evento , se houver */
        /* Trata Retorno do WebService */
        run reset in hGenXML.
        run loadXMLFromLongChar in hGenXML (cReturnValue).
        do iId = 1 to 9:
        ...
        if cCodigoErro = "000" /*and cProtocolo <> ? and cProtocolo <> ""*/ then do: 
            ...
            do iId = 10 to 20:
                ...
                if cChaveAcesso = "" or cChaveAcesso = ? then do:
                    run loadValue in hGenXML ("sequencial", iId, output cChaveAcesso) .
                    cChaveAcesso = PrintChar(cChaveAcesso).
                end.
            end.
            ...
            if not avail int_ndd_retorno then do:
                ...
                assign  int_ndd_retorno.id          = iId
                        ...
                        int_ndd_retorno.dt_envio    = today
                        int_ndd_retorno.hr_envio    = time.
            end.
...
procedure pi-processa-retorno-Cold:
    ...
    cCodigoErro = string(OnlyNumbers(string(findTag(cReturnValue,'codigo',1)))).
    ...
    if cCodigoErro <> "000" then do:
        ...
        iUltNSU     = dec(OnlyNumbers(string(findTag(cReturnValue,'ultNSU',1)))) NO-ERROR.
        ...
        do while i-aux > 0 
            ...
            if cChildren <> "" then do:
                ...
                IF i-tiposervico-consult-cold = 4 /*xml*/ THEN DO:

                    ASSIGN cFileDocument = cCaminhoTMP + nota-fiscal.cod-chave-aces-nf-eletro + "_"
                                                       + string(today,"99-99-9999") + "_" 
                                                       + replace(string(time,"HH:MM:SS"),':','_') + ".xml".
                    COPY-LOB FROM memptrXML to FILE cFileDocument.
                END.
                ELSE IF i-tiposervico-consult-cold = 0 /*danfe*/ THEN DO:
                    ASSIGN cFileDocument = cCaminhoTMP + nota-fiscal.cod-chave-aces-nf-eletro + "_"
                                                       + string(today,"99-99-9999") + "_" 
                                                       + replace(string(time,"HH:MM:SS"),':','_') + ".pdf".
                    COPY-LOB FROM memptrXML to FILE cFileDocument.
                END.
                ELSE
                    COPY-LOB FROM memptrXML to cChildren-aux.
                ...
                */
                ...
                run pi-abre-xml-string(input cChildren-aux, input pGrava, output l-okCold).

                if not l-okCold then l-aux = no.
            end.
   ...
   // if not l-log and l-aux and pGrava then os-delete value(cFile) no-error. /* - ser∆o movidos ap¢s re-processamento no int500a */
...
procedure pi-ConsultarColdSincrono:
    ...
    if l-log then do:
        cFile = cCaminhoTMP + "eformsConsultarColdSincronoHeader_" + trim(cChaveNFe) + ".xml".
        copy-lob cHeader to FILE cFile.
    end.
...
PROCEDURE pi-retorna-document:
    DEFINE OUTPUT PARAMETER pFile AS CHARACTER   NO-UNDO.

    ASSIGN pFile = cFileDocument.
...
procedure pi-abre-xml-string: 
    ...
    if  not pGrava or (
        ...
        if pGrava then do: 
            ...
            find first NDD_ENTRYINTEGRATION EXCLUSIVE-LOCK where 
          ...
          /*  /*if l-log then*/ put stream str-log unformatted 
            ...
            find first tt-NDD_ENTRYINTEGRATION no-lock where 
                ...
                tt-NDD_ENTRYINTEGRATION.CNPJDEST       = int64(cDest) no-error.
...
procedure pi-inserirDocumento:

    DISP "kleber - sistema = " opsys.
    if opsys <> "Unix" then do:
        copy-lob int_ndd_envio.documentdata to cXML.
        if l-log then do: 
        ...
        /* Gera Header do envio para NDD */
        run pi-geraHeaderInserirDocumentoNDD ("1" /* Emissao */, output cHeader).
        if l-log then do: 
        ...
        if l-connected then do:
            ...
            if cProtocolo = ? or cProtocolo = "0" or cProtocolo = "" then do:
                cMensagem = "".
                cCodigoErro = "".
                ...
                assign  int_ndd_envio.protocolo    = cProtocolo
                        ...
                        int_ndd_envio.statusnumber = 0. /* liberado para consultar protocolo */
