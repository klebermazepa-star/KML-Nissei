if  cTranEvent = "add" or cTranAction = "add" then do:
    ...
    ASSIGN tt_integr_apb_lote_fatura.tta_cod_estab                = BranchCode
        ...
        OUTPUT TO VALUE (SESSION:TEMP-DIRECTORY + 'ttAccountPayableDocumentI_00.csv').
        ...
        for each ttAccountPayableDocumentI_00:
            ...
            PUT UNFORMATTED 'ttAccountPayableDocumentI_00' + ' ; ' +
                            'RECID(tt_integr_apb_lote_fatura)'                               + ' ; ' +
                            string(ttAccountPayableDocumentI_00.Sequence)                 + ' ; ' +
                            STRING(ttAccountPayableDocumentI_00.SupplierCode)             + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DocumentSerieCode)         + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DocumentNumber)            + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DocumentParcelNumber)      + ' ; ' +
                            string(ttAccountPayableDocumentI_00.PrintDate)                         + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DueDate)                           + ' ; ' +
                            string(ttAccountPayableDocumentI_00.PaymentForecastDate)               + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DiscountDate)                      + ' ; ' +
                            string(ttAccountPayableDocumentI_00.CurrencyCode)              + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DocumentValue)                     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.CollectorCode)                    + ' ; ' +
                            'RECID(tt_integr_apb_item_lote_fatura)'                          + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DiscountValue)                     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.PortfolioCode)                     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DiscountPercentage)                + ' ; ' +
                            STRING(ttAccountPayableDocumentI_00.DaysLate)                 + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DaysLateInterest)                  + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DayLateInterestPercentage)         + ' ; ' +
                            string(ttAccountPayableDocumentI_00.Penalty)                           + ' ; ' +
                            string(ttAccountPayableDocumentI_00.InsurancePolicyNumber)     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.InsurerCode)               + ' ; ' +
                            string(ttAccountPayableDocumentI_00.LessorCode)                + ' ; ' +
                            string(ttAccountPayableDocumentI_00.LeasingContractNumber)     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.StandartHistoryDescription)+ ' ; ' +
                            string(ttAccountPayableDocumentI_00.ParcelQuantity)                    + ' ; ' +
                            STRING(ttAccountPayableDocumentI_00.Days)                     + ' ; ' +
                            string(ttAccountPayableDocumentI_00.DueForecastType)           + ' ; ' +
                            string(ttAccountPayableDocumentI_00.PaymentMethodCode)         + ' ; ' +
                            string(ttAccountPayableDocumentI_00.QuotationValue)                    + ' ; ' +
                            STRING(ttAccountPayableDocumentI_00.InvestmentOrder)          + ' ; ' +
                            string(ttAccountPayableDocumentI_00.TenNineNineValue)                  + ' ; ' +
                            string(ttAccountPayableDocumentI_00.TaxIdentNumber)                    + ' ; ' +
                            "" + '~n' .                                                            
            ...
            for each ttPendentTaxInformation
                ...
                PUT UNFORMATTED 'ttPendentTaxInformation' + ' ; ' +
                                'recid(tt_integr_apb_item_lote_fatura)'        + ' ; ' +
                                '?'                                            + ' ; ' +
                                CountryCode                                  + ' ; ' +
                                string(ttPendentTaxInformation.StateCode)            + ' ; ' +
                                string(ttPendentTaxInformation.TaxCode)              + ' ; ' +
                                string(ttPendentTaxInformation.TaxClassification)    + ' ; ' +
                                string(ttPendentTaxInformation.DocumentClassCode)    + ' ; ' +
                                string(ttPendentTaxInformation.DocumentSerieCode)    + ' ; ' +
                                string(ttPendentTaxInformation.DocumentNumber)       + ' ; ' +
                                string(ttPendentTaxInformation.DocumentParcelNumber) + ' ; ' +
                                string(ttPendentTaxInformation.TaxClassification)    + ' ; ' +
                                string(ttPendentTaxInformation.AccountPlanCode)      + ' ; ' +
                                string(ttPendentTaxInformation.AccountCode)          + ' ; ' +
                                string(ttPendentTaxInformation.RevenueTax)           + ' ; ' +
                                string(ttPendentTaxInformation.INSSDeduction)        + ' ; ' +
                                string(ttPendentTaxInformation.DependentDeduction )  + ' ; ' +
                                string(ttPendentTaxInformation.PensionDeduction)     + ' ; ' +
                                string(ttPendentTaxInformation.OtherDeductions)      + ' ; ' +
                                string(ttPendentTaxInformation.TaxBaseValue)         + ' ; ' +
                                string(ttPendentTaxInformation.TaxPercentage)        + ' ; ' +
                                string(ttPendentTaxInformation.PaidTax)              + ' ; ' +
                                string(ttPendentTaxInformation.TaxValue)             + ' ; ' +
                                string(ttPendentTaxInformation.DueDate)              + ' ; ' +
                                string(ttPendentTaxInformation.CurrencyCode)         + ' ; ' +
                                string(ttPendentTaxInformation.EconomicPurposeValue) + ' ; ' +
                                ""                                           + ' ; ' +
                                STRING(ttPendentTaxInformation.SupplierCode) + ' ; ' +
                                string(ttPendentTaxInformation.DeductionValue)       + ' ; ' +
                                '0'                                            + ' ; ' +
                                '0'                                            + ' ; ' +
                                '0'                                            + ' ; ' +
                                ""                                           + ' ; ' +
    ...
    run pi_main_code_apb925za in v_hdl_apb925za (input 1,
