if  cTranAction = "add" then do:
    ...
    do trans:
        ...
        assign v_num_seq = 0.
        ASSIGN v_cartao_seq = 1.
    
        OUTPUT TO C:\Temp\tit_acr_cartao.csv.
        ...
        for each ttCardAccountReceivableIn_00 no-lock:
            ...
            if ttCardAccountReceivableIn_00.CardSequence = 0 then do:
                assign v_erro_codigo = 0351
                       v_erro_descri = "O CARDSEQUENCE n∆o foi informado."
                       v_ajuda       = "O campo CARDSEQUENCE do XML Ç de preenchimento obrigat¢rio, verifique.".
            end.
            ...
            find first admdra_cart_esp 
                where admdra_cart_esp.cod_admdra  = ttCardAccountReceivableIn_00.ManagerNumber
                  and admdra_cart_esp.cod_estab   = ttCardAccountReceivableIn_00.BranchCode
                  and admdra_cart_esp.cod_empresa = ttCardAccountReceivableIn_00.CompanyCode
                no-lock no-error.
            ...
            if not avail admdra_cart_esp then do:
                ...
                assign tt_log_erro.ttv_num_cod_erro = 0309
                       tt_log_erro.ttv_des_msg_erro = "A Operadora utilizada n∆o est† cadastrada".
                ...
                assign tt_erro_implanta.tta_cod_erro   = 0309
                       tt_erro_implanta.tta_informacao = ttCardAccountReceivableIn_00.BranchCode        + "," +
                                                         ...
                                                         ""                                             + "," +
                                                         ttCardAccountReceivableIn_00.DocumentParcelNumber
                       tt_erro_implanta.tta_des_erro   = "A Operadora utilizada n∆o est† cadastrada."
            ...
            if not avail tit_acr then do:
                ...
                FIND first tit_acr_cartao where tit_acr_cartao.cod_empresa           = ttCardAccountReceivableIn_00.CompanyCode
                                            and tit_acr_cartao.cod_estab             = tit_acr.cod_estab
                                           ...
                                           /* AND tit_acr_cartao.num_seq               = ttCardAccountReceivableIn_00.CardSequence */ NO-LOCK NO-ERROR.

                PUT UNFORMATTED ttCardAccountReceivableIn_00.CompanyCode           + " ; " +
                                tit_acr.cod_estab                                  + " ; " +
                                ttCardAccountReceivableIn_00.ProofSell             + " ; " +
                                ttCardAccountReceivableIn_00.CardAuthorize         + " ; " +
                                string(ttCardAccountReceivableIn_00.DateSell)      + " ; " +
                                ttCardAccountReceivableIn_00.DocumentParcelNumber  + " ; " +
                                STRING(ttCardAccountReceivableIn_00.ParcelTotal)   + " ; " +
                                admdra_cart_esp.cod_admdra + "~n".      
                ...
                IF AVAIL tit_acr_cartao THEN DO:
                    ...
                    assign tt_log_erro.ttv_num_cod_erro = 0313
                           tt_log_erro.ttv_des_msg_erro = "O T°tulo de Cart∆o j† est† cadastrado na tabela filho (tit_acr_cartao)." +
                                                          ...
                                                          " CARD SEQ: " + string(ttCardAccountReceivableIn_00.CardSequence).
                    ...
                    assign tt_erro_implanta.tta_cod_erro   = 0313
                           tt_erro_implanta.tta_informacao = tit_acr.cod_estab                                  + "," +
                                                             ...
                                                             ""                                                 + "," +
                                                             ttCardAccountReceivableIn_00.DocumentParcelNumber  + "," + 
                                                             string(ttCardAccountReceivableIn_00.CardSequence)
                ...
                /*****************************************************************************************/
                find tt_titulos_cartao where
                     tt_titulos_cartao.cod_estab      = tit_acr.cod_estab      and
        ...
        OUTPUT TO VALUE (SESSION:TEMP-DIRECTORY + "tit_cartao.csv") CONVERT TARGET "iso8859-1" .
        ...
        for each tt_titulos_cartao where
            ...
            PUT UNFORMATTED tt_titulos_cartao.cod_estab + " ; " +
                            string(v_cod_refer) + " ; " +
                            string(v_dat_transacao) + " ; " +
                            STRING((tt_titulos_cartao.val_sdo_tit_acr + tt_titulos_cartao.val_ava_maior)) + " ~n "
                .
        ...
        for each tt_alter_tit_acr_base_5:
            delete tt_alter_tit_acr_base_5.
        end.

        OUTPUT TO VALUE (SESSION:TEMP-DIRECTORY + "tit_cartao2.csv") CONVERT TARGET "iso8859-1" .

        for each tt_titulos_cartao no-lock:
            ...
            */
            ...
            /* INICIO: prepara rateio do t°tulo */
            for each val_tit_acr of tit_acr
                 where val_tit_acr.cod_finalid_econ = v_cod_finalid_econ no-lock:

                for first tt_rateio_un_ff
                    where tt_rateio_un_ff.cod_unid_negoc       = val_tit_acr.cod_unid_negoc
                      and tt_rateio_un_ff.cod_tip_fluxo_financ = val_tit_acr.cod_tip_fluxo_financ
                      and tt_rateio_un_ff.cod_estab            = tit_acr.cod_estab
                      and tt_rateio_un_ff.num_id_tit_acr       = tit_acr.num_id_tit_acr: 
                end.
                if not avail tt_rateio_un_ff then do:
                    create tt_rateio_un_ff.
                    assign tt_rateio_un_ff.cod_unid_negoc       = val_tit_acr.cod_unid_negoc
                           tt_rateio_un_ff.cod_tip_fluxo_financ = val_tit_acr.cod_tip_fluxo_financ
                           tt_rateio_un_ff.cod_estab            = tit_acr.cod_estab
                           tt_rateio_un_ff.num_id_tit_acr       = tit_acr.num_id_tit_acr.
                end.
                assign tt_rateio_un_ff.val_perc_rat_val = tt_rateio_un_ff.val_perc_rat_val + val_tit_acr.val_perc_rat.
            end.
            /* FIM: prepara rateio do t°tulo */
    
            /* IN÷CIO: Efetua o rateio da alteraá∆o de valor */
            for each tt_rateio_un_ff
               where tt_rateio_un_ff.cod_estab      = tit_acr.cod_estab
                 and tt_rateio_un_ff.num_id_tit_acr = tit_acr.num_id_tit_acr:
                
                    create tt_alter_tit_acr_rateio.
                    assign v_num_seq_aprop_ctbl_pend_acr_2                         = v_num_seq_aprop_ctbl_pend_acr_2 + 1
                           tt_alter_tit_acr_rateio.tta_cod_estab                   = tt_titulos_cartao.cod_estab
                           tt_alter_tit_acr_rateio.tta_num_id_tit_acr              = tt_titulos_cartao.num_id_tit_acr
                           ...
                           tt_alter_tit_acr_rateio.tta_cod_unid_negoc              = tt_rateio_un_ff.cod_unid_negoc
                           tt_alter_tit_acr_rateio.tta_cod_plano_ccusto            = ""
                           tt_alter_tit_acr_rateio.tta_cod_ccusto                  = ""
                           tt_alter_tit_acr_rateio.tta_cod_tip_fluxo_financ        = tt_rateio_un_ff.cod_tip_fluxo_financ
                           tt_alter_tit_acr_rateio.tta_num_seq_aprop_ctbl_pend_acr = v_num_seq_aprop_ctbl_pend_acr_2
                           tt_alter_tit_acr_rateio.tta_val_aprop_ctbl              = abs((tt_titulos_cartao.val_sdo_tit_acr - tt_titulos_cartao.tot_val_adm_cartao) - tt_titulos_cartao.val_sdo_tit_acr) * tt_rateio_un_ff.val_perc_rat_val / 100
                    ...
                    if tt_alter_tit_acr_rateio.tta_val_aprop_ctbl > v_val_aprop_rat then
                    assign v_val_aprop_rat   = tt_alter_tit_acr_rateio.tta_val_aprop_ctbl
                           v_recid_tt_rateio = recid(tt_alter_tit_acr_rateio).

                    assign v_val_tot_aprop_rat = v_val_tot_aprop_rat + tt_alter_tit_acr_rateio.tta_val_aprop_ctbl.    
            end.
    
            find first tt_alter_tit_acr_rateio where 
                 recid(tt_alter_tit_acr_rateio) = v_recid_tt_rateio exclusive-lock no-error.
            if avail tt_alter_tit_acr_rateio then
