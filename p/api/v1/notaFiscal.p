...
PROCEDURE pi-integra-cd:
    ...
    oJson:WriteFile("c:\temp\EnvioIntegraLoja.json") no-error.
...
PROCEDURE pi-integra-loja:
  ...
  //  ASSIGN c-mensagem = c-mensagem + "1.1 ".
    ...
    ASSIGN oURI        = OpenEdge.Net.URI:Parse("https://api.cosmospro.com.br")
           //oURI:Path   = '/api/ExecuteCustomAction/ExecuteAction?ActionName=NotasFiscaisCompra&api-version=1.0&token=' + c-token.
           oURI:Path   = '/api/ExecuteCustomAction/ExecuteAction?ActionName=NotasFiscaisCompraHML&api-version=1.0&token=' + c-token.
  ...
  log-manager:write-message("KML - dentro conexao -  " + cEmit + " - " + cSerie + " - " + cNF ) NO-ERROR.
    ...
    FOR FIRST INT_DS_NOTA_ENTRADA NO-LOCK
        ...
        FOR EACH INT_DS_NOTA_ENTRADA_PRODUT NO-LOCK
            ...
            log-manager:write-message("KML - dentro conexao - criou campo Data validade Loja  " + STRING(tt-INT_DS_NOTA_ENTRADA_PRODUT.NEP_DATAVALIDADE_D)) NO-ERROR.
  ...
  //  ASSIGN c-mensagem = c-mensagem + "2 ".
    ...
    log-manager:write-message("JSON gerado: " + STRING(lcJson)) NO-ERROR.
    
    
    COPY-LOB FROM lcJson TO FILE "/mnt/shares/TEMP/EnvioIntegraLoja.json".
    //oJson:WRITEfile("T:\EnvioIntegraLoja.json") no-error. 
    
    //log-manager:write-message("KML - antes enviar procfit - json  " + string(oJson) ) NO-ERROR.
    log-manager:write-message("Erro WriteFile: " + ERROR-STATUS:GET-MESSAGE(1)) NO-ERROR.
