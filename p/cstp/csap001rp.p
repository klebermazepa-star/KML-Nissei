...
define temp-table ttTitulo NO-UNDO
    ...
    FIELD cb4_tit_ap_bco_cobdor     LIKE tit_ap.cb4_tit_ap_bco_cobdor 
    ...
    FIELD val_pagto                 LIKE tit_ap.val_pagto             
    ...
    do {&throws}:
        for each bord_ap fields(cod_estab_bord cod_portador num_bord_ap)
            ...
            where item_bord_ap.cod_estab_bord  = bord_ap.cod_estab_bord
              ...
              //AND item_bord_ap.ind_sit_item_bord_ap <>  "Estornado" KML Lohan, Altera‡Æo de
            ...
            where tit_ap.cod_estab       = item_bord_ap.cod_estab
              ...
              and tit_ap.val_sdo_tit_ap   > 0 // = item_bord_ap.val_pagto.
    ...
    do {&throws}:
        ...
        /*
                assign 
                   ...
                   */
                ASSIGN v_imp_parametro    = emsfnd.ped_exec_param.log_dwb_print_parameters
        ...
        FOR EACH tt-param:
            ...
            assign 
                empresaIni         = tt-param.empresaIni        
                empresaFim         = tt-param.empresaFim        
                estabelecimentoIni = tt-param.estabelecimentoIni
                estabelecimentoFim = tt-param.estabelecimentoFim
                dataVencimentoIni  = tt-param.dataVencimentoIni 
                dataVencimentoFim  = tt-param.dataVencimentoFim 
                fornecedorIni      = tt-param.fornecedorIni     
                fornecedorFim      = tt-param.fornecedorFim     
                matrizIni          = tt-param.matrizIni         
                matrizFim          = tt-param.matrizFim         
                borderoIni         = tt-param.borderoIni        
                borderoFim         = tt-param.borderoFim        
                portadorIni        = tt-param.portadorIni       
                portadorFim        = tt-param.portadorFim       
                especieIni         = tt-param.especieIni        
                especieFim         = tt-param.especieFim        
                titulosVinculados  = tt-param.titulosVinculados 
                titulosBordero     = tt-param.titulosBordero    
                notasRelacionadas  = tt-param.notasRelacionadas 
                v_imp_parametro    = tt-param.v_imp_parametro.


            find emsfnd.ped_exec_param 
                where emsfnd.ped_exec_param.num_ped_exec = v_num_ped_exec_corren 
                no-lock no-error.
            IF AVAIL emsfnd.ped_exec_param  THEN DO:

                ASSIGN arquivo            = emsfnd.ped_exec_param.cod_dwb_file
                       v_cod_dwb_output   = emsfnd.ped_exec_param.cod_dwb_output
                       c-impressora       = emsfnd.ped_exec_param.nom_dwb_printer
                       c-layout           = emsfnd.ped_exec_param.cod_dwb_print_layout
                       v_cod_dwb_user     = emsfnd.ped_exec_param.cod_usuar.
        ...
        do:  /* seta a saida da impressao */
            ...
            case v_cod_dwb_output:
                ...
                when 'Arquivo' /*l_file*/  then do:
                    ...
                    if opsys = 'UNIX' then do:
                        output stream s_1 to VALUE ("/mnt/shares/p/cjem8f/rpw/prod/rpw-fila1/" + c-impressora)
                           paged page-size value(v_rpt_s_1_lines) convert target 'iso8859-1'.
                    END.
                    ELSE DO:                                                //"rpw-8080\svc_datasul\" 
                        output stream s_1 to VALUE ("/mnt/shares/p/cjem8f/rpw/prod/rpw-fila1/" + c-impressora) //("/mnt/shares/RPW/prod/rpw-fila1/" +  c-impressora)
                           paged page-size value(v_rpt_s_1_lines) convert target 'iso8859-1'.
                    END.
                end.
            end.
    ...
    do {&throws}:
        find first item_bord_ap use-index itmbrdp_tit_ap
            where item_bord_ap.cod_estab        = tit_ap.cod_estab      
              ...
              AND item_bord_ap.ind_sit_item_bord_ap <> "Baixado"
              AND item_bord_ap.ind_sit_item_bord_ap <> "Estornado"
            no-lock no-error.
...
------------------------------------------------------------------------------*/
    ...
    DEF BUFFER bf-fornecedor FOR emsuni.fornecedor .
    ...
    do {&throws}:
        ...
        if avail fornecedor then do:
            ...
            if avail pessoa_jurid then do:
                find first bf-fornecedor
                    where bf-fornecedor.num_pessoa = pessoa_jurid.num_pessoa_jurid_matriz
                    no-lock no-error.
                IF NOT AVAIL bf-fornecedor THEN DO:
                    RUN utp/ut-msgs.p ("show" , "17242" , "A Matriz do Fornecedor nÆo esta cadastrada como Fornecedor" + "~~" + 
                                       "Fornecedor: " + STRING(tit_ap.cdn_fornecedor)+ "~n" + 
                                       "Pessoa (Fornecedor): " + STRING(fornecedor.num_pessoa)+ "~n" + 
                                       "Pessoa (Matriz): " + STRING(pessoa_jurid.num_pessoa_jurid_matriz) )
                        .
                    ASSIGN doNext = true.
                END.
                ELSE if bf-fornecedor.cdn_fornecedor < matrizIni or
                    bf-fornecedor.cdn_fornecedor > matrizFim then
                    assign 
                        doNext = true.
            end.
        end.
