...
DEFINE VARIABLE cb-prefeituras AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 0 
     ...
     LIST-ITEM-PAIRS "''",0
     DROP-DOWN-LIST
     SIZE 46 BY .88 TOOLTIP "Selecionar Prefeitura" NO-UNDO.
...
DEFINE FRAME fpage0
     ...
     cb-prefeituras AT ROW 1.25 COL 40 COLON-ALIGNED WIDGET-ID 12
     bt-buscar AT ROW 1.25 COL 92 WIDGET-ID 6
...
DO WITH FRAME fPage0:
    ...
    FOR EACH cst_prefeitura NO-LOCK:
        cb-prefeituras:ADD-LAST(cst_prefeitura.nome + " - " + STRING(cst_prefeitura.cod_prefeitura), cst_prefeitura.cod_prefeitura).
    END.
...
FOR FIRST cst_prefeitura NO-LOCK
    WHERE cst_prefeitura.cod_prefeitura = INPUT FRAME fPage0 cb-prefeituras
    , 
    EACH estabelec NO-LOCK
    WHERE estabelec.cidade = cst_prefeitura.cidade 
    ,
...
IF NOT AVAIL cst_prefeitura THEN
    FIND FIRST cst_prefeitura NO-LOCK
        WHERE cst_prefeitura.cod_prefeitura = INPUT FRAME fPage0 cb-prefeituras NO-ERROR.

//RUN VALUE("cstp/csap008a" + STRING(INPUT FRAME fPage0 cb-prefeituras) + ".p")
RUN VALUE(cst_prefeitura.layout)
