...
PROCEDURE pi-cancelamento:
    ...
    FOR FIRST tt-nota-fiscal-rest,
        ...
        FOR FIRST nota-fiscal NO-LOCK
            ...
            AND   nota-fiscal.serie       = tt-nota-fiscal-rest.serie
            AND   nota-fiscal.nr-nota-fis = tt-nota-fiscal-rest.nr-nota: END.

        IF NOT AVAIL nota-fiscal THEN DO:
            jsonOutput:ADD("ERRO","Nota Fiscal n∆o encontrada!").
            RETURN "OK".
        END.

        IF nota-fiscal.dt-cancela <> ? THEN DO:
            jsonOutput:ADD("ERRO","Nota Fiscal j† cancelada ou em processo de cancelamento!").
            RETURN "OK".
        END.
        ...
        FOR EACH RowErrors:
            ...
            ASSIGN tt-erros.cod-erro  = RowErrors.ErrorNumber
                   ...
                   tt-erros.l-erro    = IF RowErrors.ErrorSubType = "ERROR" THEN YES ELSE NO.
        
            IF  RowErrors.ErrorNumber = 17006
            AND (   RowErrors.ErrorDescription BEGINS "NF-e em processo de Cancelamento"
                 OR RowErrors.ErrorDescription BEGINS "NF-e em processo de Inutilizaªío") THEN
    ...
    IF NOT AVAIL tt-nota-fiscal-rest THEN
        jsonOutput:ADD("ERRO","Erro na execuá∆o da rotina!").

    IF CAN-FIND(FIRST tt-erros
                WHERE tt-erros.l-erro) THEN DO:
        ASSIGN jsonArrayErros = NEW JsonArray().
        jsonArrayErros:READ(TEMP-TABLE tt-erros:HANDLE).
        jsonOutput:ADD("ERROS",jsonArrayErros).
    END.
...
PROCEDURE pi-busca-danfe-xml:
    ...
    IF NOT l-ok THEN
        jsonOutput:ADD("ERRO", "N∆o encontrado Danfe/XML para essa nota.").
...
PROCEDURE pi-recebimento:  //KML
    ...
    FOR EACH Recebimento:
        ...
        DO TRANS ON ERROR UNDO, LEAVE:
           ...
           // ASSIGN c-result = "retorno int500a" .                                                        
            ...
            IF AVAIL estabelec AND
                (estabelec.cod-estabel = "003" OR
                 estabelec.cod-estabel = "005" OR
                 estabelec.cod-estabel = "017" OR
                 estabelec.cod-estabel = "014" OR
                 estabelec.cod-estabel = "010" OR
                 estabelec.cod-estabel = "023" OR
                 estabelec.cod-estabel = "003" OR
                 estabelec.cod-estabel = "005" OR
                 estabelec.cod-estabel = "026" OR
                 estabelec.cod-estabel = "044" OR
                 estabelec.cod-estabel = "051" OR
                 estabelec.cod-estabel = "072" OR
                 estabelec.cod-estabel = "084" OR
                 estabelec.cod-estabel = "091" OR
                 estabelec.cod-estabel = "149" OR
                 estabelec.cod-estabel = "154" OR
                 estabelec.cod-estabel = "229" OR
                 estabelec.cod-estabel = "238" OR
                 estabelec.cod-estabel = "285" OR
                 estabelec.cod-estabel = "292" OR
                 estabelec.cod-estabel = "296" OR
                 estabelec.cod-estabel = "331" OR
                 estabelec.cod-estabel = "357" )
                 THEN DO:

                // KML - colocar filtro por estabelecimento
                RUN pi-procfit (INPUT lcxml, OUTPUT l-ok-procfit).

                IF c-mensagem = "" THEN
                DO:
                    ASSIGN c-result = "c-mensagem em branco" .
                    
                END.
                ELSE
                    ASSIGN c-result = c-mensagem.
            END.
            ELSE DO:

                ASSIGN l-ok-procfit = YES. // como n∆o sera integrado via api marca como ok
                ASSIGN c-result = c-result.  // apenas para reforáar que ele manter† a mensagem de retorno do int500 e n∆o da procfit
...
PROCEDURE pi-procfit:
    ...
    IF CAN-FIND(FIRST int_ds_nota_entrada 
        ...
        RUN pi-integra-loja (OUTPUT p-ok).
        IF p-ok = NO THEN RETURN "OK".
    END.
...
PROCEDURE pi-integra-cd:
    ...
    IF c-token = "" THEN c-token = "84d19f58-716a-4686-9531-8b167c2f1e1e".
...
PROCEDURE pi-integra-loja:
  ...
  //  ASSIGN c-mensagem = c-mensagem + "2 ".
   ...
   // ASSIGN c-mensagem = c-mensagem + "3 ".
    ...
    LOG-MANAGER:WRITE-MESSAGE("retorno - " + c-retorno) NO-ERROR.
