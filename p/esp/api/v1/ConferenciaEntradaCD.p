    ...
    DATASET dsconferencia:HANDLE:READ-JSON('JsonObject',jsonAux).
     ...
     FOR FIRST notas: 
        ...
        FIND FIRST ems2mult.emitente NO-LOCK
            WHERE emitente.cgc = notas.cnpj_empresa NO-ERROR.
            
        IF LENGTH(TRIM(notas.NOTAFISCAL)) < 7 THEN ASSIGN notas.NOTAFISCAL = TRIM(STRING(INTEGER(notas.NOTAFISCAL),">>9999999")).     
     
     
        FIND LAST int_ds_docto_wms EXCLUSIVE-LOCK
             WHERE int_ds_docto_wms.doc_numero_n = notas.NOTAFISCAL
             AND   int_ds_docto_wms.doc_serie_s  = notas.serie
             AND   int_ds_docto_wms.doc_origem_n = emitente.cod-emitente
             //AND   int_Ds_docto_wms.situacao     = 10 
             NO-ERROR.
        ...
        IF NOT AVAIL int_ds_docto_wms  THEN DO:
            ...
            ASSIGN int_ds_docto_wms.doc_numero_n        = notas.NOTAFISCAL                          
                   ...
                   int_ds_docto_wms.situacao            = INT(notas.situacao) /* Inclus∆o */                                              
        ...
        IF c-retorno = "OK" THEN DO:
            ...
            jsonOutput:ADD("nen_cnpj_origem_s", notas.cnpj_empresa).
