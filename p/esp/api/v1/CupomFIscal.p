...
PROCEDURE pi-gera-cupom:
    ...
    log-manager:write-message("KML - entrou procedure cupom - versao 14/10/2024 - 20:02") no-error.
...
/*                          OUTPUT TABLE tt-nota-devolucao). */
    ...
    IF CAN-FIND(FIRST cupom_fiscal) THEN DO:
        ...
        log-manager:write-message("KML - antes FOR EACH cupom_fiscal") no-error.
    
        FOR EACH cupom_fiscal:
        
            log-manager:write-message("KML - antes valida‡Æo data_emissao - " + string(cupom_fiscal.data_emissao)) no-error.
        
            IF cupom_fiscal.data_emissao > TODAY  THEN
            DO:
                log-manager:write-message("KML - Entrou valida‡Æo data_emissao - " + string(cupom_fiscal.data_emissao)) no-error.
                
                CREATE RowErrors.
                ASSIGN RowErrors.ErrorNumber = 17001
                       RowErrors.ErrorDescription = "Data do Cupom diferente da data atual " + string(cupom_fiscal.data_emissao)
                       RowErrors.ErrorHelp = "Data do Cupom diferente da data atual"
            ...
            FIND FIRST estabelec NO-LOCK
                 WHERE estabelec.cgc =  cupom_Fiscal.emitente_cnpj NO-ERROR.
             ...
             DO:
                ...
                FIND FIRST nota-fiscal EXCLUSIVE-LOCK
                    WHERE nota-fiscal.cod-estabel   = estabelec.cod-estabel  
                      AND nota-fiscal.serie         = cupom_Fiscal.serie 
                      AND nota-fiscal.nr-nota-fis   = string(cupom_Fiscal.numero, "9999999")  NO-ERROR.
                ...
                DO:
                    ...
                    CREATE RowErrors.
                    ASSIGN RowErrors.ErrorNumber = 17001
                           RowErrors.ErrorDescription = "Cupom J  Existe na Base"
                           RowErrors.ErrorHelp = "Cupom J  Existe na Base"
                           RowErrors.ErrorSubType = "ERROR".
        ...
        ASSIGN tt-param.destino         = 1
               ...
               tt-param.r-int_ds_nota_loja = r-int-ds-nota-loja.   
         ...
         ELSE DO:
            ...
            END. 
             ...
             FIND FIRST int_ds_nota_loja EXCLUSIVE-LOCK 
                  WHERE ROWID(int_ds_nota_loja) = r-int-ds-nota-loja NO-ERROR.
                  
             IF AVAIL int_ds_nota_loja  THEN
             DO:
             
                FOR EACH int_ds_nota_loja_item OF int_ds_nota_loja EXCLUSIVE-LOCK:
                    
                    DELETE int_ds_nota_loja_item.
                    
                END.
                
                RELEASE int_ds_nota_loja_item.

                FOR EACH int_ds_nota_loja_cartao OF int_ds_nota_loja EXCLUSIVE-LOCK:
                
                    DELETE int_ds_nota_loja_cartao.
                    
                END.
                
                RELEASE int_ds_nota_loja_cartao.
                
                DELETE int_ds_nota_loja.
...
PROCEDURE gera-int-ds-nota-loja:
    ...
    FOR EACH cupom_fiscal:
        ...
        ASSIGN int_ds_nota_loja.tipo_movto              = 1                                     
               ...
               log-manager:write-message("KML - criou int_ds_nota_loja rowid - " + STRING(ROWID(int_ds_nota_loja)) ) no-error.
               ASSIGN r-int-ds-nota-loja = ROWID(int_ds_nota_loja).
