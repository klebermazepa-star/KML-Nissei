...
*******************************************************************************/
{include/i-prgvrs.i esapi501-G01 TOTVS}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_id-codigo bt-ok bt-cancela 
&Scoped-Define DISPLAYED-OBJECTS c_id-codigo 
...
DEFINE VARIABLE c_id-codigo AS CHARACTER FORMAT "x(4)":U 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
     SIZE 8.14 BY 1.25 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_id-codigo AT ROW 1.88 COL 28.86 COLON-ALIGNED WIDGET-ID 2
...
DO:
   FIND es-api-empresa NO-LOCK
        WHERE es-api-empresa.id-codigo EQ INPUT FRAME {&FRAME-NAME} c_id-codigo 
        NO-ERROR.
   
   IF NOT AVAILABLE es-api-empresa 
   THEN DO:
      {utp/ut-table.i db es-api-empresa 1}
      RUN utp/ut-msgs.p(INPUT "show",
                        INPUT 2,
                        INPUT RETURN-VALUE).
      RETURN NO-APPLY.
   END.
   ASSIGN p-row-tabela = ROWID(es-api-empresa).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_id-codigo 
      WITH FRAME d-vapara.
  ENABLE rt-button c_id-codigo bt-ok bt-cancela 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esapi501-G01" "TOTVS"}
