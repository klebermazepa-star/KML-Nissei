...
*******************************************************************************/
{include/i-prgvrs.i esapi502-G01 TOTVS}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_id-aplicacao bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_id-aplicacao 
...
DEFINE VARIABLE c_id-aplicacao AS CHARACTER FORMAT "x(3)":U 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.25 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_id-aplicacao AT ROW 1.92 COL 29.86 COLON-ALIGNED WIDGET-ID 2
...
DO:
   FIND FIRST es-api-aplicacao NO-LOCK
        WHERE es-api-aplicacao.id-aplicacao = INPUT FRAME {&FRAME-NAME} c_id-aplicacao 
        NO-ERROR.
   IF NOT AVAILABLE es-api-aplicacao 
   THEN DO:
      {utp/ut-table.i db es-api-aplicacao 1}
   ...
   END.
   ASSIGN 
      p-row-tabela = ROWID(es-api-aplicacao).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_id-aplicacao 
      WITH FRAME d-vapara.
  ENABLE rt-button c_id-aplicacao bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esapi502-G01" "TOTVS"}
