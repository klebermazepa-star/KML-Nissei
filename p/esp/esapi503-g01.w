...
*******************************************************************************/
{include/i-prgvrs.i esapi503-G01 TOTVS}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_id-uri bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_id-uri 
...
DEFINE VARIABLE c_id-uri AS CHARACTER FORMAT "x(16)":U 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
     SIZE 16 BY 1.25 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_id-uri AT ROW 1.79 COL 26 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND es-api-URI NO-LOCK
    WHERE es-api-URI.id-URI EQ INPUT FRAME {&FRAME-NAME} c_id-uri NO-ERROR.

IF NOT AVAILABLE es-api-URI THEN DO:
    {utp/ut-table.i db es-api-URI 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-api-URI).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_id-uri 
      WITH FRAME d-vapara.
  ENABLE rt-button c_id-uri bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esapi503-G01" "TOTVS"}
