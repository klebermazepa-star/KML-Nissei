...
*******************************************************************************/
{include/i-prgvrs.i ESAPI540 9.99.99.999}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-portador i_modalidade bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-portador i_modalidade 
...
DEFINE VARIABLE i_modalidade AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Modalidade" 
...
DEFINE VARIABLE i_cod-portador AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Portador" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-portador AT ROW 1.46 COL 26.14 COLON-ALIGNED WIDGET-ID 2
     i_modalidade AT ROW 2.58 COL 26.14 COLON-ALIGNED WIDGET-ID 6
...
DO:
   FIND FIRST es-bv-param NO-LOCK
        WHERE es-bv-param.cod-portador = INPUT FRAME {&FRAME-NAME} i_cod-portador
          AND es-bv-param.modalidade   = INPUT FRAME {&FRAME-NAME} i_modalidade
        NO-ERROR.

   IF NOT AVAIL es-bv-param 
   THEN DO:
      RUN utp/ut-msgs.p(INPUT "show",
                        INPUT 17006,
                        INPUT "Registro n∆o encontrado").
      RETURN NO-APPLY.
   END.
   ASSIGN p-row-tabela = ROWID(es-bv-param).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-portador i_modalidade 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-portador i_modalidade bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESAPI540" "9.99.99.999"}
