...
*******************************************************************************/
{include/i-prgvrs.i ESLF001 1.0.0.001}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESLF001 LFP}
&ENDIF
...
DEFINE VARIABLE v_log_tabela AS LOGICAL INITIAL no 
     LABEL "Gerar Gr†fico Excel" 
     VIEW-AS TOGGLE-BOX
     SIZE 22 BY .83 NO-UNDO.

DEFINE VARIABLE EDITOR-1 AS CHARACTER INITIAL "Informe PER°ODO a ser validado, onde se necess†rio cria um registro no primeiro dia do màs, na aba da CAT42 no LF0203 para evitar Saldo Negativo, utilizando o Saldo Final Anterior que existe ou n∆o no LF0203 para o c†lculo.        Informar Màs/Ano - MM/AAA" 
...
DEFINE VARIABLE cb-limpa-dados AS LOGICAL INITIAL yes 
     LABEL "Limpar Registros do LF0203 antes de Executar" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-sel
     EDITOR-1 AT ROW 1.5 COL 12 NO-LABEL WIDGET-ID 52
     c-periodo-mes AT ROW 6.21 COL 35.43 COLON-ALIGNED WIDGET-ID 2
     c-periodo-ano AT ROW 6.21 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 56
     c-cod-estabel AT ROW 7.25 COL 35.43 COLON-ALIGNED WIDGET-ID 4
     cb-limpa-dados AT ROW 9 COL 21 WIDGET-ID 60
     "/" VIEW-AS TEXT
          SIZE 1.14 BY .67 AT ROW 6.25 COL 41.86 WIDGET-ID 54
     RECT-23 AT ROW 1.25 COL 11 WIDGET-ID 58
    WITH 1 DOWN NO-BOX NO-HIDE KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.14 ROW 2.83
         SIZE 76.86 BY 10.62.
...
/* SETTINGS FOR FRAME f-pg-sel
   NOT-VISIBLE                                                          */
/* SETTINGS FOR EDITOR EDITOR-1 IN FRAME f-pg-sel
   NO-ENABLE                                                            */
ASSIGN 
       EDITOR-1:AUTO-INDENT IN FRAME f-pg-sel      = TRUE
       EDITOR-1:READ-ONLY IN FRAME f-pg-sel        = TRUE.

/* SETTINGS FOR RECTANGLE RECT-23 IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
{utp/ut9000.i "ESLF001" "1.0.0.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN c-arquivo = replace(c-arquivo,".LST":U, ".xls":U)
           c-arq-old = replace(c-arq-old,".LST":U, ".xls":U)
           c-arq-old-batch = replace(c-arq-old-batch,".LST":U, ".xls":U).
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY EDITOR-1 c-periodo-mes c-periodo-ano c-cod-estabel cb-limpa-dados 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE c-periodo-mes c-periodo-ano c-cod-estabel cb-limpa-dados 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
   ...
   /* for each tt-digita no-lock:
    ...
    /* Coloque aqui as validaá‰es das outras p†ginas, lembrando que elas devem 
      ...
      IF INPUT FRAME f-pg-sel c-periodo-mes < 1 OR
         INPUT FRAME f-pg-sel c-periodo-mes > 12 THEN DO:
        MESSAGE "Màs n∆o v†lido. Favor alterar"
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
            ASSIGN c-periodo-mes:SCREEN-VALUE = "00".

            return error.
      END.    
    
      IF INPUT FRAME f-pg-sel c-periodo-ano < 2018 THEN DO:
        MESSAGE "Ano n∆o pode ser menor que 2018. Favor alterar."
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
            ASSIGN c-periodo-ano:SCREEN-VALUE = "0000".
      ...
      IF NOT AVAIL estabelec THEN DO:
        MESSAGE "Estabelecimento n∆o existe. Favor alterar."
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
            return error.
      END.
    ...
    assign tt-param.usuario             = c-seg-usuario
           ...
           tt-param.c-periodo-mes       = INPUT FRAME f-pg-sel c-periodo-mes
           tt-param.c-periodo-ano       = INPUT FRAME f-pg-sel c-periodo-ano
           tt-param.c-limpa-dados       = INPUT FRAME f-pg-sel cb-limpa-dados.
    ...
    {include/i-rprun.i esp/eslf001rp.p}
