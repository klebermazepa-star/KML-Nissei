...
*******************************************************************************/
{include/i-prgvrs.i esof001 12.1.32.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i esof001 MOF}
&ENDIF
...
DEF VAR caminho-1           AS CHAR    NO-UNDO.
DEF VAR caminho-2           AS CHAR    NO-UNDO.
{include/i-imdef.i}
...
DEFINE VARIABLE c-arquivo-entrada AS CHARACTER 
     VIEW-AS EDITOR MAX-CHARS 256
     SIZE 40 BY .88 TOOLTIP "Arquivo com os movimentos a serem importados."
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE c-arquivo-itens AS CHARACTER 
     VIEW-AS EDITOR MAX-CHARS 256
     SIZE 40 BY .88 TOOLTIP "Arquivo com a base de itens."
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE text-entrada-2 AS CHARACTER FORMAT "X(256)":U INITIAL "Arquivo de itens" 
      VIEW-AS TEXT 
...
DEFINE FRAME f-pg-par
     bt-arquivo-entrada-2 AT ROW 6.5 COL 43 HELP
          "Selecione o arquivo de itens." WIDGET-ID 10
     c-arquivo-itens AT ROW 6.54 COL 3.29 HELP
          "Arquivo com a base de itens." NO-LABEL WIDGET-ID 4
     c-arquivo-entrada AT ROW 9.88 COL 3.29 HELP
          "Arquivo com os movimentos a serem importados." NO-LABEL
     bt-arquivo-entrada AT ROW 9.88 COL 43.14 HELP
          "Escolha do nome do arquivo"
     text-entrada-2 AT ROW 5.5 COL 4.14 NO-LABEL WIDGET-ID 8
     text-entrada AT ROW 8.83 COL 4.14 NO-LABEL
     RECT-8 AT ROW 9.17 COL 2
     RECT-10 AT ROW 5.83 COL 2 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
DO:
  //  {include/i-imarq.i c-arquivo-entrada f-pg-par "'*.XLS(x)' '*.xls*' }

    DEFINE VARIABLE procname-2 AS CHARACTER NO-UNDO.
    DEFINE VARIABLE OKpressed-2 AS LOGICAL INITIAL TRUE.

    SYSTEM-DIALOG GET-FILE procname-2
    TITLE      "Selecionar Arquivo..."
    //FILTERS    "Selecionando Arquivos (*.XLS(x))"   "*.XLS*"
    FILTERS    "Selecionando Arquivos (*.CSV)"   "*.CSV"
    MUST-EXIST
    USE-FILENAME
    INITIAL-DIR caminho-2
    UPDATE OKpressed-2.

    IF OKpressed-2 = TRUE THEN c-arquivo-entrada = procname-2.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-arquivo-entrada-2 C-Win
ON CHOOSE OF bt-arquivo-entrada-2 IN FRAME f-pg-par
DO:
  //  {include/i-imarq.i c-arquivo-entrada-2 f-pg-par "'*.XLS(x)' '*.xls*' }

    DEFINE VARIABLE procname-1 AS CHARACTER NO-UNDO.
    DEFINE VARIABLE OKpressed-1 AS LOGICAL INITIAL TRUE.

    SYSTEM-DIALOG GET-FILE procname-1
    TITLE      "Selecionar Arquivo..."
    //FILTERS    "Selecionando Arquivos (*.XLS(x))"   "*.XLS*"
    FILTERS    "Selecionando Arquivos (*.CSV)"   "*.CSV"
    MUST-EXIST
    USE-FILENAME
    INITIAL-DIR caminho-1
    UPDATE OKpressed-1.

    IF OKpressed-1 = TRUE THEN c-arquivo-itens = procname-1.
    DISP c-arquivo-itens WITH FRAME f-pg-par.
END.
...
{utp/ut9000.i "ESOF001" "12.1.32.000"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    {include/i-imvrf.i &programa=ESOF001 &versao-layout=001}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-arquivo-itens c-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
  ENABLE RECT-8 RECT-10 bt-arquivo-entrada-2 c-arquivo-itens c-arquivo-entrada 
         bt-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
do  on error undo, return error
    ...
    assign file-info:file-name = input frame f-pg-par c-arquivo-itens.
    if file-info:pathname = ? and
       input frame f-pg-log rs-execucao = 1 then do:
        run utp/ut-msgs.p (input "show":U,
                           input 326,
                           input c-arquivo-itens).                               
        apply 'mouse-select-click':U to im-pg-par in frame f-import.
        apply 'entry':U to c-arquivo-itens in frame f-pg-par.                
        return error.
    end. 
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.arq-itens       = input frame f-pg-par c-arquivo-itens
    ...
    {include/i-imrun.i esp/esof001rp.p}
