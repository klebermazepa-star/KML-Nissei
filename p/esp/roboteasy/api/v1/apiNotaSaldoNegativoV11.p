...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 11
.
    ...
    DO TRANS ON ERROR UNDO, RETRY:
        ...
        ELSE
            ASSIGN tt-retorno.status-cod = "NOK"
                   ...
                   tt-retorno.status-obs = tt-retorno.status-obs + STRING(tt-pedido.num-pedido) + " (" + trim(string(i-exec)) + ")".
