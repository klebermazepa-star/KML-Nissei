...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-exec                   AS inte                     SERIALIZE-NAME "statusExec" 
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 12
.
...
def var i-cont-exec          as inte no-undo.
def var i-exec          as inte no-undo initial 5.
...
PROCEDURE pi-main:
    ...
    ASSIGN tt-retorno.status-cod  = "NOK" 
           tt-retorno.status-exec = i-cont-exec.
...
bloco-exec:
do  i-cont-exec = 1 to i-exec:
    RUN pi-main.
    for first tt-retorno    
        where tt-retorno.status-cod = "OK":
        
        for each b-tt-retorno
    ...
    DO TRANS ON ERROR UNDO, RETRY:
        ...
        ELSE
            ASSIGN tt-retorno.status-cod = "NOK"
                   ...
                   tt-retorno.status-obs = tt-retorno.status-obs + STRING(tt-pedido.num-pedido) + " (" + trim(string(i-cont-exec)) + ")".

        //--------------------------------------------------------------------------------

        FOR FIRST tt-retorno
            WHERE tt-retorno.status-cod = "NOK"
            and   tt-retorno.status-exec = i-cont-exec:
