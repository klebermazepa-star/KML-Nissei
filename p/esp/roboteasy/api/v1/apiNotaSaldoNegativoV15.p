...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 15
.
...
PROCEDURE pi-gerar-nota:
    ...
    DO TRANS ON ERROR UNDO, RETRY:
        ...
        ELSE DO:
            ...
            // validar nota pela tabela NOTA-FISCAL quando o arq estiver em branco
            RUN pi-validar-nota. // altera o status p/ OK qdo encontra a nota
            
            IF  RETURN-VALUE <> "OK" THEN DO:
            
                IF  L-TRACE THEN ASSIGN TT-RETORNO.STATUS-TRACE = TT-RETORNO.STATUS-TRACE + "|150".

                // se a pi-validar-nota n∆o encontrar a nota ent∆o confirma a execuá∆o com erro
                ASSIGN tt-retorno.status-cod = "NOK"
                       tt-retorno.status-obs = if  l-erro
                                               then "Verificar relt¢rio da execuá∆o porque ocorreu um erro na geraá∆o da nota para o pedido: "
...
PROCEDURE pi-validar-nota:

    def var i-qtd-itens-pedido  as inte no-undo.
    def var i-qtd-itens-nota    as inte no-undo.
    ...
    and avail emitente then do:
        ...
        for first nota-fiscal USE-INDEX ch-distancia no-lock // ler notas da data corrente
        ...
        if  avail nota-fiscal then do:
            ...
            assign i-qtd-itens-pedido = 0
                   i-qtd-itens-nota   = 0.
   
            for each int_ds_pedido_produto of int_ds_pedido no-lock:
                assign i-qtd-itens-pedido = i-qtd-itens-pedido + 1.
                for first it-nota-fisc fields (it-codigo) no-lock of nota-fiscal
                    where it-nota-fisc.it-codigo = trim(string(int_ds_pedido_produto.ppr_produto_n)):
                    assign i-qtd-itens-nota = i-qtd-itens-nota + 1.
                end.
            end.
            
            if  i-qtd-itens-pedido = i-qtd-itens-nota then do:
            
                IF  L-TRACE THEN ASSIGN TT-RETORNO.STATUS-TRACE = TT-RETORNO.STATUS-TRACE + "|210".

                ASSIGN tt-pedido.num-nota     = nota-fiscal.nr-nota-fis
                       tt-pedido.serie-nota   = nota-fiscal.serie
                       tt-pedido.nat-operacao = nota-fiscal.nr-nota-fis
                       tt-pedido.val-nota     = nota-fiscal.vl-tot-nota.
                ASSIGN tt-retorno.status-cod = "OK"
                       tt-retorno.status-obs = "VALIDAÄ«O DA NOTA: Nota gerada com sucesso: " + tt-pedido.num-nota.
                return "OK".
            END.
