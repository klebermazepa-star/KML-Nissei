...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 19
.
...
PROCEDURE pi-main:
    ...
    run pi-buscar-notas. // busca as notas independente do status-cod OK / NOK               
...
PROCEDURE pi-input-json-retorno :
    ...
    DEFINE VARIABLE oJsonArrayNotas AS JsonArray         NO-UNDO.

    // ADICIONAR DADOS NO JSON DE RETORNO
    oJsonArrayNotas = NEW JsonArray().
    oJsonArrayNotas:READ(TEMP-TABLE tt-notas:HANDLE).
    jsonObjectOutput:ADD("notas", oJsonArrayNotas).
...
PROCEDURE pi-gerar-nota:
    ...
    DO TRANS ON ERROR UNDO, RETRY:
        ...
        IF  l-erro THEN DO:
            ...
            ASSIGN tt-retorno.status-cod = "NOK"
                   tt-retorno.status-obs = "Verificar relt¢rio da execuá∆o porque ocorreu um erro na geraá∆o da nota para o pedido: " + STRING(tt-pedido.num-pedido).
        END.
        ...
        IF  L-TRACE THEN ASSIGN TT-RETORNO.STATUS-TRACE = TT-RETORNO.STATUS-TRACE + "|145".
    ...
    IF  L-TRACE THEN ASSIGN TT-RETORNO.STATUS-TRACE = TT-RETORNO.STATUS-TRACE + "|155".
    
    for first tt-notas:
        IF  L-TRACE THEN ASSIGN TT-RETORNO.STATUS-TRACE = TT-RETORNO.STATUS-TRACE + "|156".
    end.
...
PROCEDURE pi-buscar-notas:
    ...
    and avail emitente then do:
        ...
        for each nota-fiscal USE-INDEX ch-distancia no-lock // ler notas da data corrente
            ...
            and   nota-fiscal.dt-emis-nota = 12/19/2025
