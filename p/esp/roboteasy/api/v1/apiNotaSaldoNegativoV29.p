...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 29
.
...
def temp-table tt-pedido no-undo serialize-name "pedido"
    ...
    field ativarTrace   as logical  serialize-name "ativarTrace"
...
PROCEDURE pi-main:
    ...
    IF  AVAIL tt-pedido THEN DO:
        ...
        assign l-trace = tt-pedido.ativarTrace.
...
PROCEDURE pi-input-json-retorno :
    ...
    assign i-qt-erros = 0.
    for each tt-erros:
        assign i-qt-erros = i-qt-erros + 1.
    end.

    assign i-qt-notas = 0.
    for each tt-notas:
        assign i-qt-notas = i-qt-notas + 1.
    end.
    
    // sempre vai atualizar para retorno OK ou NOK                
    ASSIGN tt-retorno.status-obs = "QTD ERROS = " + trim(string(i-qt-erros)) + " QTD NOTAS = " + trim(string(i-qt-notas)).
