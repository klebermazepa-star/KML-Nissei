...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 5
...
PROCEDURE pi-log-datasul:

    DEF INPUT PARAM p-c-opcao AS CHAR NO-UNDO.
    
    IF  p-c-opcao = "ativar" THEN DO:
        RUN roboteasy/apiAtivarClientLog.p PERSISTENT SET h-api-log.
        RUN ativarLog IN h-api-log (INPUT tt-pedido.log-datasul-pasta, INPUT tt-pedido.log-datasul-arquivo).
    END.

    IF  p-c-opcao = "desativar" AND VALID-HANDLE(h-api-log) THEN DO:
