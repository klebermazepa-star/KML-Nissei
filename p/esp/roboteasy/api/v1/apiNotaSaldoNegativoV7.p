...
def var l-trace     as logical  no-undo initial no.
def var h-api-log   as handle   no-undo.        .
...
DEF TEMP-TABLE tt-retorno NO-UNDO SERIALIZE-NAME "retorno" 
    ...
    FIELD status-versao                 AS INT                      SERIALIZE-NAME "statusVersao" INITIAL 7
    FIELD arq-log-datasul               AS CHAR                     SERIALIZE-NAME "arquivoLogDatasul"
.
...
def temp-table tt-pedido no-undo serialize-name "pedido"
    ...
    field log-datasul-ativar    as char     serialize-name "logDatasulAtivar"
    field log-datasul-pasta     as char     serialize-name "logDatasulPasta"
    field log-datasul-arquivo   as char     serialize-name "logDatasulArquivo"
.
...
PROCEDURE pi-main:
    ...
    IF  AVAIL tt-pedido THEN DO:
        ...
        IF  tt-pedido.log-datasul-ativar = "Sim" THEN
            RUN pi-log-datasul-ativar (tt-pedido.log-datasul-pasta, tt-pedido.log-datasul-arquivo).
...
PROCEDURE pi-log-datasul-ativar:
    DEF INPUT PARAM p-c-pasta   AS CHAR NO-UNDO.
    DEF INPUT PARAM p-c-arquivo AS CHAR NO-UNDO.
    RUN esp/roboteasy/apiAtivarClientLog.p PERSISTENT SET h-api-log.
    RUN ativarLog IN h-api-log (INPUT p-c-pasta, INPUT p-c-arquivo).
END PROCEDURE.

PROCEDURE pi-log-datasul-deativar:
     RUN desativarLog IN h-api-log.
     DELETE PROCEDURE h-api-log.
END PROCEDURE.
