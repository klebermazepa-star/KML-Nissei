...
*******************************************************************************/
{include/i-prgvrs.i zoomespdoc001 1.00.00.001KML}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i zoomespdoc001 mut}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           zoomespdoc001
&GLOBAL-DEFINE Version           1.00.00.001KML

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Codigo

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  custom.esp-tipo-documento.tipo-documento
...
&GLOBAL-DEFINE ttTable1          tt-esp-tipo-documento
&GLOBAL-DEFINE hDBOTable1        hesp-tipo-documento
&GLOBAL-DEFINE DBOTable1         dboesp-tipo-documento
...
&Scoped-define INTERNAL-TABLES tt-esp-tipo-documento

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ~
tt-esp-tipo-documento.tipo-documento tt-esp-tipo-documento.Descricao ~
tt-esp-tipo-documento.obrigatorio tt-esp-tipo-documento.bloqueia-vendas 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-esp-tipo-documento NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-esp-tipo-documento NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-esp-tipo-documento
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-esp-tipo-documento
...
DEFINE QUERY brTable1 FOR 
      tt-esp-tipo-documento SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-esp-tipo-documento.tipo-documento COLUMN-LABEL "Tipo Documento" FORMAT "x(20)":U
            WIDTH 14.43
      tt-esp-tipo-documento.Descricao FORMAT "x(50)":U WIDTH 32.86
      tt-esp-tipo-documento.obrigatorio COLUMN-LABEL "Obrigatorio" FORMAT "sim/nao":U
            WIDTH 12
      tt-esp-tipo-documento.bloqueia-vendas COLUMN-LABEL "Bloqueia Vendas" FORMAT "sim/nao":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-esp-tipo-documento"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   > Temp-Tables.tt-esp-tipo-documento.tipo-documento
"tt-esp-tipo-documento.tipo-documento" "Tipo Documento" "x(20)" "character" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-esp-tipo-documento.Descricao
"tt-esp-tipo-documento.Descricao" ? "x(50)" "character" ? ? ? ? ? ? no ? no no "32.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-esp-tipo-documento.obrigatorio
"tt-esp-tipo-documento.obrigatorio" "Obrigatorio" ? "logical" ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-esp-tipo-documento.bloqueia-vendas
"tt-esp-tipo-documento.bloqueia-vendas" "Bloqueia Vendas" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/espdoc001.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esp/boespdoc001.p":U THEN DO:
       
        {btb/btb008za.i1 esp/boespdoc001.p YES}
        {btb/btb008za.i2 esp/boespdoc001.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/openqueries.i &Query="ByTipo-documento"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "tipo-documento":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.tipo-documento).
            WHEN "Descricao":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.Descricao).
            WHEN "obrigatorio":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.obrigatorio).
            WHEN "bloqueia-vendas":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.bloqueia-vendas).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintByTipo-documento IN {&hDBOTable1} ( INPUT CodigoInicial,
                                                               INPUT CodigoFinal).
