...
*******************************************************************************/
{include/i-prgvrs.i z01cp0101 1.00.00.001KML}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i z01cp0101 mut}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01cp0101
...
&GLOBAL-DEFINE FieldsRangePage1  custom.esp-cond-pagto.cond-pagto
/* &GLOBAL-DEFINE FieldsRangePage2  esp-cond-pagto.especie */
...
&GLOBAL-DEFINE ttTable1          tt_esp-cond-pagto
&GLOBAL-DEFINE hDBOTable1        hbocond-pagto
&GLOBAL-DEFINE DBOTable1         esp-cond-pagto

/* &GLOBAL-DEFINE ttTable2          tt_esp-cond-pagto2 */
/* &GLOBAL-DEFINE hDBOTable2        hbocond-pagto2     */
/* &GLOBAL-DEFINE DBOTable2         esp-cond-pagto     */
...
&Scoped-define INTERNAL-TABLES tt_esp-cond-pagto

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt_esp-cond-pagto.cond-pagto ~
tt_esp-cond-pagto.especie tt_esp-cond-pagto.portador 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt_esp-cond-pagto NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt_esp-cond-pagto NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt_esp-cond-pagto
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt_esp-cond-pagto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt_esp-cond-pagto.cond-pagto FORMAT "x(12)":U WIDTH 18.43
      tt_esp-cond-pagto.especie FORMAT "x(2)":U WIDTH 25.43
      tt_esp-cond-pagto.portador FORMAT "x(5)":U WIDTH 30.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt_esp-cond-pagto"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt_esp-cond-pagto.cond-pagto
"tt_esp-cond-pagto.cond-pagto" ? ? "character" ? ? ? ? ? ? no ? no no "18.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt_esp-cond-pagto.especie
"tt_esp-cond-pagto.especie" ? ? "character" ? ? ? ? ? ? no ? no no "25.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt_esp-cond-pagto.portador
"tt_esp-cond-pagto.portador" ? ? "character" ? ? ? ? ? ? no ? no no "30.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/escp0101.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esp/boescp0101.p":U THEN DO:
       
        {btb/btb008za.i1 esp/boescp0101.p YES}
        {btb/btb008za.i2 esp/boescp0101.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/openqueries.i &Query="ByCond-pagto"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cond-pagto":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cond-pagto) .
            WHEN "especie":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.especie).
            WHEN "portador":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.portador).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintBycond-pagto IN {&hDBOTable1} (   INPUT CodigoInicial,
                                                               INPUT CodigoFinal).
