...
assign l-funcao-SGT = can-find(first ems2mult.funcao
...
DEFINE BUFFER bf-unid-feder-est  FOR ems2mult.unid-feder. /* Leitura Unid-Feder do Estabelecimento  */
DEFINE BUFFER bf-cidade-est      FOR mgdis.cidade.     /* Leitura Cidade     do Estabelecimento  */
DEFINE BUFFER bf-emitente-est    FOR ems2mult.emitente.   /* Leitura Emitente   do Estabelecimento  */
DEFINE BUFFER bf-unid-feder-emit FOR ems2mult.unid-feder. /* Leitura Unid-Feder do Emitente         */
DEFINE BUFFER bf-cidade-emit     FOR mgdis.cidade.     /* Leitura Cidade     do Emitente         */
DEFINE BUFFER bf-cidade-entrega  FOR mgdis.cidade.     /* Leitura Cidade     do Local de Entrega */
DEFINE BUFFER bf-cidade-nf       FOR mgdis.cidade.     /* Leitura Cidade     da Nota Fiscal      */
...
DEFINE BUFFER bf-pais            FOR mgadm.pais.       /* Leitura Pa°s do Emitente */
...
FUNCTION fn-trata-caracteres RETURNS CHAR (INPUT p-string AS CHAR ).
    ...
    END.
    ELSE
        ASSIGN c-trata-caracteres = replace(c-trata-caracteres, '&', 'E').
        
    RETURN c-trata-caracteres.
END FUNCTION.
...
/*---------- INTEGRACAO TSS SINCRONO -----------
DEFINE TEMP-TABLE ttNFETSSCancel NO-UNDO
   FIELD ID   AS CHAR
   FIELD XML  AS CLOB.
           
DEFINE TEMP-TABLE ttNFETSSCancelRet NO-UNDO
   FIELD k    AS ROWID
   FIELD STR  AS CHAR LABEL "STRING":U.
------- FIM - INTEGRACAO TSS SINCRONO --------*/
