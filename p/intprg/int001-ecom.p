/****************************************************************************
**
**    Programa: int001-ecom.p - Atualizar Movimentos de Caixa e bancos 
...
DEF INPUT PARAMETER TABLE FOR tt-despesas.
DEF OUTPUT PARAMETER TABLE FOR tt-retorno.

//log-manager:write-message("encontrou" + tt-despesas.cnpj) no-error.
...
log-manager:write-message("KML - Dentro API - TESTE - " +  "ENTROU NA API ANTES TUDO") no-error.
...
PROCEDURE pi_imp_movto_cta_corren_procfit:
    ...
    DEFINE VARIABLE ID-NUM AS INTEGER     NO-UNDO.
    ...
    /*************************** Buffer Definition End **************************/
    FIND FIRST tt-despesas NO-ERROR.
    
    FIND FIRST estabelec NO-LOCK
    WHERE  estabelec.cgc = tt-despesas.cnpj_filial NO-ERROR.
    
    IF NOT AVAIL estabelec THEN
    DO:
        CREATE tt-retorno.
        ASSIGN tt-retorno.desc-erro = "Estabelecimento n∆o cadastrado".  
    END.
    
    FIND LAST int_dp_fundo_fixo EXCLUSIVE-LOCK
        WHERE int_dp_fundo_fixo.cnpj_estab = estabelec.cgc
        AND   int_dp_fundo_fixo.cod_docto  = tt-despesas.codigo_documento NO-ERROR. // usando for last n∆o necessariamente preciso usar where
        //AND   int_dp_fundo_fixo.num_id_ffixo > 0 NO-ERROR.
        
    log-manager:write-message("KML - DENTRO API - TESTE - " + estabelec.cgc) no-error.
    log-manager:write-message("KML - DENTRO API - TESTE - " + tt-despesas.codigo_documento) no-error.
        
    IF NOT AVAIL int_dp_fundo_fixo THEN
    DO:
        FIND LAST int_dp_fundo_fixo NO-LOCK NO-ERROR.
        
        IF AVAILABLE int_dp_fundo_fixo THEN
            ASSIGN id-num = int_dp_fundo_fixo.num_id_ffixo + 1.
                                                  
        CREATE int_dp_fundo_fixo.
        ASSIGN int_dp_fundo_fixo.cnpj_estab      = estabelec.cgc
               int_dp_fundo_fixo.cod_ccusto      = tt-despesas.centro_custo
               int_dp_fundo_fixo.cod_docto       = tt-despesas.codigo_documento
               int_dp_fundo_fixo.cod_empresa     = tt-despesas.empresa
               int_dp_fundo_fixo.cod_estab       = estabelec.cod-estabel
               int_dp_fundo_fixo.situacao        = 1
               int_dp_fundo_fixo.cod_unid_negoc  = tt-despesas.unidade_negocio
               int_dp_fundo_fixo.cta_ctbl        = tt-despesas.conta_contabil
               int_dp_fundo_fixo.dat_movto       = tt-despesas.data_movimento
               int_dp_fundo_fixo.des_historico   = tt-despesas.historico
               int_dp_fundo_fixo.natureza        = tt-despesas.natureza
               int_dp_fundo_fixo.num_id_ffixo    = id-num
               int_dp_fundo_fixo.tipo_movto      = tt-despesas.tipo_movimento
               int_dp_fundo_fixo.val_movto       = tt-despesas.valor_movimento.
                       
    END.
    
    ELSE DO:
        CREATE tt-retorno.
        ASSIGN tt-retorno.desc-erro = "Registro j† existe na tabela".
        RETURN "NOK".
    END.
    
    log-manager:write-message("KML - DENTRO API - Movimento - " + tt-despesas.tipo_movimento) no-error.
    
    FOR LAST int_dp_fundo_fixo NO-LOCK 
        WHERE int_dp_fundo_fixo.cod_estab = estabelec.cod-estabel
        AND   int_dp_fundo_fixo.cod_docto = tt-despesas.codigo_documento /* N∆o integrado */
                                   ...
                                   .MESSAGE tt_movto_cta_corren_import.tta_cod_docto_movto_cta_bco
                                    ...
                                    FIRST tt_movto_ffixo NO-LOCK WHERE
                                        tt_movto_ffixo.ttv_num_id_ffixo = int(tt_movto_cta_corren_import.tta_cod_docto_movto_cta_bco):
                                            ...
                                            CREATE tt-retorno.
                                            ASSIGN tt-retorno.l-gerada  = NO
                                                 tt-retorno.docto-ff  = string(int_dp_fundo_fixo.cod_docto)
                                                 tt-retorno.desc-erro = tt_import_movto_valid_cmg.ttv_des_mensagem + " Estab.: " + int_dp_fundo_fixo.cod_estab + " Docto.: " + int_dp_fundo_fixo.cod_docto +
                                                                    " Conta: " + int_dp_fundo_fixo.cta_ctbl  /* + " CCusto: " + int_dp_fundo_fixo.cod_ccusto */ .
                                                                    
                                            DELETE int_dp_fundo_fixo.
                                    END.  
                END.   
                ELSE DO:
                           ...
                           tt_movto_ffixo.ttv_num_id_ffixo = int(tt_movto_cta_corren_import.tta_cod_docto_movto_cta_bco):
                            ...
                            IF AVAIL int_dp_fundo_fixo THEN DO:
                                ...
                                IF AVAIL movto_cta_corren THEN DO:
                                    ...
                                    CREATE tt-retorno.
                                    ASSIGN tt-retorno.l-gerada = YES
                                           tt-retorno.docto-ff = string(int_dp_fundo_fixo.cod_docto) 
                                           tt-retorno.desc-erro = "Registro Integrado -" + " Estab.: " + int_dp_fundo_fixo.cod_estab + " Docto.: " + int_dp_fundo_fixo.cod_docto +
                                                                " Conta: " + int_dp_fundo_fixo.cta_ctbl. /* + " CCusto: " + int_dp_fundo_fixo.cod_ccusto */                                                                        
