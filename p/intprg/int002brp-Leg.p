/********************************************************************************
*******************************************************************************/
{include/i-prgvrs.i int002bRP-Leg 2.06.00.001}  
{include/i_fnctrad.i}
/******************************************************************************
**
**       PROGRAMA: int002bRP-Leg
**
**       DATA....: 11/2015
**
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "INT002brp-Leg.txt"
           tt-param.destino = 3
...
assign c-programa = "Int002brp-Leg"
       c-versao   = "2.06"
       c-revisao  = "001"
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENRYINTEGRATION_NEW_LEG NO-LOCK WHERE       
             /*NDD_ENRYINTEGRATION_NEW_LEG.emissiondate       >= datetime(08/03/2016) AND 
             NDD_ENRYINTEGRATION_NEW_LEG.emissiondate       <= datetime(08/03/2016) AND*/
             NDD_ENRYINTEGRATION_NEW_LEG.serie              <> 10                   AND  
             NDD_ENRYINTEGRATION_NEW_LEG.kind    = 0 /* Envio */                    AND   
             NDD_ENRYINTEGRATION_NEW_LEG.status_ = 0 /* campo status pendente */
             query-tuning(no-lookahead):  
     
        /*IF NDD_ENRYINTEGRATION_NEW_LEG.emissiondate <> DATETIME(?) 
        THEN DO:
        
            ASSIGN l-estab = NO.

            FOR FIRST b-estab NO-LOCK WHERE 
                      b-estab.cgc = trim(string(NDD_ENRYINTEGRATION_NEW_LEG.cnpjemit)) , 
                FIRST estabelec NO-LOCK WHERE
                      estabelec.cgc = trim(string(NDD_ENRYINTEGRATION_NEW_LEG.cnpjdest)):
        ...
        p-h-raiz:GET-CHILD(h-niveis,i-qtd-niveis).
            ...
            THEN DO:
                ...
                THEN DO: 
                    ...
                    FIND FIRST b-NDD_ENRYINTEGRATION_NEW_LEG WHERE
                               rowid(b-NDD_ENRYINTEGRATION_NEW_LEG) = rowid(NDD_ENRYINTEGRATION_NEW_LEG) NO-ERROR.
                    IF AVAIL b-NDD_ENRYINTEGRATION_NEW_LEG 
                    THEN DO:
