/********************************************************************************
** Programa : int002i
** Funá∆o   : Calcula impostos
** Data     : 12/2015
********************************************************************************/
{include/i-prgvrs.i int002i 2.06.00.001}  
{utp/ut-glob.i}
...
DEFINE TEMP-TABLE tt-impostos LIKE tt-item-doc-est.
...
run pi-seta-titulo in h-acomp (input "Calculando Impostos.").
         ...
         for each tt-docum-est WHERE
            ...
            and natur-oper.nota-rateio then do:    
                ...
                FIND FIRST docum-est
                        OF tt-docum-est NO-LOCK NO-ERROR.
                IF AVAIL docum-est THEN DO:  
                     ...
                     IF AVAIL b-docum-est THEN DO:
                       ...
                       FIND FIRST int-ds-docto-xml NO-LOCK WHERE
                                  ...
                                  int-ds-docto-xml.serie        = b-docum-est.serie NO-ERROR.
                       IF AVAIL int-ds-docto-xml THEN
                          ASSIGN substring(b-docum-est.char-1,93,60) = IF int-ds-docto-xml.chNfe = ? THEN "" ELSE int-ds-docto-xml.chNfe. 
                     ...
                     FOR EACH item-doc-est OF docum-est EXCLUSIVE-LOCK,
                           ...
                           FOR EACH rat-lote EXCLUSIVE-LOCK WHERE
...
/*                                                                   */
         FIND FIRST int-ds-doc-erro NO-LOCK WHERE
                    ...
                    int-ds-doc-erro.descricao    = tt-valida-erro.ErrorDescription NO-ERROR.
         IF NOT AVAIL int-ds-doc-erro THEN DO:
            CREATE int-ds-doc-erro.
            ASSIGN int-ds-doc-erro.serie-docto  = tt-nota.serie-nota   
                   int-ds-doc-erro.cod-emitente = tt-nota.cod-emitente 
                   int-ds-doc-erro.nro-docto    = tt-nota.nro-docto 
                   int-ds-doc-erro.tipo-nota    = tt-nota.tipo-nota
                   int-ds-doc-erro.tipo-erro    = tt-valida-erro.ErrorSubType 
                   int-ds-doc-erro.cod-erro     = tt-valida-erro.ErrorNumber   
                   int-ds-doc-erro.descricao    = tt-valida-erro.ErrorDescription.
         END.
     END.
                 ...
                 THEN DO:
                    ...
                    IF AVAIL docum-est  THEN DO:
                       ...
                       FIND FIRST int-ds-docto-xml NO-LOCK WHERE
                                  ...
                                  int-ds-docto-xml.serie        = docum-est.serie NO-ERROR.
                       IF AVAIL int-ds-docto-xml THEN
                          ASSIGN substring(docum-est.char-1,93,60) = IF int-ds-docto-xml.chNfe = ? THEN "" ELSE int-ds-docto-xml.chNfe. 
...
PROCEDURE pi-atualiza-duplicatas:
   ...
   for each dupli-apagar EXCLUSIVE-LOCK where 
       ...
       IF DAY(dupli-apagar.dt-vencim) <= 5 THEN 
          ASSIGN c-dt-venc = "05/" + STRING(MONTH(dupli-apagar.dt-vencim)) + "/" + STRING(YEAR(dupli-apagar.dt-vencim)).
       ELSE IF DAY(dupli-apagar.dt-vencim) > 5   AND  
               DAY(dupli-apagar.dt-vencim) <= 10 THEN
           ASSIGN c-dt-venc = "10/" + STRING(MONTH(dupli-apagar.dt-vencim)) + "/" + STRING(YEAR(dupli-apagar.dt-vencim)).
       ELSE IF DAY(dupli-apagar.dt-vencim) > 10   AND  
               DAY(dupli-apagar.dt-vencim) <= 15 THEN 
           ASSIGN c-dt-venc = "15/" + STRING(MONTH(dupli-apagar.dt-vencim)) + "/" + STRING(YEAR(dupli-apagar.dt-vencim)).
        ELSE IF DAY(dupli-apagar.dt-vencim) > 15   AND  
               DAY(dupli-apagar.dt-vencim) <= 20 THEN 
           ASSIGN c-dt-venc = "20/" + STRING(MONTH(dupli-apagar.dt-vencim)) + "/" + STRING(YEAR(dupli-apagar.dt-vencim)).
        ELSE IF DAY(dupli-apagar.dt-vencim) > 20   AND  
                DAY(dupli-apagar.dt-vencim) <= 25 THEN 
           ASSIGN c-dt-venc = "25/" + STRING(MONTH(dupli-apagar.dt-vencim)) + "/" + STRING(YEAR(dupli-apagar.dt-vencim)).
        ELSE DO: 
           ASSIGN dt-vencto = dupli-apagar.dt-vencim + 8
                  c-dt-venc = "05/" + STRING(MONTH(dt-vencto)) + "/" + STRING(YEAR(dt-vencto)).
        END.

       ASSIGN dupli-apagar.dt-vencim = DATE(c-dt-venc). 
   END.                
...
PROCEDURE PI-ATUALIZA-ITEM-DOC-EST :
    ...
    RUN setDefaultsItem IN h-boin176.
   
    RUN calculate_ICMS_SubsTrib_ICMSComp IN h-boin176 (tt2-item-doc-est.cod-emitente,
                                                       tt2-item-doc-est.serie-docto,
                                                       tt2-item-doc-est.nro-docto,
                                                       tt2-item-doc-est.nat-operacao).
    
    FOR EACH item-doc-est NO-LOCK WHERE 
             item-doc-est.nro-docto    = tt2-item-doc-est.nro-docto    AND 
             item-doc-est.serie-docto  = tt2-item-doc-est.serie-docto  AND 
             item-doc-est.cod-emitente = tt2-item-doc-est.cod-emitente AND 
             item-doc-est.nat-operacao = tt2-item-doc-est.nat-operacao AND
             item-doc-est.sequencia    = tt2-item-doc-est.sequencia  :

        CREATE tt-impostos.
        BUFFER-COPY item-doc-est TO tt-impostos.
...
PROCEDURE PI-GERA-ERROS :
    ...
    for each RowErrors NO-LOCK where 
        ...
        /* MESSAGE  RowErrors.ErrorNumber       SKIP  
