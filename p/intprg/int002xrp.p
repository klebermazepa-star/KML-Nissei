...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "c:/nass/int002xrp"
           tt-param.destino = 3
...
THEN DO:
   ...
   IF AVAIL int-ds-it-docto-xml THEN DO:
      ...
      FIND FIRST int-ds-docto-xml WHERE
                 ...
                 int-ds-docto-xml.nNF          = int-ds-it-docto-xml.nNF          AND
        ...
        FOR EACH tt-it-docto-xml WHERE
            ...
            then do:
                ...
                IF NOT AVAIL pedido-compr THEN DO:
                       ...
                       RUN pi-gera-erro(INPUT 4,    
                                        INPUT "Pedido " + string(tt-it-docto-xml.num-pedido) + " NÆo cadastrado").   
            ...
            THEN DO:
               ...
               IF AVAIL ITEM-fornec THEN
                 ASSIGN int-ds-it-docto-xml.qCom-forn    = tt-it-docto-xml.qCom
                        ...
                        int-ds-it-docto-xml.item-do-forn = item-fornec.item-do-forn  
                        tt-it-docto-xml.item-do-forn     = item-fornec.item-do-forn
                        tt-it-docto-xml.ucom-forn        = tt-it-docto-xml.ucom. 
               ELSE 
                   ASSIGN tt-it-docto-xml.item-do-forn  = "" 
                          tt-it-docto-xml.xprod         = "".

               ASSIGN l-gera-nota = YES.
            ...
            THEN DO:
                  ...
                  IF AVAIL ITEM THEN
                      ASSIGN tt-it-docto-xml.xprod     = ITEM.desc-item
                             int-ds-it-docto-xml.xprod = ITEM.desc-item.
                  
                 if avail item-fornec AND l-gera-nota = NO 
                 then DO:
                  
                     IF item-fornec.unid-med-for = int-ds-it-docto-xml.uCom-forn 
                     THEN DO:
                     
                        RUN retornaIndiceConversao in h-boin176 (input int-ds-it-docto-xml.it-codigo,
                                                                 input int-ds-it-docto-xml.cod-emitente,
                                                                 input int-ds-it-docto-xml.uCom-forn,
                                                                 output de-fator). 
                     END.
                     ELSE DO:
                        /* IF AVAIL ITEM 
                        THEN DO:
                            ...
                            find tab-conv-un where 
                                 ...
                                 tab-conv-un.unid-med-for = int-ds-it-docto-xml.uCom-forn no-lock no-error.
                            IF AVAIL tab-conv-un THEN
                               assign de-fator = if avail tab-conv-un THEN tab-conv-un.fator-conver / exp(10, tab-conv-un.num-casa-dec) else 1.
                  ...
                  ASSIGN int-ds-it-docto-xml.qCom   = tt-it-docto-xml.qCom-forn / de-fator
                         ...
                         int-ds-it-docto-xml.vUnCom = tt-it-docto-xml.vProd / int-ds-it-docto-xml.qCom.
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK WHERE     
             NDD_ENTRYINTEGRATION.ENTRYINTEGRATIONID >= 85000 AND
