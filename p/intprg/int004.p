...
FOR EACH int_ds_cliente WHERE 
         int_ds_cliente.situacao   = 1 /* Pendente */ AND 
         int_ds_cliente.tipo_movto = 1 /* Inclus∆o */:
    FIND FIRST emitente USE-INDEX nome WHERE
               emitente.nome-abrev = int_ds_cliente.nome_abrev EXCLUSIVE-LOCK NO-ERROR.
    IF AVAIL emitente THEN DO:

        IF int_ds_cliente.natureza = 1 /*Pessoa fisica*/ THEN DO:
        
           FIND FIRST b-emitente USE-INDEX cgc WHERE
                      b-emitente.cgc = string(int(int_ds_cliente.cgc), "9999999999") NO-LOCK NO-ERROR.
           IF NOT AVAIL b-emitente THEN DO:
                ASSIGN emitente.cgc = int_ds_cliente.cgc.
                ASSIGN int_ds_cliente.nome_abrev = "CL" + SUBSTR(int_ds_cliente.cgc,1,10).
           END.

        END.
        ELSE DO:
           FIND FIRST b-emitente USE-INDEX cgc WHERE
                      b-emitente.cgc = string(int(int_ds_cliente.cgc), "99999999999999") NO-LOCK NO-ERROR.
           IF NOT AVAIL b-emitente THEN DO:
                ASSIGN emitente.cgc = int_ds_cliente.cgc.
                ASSIGN int_ds_cliente.nome_abrev = "CL" + SUBSTR(int_ds_cliente.cgc,1,10).
           END.
...
FOR EACH int_ds_cliente 
   WHERE int_ds_cliente.situacao = 1 /* Pendente */
    ...
    IF int_ds_cliente.natureza = 1 /*Pessoa Fisica */ THEN
        ASSIGN int_ds_cliente.cgc = string(int64(INT_ds_cliente.cgc), "99999999999").
    ELSE
        ASSIGN int_ds_cliente.cgc = string(int64(INT_ds_cliente.cgc), "99999999999999").

    IF int_ds_cliente.cgc BEGINS "79430682" 
    OR int_ds_cliente.cgc = "13495487000172" THEN DO:
       ASSIGN int_ds_cliente.situacao = 2
              int_ds_cliente.envio_status = 08. /* Integrado */
       NEXT.
    END.
    ...
    IF int_ds_cliente.tipo_movto = 1 THEN DO: /* Inclus∆o */
       ...
       END.       
          ...
          for first emitente fields (cgc nome-emit cod-emitente nome-abrev) no-lock where 
          ...
          if avail emitente then do:
             IF emitente.cgc = int_ds_cliente.cgc THEN
                ...
                RUN intprg/int999.p (INPUT "CLI", 
                                  INPUT STRING(emitente.cgc),
                                  INPUT "Nome Abreviado " + emitente.nome-abrev + " j† utilizado no cliente - CPF/CNPJ: " + string(emitente.cgc) + ". Origem: " + c-sistema-orig + ".",
                                  INPUT 1, /* 1 - Pendente */
                                  INPUT c-seg-usuario,
                                  INPUT "int004.p").
             NEXT.
