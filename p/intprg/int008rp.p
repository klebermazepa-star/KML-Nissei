...
DEF BUFFER b-int_ds_ped_compr    FOR int_ds_ped_compr.
...
def var de-preco-fornec         like int_ds_ordem_compra.preco_fornec   no-undo.
def var de-preco-unit           like int_ds_ordem_compra.preco_unitario no-undo.
...
for EACH int_ds_ped_compra WHERE  
         int_ds_ped_compra.cod_estabel     >= tt-param.cod-estabel-ini AND 
         int_ds_ped_compra.cod_estabel     <= tt-param.cod-estabel-fim AND
         int_ds_ped_compra.data_pedido     >= tt-param.dt-periodo-ini  AND 
         int_ds_ped_compra.data_pedido     <= tt-param.dt-periodo-fim  AND
         int_ds_ped_compra.num_pedido_orig >= tt-param.nr-pedido-ini   AND  
         int_ds_ped_compra.num_pedido_orig <= tt-param.nr-pedido-fim   AND
         int_ds_ped_compra.num_pedido_orig <> ?                        AND     
         int_ds_ped_compra.situacao         = 1:   /* Pendente */

    IF VALID-HANDLE(h-acomp) THEN
       RUN pi-acompanhar IN h-acomp(INPUT "Selecionando Pedidos: " + STRING(int_ds_ped_compra.num_pedido_orig)).
	
	/* KML - Guilherme Nichele 28/11/2024 */
	FIND FIRST centro_distribuicao NO-LOCK WHERE 
        centro_distribuicao.cod-estabel = string (int_ds_ped_compra.cod_estabel) no-error.
	
   
    if int_ds_ped_compra.cod_estabel = int(centro_distribuicao.cod-estabel) THEN
       ASSIGN int_ds_ped_compra.situacao = 9. 
    ELSE
       ASSIGN int_ds_ped_compra.situacao = 2. 
...
bloco-altera:
FOR EACH int_ds_ped_compra WHERE  
         int_ds_ped_compra.situacao         = 9                       AND
         int_ds_ped_compra.data_pedido     >= tt-param.dt-periodo-ini AND 
         int_ds_ped_compra.data_pedido     <= tt-param.dt-periodo-fim AND
         int_ds_ped_compra.num_pedido_orig >= tt-param.nr-pedido-ini  AND  
         int_ds_ped_compra.num_pedido_orig <= tt-param.nr-pedido-fim  AND
         int_ds_ped_compra.num_pedido_orig <> ?                       AND     
         int_ds_ped_compra.tipo_movto       = 2  /* Alteraá∆o */
         BREAK BY int_ds_ped_compra.num_pedido_orig
               BY int_ds_ped_compra.sequencia:
                           
    IF VALID-HANDLE(h-acomp) THEN
       RUN pi-acompanhar IN h-acomp(INPUT "Alterando Pedidos: " + STRING(int_ds_ped_compra.num_pedido_orig)).

    IF LAST-OF(int_ds_ped_compra.sequencia) THEN DO:
       ASSIGN l-ce-atual = NO.
       EMPTY TEMP-TABLE tt-docum.
       FOR EACH item-doc-est USE-INDEX itmdctst-09 WHERE 
                item-doc-est.num-pedido = int_ds_ped_compra.num_pedido_orig NO-LOCK:
           ...
           IF AVAIL docum-est THEN DO:
              ...
              FOR FIRST int_ds_docto_wms WHERE  
                        int_ds_docto_wms.doc_numero_n = docum-est.nro-docto AND
                        int_ds_docto_wms.doc_serie_s  = docum-est.serie     AND
                        int_ds_docto_wms.doc_origem_n = docum-est.cod-emitente NO-LOCK:
              END.  
              IF AVAIL int_ds_docto_wms THEN DO:
                 IF int_ds_docto_wms.situacao <> 0 THEN DO:
                    RUN intprg/int999.p (INPUT "PCOMPR", 
                                         INPUT string(int_ds_ped_compra.num_pedido_orig),
                                         INPUT "Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                               " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf) + " n∆o Alterado. Pedido vinculado com Nota Fiscal movimentada no WMS.", 
       ...
       IF l-ce-atual = NO THEN DO:
          FOR EACH tt-docum:
              ...
              IF AVAIL docum-est THEN DO:
                 ...
                 for each cst_fat_devol where
                          ...
                          cst_fat_devol.cod_emitente = docum-est.cod-emitente:
          ...
          FOR EACH pedido-compr WHERE
                   pedido-compr.num-pedido = int_ds_ped_compra.num_pedido_orig:
              ...
              DELETE pedido-compr.
          END.
          ASSIGN int_ds_ped_compra.tipo_movto = 1.
       END.
       ELSE DO:
           RUN intprg/int999.p (INPUT "PCOMPR", 
                                INPUT string(int_ds_ped_compra.num_pedido_orig),
                                INPUT "Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                      " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf) + " n∆o Alterado. Pedido vinculado com Nota Fiscal j† atualizada no Recebimento/Estoque.", 
                                INPUT 1, /* 1 - Pendente ou 2 - Integrado */
                                INPUT c-seg-usuario,
                                INPUT "int008rp.p").  
           ASSIGN int_ds_ped_compra.situacao = 2.
...
FOR EACH int_ds_ped_compra WHERE  
         ...
         int_ds_ped_compra.num_pedido_orig <= tt-param.nr-pedido-fim  AND
         int_ds_ped_compra.num_pedido_orig <> ?                       AND     
         int_ds_ped_compra.tipo_movto       = 3  /* Cancelamento */
         BREAK BY int_ds_ped_compra.num_pedido_orig
               BY int_ds_ped_compra.sequencia:
                           
    IF VALID-HANDLE(h-acomp) THEN
       RUN pi-acompanhar IN h-acomp(INPUT "Cancelando Pedidos: " + STRING(int_ds_ped_compra.num_pedido_orig)).
    ...
    IF LAST-OF(int_ds_ped_compra.sequencia) THEN DO:
              ...
              IF AVAIL int_ds_docto_wms THEN DO:
                 IF int_ds_docto_wms.situacao <> 0 THEN DO:
                    RUN intprg/int999.p (INPUT "PCOMPR", 
                                         INPUT string(int_ds_ped_compra.num_pedido_orig),
                                         INPUT "Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                               " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf) + " n∆o Cancelado. Pedido vinculado com Nota Fiscal movimentada no WMS.", 
                                         INPUT 1, /* 1 - Pendente ou 2 - Integrado */
                                         INPUT c-seg-usuario,
                                         INPUT "int008rp.p").  
                    ASSIGN int_ds_ped_compra.situacao = 2.
       ...
       ELSE DO:
           RUN intprg/int999.p (INPUT "PCOMPR", 
                                INPUT string(int_ds_ped_compra.num_pedido_orig),
                                INPUT "Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                      " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf) + " n∆o Cancelado. Pedido vinculado com Nota Fiscal j† atualizada no Recebimento/Estoque.", 
                                INPUT 1, /* 1 - Pendente ou 2 - Integrado */
                                INPUT c-seg-usuario,
                                INPUT "int008rp.p").  
           ASSIGN int_ds_ped_compra.situacao = 2.
...
PROCEDURE pi-inclusao-pedidos.
    ...
    FOR EACH int_ds_ped_compra NO-LOCK WHERE  
             ...
             int_ds_ped_compra.num_pedido_orig <= tt-param.nr-pedido-fim  AND
             int_ds_ped_compra.num_pedido_orig <> ?                       AND     
             (int_ds_ped_compra.tipo_movto = 1 OR int_ds_ped_compra.tipo_movto = 0 OR int_ds_ped_compra.tipo_movto = ?):  /* Inclus∆o */   
                               
        IF VALID-HANDLE(h-acomp) THEN
           RUN pi-acompanhar IN h-acomp(INPUT "Gerando Pedidos: " + STRING(int_ds_ped_compra.num_pedido_orig)).
        ...
        IF AVAIL pedido-compr THEN DO:
           DO TRANS: 
               FOR EACH b-int_ds_ped_compra where
                        b-int_ds_ped_compra.num_pedido_orig = int_ds_ped_compra.num_pedido_orig AND
                       (b-int_ds_ped_compra.situacao        = 9 OR b-int_ds_ped_compra.situacao = 1):         
                   ASSIGN b-int_ds_ped_compra.situacao = 2.
               END.
           END.
        ...
        assign c-cgc = string(int_ds_ped_compra.cnpj_cpf) no-error.
        ...
        IF int_ds_ped_compra.cod_cond_pag < 200 THEN DO:
           create tt-erro.
           assign tt-erro.cd-erro  = 51
                  tt-erro.mensagem = "Condiá∆o de Pagamento " + STRING(int_ds_ped_compra.cod_cond_pag) + " inv†lida para geraá∆o de Pedido de Compra no Datasul.". 
        END.
    
        assign l-item = no.
           
        for each int_ds_ordem_compra no-lock where
                 int_ds_ordem_compra.num_pedido_orig = int_ds_ped_compra.num_pedido_orig AND
                 int_ds_ordem_compra.sequencia       = int_ds_ped_compra.sequencia,
            first item no-lock where
                  item.it-codigo = string(int_ds_ordem_compra.it_codigo):  
            ...
            for first item-uni-estab fields(cod-estabel it-codigo cod-obsoleto) where 
                      item-uni-estab.cod-estabel = string(int_ds_ped_compra.cod_estabel,"999") and 
        ...
        if l-item = NO then do:
           ...
           assign tt-erro.cd-erro  = 48
                  tt-erro.mensagem = "Pedido " + STRING(int_ds_ped_compra.num_pedido_orig) + " n∆o possui itens.".  
        end. 
        ...
        IF NOT AVAIL b-emitente THEN DO:
           ...
           assign tt-erro.cd-erro  = 47
                  tt-erro.mensagem = "Fornecedor " + STRING(i-cod-emitente) + " n∆o cadastrado. CNPJ: " + string(int_ds_ped_compra.cnpj_cpf).     
        end.
        ELSE DO:
           /*** Forncederos ativos ***/
           ...
           IF i-ativo <> 1 THEN DO:
              IF i-ativo = 0 THEN DO:
                 ...
                 ASSIGN tt-erro.cd-erro  = 52
                        tt-erro.mensagem = "Nenhum Fornecedor Ativo para o CNPJ: " + string(int_ds_ped_compra.cnpj_cpf).
              END.
              IF i-ativo > 1 THEN DO:
                 CREATE tt-erro.
                 ASSIGN tt-erro.cd-erro  = 52
                        tt-erro.mensagem = "Mais de um Fornecedor ativo para o CNPJ: " + string(int_ds_ped_compra.cnpj_cpf).
              END.
           END. 
           ELSE DO:
               FIND FIRST tt-dist-emitente WHERE
        ...
        ELSE DO:
           ...
           IF AVAIL dist-emitente THEN DO:
              IF dist-emitente.idi-sit-fornec <> 1 THEN DO:
                 CREATE tt-erro.
                 ASSIGN tt-erro.cd-erro  = 52
                        tt-erro.mensagem = "Fornecedor " + STRING(emitente.cod-emitente) + " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf) + " Inativo ou possui Restriá∆o de Compras.".
              END.
           END.
        END.*/
        ...
        if not can-find(first tt-erro) then do: 
           ...
           ASSIGN i-num-pedido = int_ds_ped_compra.num_pedido_orig. 
           ...
           IF (int_ds_ped_compra.cod_cond_pag = ? 
           OR int_ds_ped_compra.cod_cond_pag = 0) THEN
              ASSIGN i-cond-pagto = i-cond-pagto.
           ELSE 
              ASSIGN i-cond-pagto = int_ds_ped_compra.cod_cond_pag.
           ...
           assign ttpedido-compr.num-pedido       = i-num-pedido
                  ...
                  ttpedido-compr.data-pedido      = int_ds_ped_compra.data_pedido 
                  fi-cod-estab                    = string(int_ds_ped_compra.cod_estabel,"999")
            ...
            for each int_ds_ordem_compra no-lock where
               ...
               find item-fornec no-lock where 
                    item-fornec.it-codigo    = string(int_ds_ordem_compra.it_codigo) and
                    item-fornec.cod-emitente = ttpedido-compr.cod-emitente no-error.
               if NOT available item-fornec then do:                                                              
                  create item-fornec.
                  assign item-fornec.it-codigo    = string(int_ds_ordem_compra.it_codigo)
                         item-fornec.cod-emitente = ttpedido-compr.cod-emitente
                         item-fornec.item-do-forn = "" /* int_ds_ordem_compra.item-do-forn */ 
               ...
               FIND FIRST item-fornec-umd WHERE 
                          item-fornec-umd.it-codigo    = string(int_ds_ordem_compra.it_codigo) AND
               ...
               IF NOT AVAIL item-fornec-umd THEN DO:
                  ...
                  ASSIGN item-fornec-umd.it-codigo    = string(int_ds_ordem_compra.it_codigo)
               ...
               for first item-fornec-estab where 
                         item-fornec-estab.it-codigo    = string(int_ds_ordem_compra.it_codigo) and
               ...
               if NOT available item-fornec-estab THEN DO:
                  ...
                  ASSIGN item-fornec-estab.it-codigo    = string(int_ds_ordem_compra.it_codigo)
            ...
            DO TRANS: 
                ...
                ELSE DO:
                    ...
                    for each int_ds_ordem_compra no-lock where
                        ...
                        IF VALID-HANDLE(h-acomp) THEN
                           RUN pi-acompanhar IN h-acomp(INPUT "Pedido: " + STRING(int_ds_ped_compra.num_pedido_orig) + " Item: " + STRING(int_ds_ordem_compra.it_codigo)).
                                                      
                        FIND FIRST ITEM NO-LOCK WHERE
                                   ITEM.it-codigo = string(int_ds_ordem_compra.it_codigo) NO-ERROR.
                        ...
                        if int_ds_ordem_compra.preco_fornec <= 0 then 
                           assign de-preco-fornec = 0.01.
                        else 
                           assign de-preco-fornec = int_ds_ordem_compra.preco_fornec.
                           
                        if int_ds_ordem_compra.preco_unitario <= 0 then 
                           assign de-preco-unit = 0.01.
                        else 
                           assign de-preco-unit = int_ds_ordem_compra.preco_unitario.     
                        ...
                        assign /* tt-ordem-compra-bo.ind-tipo-movto = 1 */
                               ...
                               tt-ordem-compra-bo.it-codigo      = string(int_ds_ordem_compra.it_codigo)   /* tabela tutorial */                                                  
                               ...
                               tt-ordem-compra-bo.qt-solic       = int_ds_ordem_compra.qt_solic          /* tabela tutorial */
						...
						FIND FIRST 	centro_distribuicao NO-LOCK
							WHERE 	centro_distribuicao.cod-estabel = tt-ordem-compra-bo.cod-estabel no-error.
							
                        assign tt-ordem-compra-bo.conta-contabil = tt-ordem-compra-bo.ct-codigo + tt-ordem-compra-bo.sc-codigo
                               tt-ordem-compra-bo.dep-almoxar    = if avail centro_distribuicao then centro_distribuicao.cod-estabel else " ". 
                        ...
                        DO WHILE not AVAIL ordem-compra:               
                           ...
                           FIND FIRST int_ds_nr_ordem NO-LOCK NO-ERROR.
                           IF AVAIL int_ds_nr_ordem THEN DO:
                              ASSIGN i-num-ordem = int_ds_nr_ordem.nr_ordem_compra + 1.
                              FIND FIRST int_ds_nr_ordem EXCLUSIVE-LOCK NO-ERROR.
                              ASSIGN int_ds_nr_ordem.nr_ordem_compra = i-num-ordem.
                              RELEASE int_ds_nr_ordem.
                           END.
                        ...
                        IF NOT CAN-FIND(FIRST rowerrors where rowErrors.ErrorSubType = "Error") THEN DO:
                           ...
                           IF AVAIL ordem-compra THEN DO:
                              ...
                              if avail b-int_ds_ordem_compra then 
                                 assign b-int_ds_ordem_compra.numero_ordem = tt-ordem-compra-bo.numero-ordem.
        ...
        do trans:
            if can-find (first tt-erro) then do:
               ...
               RUN intprg/int999.p (INPUT "PCOMPR", 
                                    INPUT string(int_ds_ped_compra.num_pedido_orig),
                                    INPUT tt-erro.mensagem + " Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                                             " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf), 
                                    INPUT 1, /* 1 - Pendente ou 2 - Integrado */
                                    INPUT c-seg-usuario,
                                    INPUT "int008rp.p").  
               ...
               if avail b-int_ds_ped_compra then DO:
                
                  assign b-int_ds_ped_compra.situacao   = 2
                         b-int_ds_ped_compra.num_pedido = ttpedido-compr.num-pedido.
    
                  PUT b-int_ds_ped_compra.num_pedido     AT 01 
                      int_ds_ped_compra.num_pedido_orig  AT 15 SKIP.
               ...
               RUN intprg/int999.p (INPUT "PCOMPR", 
                                   INPUT string(int_ds_ped_compra.num_pedido_orig),
                                   INPUT "Registro Integrado -" + " Pedido: " + string(int_ds_ped_compra.num_pedido_orig) + " Estab.: " + string(int_ds_ped_compra.cod_estabel) + 
                                                                  " CNPJ: " + string(int_ds_ped_compra.cnpj_cpf),
...
RUN intprg/int888.p (INPUT "PCOMPR",
                     INPUT "int008rp.p").
