...
/********************************************************************************
** Programa: int013 - Cadastro de->para cfop X Nat. Opera‡Æo PRS -> Datasul / NFE
**
** Versao : 12 - 23/02/2016 - Alessandro V Baccin
**
********************************************************************************/
{include/i-prgvrs.i INT013 2.12.01.AVB}
...
DEFINE VARIABLE h_int013-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int013-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 89.86 BY 15.25 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Natureza Op. Datasul X Sysfarma"
         HEIGHT             = 15.25
         WIDTH              = 89.86
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Natureza Op. Datasul X Sysfarma */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Natureza Op. Datasul X Sysfarma */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int013-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int013-viewer ).
       RUN set-position IN h_int013-viewer ( 2.46 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.17 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int013-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int013-zoom.w,
                     ProgVaPara = intprg/int013-vapara.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_int013-query ).
       RUN set-position IN h_int013-query ( 2.58 , 82.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.42 , 6.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_int013-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int013-query , 'Record':U , h_int013-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_int013-viewer ).

       /* Links to SmartQuery h_int013-query. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int013-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int013-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int013-query ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int013-viewer ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i INT013 2.12.01.AVB}
