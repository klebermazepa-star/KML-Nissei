...
*******************************************************************************/
{include/i-prgvrs.i ZINT015 2.06.01.AVB}
...
DEFINE VARIABLE h_INT015-browser AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 18.25 COL 2
     bt-ok AT ROW 19.46 COL 3
     bt-cancela AT ROW 19.46 COL 14
     bt-ajuda AT ROW 19.46 COL 79
     rt-button AT ROW 19.25 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de De->Para Tp Pedido Naturezas"
         HEIGHT             = 19.71
         WIDTH              = 90
         MAX-HEIGHT         = 25.92
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de De->Para Tp Pedido Naturezas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de De->Para Tp Pedido Naturezas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 18.08 , 89.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT015-browser.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT015-browser ).
       RUN set-position IN h_INT015-browser ( 3.00 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 15.17 , 80.14 ) */

       /* Links to SmartBrowser h_INT015-browser. */
       RUN add-link IN adm-broker-hdl ( h_INT015-browser , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT015-browser ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ZINT015" "2.06.01.AVB"}
