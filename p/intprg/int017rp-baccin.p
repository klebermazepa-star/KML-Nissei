/********************************************************************************
** Programa: INT017 - Gera‡Æo notas fiscais a partir de Pedidos do Tutorial/PRS
**
** Versao : 12 - 05/03/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT017RP 2.12.01.AVB}
...
def var c-informacao         as char format "x(120)".
...
procedure pi-importa:
    ...
    for each int-ds-pedido exclusive where 
             int-ds-pedido.situacao     = 1 /* AND 
             int-ds-pedido.ped-codigo-n =  66067230 */ :
        ...
        if  int-ds-pedido.ped-tipopedido-n = 1 or /* transf cd -> filial s¢ pedidos */
            ...
            int-ds-pedido.ped-tipopedido-n = 3 /* BALANCO MANUAL FILIAL */ OR
        ...
        for each int-ds-pedido-produto exclusive of int-ds-pedido,
            ...
            assign  c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                    ...
                    c-nr-docum          = if int-ds-pedido-produto.nen-notafiscal-n <> ? and
                                             int-ds-pedido-produto.nen-notafiscal-n <> 0 then trim(string(int-ds-pedido-produto.nen-notafiscal-n,">>9999999")) else ""
                    c-serie-docum       = if int-ds-pedido-produto.nen-serie-s <> ? then int-ds-pedido-produto.nen-serie-s else ""

                    i-seq-docum         = if int-ds-pedido-produto.nep-sequencia-n <> ? then int-ds-pedido-produto.nep-sequencia-n else 0.
...
procedure valida-registro-pedido.
    ...
    assign  c-uf-origem    = estabelec.estado
            c-nome-abrev   = ""
            i-cod-emitente = 0
            c-nr-tabpre    = ""
            i-canal-vendas = 0
...
procedure valida-registro-item.
   ...
   then do:  /* Nota de origem de uma devolu»’o no Sysfarma */
       ...
       THEN DO:
           ...
           THEN DO:
               ...
               IF int-ds-docto-xml.situacao = 1 AND /* Pendente */
               ...
               THEN DO : 
                    ...
                    assign i-erro = 39
                           ...
                           c-serie-docum + " - " + 
                           c-nr-docum    + " - " + string(i-seq-docum) + " - " + c-it-codigo
                           l-movto-com-erro = yes.
       ...
       ELSE DO: /* Notas <> de PEPSICO */
           ...
           for first item-doc-est 
            ...
            /* item-doc-est.sequencia    = i-seq-docum and */
               ...
               item-doc-est.lote         = c-numero-lote: end.
           if not avail item-doc-est then do:
                assign i-erro = 18
                       c-informacao = " Nao existe " +
                                      c-nr-pedcli + " - " + 
                                      c-serie-docum + " - " + 
                                      c-nr-docum + " - " + 
                                      string(i-seq-docum) + " - " +
   ...
   if avail natur-oper and 
     (natur-oper.especie-doc = "NFD" or
      ...
      c-nr-docum = "" then do:
       assign i-erro = 28
              c-informacao = "Ped: " + c-nr-pedcli + " Nat: " + 
                          c-natur + " Esp: " + natur-oper.especie-doc
           l-movto-com-erro = yes.
       run gera-log. 
       return.
   end.
   ...
   if trim(c-nr-docum) <> "" then do:
        ...
        if not avail item-doc-est then do:
            assign i-erro = 18
                   c-informacao = "Ped: " + c-nr-pedcli + " Ser: " + 
                                  c-serie-docum + " Doc: " + 
                                  c-nr-docum + " Lot: " + 
                                  c-numero-lote + " It: " +
                                  c-it-codigo
...
procedure cria-tt-ped-item-prs.
   ...
   assign tt-ped-item-prs.numero-lote   = c-numero-lote
          ...
          tt-ped-item-prs.nr-docum      = c-nr-docum          
   ...
   assign de-vl-liq         = de-vl-liq + tt-ped-item-prs.vl-liq-it
          de-vl-tot-ped     = de-vl-tot-ped + tt-ped-item-prs.vl-tot-it .
...
procedure pi-busca-saldo-terc:
    ...
    if de-qt-devol > 0 and not l-movto-com-erro then do:
        assign i-erro = 19
               c-informacao = "Ser: " + c-serie-docum + " Doc:" + 
                              c-nr-docum + " Lote: " + 
                              c-numero-lote + " It: " +
                              c-it-codigo
               l-movto-com-erro = yes.
...
procedure pi-devolucoes:
    ...
    assign 
        i-seq-wt-it-last  = i-seq-wt-it-docto
        i-seq-wt-it-docto = 0.
    
    assign i-seq-wt-it-docto = 1.
    for last wt-it-docto
        where wt-it-docto.seq-wt-docto = i-seq-wt-docto no-lock:
...
procedure pi-erro-nota:
    ...
    for each RowErrors:
        ...
        assign i-erro = 22
               c-informacao = "Est: " + c-cod-estabel + " Cli: " + string(i-cod-emitente) + " Ped:" + c-nr-pedcli + " Bo:" 
                              + c-bo + " Nat:" + c-natur + " Ser:" + c-serie + " It:" + c-it-codigo + " - " +
    ...
    IF AVAIL int-ds-docto-xml THEN DO:
            ...
            IF AVAIL tt-nota THEN DO:
                 ...
                 IF AVAIL int-ds-it-docto-xml THEN DO:
                     ...
                     ASSIGN tt-item-doc-est.serie-docto    = tt-docum-est.serie-docto
                            ...
                            tt-item-doc-est.lote           = c-numero-lote
        ...
        THEN DO:
            ...
            FOR EACH int-ds-doc-erro WHERE
                 ...
                 assign i-erro = 22 
                        c-informacao = c-cod-estabel + " - " + c-nr-pedcli + " - " 
                                       + " - " + c-natur + " - " + c-it-codigo + " - " +
                                      string(int-ds-doc-erro.cod-erro) + '-' + 
    ...
    INPUT FROM value(tt-param-re.arquivo) CONVERT TARGET "iso8859-1".
        ...
        FOR EACH tt-erro-nota WHERE
            ...
            BREAK BY tt-erro-nota.cod-erro:
             ...
             assign i-erro = 18
                    c-informacao = tt-erro-nota.nro-docto + " - " + 
                                   tt-erro-nota.serie + " - " + 
                                   STRING(tt-erro-nota.cod-erro) + " - " +
