...
{include/i-prgvrs.i INT017RP 2.12.02.AVB}
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "INT017.txt"
           tt-param.destino = 3
...
def var tb-erro              as char format "x(50)" extent 39 init
                             ["01. Faturado com Sucesso                          ",
                              ...
                              "23. Transportador de redespacho  NAO Cadastrado   ",
...
procedure pi-importa:
    ...
    for each int_ds_pedido no-lock where 
        ...
        if  int_ds_pedido.ped_tipopedido_n = 1 or /* transf cd -> filial s¢ pedidos */
            ...
            int_ds_pedido.ped_tipopedido_n = 2 or /* Retirada loja -> CD - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 19 or  /* Transf Lj->Lj - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 48 or /* Substitiocao Cupom INT048 - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 15 or /* DEVOLUCAO FILIAL - FORNECEDOR - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 32 or /* DEVOLUCAO FILIAL - FORNECEDOR (MED. CONTROLADO) - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 33 or /* TRANSFERENCIA FILIAL - FORNECEDOR (MED. CONTROLADO) - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 39 or /* ATIVO IMOBILIZADO FILIAL => FILIAL - 1 nota X caixa INT047 */
            int_ds_pedido.ped_tipopedido_n = 46 OR /* RETIRADA FILIAL - PROPRIA FILIAL - 1 nota X caixa INT047 */
            ...
            int_ds_pedido.ped_tipopedido_n = 38 OR /* ESTORNO */
            int_ds_pedido.ped_tipopedido_n = 36 OR /* BALAN€O GERAL CONTROLADOS FILIAL */ 
            int_ds_pedido.ped_tipopedido_n = 51 OR /* ESTORNO */ 
            int_ds_pedido.ped_tipopedido_n = 52 OR /* ESTORNO */ 
            int_ds_pedido.ped_tipopedido_n = 53    /* ESTORNO */ then next.
        ...
        for each int_ds_pedido_produto no-lock of int_ds_pedido,
            ...
            assign  c-it-codigo         = trim(string(int_ds_pedido_produto.ppr_produto_n))
                    ...
                    c-numero_caixa      = (if c-numero_caixa = ""
                                           then string(int_ds_pedido_retorno.rpp_caixa_n)
                                           else c-numero_caixa + "," + string(int_ds_pedido_retorno.rpp_caixa_n))
...
procedure valida-registro-item.
   ...
   then do:  /* Nota de origem de uma devolu?’o no Sysfarma */
       ...
       FIND FIRST int_ds_ext_emitente NO-LOCK WHERE
                  int_ds_ext_emitente.cod-emitente = i-cod-emitente NO-ERROR. 
       IF AVAIL int_ds_ext_emitente AND 
                int_ds_ext_emitente.gera-nota = YES  /* Quando for Pepsico Verifica se a nota estÿ pendente de atualiza‡Æo */ 
       THEN DO:
           ...
           THEN DO:
               ...
               IF int_ds_docto_xml.situacao = 0 AND /* Nunca */
                  int_ds_docto_xml.tot-desconto = 0
               THEN DO : 
                 
                    assign i-erro = 39
                           c-informacao = "Pedido: " + c-nr-pedcli   + " - " + 
                           "S‚rie: " + c-serie-docum + " - " + 
                           "Docto.: " + c-nr-docum    + " - " + "Seq.: " + string(i-seq-docum) + " - " + "Item: " + c-it-codigo
                           l-movto-com-erro = yes.
               ...
               ELSE DO:   
                    ...
                    FIND FIRST tt-nota-origem WHERE
                               ...
                               tt-nota-origem.tipo_nota      = int_ds_docto_xml.tipo_nota     AND 
                    ...
                    THEN DO:
                        ...
                        ASSIGN tt-nota-origem.nome-abrev     = c-nome-abrev
                               ...
                               tt-nota-origem.tipo_nota      = int_ds_docto_xml.tipo_nota   
   ...
   */
   ...
   then do:  
       ...
       if not avail item-doc-est then do:
            ...
            if avail int_ds_ext_emitente and
                     int_ds_ext_emitente.gera_nota = yes /* PEPSICO */
            then do:
                /* notas digitadas */
                if can-find (first int_ds_docto_xml no-lock where
                             int_ds_docto_xml.serie        = c-serie-docum and
                             int64(int_ds_docto_xml.nNF)   = int64(c-nr-docum) and
                             int_ds_docto_xml.cod_emitente = i-cod-emitente) then do:
                    ...
                    assign tt-param-int012.arquivo = session:temp-directory + "INT073" + ".tmp":U.
                    raw-transfer tt-param-int012 to raw-param.
                    run intprg/int073rp.p (input raw-param,
                                           input table tt-raw-digita).
                end.
                /* notas XML */
                if can-find (first int_ds_nota_entrada no-lock where 
                             int_ds_nota_entrada.nen_cnpj_origem_s = c-cgc and
                             int_ds_nota_entrada.nen_serie_s       = c-serie-docum and
                             int_ds_nota_entrada.nen_notafiscal_n  = int64(c-nr-docum)) then do:
                    empty temp-table tt-param-int012.
                    create  tt-param-int012.
                    assign  tt-param-int012.destino          = 3
                            tt-param-int012.usuario          = c-seg-usuario
                            ...
                            tt-param-int012.serie-docto-fim  = c-serie-docum
                            tt-param-int012.serie-docto-ini  = c-serie-docum.
                    assign tt-param-int012.arquivo = session:temp-directory + "INT012" + ".tmp":U.
                    raw-transfer tt-param-int012 to raw-param.
                    run intprg/int012rp.p (input raw-param,
                                           input table tt-raw-digita).
                end.
   ...
   if trim(c-nr-docum) <> "" then do:
        for first item-doc-est no-lock where 
            ...
            /*item-doc-est.sequencia    = i-seq-docum    and*/
            item-doc-est.it-codigo    = c-it-codigo    and
            item-doc-est.lote         = c-numero_lote: 
            i-seq-docum = item-doc-est.sequencia.
        end.
        if not avail item-doc-est then do:
            assign i-erro = 18
...
procedure cria-tt-ped-item-prs.
   ...
   for first tt-ped-item-prs where
       ...
       tt-ped-item-prs.numero_lote  = c-numero_lote: end.
            ...
            */
            run criaAlteraWtFatSerLote in h-bodi317sd 
                    (input yes,
                     ...
                     input "",
                     input if item.tipo-con-est = 3 then tt-ped-item-prs.numero_lote else "",
...
/* Atualiza as quantidades na nota de origem e gera uma nota de entrada com uma 
   serie espec­fica para devolu?’o cria um registro no int0002 e atualiza automaticamente no re1001 */
    ...
    FIND FIRST int_ds_docto_xml NO-LOCK WHERE
               ...
               int_ds_docto_xml.tipo_nota     = tt-nota-origem.tipo_nota     AND 
    ...
    IF AVAIL int_ds_docto_xml THEN DO:
        ...
        FOR EACH int_ds_pedido_produto NO-LOCK WHERE
            ...
            ASSIGN c-numero_lote = if trim(int_ds_pedido_retorno.rpp_lote_s) <> "0" and 
                                      trim(int_ds_pedido_retorno.rpp_lote_s) <> "?" and
                                      trim(int_ds_pedido_retorno.rpp_lote_s) <> ? then trim(int_ds_pedido_retorno.rpp_lote_s) else "".
            ...
            FIND FIRST int_ds_it_docto_xml EXCLUSIVE-LOCK WHERE
                       ...
                       int_ds_it_docto_xml.it_codigo    = string(int_ds_pedido_produto.ppr_produto_n) NO-ERROR.
            IF AVAIL int_ds_it_docto_xml 
            THEN DO:
                IF int_ds_pedido_produto.ppr_quantidade_n >= int_ds_it_docto_xml.qcom 
                THEN ASSIGN int_ds_it_docto_xml.situacao = 4. /* Devolvida */
                ELSE ASSIGN int_ds_it_docto_xml.qcom = int_ds_it_docto_xml.qcom - int_ds_pedido_produto.ppr_quantidade_n.
            ...
            IF AVAIL tt-nota THEN DO:
                 ...
                 IF AVAIL int_ds_it_docto_xml THEN DO:
                     ...
                     for first item fields (it-codigo tipo-con-est) no-lock where 
                         item.it-codigo = int_ds_it_docto_xml.it_codigo: end.
                     ...
                     ASSIGN tt-item-doc-est.serie-docto    = tt-docum-est.serie-docto
                            ...
                            tt-item-doc-est.sequencia      = int_ds_it_docto_xml.sequencia
                            tt-item-doc-est.it-codigo      = int_ds_it_docto_xml.it_codigo
                            tt-item-doc-est.lote           = if item.tipo-con-est = 3 /* lote */ then c-numero_lote else ""
                            tt-item-doc-est.num-pedido     = int_ds_it_docto_xml.num_pedido    
                            tt-item-doc-est.numero-ordem   = int_ds_it_docto_xml.numero_ordem
                            tt-item-doc-est.numero-ordem   = 0
                            tt-item-doc-est.parcela        = 0.
                          
                     find first item no-lock where
                                item.it-codigo = int_ds_it_docto_xml.it_codigo no-error. 

                     FIND FIRST item-uni-estab NO-LOCK WHERE
                                item-uni-estab.cod-estabel = int_ds_docto_xml.cod_estab AND
                                item-uni-estab.it-codigo   = int_ds_it_docto_xml.it_codigo NO-ERROR.
        ...
        FOR EACH int_ds_doc_erro WHERE
                 ...
                 int_ds_doc_erro.tipo_nota    = tt-nota.tipo_nota   :
                   ...
                   THEN DO:
                        ...
                        THEN DO:
                            ...
                            FOR EACH tt-docto-xml:
                                ...
                                /*ASSIGN int_ds_docto_xml.id_sequencial = NEXT-VALUE(seq-int-ds-docto-xml). /* Prepara‡Æo para integra‡Æo com Procfit */*/
