...
/********************************************************************************
** Programa: int018 - Cadastro de->para CUPOM FISCAL X Nat. Opera‡Æo 
**                    Tutorial/PRS -> Datasul
**
** Versao : 12 - 03/03/2016 - Alessandro V Baccin
**
********************************************************************************/
{include/i-prgvrs.i INT018 2.06.02.AVB}
...
DEFINE VARIABLE h_INT018-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_INT018-viewer AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Naturezas de Opera‡Æo X Emitente"
         HEIGHT             = 9.67
         WIDTH              = 100.86
         MAX-HEIGHT         = 25.33
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Naturezas de Opera‡Æo X Emitente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Naturezas de Opera‡Æo X Emitente */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 83.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT018-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT018-viewer ).
       RUN set-position IN h_INT018-viewer ( 3.04 , 2.86 ) NO-ERROR.
       /* Size in UIB:  ( 6.75 , 97.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT018-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int018-zoom.w,
                     ProgVaPara = intprg/int018-vapara.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_INT018-query ).
       RUN set-position IN h_INT018-query ( 1.00 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.71 , 10.43 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_INT018-viewer. */
       RUN add-link IN adm-broker-hdl ( h_INT018-query , 'Record':U , h_INT018-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_INT018-viewer ).

       /* Links to SmartQuery h_INT018-query. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_INT018-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_INT018-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_INT018-query ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT018-viewer ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT018" "2.06.02.AVB"}
