...
def temp-table tt-int_ds_nota_loja-8 like int_ds_nota_loja
...
procedure pi-importa:
    ...
    EMPTY TEMP-TABLE tt-int_ds_nota_loja-8.

    FOR EACH estabelec USE-INDEX codigo WHERE 
             estabelec.cod-estabel >= int_ds_ger_cupom.cod_estab_ini_8 AND
             estabelec.cod-estabel <= int_ds_ger_cupom.cod_estab_fim_8 NO-LOCK:
        ...
        for each int_ds_nota_loja no-lock USE-INDEX sit_cnpj_emiss where 
            ...
            FOR FIRST tt-int_ds_nota_loja-8 WHERE 
                tt-int_ds_nota_loja-8.rowid-nota-loja = ROWID(int_ds_nota_loja): END.
            IF NOT AVAIL tt-int_ds_nota_loja-8 THEN DO:
                CREATE tt-int_ds_nota_loja-8.
                BUFFER-COPY int_ds_nota_loja TO tt-int_ds_nota_loja-8.
                ASSIGN tt-int_ds_nota_loja-8.rowid-nota-loja = ROWID(int_ds_nota_loja).
            END.

        end.
    END.

    FOR EACH tt-int_ds_nota_loja-8 USE-INDEX data:
        FOR FIRST int_ds_nota_loja WHERE ROWID(int_ds_nota_loja) = tt-int_ds_nota_loja-8.rowid-nota-loja NO-LOCK:
        END.
