/********************************************************************************
** Programa: INT020manual-rp - Geracao notas fiscais a partir de Cupom Fiscal PRS
**
** Versao : 12 - 20/03/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT0manual-rp 2.12.01.AVB}
{cdp/cdcfgdis.i}
...
DEF VAR i-reg AS INT FORMAT ">,>>>,>>9" NO-UNDO.
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "INT020manual-rp.txt"
           tt-param.destino = 3
...
assign c-programa       = "INT020manual-rp"
       c-versao         = "2.12"
       c-revisao        = ".01.AVB"
...
procedure pi-importa:
    ...
    for each int-ds-nota-loja no-lock USE-INDEX situacao where 
             int-ds-nota-loja.situacao = 1 AND
             int-ds-nota-loja.emissao  >= 11/01/2017
             query-tuning(no-lookahead):
            
        ASSIGN i-reg = i-reg + 1.

        run pi-acompanhar in h-acomp (input "Carregando informa‡äes: " + STRING(i-reg)).
