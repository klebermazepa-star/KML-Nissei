           ...
           de-valor-vale    <> 0 then do:
            for each tt-int-ds-nota-loja-cartao no-lock where 
                ...
                IF AVAIL tt-docto THEN
                   ASSIGN tt-docto.vl-mercad = tt-docto.vl-mercad - tt-int-ds-nota-loja-cartao.taxa_admin.  

            end.
        end.
        else do i-cont = 1 to i-num-parcelas:
            assign i-seq-fat-duplic = i-seq-fat-duplic + 1.
            create tt-fat-duplic.
            assign tt-fat-duplic.seq-tt-fat-duplic = i-seq-fat-duplic
                   tt-fat-duplic.seq-tt-docto      = i-nr-registro
                   ...
                   tt-fat-duplic.vl-parcela        = de-vl-tot-cup * de-per-pg-dup[i-cont] / 100
                   tt-fat-duplic.vl-comis          = 0
                   tt-fat-duplic.vl-acum-dup       = de-vl-tot-dup + tt-fat-duplic.vl-parcela
       ...
       de-valor-vale    <> 0) then do:
        if not can-find(first int-ds-nota-loja-cartao no-lock where
            ...
            for each int-ds-nota-loja-cartao no-lock where 
                ...
                for first int-ds-loja-natur-oper no-lock where rowid(int-ds-loja-natur-oper) = r-rowid: end.
                if avail int-ds-loja-natur-oper then do:
                    assign tt-int-ds-nota-loja-cartao.cod-esp = trim(int-ds-loja-natur-oper.cod-esp).
                    /*
                    if tt-int-ds-nota-loja-cartao.cod-esp = "" or 
                       not can-find (first esp-doc no-lock where 
                                     esp-doc.cod-esp = trim(int-ds-loja-natur-oper.cod-esp)) then do:
                        assign i-erro = 43
                               c-informacao = c-num-nota    + " - " + 
                                              " "           + " - " + 
                                              tt-int-ds-nota-loja-cartao.cod-esp
                               l-cupom-com-erro = yes.
                        run gera-log. 
                        return.
                    end.
                    */
                    ...
                    for first cond-pagto fields (cod-vencto) no-lock where 
                        cond-pagto.cod-cond-pag = int-ds-loja-natur-oper.cod-cond-pag:
                        assign tt-int-ds-nota-loja-cartao.cod-vencto   = cond-pagto.cod-vencto
                               tt-int-ds-nota-loja-cartao.cod-cond-pag = int-ds-loja-natur-oper.cod-cond-pag.
                    end.
                    ...
                    assign tt-int-ds-nota-loja-cartao.cod-portador  = int-ds-loja-natur-oper.cod-portador
                           tt-int-ds-nota-loja-cartao.modalidade    = int-ds-loja-natur-oper.modalidade.
                ...
                if not avail int-ds-loja-natur-oper or not avail natur-oper then do:
                    assign i-erro = 21
                           c-informacao = c-num-nota    + " - " + 
                                          " "           + " - " + 
   ...
   */
   ...
   for first int-ds-loja-natur-oper no-lock where rowid(int-ds-loja-natur-oper) = r-rowid:
       ...
       if  c-cod-esp-princ = "" or i-cod-portador = ? or i-cod-cond-pag = ? then do:
           assign  c-serie         = int-ds-loja-natur-oper.serie
                   ...
                   c-cod-esp-princ = int-ds-loja-natur-oper.cod-esp.
           ...
           for first ser-estab /*exclusive-lock*/ no-lock where
   ...
   if not avail natur-oper or not avail int-ds-loja-natur-oper then do:
       assign i-erro = 21
              c-informacao = c-num-nota    + " - " + 
                             c-natur       + " - " + 
                             c-condipag    + " - " + 
