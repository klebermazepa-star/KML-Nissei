...
DEF VAR c-codigo-convenio              AS CHARACTER   NO-UNDO.
        ...
        for each tt-int_ds_nota_loja_cartao no-lock where 
            ...
            buffer-copy tt-fat-duplic to cst_fat_duplic
                assign cst_fat_duplic.cod_portador  = tt-int_ds_nota_loja_cartao.cod-portador
                       ...
                       cst_fat_duplic.autorizacao   = tt-int_ds_nota_loja_cartao.autorizacao
                       cst_fat_duplic.condipag      = tt-int_ds_nota_loja_cartao.condipag
                       cst_fat_duplic.nsu_admin     = tt-int_ds_nota_loja_cartao.nsu_admin /* retirar apos aumentado o campo */
                       cst_fat_duplic.nsu_numero    = tt-int_ds_nota_loja_cartao.nsu_numero /* retirar apos aumentado o campo */
                       cst_fat_duplic.taxa_admin    = tt-int_ds_nota_loja_cartao.taxa_admin
                       cst_fat_duplic.cod_estabel   = c-cod-estabel
                       cst_fat_duplic.serie         = c-serie
                       cst_fat_duplic.nr_fatura     = c-num-nota.
            IF cst_fat_duplic.nsu_numero = "" OR cst_fat_duplic.nsu_numero = "?"  THEN DO:
                ASSIGN cst_fat_duplic.nsu_numero    = tt-int_ds_nota_loja_cartao.nsu_admin.
            END.
        end.
...
procedure valida-registro-cabecalho.
   ...
   /* /* validar totais do cupom se estabelecimento Ç Procfit */
    ...
    FIND first ser-estab no-lock where
         ser-estab.serie = c-serie and
         ser-estab.cod-estabel = c-cod-estabel NO-ERROR.
    if not avail ser-estab then do:
       assign i-erro = 36   
              c-informacao = "Nr. Nota: "   + c-num-nota        + " - " + 
                             "Estab: "      + c-cod-estabel     + " - " +
                             "SÇrie: "      + IF c-serie = ? THEN "?" ELSE c-serie 
              l-cupom-com-erro = yes.
       run gera-log(OUTPUT i-status,
                    OUTPUT c-retorno). 
       return.
    ...
    for first param-estoq no-lock query-tuning(no-lookahead): 
        ...
        if (c-aux = string(year(d-dt-emissao),"9999") + string(month(d-dt-emissao),"99") and
           ...
           /* ATIVAR QUANDO ê NECESSµRIO ALTERAR DATA DOS CUPONS FISCAIS  
           
           
           IF c-convenio = "N" and int_ds_nota_loja.emissao < 02/01/2019 THEN DO:

                FIND FIRST b_int_ds_nota_loja EXCLUSIVE-LOCK
                    WHERE ROWID(b_int_ds_nota_loja) = ROWID(int_ds_nota_loja) NO-ERROR.
                ASSIGN b_int_ds_nota_loja.emissao = 02/01/2019.  
    ...
    /*  KML - 13/09/2024  - Retirado regra para o projeto VNDA
    ...
    */
    ...
    MESSAGE "Antes DA VALIDAÄ«O - KML"
        VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
       ...
       de-valor-vale    <> 0 )*/ then do:
        ...
        if trim(c-condipag) <> "" and c-condipag <> ? and
           trim(c-condipag) <> "0" then do:
            ...
            MESSAGE "Antes DA VALIDAÄ«O - KML 2"
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
                            ...
                            ELSE DO:
                                ...
                                ELSE DO:
                                    ...
                                    FOR EACH int_ds_nota_loja_cartao NO-LOCK WHERE 
                                        ...
                                        MESSAGE "TESTE KML " SKIP
                                                int_ds_nota_loja_cartao.cod_adquirente
                                            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
        ...
        /* foráar convànio como condiá∆o proncipal do cupom para ft0603 - AVB 15/01/2019 */
        for first tt-int_ds_nota_loja_cartao no-lock where 
            tt-int_ds_nota_loja_cartao.cnpj_filial = int_ds_nota_loja.cnpj_filial   and
            tt-int_ds_nota_loja_cartao.serie       = int_ds_nota_loja.serie         and
            tt-int_ds_nota_loja_cartao.num_nota    = int_ds_nota_loja.num_nota      and
            tt-int_ds_nota_loja_cartao.cod-esp     = "CV":
            for first int_ds_loja_cond_pag no-lock where 
                int_ds_loja_cond_pag.condipag = trim(string(int(tt-int_ds_nota_loja_cartao.condipag),">>>99"))
                query-tuning(no-lookahead):
                ...
                assign  i-cod-cond-pag  = int_ds_loja_cond_pag.cod_cond_pag 
                        ...
                        i-modalidade    = int_ds_loja_cond_pag.modalidade
                        c-cod-esp-princ = int_ds_loja_cond_pag.cod_esp.
                        c-condipag      = trim(string(int(int_ds_loja_cond_pag.condipag),">>>99")).
        ...
        end. /* firs int_ds_nota_loja_cartao...CV */

        /* INICIO - foráar CHEQUE como condiá∆o proncipal do cupom para ft0603, caso n∆o exista nenhum convenio relacionado ao Cupom - 13/03/2019 */
        FIND FIRST tt-int_ds_nota_loja_cartao no-lock where 
                   tt-int_ds_nota_loja_cartao.cnpj_filial = int_ds_nota_loja.cnpj_filial   and
                   tt-int_ds_nota_loja_cartao.serie       = int_ds_nota_loja.serie         and
                   tt-int_ds_nota_loja_cartao.num_nota    = int_ds_nota_loja.num_nota      and
                   tt-int_ds_nota_loja_cartao.cod-esp     = "CV" NO-ERROR.
        IF NOT AVAIL tt-int_ds_nota_loja_cartao THEN DO:

            FOR FIRST tt-int_ds_nota_loja_cartao no-lock where 
                      tt-int_ds_nota_loja_cartao.cnpj_filial = int_ds_nota_loja.cnpj_filial   and
                      tt-int_ds_nota_loja_cartao.serie       = int_ds_nota_loja.serie         and
                      tt-int_ds_nota_loja_cartao.num_nota    = int_ds_nota_loja.num_nota      and
                      tt-int_ds_nota_loja_cartao.cod-esp     = "CE":
            ...
            end. /* firs int_ds_nota_loja_cartao...CE */
        END.
        /* FIM    - foráar CHEQUE como condiá∆o proncipal do cupom para ft0603, caso n∆o exista nenhum convenio relacionado ao Cupom - 13/03/2019 */
    ...
    if (c-estado = "PR" or 
        c-estado = "SP" /* AVB 15/01/2019 */) and 
        int_ds_nota_loja.nfce_chave_s  /* chave acesso */ = ? 
...
*/
   ...
   */
   ...
   /* Alteraá∆o kleber para tipos de pedidos PapaFila */
   FIND FIRST ext_serie NO-LOCK
       WHERE ext_serie.serie = c-serie NO-ERROR.

   IF AVAIL ext_serie THEN DO:

       IF ext_serie.tp-ped-papafila <> "" THEN DO:
           ASSIGN c-tp-pedido = ext_serie.tp-ped-papafila .
       END.
   END.                                                    
...
/*   /* Alteraá∆o Kleber para atribuir natureza de consignado */
   ...
   */
   ...
   for first natur-oper 
       ...
       /*assign c-cod-esp-princ = natur-oper.cod-esp. AVB 15/01/2019 - deve ficar espÇcie das condiá‰es de pagamento */
   end.
...
procedure cria-tt-docto.
    ...
    FOR EACH int_ds_nota_loja_cartao OF int_ds_nota_loja NO-LOCK:
        
            IF int_ds_nota_loja_cartao.condipag = "12" THEN
                
                ASSIGN c-codigo-convenio = "".
                
            ELSE IF int_ds_nota_loja_cartao.condipag = "83" THEN
            
                ASSIGN c-codigo-convenio = "000985".
                 
            //KML - 13/01/2026 - ITAIPU convertida para Medme 
            //ELSE IF int_ds_nota_loja_cartao.condipag = "65" THEN 
            //
            //    ASSIGN c-codigo-convenio = "007934".
    ...
    assign tt-cst_nota_fiscal.cod_estabel       = c-cod-estabel
           ...
           tt-cst_nota_fiscal.convenio          = c-codigo-convenio 
           ...
           tt-cst_nota_fiscal.caixa              = int_ds_nota_loja.caixa    
           tt-cst_nota_fiscal.loja               = int_ds_nota_loja.loja     
           tt-cst_nota_fiscal.movimento          = int_ds_nota_loja.movimento
           tt-cst_nota_fiscal.venda              = int_ds_nota_loja.venda 
           tt-cst_nota_fiscal.data_emis_ori      = int_ds_nota_loja.movimento   
                                                          .                                                                        .
...
procedure cria-tt-it-docto.
   ...
   */
   ...
   assign tt-it-imposto.vl-biss-it        = 0
          ...
          tt-it-imposto.vl-bsubs-it       = IF int_ds_nota_loja_item.vl_cst60 > 0 THEN de-base-icms ELSE 0    /*KML - Base CST60 */ 
          tt-it-imposto.icm-complem       = 0
          tt-it-imposto.vl-icms-ent-fut   = 0
          tt-it-imposto.vl-icmsub-ent-fut = 0
          tt-it-imposto.vl-icmsub-it      = IF int_ds_nota_loja_item.vl_cst60 > 0 THEN int_ds_nota_loja_item.vl_cst60 ELSE 0     /*KML - Valor CST60 */ 
...
procedure importa-nota.    
    ...
    for each tt-notas-geradas query-tuning(no-lookahead):
        for first nota-fiscal exclusive-lock where 
            ...
            log-manager:write-message("KML - nota foi criada - " + nota-fiscal.nr-nota-fis ) no-error.
  	    ...
  	    /* acertar documento inutilizado */
            ...
            FOR EACH it-nota-fisc OF nota-fiscal NO-LOCK:
                  ...
                  log-manager:write-message("KML - item sendo ajustado - " + nota-fiscal.nr-nota-fis ) no-error.
                  ...
                  log-manager:write-message("KML - item ajustado com sucesso - " + nota-fiscal.nr-nota-fis ) no-error.
            ...
            for each cst_fat_duplic no-lock where 
                ...
                for each fat-duplic exclusive where 
                    ...
                    assign fat-duplic.int-1 = cst_fat_duplic.cod_portador
                           ...
                           fat-duplic.vl-parcela = fat-duplic.vl-parcela-me. // KML - 20/07/2023 - Corrigindo diferenáa de centavos
                ...
                log-manager:write-message("KML - duplicata foi ajustada - " + nota-fiscal.nr-nota-fis ) no-error.
            end.
            ...
            for each it-nota-fisc EXCLUSIVE of nota-fiscal:
                ...
                log-manager:write-message("KML - antes ajustar impostos do item - " + nota-fiscal.nr-nota-fis ) no-error.
                ...
                */
                ...
                log-manager:write-message("KML - impostos dos itens ajustados - " + nota-fiscal.nr-nota-fis ) no-error.
            end.
            
            //  KML 02/09/2024  - Projeto cart∆o presente.
                
            FIND FIRST estabelec  NO-LOCK 
                WHERE estabelec.cod-estabel = nota-fiscal.cod-estabel NO-ERROR.
            
            FOR EACH int_ds_nota_loja_item NO-LOCK     //  s¢ entrar quando tiver transaction id do cart∆o presente
                WHERE int_ds_nota_loja_item.cnpj_filial             = estabelec.cgc 
                  AND int_ds_nota_loja_item.serie                   = nota-fiscal.serie 
                  AND int_ds_nota_loja_item.num_nota                = trim(string(int(nota-fiscal.nr-nota-fis),"999999")) 
                  AND int_ds_nota_loja_item.produto                 = it-nota-fisc.it-codigo
                  AND int_ds_nota_loja_item.sequencia               = string(it-nota-fisc.nr-seq-fat / 10)
                  AND int_ds_nota_loja_item.transID_cartao_presente <> "": 
                  
                  
                FIND FIRST cst_cartao_presente NO-LOCK NO-ERROR.
                    
                 
                CREATE cst_it_nota_fisc.
                ASSIGN cst_it_nota_fisc.cod-estabel                = it-nota-fisc.cod-estabel
                       cst_it_nota_fisc.serie                      = it-nota-fisc.serie
                       cst_it_nota_fisc.nr-nota-fis                = it-nota-fisc.nr-nota-fis
                       cst_it_nota_fisc.it-codigo                  = it-nota-fisc.it-codigo
                       cst_it_nota_fisc.nr-seq-fat                 = it-nota-fisc.nr-seq-fat
                       cst_it_nota_fisc.dt-emis-nota               = nota-fiscal.dt-emis-nota
                       cst_it_nota_fisc.transID_cartao_presente    = int_ds_nota_loja_item.transID_cartao_presente
                       cst_it_nota_fisc.valor_total                = it-nota-fisc.vl-tot-item
                       cst_it_nota_fisc.valor_repasse              = IF AVAIL cst_cartao_presente THEN it-nota-fisc.vl-tot-item * cst_cartao_presente.percentual_repasse / 100 ELSE 0
                       cst_it_nota_fisc.percentual_repasse         = IF AVAIL cst_cartao_presente THEN cst_cartao_presente.percentual_repasse ELSE 0
                       .
            ...
            for first estabelec fields (cod-estabel cgc) no-lock where 
                ...
                log-manager:write-message("KML - antes de efetivar a nota com sucesso - " + nota-fiscal.nr-nota-fis ) no-error.
                ...
                for each int_ds_nota_loja where
                        ...
                        int_ds_nota_loja.num_nota    = trim(string(int(nota-fiscal.nr-nota-fis),"9999999")) and
                        ...
                        log-manager:write-message("KML - efetivado com sucesso - " + nota-fiscal.nr-nota-fis ) no-error.
...
procedure gera-log.
   ...
   ASSIGN i-status  = i-erro
          c-retorno = substring(trim(tb-erro[i-erro]) + " - " + c-informacao,1,500).
   ...
   IF AVAIL int_ds_nota_loja THEN DO:
       IF int_ds_nota_loja.situacao = 2 THEN DO:
           ASSIGN int_ds_nota_loja.envio_status      = 8
                  int_ds_nota_loja.retorno_validacao = "".
       END.
       ELSE IF int_ds_nota_loja.situacao = 1 THEN DO:
           ASSIGN int_ds_nota_loja.envio_status      = 9
                  int_ds_nota_loja.retorno_validacao = substring(trim(tb-erro[i-erro]) + " - " + c-informacao,1,500).
       END.
   END.
   ...
   for each  int_ds_log_aux use-index orig_chave_sit_dt 
       where int_ds_log_aux.origem = "CUPOM" 
         and int_ds_log_aux.chave  = c-chave:
      assign int_ds_log_aux.dt_movto = int_ds_nota_loja.emissao.
      IF int_ds_log_aux.dt_movto = ? THEN
         ASSIGN int_ds_log_aux.dt_movto = int_ds_log_aux.dt_ocorrencia.
   end.
       ...
       for first nota-fiscal fields (cod-estabel nr-fatura
         ...
         for each cst_fat_duplic no-lock where 
             ...
             for each fat-duplic exclusive where 
                 ...
                 assign fat-duplic.int-1 = cst_fat_duplic.cod_portador
                        fat-duplic.int-2 = cst_fat_duplic.modalidade
                        fat-duplic.vl-parcela = fat-duplic.vl-parcela-me. // Adicionando devido ao safra
         ...
         assign cst_nota_fiscal.condipag            = int_ds_nota_loja.condipag
                ...
                cst_nota_fiscal.caixa              = int_ds_nota_loja.caixa    
                cst_nota_fiscal.loja               = int_ds_nota_loja.loja     
                cst_nota_fiscal.movimento          = int_ds_nota_loja.movimento
                cst_nota_fiscal.venda              = int_ds_nota_loja.venda     .
       end.
       for each it-nota-fisc exclusive of nota-fiscal
           query-tuning(no-lookahead):
