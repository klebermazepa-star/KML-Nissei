...
def var i-seq-fat-duplic as integer.
...
procedure pi-importa:
    ...
    for each int-ds-nota-loja exclusive where 
       ...
       /* 
        ...
        /*run finaliza-cupom.*/
        if i-nr-registro mod 2000 = 0 then do:
            run importa-nota.
        end.
    end.
    run importa-nota.
...
procedure valida-registro-cabecalho.
    ...
    assign  c-nome-abrev = if int-ds-nota-loja.valor_convenio <> 0 
                           then "CONSUMIDOCV" else "CONSUMIDOR"
            i-cod-emitente = 0
            i-canal-vendas = 0
            i-cod-rep      = 1.
    ...
    for first emitente fields (cod-emitente nome-abrev cod-canal-venda 
        ...
        if not can-find(unid-feder 
           ...
           run pi-erros-nota (input 56, input trim(return-value) + " nÆo cadastrado").
           return.
        end.      
    end.
    ...
    if de-valor-cartaod <> 0 or de-valor-cartaoc <> 0 then do:
        ...
        if not avail int-ds-nota-loja-cartao then do:
            ...
            assign i-erro = 40
                   c-informacao = c-num-nota + " - " + c-cod-estabel + " - " + 
                                  string(int-ds-nota-loja-cartao.valor) + " - " + string(de-valor-cartaoc) + " - " + string(de-valor-cartaod)
                   l-cupom-com-erro = yes.
            run gera-log. 
            return.
        end.
   ...
   */
   ...
   if can-find(nota-fiscal
      ...
      and   nota-fiscal.nr-nota-fis = c-num-nota) then do:
       ...
       run pi-erros-nota (input 1, input trim(return-value) + " j  existe").
       return.
   end.
...
procedure cria-tt-docto.
    ...
    create tt-docto.
    assign tt-docto.fat-nota         = IF serie.ind-nff then 1 /* NFF */ELSE 2
...
procedure importa-nota.
    ...
    if  can-find (first tt-docto) and
        can-find (first tt-it-docto) then
