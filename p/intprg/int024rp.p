...
{include/i-prgvrs.i INT024RP 2.12.04.AVB}
{cdp/cdcfgdis.i}
...
for each int-ds-devolucao-cupom WHERE
    ...
    /*
    and int-ds-devolucao-cupom.cnpj_filial_dev = "79430682002842" 
    AND int-ds-devolucao-cupom.numero_dev = "046546" 
    AND int-ds-devolucao-cupom.numero_cup = "013835"
    */
    ...
    for each estabelec 
        ...
        cst-estabelec.dt-fim-operacao >= /*int-ds-devolucao-cupom.data*/ TODAY
        query-tuning(no-lookahead):
    ...
    if not avail item-estab and not avail item-uni-estab then do:
        run pi-gera-log (input "Item n∆o cadastrado no estabelecimento. Item: " + string(int(int-ds-devolucao-cupom.produto)) + "/" + "Estab.: " + c-cod-estab-dev,
                         input 1).
        next.
    end.
    ...
    for each ser-estab no-lock where 
        ...
        for each it-nota-fisc no-lock where 
            ...
            it-nota-fisc.dt-emis-nota >= today - 360
            query-tuning(no-lookahead): 
    ...
    if last-of (int-ds-devolucao-cupom.numero_dev)  then do:
        ...
        if avail docum-est then do:
            run pi-gera-log (input "Documento de estoque j† cadastrado. Filial: " + 
                             c-cod-estab-dev + " Emitente: " + string(docum-est.cod-emitente) + 
