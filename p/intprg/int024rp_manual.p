...
for EACH int-ds-devolucao-cupom WHERE
    int-ds-devolucao-cupom.situacao = 9
    /*
    and int(int-ds-devolucao-cupom.numero_dev) = 55
    AND int-ds-devolucao-cupom.cnpj_filial_dev <> int-ds-devolucao-cupom.cnpj_filial_cup
    */
    ...
    /*
    DISPLAY 
        int-ds-devolucao-cupom.cnpj_filial_dev
        int-ds-devolucao-cupom.cnpj_filial_cup
        int-ds-devolucao-cupom.numero_dev
        int-ds-devolucao-cupom.numero_cup
        int-ds-devolucao-cupom.produto
        int-ds-devolucao-cupom.quantidade
        int-ds-devolucao-cupom.sequencia
        WITH WIDTH 300 STREAM-IO FRAME f-24.
    DOWN WITH FRAME f-24.
    */
        ...
        */
        ...
        do on error undo, leave:
            ...
            for first docum-est exclusive-lock of tt-docum-est-nova:
                        ...
                        if cst-fat-devol.cod-estabel = cst-fat-devol.cod-estab-comp then 
                            ...
                            for last item-doc-est no-lock where 
                                ...
                                FOR EACH nota-fiscal EXCLUSIVE-LOCK WHERE
                                    nota-fiscal.cod-emitente = cst-fat-devol.cod-emitente and
                                    nota-fiscal.serie        = cst-fat-devol.serie-docto  and
                                    nota-fiscal.nat-operacao = cst-fat-devol.nat-operacao and
                                    nota-fiscal.nr-nota-fis  = item-doc-est.nro-docto:
                ...
                run pi-gera-log("Estab: " + tt-movto.cod-estabel +
                                " Emitente: " + trim(string(tt-movto.cod-emitente)) +
                                " SÇrie: " + tt-movto.serie        +
                                " NF: "  + tt-movto.nr-nota-fis  +
                ...
                for each bint-ds-devolucao-cupom exclusive where
                    ...
                    query-tuning(no-lookahead):
                    assign  bint-ds-devolucao-cupom.situacao = 3 /* processada */.
                    release bint-ds-devolucao-cupom.
                end.
            end.
            run pi-elimina-tt.
    ...
    for each tt-movto break by tt-movto.cod-esp:
        ...
        if last-of (tt-movto.cod-esp) then do:
            ...
            assign  tt-cst-fat-devol.cod-estabel    = tt-movto.cod-estabel
                    ...
                    tt-cst-fat-devol.flag-atualiz   = YES.
        end.
    end.
