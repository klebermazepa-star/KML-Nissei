...
/*
FOR FIRST estabelec WHERE estabelec.cod-estabel = "010":
    ASSIGN c-cgc = estabelec.cgc.
END.
*/
for-dev:
for each int-ds-devolucao-cupom where
    int-ds-devolucao-cupom.situacao = 1
    /*
    AND int-ds-devolucao-cupom.cnpj_filial_dev = c-cgc
    */

    /*and int(int-ds-devolucao-cupom.numero_dev) = 426*/
    ...
    for first item 
        fields (it-codigo class-fiscal tipo-con-est peso-liquido aliquota-ipi ct-codigo
                conta-aplicacao tipo-contr cd-trib-icm cd-trib-ipi cd-trib-iss sc-codigo)
        no-lock where 
    ...
    for each ser-estab no-lock where 
        ...
        for each it-nota-fisc no-lock where 
            ...
            it-nota-fisc.nr-nota-fis = trim(string(int(int-ds-devolucao-cupom.numero_cup),">>9999999")) and
    ...
    if not can-find(first tt-it-nota-fisc no-lock where
                    ...
                    tt-it-nota-fisc.nr-nota-fis = trim(string(int(int-ds-devolucao-cupom.numero_cup),">>9999999")) and
    ...
    for first tt-it-nota-fisc no-lock where 
        ...
        assign tt-movto.cod-estabel     = c-cod-estabel
               ...
               tt-movto.valor-mercad    = if int-ds-devolucao-cupom.valor_unit <> 0 
                                          ...
                                          else it-nota-fisc.vl-preuni * int-ds-devolucao-cupom.quantidade
    ...
    */             
    ...
    for first nota-fiscal no-lock of tt-it-nota-fisc: 
       ...
       */
       ...
       assign tt-movto.cod-emitente = i-cod-cliente.
       ...
       for last ser-estab no-lock where 
           ...
           ser-estab.forma-emis = 1: /* automatica */
        ...
        for each cst-fat-devol no-lock where
            ...
            if not avail it-nota-fisc then do:
                for each docum-est no-lock where 
                    ...
                    item-doc-est.it-codigo = tt-movto.it-codigo and
                    item-doc-est.nr-pedcli = tt-movto.nr-nota-fis,
...
procedure PiGeraMovimento:
    ...
    for each tt-movto
        ...
        assign tt-item-doc-est-nova.registro           = 3
               ...
               tt-item-doc-est-nova.base-pis           = 0
               tt-item-doc-est-nova.valor-pis          = 0
               tt-item-doc-est-nova.aliquota-pis       = 0
               tt-item-doc-est-nova.base-cofins        = 0
               tt-item-doc-est-nova.valor-cofins       = 0
               tt-item-doc-est-nova.aliquota-cofins    = 0
...
procedure pi-gera-tt-fat-devol:
    ...
    for each tt-movto break by tt-movto.cod-esp:
        ...
        if last-of (tt-movto.cod-esp) then do:
            ...
            assign  tt-cst-fat-devol.cod-estabel    = c-cod-estabel
