...
display stream str-rp
        ...
        SKIP(1)
        WITH NO-BOX STREAM-IO down width 300.

IF tt-param.l-pendentes = NO THEN DO:
   for each tt-int_ds_nota_entrada where 
            tt-int_ds_nota_entrada.situacao = 2 AND 
            tt-int_ds_nota_entrada.nen_observacao <> "Aguardando Retorno Conferencia - PROCFIT":
   
       run pi-acompanhar in h-acomp (input "Atualizadas: " + string(tt-int_ds_nota_entrada.nen_notafiscal_n) + "/" + tt-int_ds_nota_entrada.nen_serie_s).
   
       for first emitente no-lock where emitente.cgc = tt-int_ds_nota_entrada.nen_cnpj_origem_s: 
       end.
   
       ASSIGN c-sistema-destino = ""
              c-cod-estabel     = "".
       for each estabelec 
       ...
       display stream str-rp
           ...
           with NO-BOX width 550 stream-io down.
   end.
END.

put stream str-rp skip(1)
    "Aguardando Retorno Conferencia - PROCFIT:"
    skip(1).

for each tt-int_ds_nota_entrada where 
         tt-int_ds_nota_entrada.situacao  = 1 OR
         (tt-int_ds_nota_entrada.situacao = 2 AND tt-int_ds_nota_entrada.nen_observacao = "Aguardando Retorno Conferencia - PROCFIT"): 

    run pi-acompanhar in h-acomp (input "Pend. Importa‡Æo: " + string(tt-int_ds_nota_entrada.nen_notafiscal_n) + "/" + tt-int_ds_nota_entrada.nen_serie_s).
...
for each tt-int_ds_nota_entrada where 
    ...
    run pi-acompanhar in h-acomp (input "Pend. INT520: " + string(tt-int_ds_nota_entrada.nen_notafiscal_n) + "/" + tt-int_ds_nota_entrada.nen_serie_s).
...
for each tt-docum-est no-lock
    ...
    run pi-acompanhar in h-acomp (input "Pend. RE1001: " + string(tt-docum-est.nro-docto) + "/" + tt-docum-est.serie-docto).
    ...
    display STREAM str-RP
        ...
        with NO-BOX width 550 stream-io.
end.
...
procedure pi-leitura:
    ...
    for each int_ds_nota_entrada no-lock where
             ...
             int_ds_nota_entrada.situacao               <> 9 /* Notas Recusadas */
        on stop undo, leave:

        run pi-acompanhar in h-acomp (input "Nota: " + string(int_ds_nota_entrada.nen_notafiscal_n) + "/" + int_ds_nota_entrada.nen_serie_s + "  Estab: " + estabelec.cod-estabel).

        IF  int_ds_nota_entrada.tipo_nota <> 1 /* Compras */
        AND int_ds_nota_entrada.tipo_nota <> 3 /* Transferˆncia */ THEN
            NEXT for-nota.

        IF tt-param.i-tipo-nota <> 5 THEN DO:
           IF  tt-param.i-tipo-nota = 1
           AND int_ds_nota_entrada.tipo_nota <> 1 THEN
               NEXT for-nota.
           IF  tt-param.i-tipo-nota = 3
           AND int_ds_nota_entrada.tipo_nota <> 3 THEN
               NEXT for-nota.
        END.        

        if int_ds_nota_entrada.nen_cfop_n = 6949 or 
           int_ds_nota_entrada.nen_cfop_n = 5949 then 
           next.

        for first cst_estabelec no-lock WHERE 
                  cst_estabelec.cod_estabel = estabelec.cod-estabel: 
        end.
        if cst_estabelec.dt_fim_operacao < int_ds_nota_entrada.nen_datamovimentacao_d then 
           next.
        ...
        for first tt-int_ds_nota_entrada where
                  ...
                  tt-int_ds_nota_entrada.nen_notafiscal_n  = int_ds_nota_entrada.nen_notafiscal_n: 
        end.
        if not avail tt-int_ds_nota_entrada then do:
           ...
           IF tt-int_ds_nota_entrada.nen_observacao MATCHES "*Nota fiscal emitida pela*" THEN
        ...
        if int_ds_nota_entrada.situacao        >= 2 and
           ...
           ASSIGN i-conferida = i-conferida + 1
                  tt-int_ds_nota_entrada.nen_observacao = "Atualizada". /*"Conferida".*/
        end.
        for first emitente NO-LOCK WHERE emitente.cgc = int_ds_nota_entrada.nen_cnpj_origem_s: 
        end.
        if not avail emitente then do:
           assign i-emit = i-emit + 1
                  tt-int_ds_nota_entrada.nen_observacao = "Fornecedor nao cadastrado".
        end.
        for first docum-est where 
            ...
            ASSIGN i-docum = i-docum + 1
                   tt-int_ds_nota_entrada.nen_observacao = "Pendente RE1001".
    
            if  docum-est.dt-atualiza <> ? 
            and (docum-est.CE-atual or
                 docum-est.cr-atual or
                 docum-est.ap-atual or
                 docum-est.of-atual) then do:
                ASSIGN i-atualiz = i-atualiz + 1
                       tt-int_ds_nota_entrada.nen_observacao = "Atualizada".
            end.
            else do:
                for first tt-docum-est of docum-est: 
                end.
        ...
        and int_ds_nota_entrada.situacao >= 2 
        AND int_ds_nota_entrada.nen_conferida_n = 1 
        AND int_ds_nota_entrada.nen_datamovimentacao_d <> ? then do:
            if int_ds_nota_entrada.situacao = 2 THEN DO: 
               assign tt-int_ds_nota_entrada.nen_observacao = "Aguardando Retorno Conferencia - PROCFIT"
                      i-faltantes                           = i-faltantes + 1.
            END.
        end. */

        if int_ds_nota_entrada.situacao = 1 THEN DO: 
           assign tt-int_ds_nota_entrada.nen_observacao = "Aguardando Retorno Conferencia - PROCFIT"
                  i-faltantes                           = i-faltantes + 1.
        END.
