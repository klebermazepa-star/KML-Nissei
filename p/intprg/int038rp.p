...
DEFINE VARIABLE h-cdapi1001 AS HANDLE      NO-UNDO.
{cdp/cdapi1001.i}
...
def var c-cond-redespacho    as char                    no-undo.
...
def var c-tp-transp          as char                    no-undo.
def var c-cod-transp         as char                    no-undo.
def var c-it-codigo          as char                    no-undo.
def var de-quantidade        as decimal                 no-undo.
def var c-natur              like ped-venda.nat-operacao    no-undo.
def VAR c-natur-ent          like natur-oper.nat-operacao   no-undo.
def var de-vl-uni            like item-uni-estab.preco-base no-undo.
...
def var de-preco             like ems2dis.preco-item.preco-venda    no-undo.
def var de-preco-liq         like ped-item.vl-preuni        no-undo.
def var c-cod-refer          as char                    no-undo.
def var c-cod-entrega        as char                    no-undo.
def var c-cidade-cif         as char                    no-undo.
def var c-redespacho         as char                    no-undo.
def var i-canal-vendas       as integer                 no-undo.
def var c-cod-rota           as char                    no-undo.
def var i-nr-registro        as integer init 0          no-undo.
def var c-informacao         as char format "x(220)"    no-undo.
...
def var d-dt-entrega         as date                    no-undo.
def var d-dt-implantacao     as date                    no-undo.
def var i-nr-sequencia       as integer                 no-undo.
def var c-nome-repres        as character               no-undo.
DEF VAR i-tipo-contr-item    AS INTEGER                 NO-UNDO.
def var c-placa              as character               no-undo.
def var c-uf-placa           as character               no-undo.
def var c-numero-caixa       as character               no-undo.
def var c-numero-lote        as character               no-undo.
def var c-tp-pedido          as character               no-undo.
def var c-docto-orig         as character               no-undo.
def var c-obs-gerada         as character               no-undo.
def var tb-erro              as char format "x(60)"     NO-UNDO extent 51 init
                             ["01. Faturado com Sucesso                          ",
                              ...
                              "50. Pedido cancelado com sucesso" ,
                              "51. Tipo de controle do item difere dos demais itens"].

def var i-qt-volumes    as integer  format ">>>>9" init 0   no-undo.
def var i-qt-caixas     as integer                          no-undo.
def var d-pes-liq-tot   as decimal format ">>,>>9.99"       no-undo.
def var de-peso-itens   as dec  format ">>>,>>9.99" init 0  no-undo. 
...
DEFINE VARIABLE c-item-controle AS CHARACTER   NO-UNDO.
...
procedure pi-importa:
    ...
    DEFINE VARIABLE cont2 AS DECIMAL        NO-UNDO.
    ...
    */
    ...
    for each tt-balanco ,
        ...
        last tt-estab where
             ...
             tt-estab.dt-fim-operacao >= int_ds_pedido.dt_geracao:

        run pi-acompanhar in h-acomp (input "Lendo Pedidos Invent rio: " + string(int_ds_pedido.ped_codigo_n)).


        IF tt-param.cancelar = YES THEN DO:

            FIND FIRST bint_ds_pedido EXCLUSIVE-LOCK
                WHERE ROWID(bint_ds_pedido) = ROWID(int_ds_pedido) NO-ERROR.
            IF AVAIL bint_ds_pedido THEN
                ASSIGN bint_ds_pedido.situacao = 3.
            RELEASE bint_ds_pedido.

           assign i-erro = 50
                  c-informacao = "Pedido: " + string(int_ds_pedido.ped_codigo_n) + " CNPJ origem: " + string(int_ds_pedido.ped_cnpj_origem)
        ...
        then do:
            ...
            FIND FIRST INT_ds_pedido_produto OF INT_ds_pedido NO-ERROR.
            FIND FIRST ITEM NO-LOCK
                WHERE ITEM.it-codigo = STRING(int_ds_pedido_produto.ppr_produto_n) NO-ERROR.
            IF AVAIL ITEM THEN
                ASSIGN i-tipo-contr-item = ITEM.tipo-contr.
            ...
            for each int_ds_pedido_produto no-lock of int_ds_pedido ,
                ...
                break by int_ds_pedido_retorno.ppr_produto_n
                 ...
                 // Kleber - altera‡Æo para dar erro de item com controle de estoque diferente do resto da nota
                
                FIND FIRST ITEM NO-LOCK
                    WHERE ITEM.it-codigo = STRING(int_ds_pedido_produto.ppr_produto_n) NO-ERROR.
                IF AVAIL ITEM THEN DO:

                    IF ITEM.tipo-contr <> i-tipo-contr-item THEN DO:
                
                        assign i-erro = 51
                               c-informacao = "Pedido: " + string(int_ds_pedido.ped_codigo_n) + " CNPJ origem: " + string(int_ds_pedido.ped_cnpj_origem)
                                              + " Item: " + STRING(int_ds_pedido_produto.ppr_produto_n).
                        l-movto-com-erro = yes.
                        run gera-log. 
                        return.
                ...
                then do:
                    if int_ds_pedido_produto.nen_notafiscal_n = ? or
                       int_ds_pedido_produto.nen_notafiscal_n = 0 then do:
                        ...
                        FIND first tt-nfe no-lock where
                            tt-nfe.ped_codigo_n = int_ds_pedido.ped_codigo_n and  
                            tt-nfe.nro-docto    = i-nro-docto NO-ERROR.
                        if not avail tt-nfe 
                        then do:
                             create tt-nfe.
        ...
        if avail tt-nfe 
        then do:
            FIND first tt-item-doc-est-ent NO-ERROR.
...
procedure pi-cria-tt-docum-est:
   ...
   for each tt-nfe no-lock where
        ...
        // KML - L¢gica para aplicar centro de custo
        IF c-cod-estabel = "973" THEN 
            ASSIGN c-sc-codigo = "00973". //era 00506
        ELSE 
            ASSIGN c-sc-codigo = STRING(INT(c-cod-estabel), "99999").
        ...
        assign tt-docum-est-ent.nro-docto    = tt-nota-ent.nro-docto
               ...
               tt-docum-est-ent.observacao   = "Pedido Balan‡o : " + string(tt-nfe.ped_codigo_n) + CHR(13) + CHR(13) + c-observacao + CHR(13) + tt-param.txt-obs 
               tt-docum-est-ent.valor-mercad = 0 
               tt-docum-est-ent.tot-valor    = 0
               tt-docum-est-ent.conta-transit = /*param-estoq.conta-fornec*/ "33101030" // KML "91107005"  alterado kleber 21/02 a pedido do pedro
               tt-docum-est-ent.ct-transit    = "33101030" // "91107005"  alterado kleber 21/02 a pedido do pedro
               tt-docum-est-ent.sc-transit    = c-sc-codigo
        ...
        for each tt-it-nfe where
             ...
             FIND first item no-lock where 
                       item.it-codigo = tt-it-nfe.it-codigo NO-ERROR.
...
/*                  else                                                                  */
/*                     assign c-cod-depos = item.-ito-pad.                            */
                
               /*Alterado dia 27/02/2026 para obedecer o deposito correto de acordo com solicitacÆo chamado 0126-021127*/ 
               ...
               IF c-cod-estabel = "977" THEN
               DO:
               
                   assign c-cod-depos = "977".
             ...
             assign tt-item-doc-est-ent.encerra-pa     = no
                    ...
                    tt-item-doc-est-ent.preco-total[1] = tt-it-nfe.vl-uni * tt-item-doc-est-ent.quantidade.    
             ...
             FIND FIRST unid-feder NO-LOCK
                    WHERE unid-feder.pais   = tt-docum-est-ent.pais
                      AND unid-feder.estado = tt-docum-est-ent.uf NO-ERROR.
             
                
             RUN pi-seta-tributos IN h-cdapi1001 (input "11").
             RUN pi-retorna-sit-tribut IN h-cdapi1001 (INPUT  1, /* tipo-nota */
                                                       INPUT  tt-docum-est-ent.cod-estabel,
                                                       INPUT  tt-item-doc-est-ent.nat-operacao,
                                                       INPUT  tt-item-doc-est-ent.class-fiscal,
                                                       INPUT  tt-item-doc-est-ent.it-codigo,
                                                       INPUT  IF AVAIL unid-feder THEN integer(unid-feder.cod-uf-ibge) ELSE 0,
                                                       INPUT  tt-docum-est-ent.cod-emitente,
                                                       INPUT  tt-docum-est-ent.dt-emissao,
                                                       OUTPUT TABLE tt-sit-tribut).
              
             FOR FIRST tt-sit-tribut
                 WHERE tt-sit-tribut.cdn-tribut = 11:
                 
                 
                 IF  tt-sit-tribut.cdn-sit-tribut <> 0
                 AND tt-sit-tribut.seq-tab-comb   <> 0 THEN
                     overlay(tt-item-doc-est-ent.char-2,840,20) /** CEST **/ = STRING(tt-sit-tribut.cdn-sit-tribut,"9999999").
                 ELSE
                     overlay(tt-item-doc-est-ent.char-2,840,20) /** CEST **/ = "". 
...
procedure pi-cria-tt-ped-venda-prs:
   ...
   for each tt-it-nfs where
       ...
       find last ems2dis.preco-item where 
...
procedure emite-nota-entrada.
    ...
    for each tt-nota-ent:
        ...
        for first nota-fiscal no-lock where
            ...
            nota-fiscal.serie-orig       = tt-docum-est-ent.serie        and
...
procedure pi-gera-itens-nota:
    ...
    for each tt-ped-item-prs no-lock of tt-ped-venda-prs:
        ...
        FIND FIRST item NO-LOCK
            WHERE item.it-codigo = tt-ped-item-prs.it-codigo NO-ERROR.

        FIND first item-uni-estab no-lock where 
            item-uni-estab.it-codigo = tt-ped-item-prs.it-codigo and
            item-uni-estab.cod-estabel = tt-ped-venda-prs.cod-estabel NO-ERROR.
        ...
        /* beneficiamentos */
        else if natur-oper.terceiros then run pi-terceiros.
        /* devolucao */
...
procedure pi-altera-itens:
  ...
  for each tt-item: 
     ...
     for first b-item FIELDS(it-codigo cod-obsoleto) EXCLUSIVE-LOCK 
         WHERE b-item.it-codigo = string(tt-item.it-codigo):
     ...
     FIND first item-uni-estab  EXCLUSIVE-LOCK
         where item-uni-estab.cod-estabel = c-cod-estabel 
           and item-uni-estab.it-codigo   = tt-item.it-codigo NO-ERROR.
