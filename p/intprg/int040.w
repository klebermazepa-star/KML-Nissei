...
*******************************************************************************/
{include/i-prgvrs.i int040 2.12.01.001}
...
DEFINE FRAME f-pg-par
     bt-arquivo-entrada AT ROW 5.25 COL 43.57 HELP
          "Escolha do nome do arquivo"
     c-arquivo-entrada AT ROW 5.29 COL 3.29 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     text-entrada AT ROW 4.25 COL 4.14 NO-LABEL
     RECT-8 AT ROW 4.58 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
DEFINE FRAME f-pg-sel
     c-estabel-ini AT ROW 2 COL 25.86 COLON-ALIGNED HELP
          "" WIDGET-ID 2
          LABEL "Estabelecimento":R15
     c-estabel-fim AT ROW 2 COL 45 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     periodo-fim AT ROW 3 COL 47 NO-LABEL WIDGET-ID 14
     periodo-ini AT ROW 3.04 COL 14.71 WIDGET-ID 16
     IMAGE-1 AT ROW 3.04 COL 36.43 WIDGET-ID 10
     IMAGE-2 AT ROW 3.04 COL 42.86 WIDGET-ID 12
...
/* SETTINGS FOR FILL-IN periodo-fim IN FRAME f-pg-sel
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN periodo-ini IN FRAME f-pg-sel
   ALIGN-L                                                              */
...
{utp/ut9000.i "int040" "2.12.01.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    {include/i-imvrf.i &programa=int040 &versao-layout=001}

    ASSIGN periodo-ini:SCREEN-VALUE IN FRAME f-pg-sel = STRING(date(08,01,year(today) - 1))
           periodo-fim:SCREEN-VALUE IN FRAME f-pg-sel = if month(date(periodo-ini:screen-value in frame f-pg-sel)) < 12
                                                                then string(date(month(date(TODAY)) + 1,01,year(date(today))) - 1)
                                                                else string(date(01,01,year(date(today)) + 1) - 1).

    ASSIGN c-arquivo-entrada:SCREEN-VALUE IN FRAME f-pg-par = SESSION:TEMP-DIRECTORY + "Notas-pepsico.csv". 
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-estabel-ini c-estabel-fim periodo-fim periodo-ini 
      WITH FRAME f-pg-sel IN WINDOW C-Win.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 c-estabel-ini c-estabel-fim 
         periodo-fim periodo-ini 
      WITH FRAME f-pg-sel IN WINDOW C-Win.
  ...
  ENABLE RECT-8 bt-arquivo-entrada c-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
do  on error undo, return error
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.dt-periodo-ini  = INPUT FRAME f-pg-sel periodo-ini
           tt-param.dt-periodo-fim  = INPUT FRAME f-pg-sel periodo-fim
           tt-param.cod-estabel-ini = INPUT FRAME f-pg-sel c-estabel-ini
           tt-param.cod-estabel-fim = INPUT FRAME f-pg-sel c-estabel-fim.
    ...
    {include/i-imrun.i intprg/int040rp.p}
