...
DEFINE BUFFER b2-int_ds_pedido FOR int_ds_pedido.
...
DEF VAR v-serie-docto       LIKE docum-est.serie-docto NO-UNDO.
...
def var tb-erro              as char format "x(60)" extent 46 init
                             ["01. Faturado com Sucesso                          ",
                              ...
                              "46. Emitente est  cadastrado apenas como fornecedor"].
...
RUN intprg/int888.p (INPUT "NF Retorno",
                     INPUT "INT041RP.P").
...
procedure pi-importa:
    ...
    for each int_ds_mod_pedido no-lock where int_ds_mod_pedido.cod_prog_dtsul = "int041",
        ...
        run pi-cria-retorno(input int(int_ds_tipo_pedido.tp_pedido)).
    end.
    ...
    */
    ...
    for EACH tt-retorno ,
        each int_ds_pedido no-lock WHERE 
             int_ds_pedido.ped_tipopedido_n = tt-retorno.ped_tipopedido_n AND
        ...
        date(int_ds_pedido.dt_geracao)     <= tt-param.dt-periodo-fim     AND 
         ...
         FIRST tt-estab WHERE
               ...
               tt-estab.dt_fim_operacao >= int_ds_pedido.dt_geracao query-tuning(no-lookahead):
        ...
        THEN DO:
            ...
            for each int_ds_pedido_produto no-lock of int_ds_pedido ,
                ...
                // KML - 06/08/2021 - verificado com a procfit que esse campo ‚ enviado com o valor total do item, dessa forma
                // Foi necess rio alterar o de-vl-uni para dividir pela quantiade
                assign de-vl-uni           = ROUND(int_ds_pedido_produto.ppr_valorbruto_n / int_ds_pedido_retorno.rpp_quantidade_n , 2 )
                ...
                IF i-seq-entrada = para-fat.nr-item-nota THEN
                 ...
                 IF NOT AVAIL tt-it-nfe THEN DO:                   
...
/*                  ELSE DO:                                                                                 */
/*                                                                                                           */
/*                     .MESSAGE "tt-it-nfe.quantidade + de-quantidade " tt-it-nfe.quantidade + de-quantidade */
/*                         SKIP "tt-it-nfe.quantidade " tt-it-nfe.quantidade                                 */
/*                         SKIP "de-quantidade " de-quantidade                                               */
...
/*                  END.                                                                                     */
        ...
        then do:
              ...
              then do:
                 FOR FIRST tt-docum-est-nova NO-LOCK: 
                     assign i-erro        = 1
                            c-informacao  = "Pedido: "    + c-nr-pedcli
                                          + " Emitente: " + c-nome-abrev
                                          + " Docto: "    + tt-docum-est-nova.nro-docto
                                          + " Serie: "    + tt-docum-est-nova.serie-docto
                                          + " Cod.Emit: " + string(tt-docum-est-nova.cod-emitente)
                                          + " Natureza: " + tt-docum-est-nova.nat-operacao.
                 END.
  ...
  /*  IF NOT AVAIL int_ds_pedido THEN DO:
       ...
       FOR EACH b2-int_ds_pedido no-lock WHERE 
             b2-int_ds_pedido.ped_codigo_n    >=  tt-param.pedido-ini        AND  
             b2-int_ds_pedido.ped_codigo_n    <=  tt-param.pedido-fim        AND
        date(b2-int_ds_pedido.dt_geracao)     >= tt-param.dt-periodo-ini     AND                
        date(b2-int_ds_pedido.dt_geracao)     <= tt-param.dt-periodo-fim     AND 
             b2-int_ds_pedido.situacao         = 1 ,
           LAST tt-retorno: 

            DISP STREAM str-rp SKIP " (ERRO) Tipo do pedido diferente do parametrizado, favor rever parametros no cadastro int115" SKIP
               "Pedido: " string(b2-int_ds_pedido.ped_codigo_n)  " CNPJ Origem: "  string(b2-int_ds_pedido.ped_cnpj_origem) FORMAT "X(20)"
               "Tipo Pedido (Procfit): "  string(b2-int_ds_pedido.ped_tipopedido_n) " Tipo Pedido encontrado: " string(tt-retorno.ped_tipopedido_n)
                    SKIP       WITH WIDTH 333 STREAM-IO.
        END.
...
DEF INPUT PARAMETER p-tipo AS INTEGER.
  ...
  CREATE tt-retorno.
  ASSIGN tt-retorno.ped_tipopedido_n = p-tipo.
...
procedure valida-pedido:
    ...
    do:
       assign i-erro = 30
              c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Destino: " + string(int_ds_pedido.ped_cnpj_destino_s)
              l-movto-com-erro = yes.
       run gera-log. 
       return.
    end.
...
PROCEDURE valida-nota-entrada:
    ...
    for first emitente no-lock where
              emitente.cgc = trim(int_ds_pedido.ped_cnpj_origem_s):
...
PROCEDURE pi-cria-tt-docum-est:
   ...
   run pi-acompanhar in h-acomp (input "Criando Nota de Entrada :" + SUBSTRING(string(tt-nfe.ped_codigo_n),2,6)).
   ...
   FOR EACH tt-nfe NO-LOCK WHERE
        ...
        assign tt-docum-est-ent.registro               = 2
               ...
               tt-docum-est-ent.observacao             = ""
               tt-docum-est-ent.ct-transit             = IF AVAIL estabelec THEN estabelec.ct-devolucao ELSE ""
               tt-docum-est-ent.sc-transit             = "" 
               tt-docum-est-ent.dt-venc-icm            = TODAY
               tt-docum-est-ent.dt-venc-ipi            = TODAY
     ...
     FOR FIRST nota-fiscal NO-LOCK
         WHERE nota-fiscal.cod-estabel = c-cod-estabel
           AND nota-fiscal.serie       = c-serie-docum
           AND nota-fiscal.nr-nota-fis = c-nr-docum:
         ASSIGN tt-docum-est-ent.valor-mercad = nota-fiscal.vl-mercad  
                tt-docum-est-ent.tot-valor    = nota-fiscal.vl-tot-nota
/*                 tt-docum-est-ent.base-icm     = nota-fiscal.vl-mercad   */
/*                 tt-docum-est-ent.base-ipi     = nota-fiscal.vl-tot-nota */
                tt-docum-est-ent.valor-outras = nota-fiscal.vl-tot-nota - nota-fiscal.vl-mercad
                tt-docum-est-ent.observacao   = nota-fiscal.observ-nota + CHR(13) 
                                              + "NF ORIGEM: " + c-serie-docum + " / " + c-nr-docum.
     END.

     /* Est  utilizando o [ (colchetes) como separador, ser  necess rio na trigger da docum-est para recuperar o numero do pedido */
     IF  tt-docum-est-ent.observacao <> ""
     THEN
         ASSIGN tt-docum-est-ent.observacao = tt-docum-est-ent.observacao + CHR(13) +
                                              "Pedido Estorno[ " + c-nr-pedcli.
     ELSE
         ASSIGN tt-docum-est-ent.observacao = "Pedido Estorno[ " + c-nr-pedcli.
     ...
     FOR EACH tt-it-nfe WHERE
         ...
         */
         ...
         assign tt-item-doc-est-ent.encerra-pa     = no
                ...
                tt-item-doc-est-ent.despesas       = IF  i-seq-item = 10 THEN tt-docum-est-ent.valor-outras ELSE 0
                ...
                tt-item-doc-est-ent.base-icm       = tt-item-doc-est-ent.preco-total.
          ...
          FOR FIRST nota-fiscal NO-LOCK WHERE
             ...
             FIRST it-nota-fisc OF nota-fiscal NO-LOCK WHERE
              ...
              ASSIGN tt-item-doc-est-ent.nat-comp     = it-nota-fisc.nat-operacao
                     ...
                     tt-item-doc-est-ent.valor-icm    = it-nota-fisc.vl-icms-it
                     tt-item-doc-est-ent.aliquota-icm = it-nota-fisc.aliquota-icm
                     tt-item-doc-est-ent.cd-trib-icm  = it-nota-fisc.cd-trib-icm
                     tt-item-doc-est-ent.cd-trib-ipi  = it-nota-fisc.cd-trib-ipi. 
              assign c-nat-origem = it-nota-fisc.nat-operacao.
          END.
          ...
          assign tt-item-doc-est-ent.nat-operacao   = c-natur-ent.
     ...
     FOR EACH int_ds_doc_erro WHERE
              ...
              int_ds_doc_erro.tipo_nota    = tt-nota-ent.tipo_nota   :
                   ...
                   FOR FIRST nota-fiscal NO-LOCK WHERE
                    ...
                    THEN DO:
                       ...
                       THEN DO:
                           ...
                           THEN DO:
                               ASSIGN OVERLAY(docum-est.char-2,166,2)  = "3". /* Finalidade NFe */
...
procedure gera-log-final:
   ...
   RUN intprg/int999.p ("NF Retorno", 
                        string(c-nr-pedcli),
                        STRING((trim(tb-erro[i-erro]) + " - " + c-informacao)),
                        int(b-int_ds_pedido.situacao), /* 1 - Pendente, 2 - Processado */ 
                        c-seg-usuario,
                        "INT041RP.P").

    /* INICIO - Grava¯Êo LOG Tabela para a PROCFIT */
    FIND FIRST b2-int_ds_pedido OF int_ds_pedido EXCLUSIVE-LOCK NO-ERROR.
    IF AVAIL b2-int_ds_pedido THEN DO:
        IF b2-int_ds_pedido.situacao = 2 THEN DO:
            ASSIGN b2-int_ds_pedido.envio_status      = 8
                   b2-int_ds_pedido.retorno_validacao = "".
        END.
        ELSE IF b2-int_ds_pedido.situacao = 1 THEN DO:
            ASSIGN b2-int_ds_pedido.envio_status      = 9
                   b2-int_ds_pedido.retorno_validacao = substring(trim(tb-erro[i-erro]) + " - " + c-informacao,1,500).
        END.
    END.
    RELEASE b2-int_ds_pedido.
...
procedure gera-log.
   ...
   RUN intprg/int999.p ("NF Retorno", 
                        string(c-nr-pedcli),
                        STRING((trim(tb-erro[i-erro]) + " - " + c-informacao)),
                        int(int_ds_pedido.situacao), /* 1 - Pendente, 2 - Processado */ 
                        c-seg-usuario,
                        "INT041RP.P").
