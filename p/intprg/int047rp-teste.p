...
procedure pi-importa:
    ...
    for each int-ds-pedido no-lock where
              ...
              int-ds-pedido.ped-codigo-n <> 58403402 and  /* Ricardo */         */
...
procedure finaliza-pedido.
    ...
    if not l-movto-com-erro then do:
        ...
        FOR EACH tt-ped-item-prs NO-LOCK WHERE 
            tt-ped-item-prs.nr-pedcli = trim(string(int-ds-pedido.ped-codigo-n)):
            de-quantidade = de-quantidade - tt-ped-item-prs.qt-pedida.
        END.
       ...
       if not avail item-doc-est then do:
            ...
            then do:
                ...
                if can-find (first int-ds-docto-xml no-lock where
                    ...
                    assign tt-param-int073.arquivo = session:temp-directory + "INT073" + ".tmp":U.
    ...
    for each tt-ped-item-prs no-lock of tt-ped-venda-prs where 
        ...
        for each wt-it-docto exclusive-lock
            ...
            for each wt-it-imposto exclusive-lock 
                where wt-it-imposto.seq-wt-docto    = wt-it-imposto.seq-wt-docto
