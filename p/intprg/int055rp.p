/********************************************************************************
**
**  Programa: int055rp.p - Integra‡Æo informa‡äes ABC Farma - Procfit -> Datasul
**
********************************************************************************/                                                                

/* include de controle de versÆo */
{include/i-prgvrs.i INT055RP 2.00.00.000}  /*** 010000 ***/

DISABLE TRIGGERS FOR LOAD OF ITEM.
DISABLE TRIGGERS FOR LOAD OF ems2dis.preco-item.
...
IF tt-param.arquivo = "" THEN 
   ASSIGN tt-param.arquivo   = "int055.txt"
          tt-param.destino   = 3
...
assign  c-programa 	    = "INT055RP"
	    c-versao	    = "2.00"
	    c-revisao	    = ".00.000"
        c-empresa       = param-global.grupo
	    c-sistema	    = "Espec¡fico"
	    c-titulo-relat  = "Atualiza Informa‡äes ABC Farma".
...
run pi-inicializar in h-acomp (input "ABC Farma").                                               

DEFINE VARIABLE i-cont-teste AS INTEGER     NO-UNDO.

ASSIGN i-cont-teste = 0.

//OUTPUT TO VALUE("C:\temp\kleber\Joao\INT055\itensAtualizados.csv") APPEND.
//PUT SKIP.
...
FOR EACH int_dp_abc_farma WHERE 
    ...
    RUN pi-acompanhar IN h-acomp(INPUT "Item: " + int_dp_abc_farma.it_codigo).
    ...
    IF NOT AVAIL ITEM THEN DO:
       RUN intprg/int999.p (INPUT "ABCFARMA", 
                            ...
                            INPUT "Item nÆo cadastrado no Datasul: " + STRING(int_dp_abc_farma.it_codigo),
                            INPUT 1, /* 1 - Pendente */
                            INPUT c-seg-usuario,
                            INPUT "int055.p").
     //  PUT UNFORMATTED "next1" SKIP.
       NEXT.
    END.
    ...
    IF NOT AVAIL unid-feder THEN DO:
       RUN intprg/int999.p (INPUT "ABCFARMA", 
                            ...
                            INPUT "UF nÆo cadastrada para o Pa¡s: Brasil / " + int_dp_abc_farma.uf,
                            INPUT 1, /* 1 - Pendente */
                            INPUT c-seg-usuario,
                            INPUT "int055.p").
      // PUT UNFORMATTED "next2" SKIP.
       NEXT.
    END.

  //  ASSIGN i-cont-teste = i-cont-teste + 1.

  //  PUT UNFORMATTED "next3; " int_dp_abc_farma.it_codigo ";" int_dp_abc_farma.uf SKIP.
//    NEXT.

  //  IF i-cont-teste > 10 THEN LEAVE.

  /*  PUT UNFORMATTED int_dp_abc_farma.it_codigo    ";"
                    int_dp_abc_farma.uf           ";"
                    int_dp_abc_farma.pmc          ";"
                    int_dp_abc_farma.pnu          ";"
                    int_dp_abc_farma.tipo_preco   ";"
                    int_dp_abc_farma.preco_fabric SKIP. */
    ...
    ASSIGN int_dp_info_abc.pmc          = int_dp_abc_farma.pmc
           ...
           int_dp_info_abc.categoria    = STRING(TODAY,"99/99/9999").         
    ...
    IF int_dp_abc_farma.uf = "PR" THEN DO:
       ...
       IF AVAIL item-uf THEN DO:*/
          for each ems2dis.preco-item where preco-item.nr-tabpre  = "PR12" AND
                                    preco-item.dt-inival <= today and
                                    preco-item.it-codigo  = int_dp_abc_farma.it_codigo:
              ASSIGN preco-item.situacao = 2.
          END.

          FIND FIRST ems2dis.preco-item WHERE 
                     preco-item.nr-tabpre    = "PR12" AND
                     preco-item.it-codigo    = int_dp_abc_farma.it_codigo AND
                     preco-item.cod-refer    = "" AND
                     preco-item.cod-unid-med = item.un AND
                     preco-item.dt-inival    = TODAY AND
                     preco-item.quant-min    = 0 NO-ERROR.
          ...
          END.
          ASSIGN preco-item.preco-venda  = int_dp_abc_farma.pmc
...
PUT "Atualiza‡Æo das informa‡äes ABC Farma finalizada com sucesso." SKIP(1)
    "Para consultar as informa‡äes atualizadas, utilize o programa NI0308.". 
