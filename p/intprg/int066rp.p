/********************************************************************************
** Programa: INT066 - Geracao notas fiscais a partir de Recebimento Copel
**
** Versao : 01 - 13/08/2018 - Resultpro
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i int066rp 2.12.01.001}
...
/* def temp-table tt-int_ds_nota_copel-cartao like int_ds_nota_copel-cartao */
...
DEFINE TEMP-TABLE tt-docto-aux NO-UNDO LIKE tt-docto 
    FIELD COR_ID_N LIKE int_ds_nota_copel.COR_ID_N.
...
def var tb-erro              as char format "x(50)" extent 41 init
                             ["01. Faturado com Sucesso                          ",
                              ...
                              "18. Documento a ser devolvido n’o existe          ",
                              ...
                              "41. Numero de nota fiscal j  existe no faturamento"].
...
assign c-programa       = "int066rp"
       c-versao         = "2.12"
       c-revisao        = ".01.001"
       c-empresa        = "* Nao Definido *"
       c-sistema        = "Faturamento"
       c-titulo-relat   = "Importacao de Notas Fiscais - COPEL".
...
/* RUN intprg/int888.p (INPUT "COPEL",       */
/*                      INPUT "int066rp.p"). */
...
procedure pi-elimina-tabelas.
   ...
   EMPTY temp-table tt-docto-aux.
end.        

procedure pi-importa:
    DEFINE VARIABLE i_NumNota AS INTEGER     NO-UNDO.
    ...
    FOR EACH int_ds_nota_copel
        WHERE int_ds_nota_copel.COR_SITUACAO_TOTVS =  1 
           BY int_ds_nota_copel.COR_DATA_D :

        run pi-acompanhar in h-acomp (input "Lendo Copel: " + string(int_ds_nota_copel.COR_ID_N)).                                                
        ...
        IF int_ds_nota_copel.COR_NUM_NOTA = "" OR int_ds_nota_copel.COR_NUM_NOTA = ? THEN DO:
            ASSIGN i_NumNota = NEXT-VALUE(idSeq_NF_COPEL).
            ASSIGN int_ds_nota_copel.COR_NUM_NOTA = trim(string(int(i_NumNota),">>>9999999")).
        END.

        FIND FIRST estabelec NO-LOCK
            WHERE estabelec.cgc = int_ds_nota_copel.COR_CNPJ_FILIAL NO-ERROR.

        FIND FIRST nota-fiscal  NO-LOCK
            WHERE nota-fiscal.serie         = int_ds_nota_copel.COR_SERIE 
              AND nota-fiscal.cod-estabel   = estabelec.cod-estabel
              AND nota-fiscal.nr-nota-fis   = trim(string(int(int_ds_nota_copel.COR_NUM_NOTA),">>>9999999"))  NO-ERROR.
    
        IF AVAIL nota-fiscal THEN DO:

           ASSIGN i-erro = 41
              c-informacao = "S‚rie: " + int_ds_nota_copel.COR_SERIE + " - " + 
                             "Nota: "  + int_ds_nota_copel.COR_NUM_NOT + " - " +
                             "CNPJ Filial: " + STRING(int_ds_nota_copel.COR_CNPJ_FILIAL) + " - " +
                             "Estabelec: " + estabelec.cod-estabel
        ...
        ASSIGN  c-num-nota          = trim(string(int(int_ds_nota_copel.COR_NUM_NOTA),">>>9999999"))
                d-dt-implantacao    = today
                d-dt-emissao        = int_ds_nota_copel.COR_DATA_D
                de-valor-dinh       = int_ds_nota_copel.COR_VALORPAGAR_N
                de-valor-cartaod    = 0
                de-valor-cartaoc    = 0
                de-valor-vale       = 0
                c-condipag          = "01"
        ...
        IF int_ds_nota_copel.COR_ORIGEM = "PROCFIT" THEN DO:
            ASSIGN c-serie = int_ds_nota_copel.COR_SERIE.
        END.
        ELSE DO:
            ASSIGN int_ds_nota_copel.COR_SERIE = "COP".
        END.
        ...
        assign  c-it-codigo         = "990001"
                de-quantidade       = 1
                de-vl-uni           = int_ds_nota_copel.COR_VALORPAGAR_N
                de-vl-liq           = int_ds_nota_copel.COR_VALORPAGAR_N
                de-desconto         = 0
                c-numero-lote       = ""
                de-aliq-icms        = 0
                de-base-icms        = 0
        ...
        if not l-cupom-com-erro and not avail tt-it-docto then do:
            assign i-erro = 37
                   c-informacao = c-serie + " - " + c-num-nota + " - " + string(int_ds_nota_copel.COR_CNPJ_FILIAL)
                   l-cupom-com-erro = yes.
            run gera-log.
        end.
...
PROCEDURE valida-registro-cabecalho.
    ...
    RUN pi-acompanhar in h-acomp (INPUT "Validando Recebimento Copel").
    ...
    for each estabelec 
        ...
        no-lock where
        estabelec.cgc = int_ds_nota_copel.COR_CNPJ_FILIAL,
        ...
        ASSIGN int_ds_nota_copel.COR_FILIAL_S = estabelec.cod-estabel.
...
/*                          output r-rowid).        */
   ...
   assign  c-natur         = ""
           ...
           i-modalidade    = ?
           c-cod-esp-princ = "".
/*    for first int-ds-loja-natur-oper no-lock where rowid(int-ds-loja-natur-oper) = r-rowid: */
...
/*    end.                                                                                    */
   ...
   assign  c-natur         = "5102CO"
           i-cod-cond-pag  = 1
           i-cod-portador  = 99901
           i-modalidade    = 6
/*            c-serie         = "COP" */
           c-cod-esp-princ = "".
   ...
   */
   ...
   then do:
       ...
       FOR FIRST estabelec 
           FIELDS (cod-estabel estado cidade 
                   cep pais endereco bairro ep-codigo) NO-LOCK WHERE
        estabelec.cgc = int_ds_nota_copel.COR_CNPJ_FILIAL,
        FIRST cst_estabelec NO-LOCK WHERE 
        cst_estabelec.cod_estabel = estabelec.cod-estabel AND
        cst_estabelec.dt_fim_operacao >= d-dt-emissao : END.
      ...
      if not avail portador then do:
         assign i-erro = 34
                c-informacao = c-cod-estabel + " - " + c-serie + " - " + c-num-nota + " - " +
...
procedure cria-tt-docto.
    ...
    CREATE tt-docto-aux.
    BUFFER-COPY tt-docto TO tt-docto-aux.
    ASSIGN  tt-docto-aux.COR_ID_N =  int_ds_nota_copel.COR_ID_N.
    ...
    assign tt-cst_nota_fiscal.cod_estabel       = c-cod-estabel
           ...
           tt-cst_nota_fiscal.condipag          = "01"
           tt-cst_nota_fiscal.convenio          = "".
   ASSIGN  tt-cst_nota_fiscal.cupom_ecf         = "".
   ASSIGN  tt-cst_nota_fiscal.nfce_chave        = ""
           tt-cst_nota_fiscal.valor_chq         = 0
           tt-cst_nota_fiscal.valor_chq_pre     = 0
           tt-cst_nota_fiscal.valor_convenio    = 0
           tt-cst_nota_fiscal.valor_dinheiro    = de-valor-dinh
           tt-cst_nota_fiscal.valor_ticket      = 0
           tt-cst_nota_fiscal.valor_vale        = 0
        .
end.
...
procedure importa-nota.
    ...
    for each tt-notas-geradas:
        for first nota-fiscal exclusive-lock where 
            ...
            for first tt-cst_nota_fiscal no-lock where
                ...
                FOR FIRST int_ds_nota_copel EXCLUSIVE-LOCK
                    WHERE int_ds_nota_copel.COR_FILIAL_S      = nota-fiscal.cod-estabel
                      AND int_ds_nota_copel.COR_SERIE         = nota-fiscal.serie      
                      AND INT(int_ds_nota_copel.COR_NUM_NOTA) = INT(nota-fiscal.nr-nota-fis). END.

                CREATE cst_nota_copel.
                ASSIGN cst_nota_copel.cod_estabel = nota-fiscal.cod-estabel
                       cst_nota_copel.serie       = nota-fiscal.serie      
                       cst_nota_copel.nr_nota_fis = nota-fiscal.nr-nota-fis .

                ASSIGN cst_nota_copel.COR_ID_N       = int_ds_nota_copel.COR_ID_N 
                       cst_nota_copel.COR_NUM_FATURA = string(DEC(SUBSTR(int_ds_nota_copel.COR_BARRAS_S,28,15))) + "-" + SUBSTR(int_ds_nota_copel.COR_BARRAS_S,43,2)
                       cst_nota_copel.COR_BARRAS_S   = int_ds_nota_copel.COR_BARRAS_S
                       cst_nota_copel.log_conciliado = NO.

                ASSIGN i-erro       = 1
                       c-informacao = nota-fiscal.cod-estabel + "/" + 
                                      nota-fiscal.serie + "/" + nota-fiscal.nr-nota-fis + "/" + nota-fiscal.nome-abrev.


                ASSIGN int_ds_nota_copel.COR_SITUACAO_TOTVS = 2 /* integrado */
                       l-cupom-com-erro = yes.
    ...
    for each tt-docto:
        if not can-find (first nota-fiscal no-lock where
            ...
            FOR FIRST tt-docto-aux OF tt-docto. END.

            for first estabelec fields (cgc) no-lock where 
                estabelec.cod-estabel = tt-docto.cod-estabel: 
                for first int_ds_nota_copel where
                    int_ds_nota_copel.COR_CNPJ_FILIAL    = estabelec.cgc and
                    /*int_ds_nota_copel.serie             = nota-fiscal.serie and*/ 
                    int_ds_nota_copel.COR_ID_N           = tt-docto-aux.COR_ID_N and
                    int_ds_nota_copel.COR_DATA_D         = tt-docto.dt-emis-nota and
                    int_ds_nota_copel.COR_SITUACAO_TOTVS = 1:
                    ...
                    assign i-erro       = 22
                           c-informacao = tt-docto.cod-estabel
                                        ...
                                        + "/" +
                                        tt-docto.nome-abrev 
                                        + " ID: " +  STRING(tt-docto-aux.COR_ID_N)
                                        + " Nota fiscal Nao integrada" .
                    run gera-log.
                end.
            end.
...
procedure gera-log.
   ...
   FIND CURRENT int_ds_nota_copel EXCLUSIVE-LOCK NO-ERROR.
   IF AVAIL int_ds_nota_copel THEN DO:
       IF int_ds_nota_copel.COR_SITUACAO_TOTVS = 2 THEN DO:
           ASSIGN int_ds_nota_copel.envio_status      = 8
                  int_ds_nota_copel.retorno_validacao = "".
       END.
       ELSE IF int_ds_nota_copel.COR_SITUACAO_TOTVS = 1 THEN DO:
           ASSIGN int_ds_nota_copel.envio_status      = 9
                  int_ds_nota_copel.retorno_validacao = substring(trim(tb-erro[i-erro]) + " - " + c-informacao,1,500).
       END.
   END.
   /* FIM    - Grava»’o LOG Tabela para a PROCFIT */


   run intprg/int999.p ("COPEL", 
                        trim(int_ds_nota_copel.COR_CNPJ_FILIAL  + "/" + c-serie  + "/" + trim(string(int(int_ds_nota_copel.COR_ID_N )))),
                        substring(trim(tb-erro[i-erro]) + " - " + c-informacao,1,500),
                        int_ds_nota_copel.COR_SITUACAO_TOTVS, /* 1 - Pendente, 2 - Processado */ 
                        c-seg-usuario,
                        "int066rp.p").
   assign c-informacao = " ".
end.
...
procedure finaliza-cupom.
   run pi-acompanhar in h-acomp (input "Finalizando cupom: " + trim(STRING(int_ds_nota_copel.COR_ID_N))).
   find current int_ds_nota_copel exclusive-lock no-error.
   if avail int_ds_nota_copel then do:
