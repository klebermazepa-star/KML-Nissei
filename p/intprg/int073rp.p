/********************************************************************************
** Programa: int073 - Importaá∆o de Notas de Entrada PEPSICO 
**
** Versao : 12 - 24/03/2017 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de vers∆o */
{include/i-prgvrs.i int073rp 2.12.06.AVB}
{cdp/cdcfgdis.i}
{utp/ut-glob.i}
...
def temp-table tt-movto no-undo
    ...
    field dt-trans          like int_ds_docto_xml.dt_trans
    ...
    field valor-guia-st     like int_ds_docto_xml.valor_guia_st
    field base-guia-st      like int_ds_docto_xml.base_guia_st
    field dt-imposto        as date
    field nNF               like int_ds_docto_xml.nNF
    field CNPJ-dest         like int_ds_docto_xml.CNPJ_dest.
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "int073.txt"
           tt-param.destino = 3
...
assign c-programa     = "int073"
       c-versao       = "2.13"
       c-revisao      = ".06.AVB"
       c-empresa      = ""
       c-sistema      = "Recebimento"
       c-titulo-relat = "Importacao Documento PEPSICO Recebimento".
...
for each int_ds_docto_xml no-lock where
    ...
    int_ds_docto_xml.situacao <> 4 and
    int_ds_docto_xml.situacao <> 5 and
    int_ds_docto_xml.situacao <> 6 and
    ...
    FIND FIRST int_ds_ext_emitente no-lock where 
        int_ds_ext_emitente.cod_emitente = int_ds_docto_xml.cod_emitente and
        int_ds_ext_emitente.gera_nota NO-ERROR.

    IF NOT AVAIL int_ds_ext_emitente THEN DO:

        run pi-gera-log (input "N∆o est† marcado o campo (Emite nota sem XML) na tela CD0401 para o Fornecedor " + string(int_ds_docto_xml.cod_emitente),
                         input 1).
        NEXT.
    ...
    /*
    for each int_ds_docto-wms exclusive where 
    ...
    */
    ...
    for first int_ds_docto_xml_dup no-lock of bint_ds_docto_xml:
        ...
        if int_ds_docto_xml.dEmi > date(int(entry(2,int_ds_docto_xml_dup.dVenc,"/")),
            ...
            empty temp-table tt-movto.
            run pi-gera-log (input "Vencimento da duplicata deve ser maior ou igual a Emiss∆o do Documento: "
                              + "SÇrie: "     + docum-est.serie-docto 
                             + " Numero: "   + docum-est.nro-docto   
                             + " Forn: "     + string(docum-est.cod-emitente)
                             + " Natureza: " + docum-est.nat-operacao
                             + " Emissao: " + string(int_ds_docto_xml.dEmi)
                             + " Vecto: " + trim(int_ds_docto_xml_dup.dVenc), 
                             input 1).
            return.
        end.

        if int_ds_docto_xml_dup.nDup matches "*GNRE*" then do:
    ...
    for first param-estoq no-lock: 
        if param-estoq.ult-fech-dia >= int_ds_docto_xml.dt_trans then do:
    ...
    for each int_ds_it_docto_xml no-lock where
        ...
        create tt-movto.
        assign tt-movto.CNPJ-dest           = int_ds_docto_xml.CNPJ_dest
               ...
               tt-movto.tot-desconto        = fnTrataNuloDec(int_ds_docto_xml.tot_desconto)
               tt-movto.chave-acesso        = if int_ds_docto_xml.chnfe <> ? then int_ds_docto_xml.chnfe else ""
               ...
               tt-movto.tipo-nota           = int_ds_docto_xml.tipo_nota
               tt-movto.dt-trans            = int_ds_docto_xml.dt_trans
               tt-movto.dt-imposto          = int_ds_docto_xml.dt_guia_st
               tt-movto.base-guia-st        = fnTrataNuloDec(int_ds_docto_xml.base_guia_st)
               ...
               tt-movto.valor-guia-st       = fnTrataNuloDec(int_ds_docto_xml.valor_guia_st).
        ...
        if int_ds_it_docto_xml.vProd < int_ds_it_docto_xml.vDesc then do:
            run pi-gera-log (input "Valor desconto maior que o valor bruto: " + string(int_ds_it_docto_xml.it-codigo) +
        ...
        */
        ...
        assign tt-movto.it-codigo       = string(int_ds_it_docto_xml.it_codigo)
               ...
               tt-movto.desconto        = fnTrataNuloDec(int_ds_it_docto_xml.vDesc)
               ...
               tt-movto.class-fiscal    = if int_ds_it_docto_xml.ncm <> ? and int_ds_it_docto_xml.ncm <> 0
                                          then string(int_ds_it_docto_xml.ncm) else item.class-fiscal.
    ...
    for each tt-movto
                ...
                for first tt-dupli-apagar: 
                   ...
                   assign tt-dupli-imp.registro              = 5
                          ...
                          tt-dupli-imp.dt-venc-imp           = date(int(entry(2,int_ds_docto_xml_dup.dVenc,"/")),
                                                                    int(entry(1,int_ds_docto_xml_dup.dVenc,"/")),
                                                                    int(entry(3,int_ds_docto_xml_dup.dVenc,"/")))
                          tt-dupli-imp.rend-trib             = tt-movto.base-guia-st
                          tt-dupli-imp.aliquota              = (int_ds_docto_xml_dup.vDup / tt-movto.base-guia-st) * 100
        ...
        if not avail tt-docum-est-nova then do:
            ...
            assign tt-docum-est-nova.registro          = 2
                   ...
                   tt-docum-est-nova.esp-docto         = if tt-movto.tipo-nota = 1 then 21 /* NFE */ else 23 /* NFT */
        ...
        if last-of (tt-movto.nr-nota-fis) then do:
                   ...
                   IF tt-erro.cd-erro = 1878 THEN DO:
                      ...
                      for each bint_ds_docto_xml exclusive-lock where
                          ...
                          for each int_ds_it_docto_xml exclusive-lock where
                              int_ds_it_docto_xml.CNPJ  = bint_ds_docto_xml.CNPJ   and
                              int_ds_it_docto_xml.serie = bint_ds_docto_xml.serie  and
                              int_ds_it_docto_xml.nNF   = bint_ds_docto_xml.nNF
                              query-tuning(no-lookahead):
                              assign int_ds_it_docto_xml.situacao = bint_ds_docto_xml.situacao.
                          end.
                      end.
                   END.
            ...
            for first docum-est no-lock of tt-docum-est-nova:
                ...
                */
                ...
                l-ok = yes.
                /* Verificado bloqueio de item pelo c†lculo de custo */
                for each item-doc-est no-lock of docum-est:
                    find first ITEM EXCLUSIVE where ITEM.it-codigo = item-doc-est.it-codigo no-wait no-error.
                    if locked(item) then l-ok = no.
                end.
                ...
                if l-ok then do:
                    ...
                    assign 
                        ...
                        tt-param-re1005.arquivo            = "int073-re1005.txt"
                    ...
                    run pi-gera-log("Emitente: " + trim(string(tt-movto.cod-emitente)) +
                                    ...
                                    " Natur. Oper.: " + c-nat-principal + 
                                    " Estab.: " + tt-movto.cod-estabel +
                                    " - " + "H† itens bloqueados. Documento importado porÇm deve ser atualizado manualmente no RE1005/RE1001!",
                                    1).
                end.
                release docum-est.
            end. /* docum-est */
...
procedure pi-gera-int_ds_doc:
    ...
    for each item-doc-est no-lock of docum-est:
        ...
        if int_ds_doc.nen_pedido_n = 0 or int_ds_doc.nen_pedido_n = ? then
