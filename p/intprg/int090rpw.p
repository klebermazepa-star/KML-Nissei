...
find ems2mult.empresa no-lock WHERE empresa.ep-codigo = v_cdn_empres_usuar no-error.
...
FOR EACH tt-processo WHERE tt-processo.alterado = YES BREAK BY tt-processo.it-codigo :
    ...
    FOR EACH tt-processo :
        ...
        AND tt-processo.estado          = "PR" THEN DO:
            FIND FIRST b-tt-processo WHERE
                       ...
                       b-tt-processo.estado          = "PR" NO-LOCK NO-ERROR.
        ...
        AND tt-processo.estado          = "SC" THEN DO:
            FIND FIRST b-tt-processo WHERE
                       ...
                       b-tt-processo.estado          = "SC" NO-LOCK NO-ERROR.
        ...
        AND tt-processo.estado          = "SP" THEN DO:
            FIND FIRST b-tt-processo WHERE
                       b-tt-processo.cod-estado-orig = "PR" AND
                       b-tt-processo.estado          = "SP" NO-LOCK NO-ERROR.
    ...
    FOR EACH tt-processo :
        ...
        IF AVAIL ITEM THEN DO:
          ...
          /* FOR FIRST int_ds_ncm_produto WHERE 
           ...
           .MESSAGE tt-processo.estado SKIP
                   "tt-processo.valor-pmpf - " tt-processo.valor-pmpf
              ...
              ASSIGN tt-processo.aliquota-icm     = 17
                ...
                IF tt-processo.cd-trib-icm = 1 THEN
                    ...
                    FIND FIRST ICMS-IT-UF WHERE
                     ...
                     ICMS-IT-UF.ESTADO    = "SC" NO-LOCK NO-ERROR. //inclusÆo kml
              ...
              IF AVAIL UNID-FEDER AND unid-feder.ind-uf-subs = YES THEN DO: 
                  ...
                  IF item-uf.per-sub-tri    > 0  THEN DO:
                     ...
                     IF item-uf.estado = item-uf.cod-estado-orig THEN
                        ...
                        FOR FIRST  b-item-uf NO-LOCK where 
...
find first unid-feder 
   where unid-feder.estado = tt-processo.estado no-error.
    ...
    IF ITEM.CD-TRIB-ICM = 1 THEN DO: /* tributado */ // momento correto de altera‡Æo
       ...
       IF AVAIL   icms-it-uf THEN DO:
          ...
          .MESSAGE tt-processo.aliquota-icm SKIP    
                  tt-processo.aliquota-icm-ale SKIP
              VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
...
PROCEDURE cria-integracao:
    ...
    FOR EACH tt-processo :
        ...
        FOR EACH Diferim-parcial-icms
            WHERE Diferim-parcial-icms.cod-item     = tt-processo.it-codigo
              AND Diferim-parcial-icms.cod-estado   = tt-processo.estado
              AND Diferim-parcial-icms.cod-estado   = tt-processo.cod-estado-orig       :
        
                   
            ASSIGN tt-processo.rede = Diferim-parcial-icms.val-perc-icms-diferim .
        
        END.

        .MESSAGE "tt-processo.rede - " tt-processo.rede SKIP
        ...
        FIND FIRST int_dp_item_fiscal USE-INDEX codigo WHERE
        ...
        IF int_dp_item_fiscal.vl_pmpf <> tt-processo.valor-pmpf THEN DO:
