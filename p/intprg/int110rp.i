...
DEF NEW SHARED TEMP-TABLE tt-item-nfs-ds-retorno NO-UNDO
    ...
    FIELD ppr_produto_n   AS INT
    FIELD rpp_caixa_n     AS INT
    FIELD rpp_lote_s      AS CHAR
...
procedure importa:
    ...
    for each int_ds_mod_pedido no-lock where int_ds_mod_pedido.cod_prog_dtsul = "int110",
        ...
        for EACH int_ds_pedido no-lock where 
         ...
         //   AND INT_ds_pedido.ped_codigo_n = 2079425      
            ...
            */
            ...
            for each int_ds_pedido_produto no-lock of int_ds_pedido,
                ...
                c-aux = if string(int_ds_pedido_retorno.rpp_caixa_n) <> ? then string(int_ds_pedido_retorno.rpp_caixa_n) else "".
                  ...
                  .MESSAGE "antes" int_ds_pedido_retorno.rpp_quantidade_n
                      VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
...
procedure cria-tt-ped-item-imp.
   ...
   FOR EACH tt-item-st-aux
       ...
       ASSIGN tt-item-nfs-ds-retorno.nr-sequencia    = i-nr-sequencia
              ...
              tt-item-nfs-ds-retorno.ppr_produto_n   = int_ds_pedido_retorno.ppr_produto_n 
              tt-item-nfs-ds-retorno.rpp_caixa_n     = int_ds_pedido_retorno.rpp_caixa_n   
              tt-item-nfs-ds-retorno.rpp_lote_s      = int_ds_pedido_retorno.rpp_lote_s    
...
procedure pi-gera-itens-nota:
    ...
    for each tt-ped-item-imp no-lock of tt-ped-venda-imp:
        ...
        /* Caso tenha achado algum erro ou advertˆncia, gera tt-erro */
        run pi-erro-nota(h-bodi317pr,'pr'). if l-movto-com-erro then undo, return error.
        ...
        run validaItemDaNota      in h-bodi317va(input  i-seq-wt-docto,
                                                 ...
                                                 output l-proc-ok-aux).
        /* Caso tenha achado algum erro ou advertˆncia, gera tt-erro */
        run pi-erro-nota(h-bodi317va,'va'). if l-movto-com-erro then undo, return error.

        ASSIGN i-tot-item = i-tot-item + 1.

    end. /* tt-ped-item-imp */
    if l-movto-com-erro then undo, return error.
end procedure.
...
procedure pi-envios:
    ...
    /* Caso tenha achado algum erro ou advertˆncia, gera tt-erro */
    run pi-erro-nota(h-bodi317sd,'sd'). if l-movto-com-erro then undo, return error.
...
procedure pi-efetiva-nota:
    ...
    /* Caso tenha achado algum erro ou advertˆncia, gera tt-erro */
    run pi-erro-nota(h-bodi317ef,'ef'). if l-movto-com-erro then undo, return error.
...
PROCEDURE pi-quebra-item-st:
    ...
    FOR FIRST item-uf NO-LOCK
        WHERE item-uf.it-codigo = c-it-codigo: END.
    ...
    IF AVAIL item-uf THEN DO:
        ...
        .MESSAGE de-quantidade
            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
        bl_itens:
        FOR EACH esp-item-entr-st NO-LOCK
            ...
            ELSE
                ...
                .MESSAGE  de-qtd-sdo-aux
                    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
            CREATE tt-item-st-aux.
            ASSIGN tt-item-st-aux.it-codigo       = c-it-codigo
                   tt-item-st-aux.quantidade      = de-qtd-sdo-aux
                   tt-item-st-aux.preco           = (item-doc-est.valor-ipi[1] / item-doc-est.quantidade) + item-doc-est.preco-unit[1]
            ...
            .MESSAGE de-quantidade-aux SKIP de-qtd-st
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
