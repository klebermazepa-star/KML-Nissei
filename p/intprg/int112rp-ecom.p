...
log-manager:write-message("KML - int112-ecom - entrou no programa no inicio") NO-ERROR.
...
log-manager:write-message("KML - int112-ecom - entrou no programa " + STRING(tt-param.nro-docto) + "-" + tt-param.serie-docto + "-" + tt-param.cnpj_origem ) NO-ERROR.
    

for-nota:
for each int_ds_nota_entrada no-lock where
    int_ds_nota_entrada.situacao >= 1 AND
   // int_ds_nota_entrada.nen_conferida_n < 2 AND    Retirado para validar dentro do fonte...
    ...
    log-manager:write-message("KML - int112-ecom - entrou  for each int_ds_nota_entrada " + STRING(int_ds_nota_entrada.nen_notafiscal_n) ) NO-ERROR.
    ...
    for each int_ds_nota_entrada_produt no-lock of int_ds_nota_entrada:
        ...
        log-manager:write-message("KML - int112-ecom - ANTES CRIAR TT-MOVTO - " + c-retorno + "nota - " + STRING(int_ds_nota_entrada.nen_notafiscal_n)) NO-ERROR.
        ...
        log-manager:write-message("KML - int112-ecom -SEGUNDO FIND NO ITEM- " + c-retorno + "nota - " + string(int_ds_nota_entrada.nen_notafiscal_n)) NO-ERROR.
        ...
        log-manager:write-message("KML - int112-ecom - ITEM UNIESTAB - " + c-retorno + "nota - " + string(int_ds_nota_entrada.nen_notafiscal_n) ) NO-ERROR.
        ...
        log-manager:write-message("KML - int112-ecom - RUN DO INT013 - " + c-nat-operacao + "nota - " + string(int_ds_nota_entrada.nen_notafiscal_n) ) NO-ERROR.
    ...
    if can-find(first tt-movto) then do:
        ...
        if de-tot-despesas <> de-tot-desp-nf then do:
            run pi-gera-log (input "Total das despesas n∆o confere. Docto: " + string(int_ds_nota_entrada.nen_notafiscal_n) + " SÇrie: " + string(int_ds_nota_entrada.nen_serie_s) + " Estab.: " + c-cod-estabel +
                             " Itens: " + trim(string(de-tot-despesas,"->>>,>>>,>>9.99")) + " Nota: " + trim(string(de-tot-desp-nf,"->>>,>>>,>>9.99")) + " Liberada alteraá∆o manual.",
                             input 1).
            for first bint_ds_nota_entrada exclusive where
                rowid(bint_ds_nota_entrada) = rowid(int_ds_nota_entrada)
                query-tuning(no-lookahead):
...
log-manager:write-message("KML - int112-ecom - depois for first - " + c-retorno + "nota - " + string(int_ds_nota_entrada.nen_notafiscal_n) ) NO-ERROR. 
...
procedure PiGeraMovimento:
    ...
    for each tt-movto
        ...
        log-manager:write-message("KML - int112-ecom - DESCOBRIR NATUREZA - " + c-nat-principal + "nota- " + string(int_ds_nota_entrada.nen_notafiscal_n)) NO-ERROR. 
        ...
        if last-of (tt-movto.nr-nota-fis) then do:
            ...
            for first docum-est no-lock of tt-docum-est-nova:
                ...
                */
                ...
                run pi-gera-log("OK - Nota - " + STRING(tt-movto.nr-nota-fis),
                                2).
                for each bint_ds_nota_entrada where
                    bint_ds_nota_entrada.nen_cnpj_origem_s = tt-movto.nen_cnpj_origem_s and
                    bint_ds_nota_entrada.nen_serie_s = tt-movto.serie and
