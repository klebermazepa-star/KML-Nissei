...
*******************************************************************************/
{include/i-prgvrs.i int115 2.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-consig 
...
DEFINE VARIABLE h_int115-browser-cesta AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-isento AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-nenhum AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-origem AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-outros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-st AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-browser-tributado AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115-viewer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadpai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-consig 
     LABEL "Consignado" 
     SIZE 12.86 BY 1.13.
...
DEFINE FRAME f-cad
     bt-consig AT ROW 1.21 COL 65 WIDGET-ID 2
     rt-button AT ROW 1 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 94.29 BY 14 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Tipo Pedido Natureza"
         HEIGHT             = 14
         WIDTH              = 94.29
         MAX-HEIGHT         = 21.21
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manuten‡Æo Tipo Pedido Natureza */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manuten‡Æo Tipo Pedido Natureza */
DO:
...
&Scoped-define SELF-NAME bt-consig
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-consig w-cadpaifilho-ambos
ON CHOOSE OF bt-consig IN FRAME f-cad /* Consignado */
DO:
   RUN intprg/int115e.w.
END.
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.17 , 78.29 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int115-viewer ).
       RUN set-position IN h_int115-viewer ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 91.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Tributado|Isento|Cesta B sica|ST|Outros|Nenhum|Nat Origem' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 6.00 , 1.29 ) NO-ERROR.
       RUN set-size IN h_folder ( 8.75 , 94.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int115-zoom.w,
                     ProgVaPara = intprg/int115-vapara.w,
                     ProgIncMod = intprg/int115d.w,
                     Implantar = no':U ,
             OUTPUT h_int115-query ).
       RUN set-position IN h_int115-query ( 3.50 , 84.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.25 , 7.00 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_int115-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_int115-viewer ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_int115-query. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_int115-query ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int115-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int115-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int115-query ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-consig:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadpai ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-viewer ,
             bt-consig:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_int115-viewer , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-tributado.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int115b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-tributado ).
       RUN set-position IN h_int115-browser-tributado ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-tributado. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-tributado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-tributado ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-isento.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-isento ).
       RUN set-position IN h_int115-browser-isento ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-isento. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-isento ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-isento ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-cesta.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-cesta ).
       RUN set-position IN h_int115-browser-cesta ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-cesta. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-cesta ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-cesta ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-st.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-st ).
       RUN set-position IN h_int115-browser-st ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-st. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-st ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-st ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-outros.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-outros ).
       RUN set-position IN h_int115-browser-outros ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-outros. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-outros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-outros ,
             h_folder , 'AFTER':U ).
    END. /* Page 5 */
    WHEN 6 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-nenhum.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-nenhum ).
       RUN set-position IN h_int115-browser-nenhum ( 8.00 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 93.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-nenhum. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-nenhum ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-nenhum ,
             h_folder , 'AFTER':U ).
    END. /* Page 6 */
    WHEN 7 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115-browser-origem.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int115c.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int115-browser-origem ).
       RUN set-position IN h_int115-browser-origem ( 8.00 , 4.14 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 91.00 ) */

       /* Links to BrowserCadastro2 h_int115-browser-origem. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-browser-origem ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115-browser-origem ,
             h_folder , 'AFTER':U ).
    END. /* Page 7 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT115" "2.12.01.AVB"}
