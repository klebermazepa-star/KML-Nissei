...
*******************************************************************************/
{include/i-prgvrs.i INT120Z 1.12.00.AVB}
...
DEFINE VARIABLE h_int120-browser-zoom AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     ...
     bt-cancela AT ROW 15.25 COL 14
     bt-ajuda AT ROW 15.25 COL 73.86
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Modalidade de Pedido"
         HEIGHT             = 15.88
         WIDTH              = 84.86
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Modalidade de Pedido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Modalidade de Pedido */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Modalidade' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.75 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 13.00 , 83.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int120-browser-zoom.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int120-browser-zoom ).
       RUN set-position IN h_int120-browser-zoom ( 3.50 , 2.86 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 80.14 ) */

       /* Links to SmartBrowser h_int120-browser-zoom. */
       RUN add-link IN adm-broker-hdl ( h_int120-browser-zoom , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int120-browser-zoom ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT120Z" "1.12.00.AVB"}
