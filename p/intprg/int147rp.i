...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "int147.txt"
           tt-param.destino = 3
...
def var tb-erro              as char format "x(50)" extent 42 initial
...
assign c-programa       = "{&c-programa-tipo}" + "RP"
       c-versao         = "2.12"
       c-revisao        = ".03.AVB"
...
/* apaga param-nf-estab para evitar erro de TSS */
for each param-nf-estab exclusive
...
procedure pi-importa:
    ...
    for each int_ds_mod_pedido no-lock where int_ds_mod_pedido.cod_prog_dtsul = "{&c-programa-tipo}",
        ...
        for each int_ds_pedido no-lock where
            ...
            /*AND int_ds_pedido.ped_codigo_n = 61625365 */
            query-tuning(no-lookahead):
            ...
            assign  c-nr-pedcli         = trim(string(int_ds_pedido.ped_codigo_n))
                    d-dt-implantacao    = /*today*/ int_ds_pedido.ped_data_d
                    d-dt-entrega        = int_ds_pedido.ped_dataentrega_d
                    d-dt-emissao        = /*int_ds_pedido.ped_data_d*/ today
            ...
            if int_ds_pedido.envio_status <> ? and c-cod-estabel <> "973" then next.
            */

            /* pedido de loja gerados no procfit com nota no procfit */
            if can-find (first int_dp_nota_saida no-lock where int_dp_nota_saida.ped_codigo_n = int_ds_pedido.ped_codigo) then do:
                assign i-erro = 42
            ...
            for each int_ds_pedido_produto no-lock of int_ds_pedido,
                ...
                assign  c-it-codigo         = trim(string(int_ds_pedido_produto.ppr_produto_n))
                        ...
                        c-obs-gerada        = c-numero_caixa
    
                        c-nr-docum          = if int_ds_pedido_produto.nen_notafiscal_n <> ? and
                                                 int_ds_pedido_produto.nen_notafiscal_n <> 0 and
                                                 /*
                                                 int_ds_pedido.ped_tipopedido_n <> 33 AND
                                                 int_ds_pedido.ped_tipopedido_n <> 2 AND
                                                 int_ds_pedido.ped_tipopedido_n <> 19 and
                                                 int_ds_pedido.ped_tipopedido_n <> 21
                                                 */ 
                                                 int_ds_mod_pedido.log_nat_origem
                                              then trim(string(int_ds_pedido_produto.nen_notafiscal_n,">>9999999")) else ""
                        c-serie-docum       = if int_ds_pedido_produto.nen_serie_s <> ? and 
                                                 /*
                                                 int_ds_pedido.ped_tipopedido_n <> 33 AND
                                                 int_ds_pedido.ped_tipopedido_n <> 2 AND
                                                 int_ds_pedido.ped_tipopedido_n <> 19 and
                        ...
                        i-seq-docum         = if int_ds_pedido_produto.nep_sequencia_n <> ? and 
                                                 /*
                                                 int_ds_pedido.ped_tipopedido_n <> 33 AND 
                                                 int_ds_pedido.ped_tipopedido_n <> 2 AND
                                                 int_ds_pedido.ped_tipopedido_n <> 19 and
                ...
                find first tt-ped-venda-prs where 
                    ...
                    tt-ped-venda-prs.numero_caixa = c-numero_caixa
                    no-error.
                ...
                if last-of(int_ds_pedido_retorno.rpp_caixa_n) then do:
            ...
            IF NOT AVAIL int_ds_pedido_retorno THEN DO:
                 assign i-erro = 37
                        c-informacao = "Ped: " + c-nr-pedcli + " Ori:" + c-cod-estabel
                        l-movto-com-erro = yes.
                 run gera-log. 
...
procedure finaliza-pedido.
    ...
    if not l-movto-com-erro then do:
        ...
        for each nota-fiscal fields (cod-estabel serie nr-nota-fis)
            no-lock where nota-fiscal.nr-pedcli = trim(string(int_ds_pedido.ped_codigo_n)) and
...
procedure valida-registro-pedido.
    ...
    if d-dt-procfit <> ? and d-dt-procfit <= d-dt-implantacao then do:
        assign i-erro = 42
               c-informacao = "Ped: " + c-nr-pedcli + " Ori:" + c-cod-estabel
               l-movto-com-erro = yes.
        run gera-log. 
    ...
    for first nota-fiscal fields (cod-estabel serie nr-nota-fis dt-cancel) no-lock where
        ...
        if nota-fiscal.dt-cancel = ? then do:
            assign i-erro = 36
                   c-informacao = "Ped: " + c-nr-pedcli + " Origem: " + string(int_ds_pedido.ped_cnpj_origem) + " Est: " + 
    ...
    */
    ...
    then do:
        for first btransporte no-lock where
            btransporte.cod-transp = int(c-redespacho)
            query-tuning(no-lookahead). end.
       ...
       if not avail item-doc-est then do:
            ...
            then do:
                ...
                if can-find (first int_ds_docto_xml no-lock where
                    ...
                    assign tt-param-int073.arquivo = "INT073-147" + ".tmp":U.
                ...
                if can-find (first int_ds_nota_entrada no-lock where 
                    ...
                    /* Oblak */
                    assign tt-param-int012.arquivo = "INT012-147" + ".tmp":U.
                    raw-transfer tt-param-int012 to raw-param.
                    run intprg/int012rp.p (input raw-param,
                                           input table tt-raw-digita).
                    /* Procfit */
                    assign tt-param-int012.arquivo = "INT112-147" + ".tmp":U.
                    raw-transfer tt-param-int012 to raw-param.
...
procedure cria-tt-ped-venda-prs.
   ...
   assign tt-ped-venda-prs.cod-portador   = if i-cod-portador <> ? then i-cod-portador else emitente.port-prefer
          ...
          tt-ped-venda-prs.cod-des-merc   = if natur-oper.consum-final 
                                            then 2 else 1
          tt-ped-venda-prs.observacoes    = c-observacao + if c-numero_caixa <> "" then " Caixa: " + c-numero_caixa else ""
...
procedure cria-tt-ped-item-prs.
   ...
   for first tt-ped-item-prs where
       ...
       tt-ped-item-prs.numero_lote  = c-numero_lote
       query-tuning(no-lookahead): end.
...
procedure pi-gera-itens-nota:
    ...
    for each tt-ped-item-prs no-lock of tt-ped-venda-prs where 
        ...
        for first item fields (it-codigo deposito-pad item.cod-localiz)
            no-lock where item.it-codigo = tt-ped-item-prs.it-codigo
            query-tuning(no-lookahead): end.
        for first item-uni-estab no-lock where 
    ...
    /*if tt-ped-item-prs.numero_lote <> "" and 
       item.tipo-con-est = 3 /* lote */ then*/ do:
        ...
        if de-qt-utilizada > 0 then do:
            assign i-erro = 33
                   c-informacao = "Ped: " + c-nr-pedcli + " Lot: " + tt-ped-item-prs.numero_lote
                   l-movto-com-erro = yes.
            run gera-log. 
            return.
        end.
...
procedure pi-saldo-terc:
    ...
    if de-qt-utilizada > 0 then do:
        ...
        for first tt-it-terc-nf where 
            tt-it-terc-nf.rw-saldo-terc = rowid(saldo-terc)
            query-tuning(no-lookahead): end.
        if not avail tt-it-terc-nf then do:
            for first item no-lock where 
                item.it-codigo = saldo-terc.it-codigo
                query-tuning(no-lookahead): end.
            ...
            for each bcomponente
                ...
                where bcomponente.cod-emitente = componente.cod-emitente 
                  ...
                  and (bcomponente.quantidade <> 0
                   OR  bcomponente.dt-retorno <= today) no-lock
                query-tuning(no-lookahead): /* NF Reajuste ate Dt Trans */ 
