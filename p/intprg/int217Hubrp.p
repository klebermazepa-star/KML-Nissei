...
{include/i-prgvrs.i int217Hubrp 2.12.06.AVB}
{cdp/cdcfgdis.i}
{utp/ut-glob.i}
...
DEF INPUT PARAMETER TABLE FOR TT-NOTA-FISCAL-EMITIDA.
...
DEFINE BUFFER b2-natur-oper FOR natur-oper.
...
def var c-bairro            like ems2mult.estabelec.bairro no-undo.
def var c-cidade            like ems2mult.estabelec.cidade no-undo.
def var c-pais              like ems2mult.estabelec.pais no-undo.
def var c-endereco          like ems2mult.estabelec.endereco no-undo.
def var i-ep-codigo         like ems2mult.estabelec.ep-codigo no-undo.
...
def BUFFER bestabelec for ems2mult.estabelec.
...
FOR EACH TT-NOTA-FISCAL-EMITIDA:
    ...
    for each int_dp_nota_saida no-lock             
        WHERE int_dp_nota_saida.nsa_notafiscal_n  = TT-NOTA-FISCAL-EMITIDA.numero 
          AND int_dp_nota_saida.nsa_serie_s       = TT-NOTA-FISCAL-EMITIDA.serie
          AND int_dp_nota_saida.nsa_cnpj_origem_s = TT-NOTA-FISCAL-EMITIDA.emitente_cnpj
        :
        ...
        for each int_ds_tipo_pedido no-lock where 
            ...
            log-manager:write-message("KML - 7 " + string(l-erro)) no-error.
            ...
            for each int_ds_mod_pedido no-lock where 
               ...
               (int_ds_mod_pedido.cod_prog_dtsul = "int147" or /* faturamentos pedidos */
                ...
                int_ds_mod_pedido.cod_prog_dtsul = "int217" OR /* importaá∆o de notas */
                int_ds_mod_pedido.cod_prog_dtsul = "int041" OR   /*estorno devoluá∆o*/
                int_ds_mod_pedido.cod_prog_dtsul = "int117" )
                :
                
                IF NOT AVAIL int_ds_mod_pedido THEN
                DO:
                    CREATE tt-erros.
                    ASSIGN tt-erros.CNPJDEV    = int_dp_nota_saida.nsa_cnpj_origem_s 
                           tt-erros.NumeroNota = STRING(int_dp_nota_saida.nsa_notafiscal_n)
                           tt-erros.desc-erro  = "Modalidade de Pedido n∆o encontrada"  
                           tt-erros.l-erro     = YES.
               ...
               MESSAGE "nem entrou ="   
                    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
                log-manager:write-message("KML - 8 " + string(l-erro)) no-error.
                ...
                log-manager:write-message("KML - 9 " + string(l-erro)) no-error.                              

                RUN pi-valida-cabecalho.
                
                log-manager:write-message("KML - 10 " + string(l-erro)) no-error.
                ...
                FIND first tt-it-docto NO-ERROR.
                if not l-erro and not avail tt-it-docto then do:
                    ...
                    log-manager:write-message("KML - 1 " + string(l-erro)) no-error.
                    next.
                end.
                ...
                if not l-erro then do:
                    ...
                    if not l-erro then do:
                        ...
                        log-manager:write-message("KML - 2 " + string(l-erro)) no-error.
                        RUN pi-importa-nota.
                        log-manager:write-message("KML - 11 " + string(l-erro)) no-error.
                        run pi-elimina-tabelas.
                    end.
                end.
        ...
        log-manager:write-message("KML - 12 " + string(l-erro)) no-error.
        run pi-elimina-tabelas.
    end. /* for-nota */
END.
...
procedure pi-cria-tt-docto.
    ...
    if  TrataNuloChar(int_dp_nota_saida.nsa_cupomorigem_s) <> "" or
        ...
        assign  tt-nota-fiscal-adc.cod-estabel              = c-cod-estabel
                ...
                tt-nota-fiscal-adc.cod-ser-docto-referado   = TrataNuloChar(int_dp_nota_saida.nsa_numeroserieecf_s). //era TrataNuloChar(int_dp_nota_saida.nen_serieorigem_s).
...
procedure pi-cria-tt-it-docto.
   ...
   log-manager:write-message("KML - DENTRO INT227 -" + STRING(int_dp_nota_saida_item.nsi_cstb_n)) no-error. 
   ...
   assign tt-it-docto.calcula             = yes
          ...
          tt-it-docto.vl-merc-liq         = truncate(TrataNuloDec(int_dp_nota_saida_item.nsi_valorliquido_n),2) // era truncate(TrataNuloDec(int_dp_nota_saida_item.nsi_valorbruto_n),2)   
    ...
    /*  KML - 26/08/2022 - Alteraáao para Feira gestante */
   IF c-nat-operacao = "5915A6" OR c-nat-operacao = "5904A1" THEN DO: 
   
       FIND FIRST b2-natur-oper NO-LOCK
           WHERE b2-natur-oper.nat-operacao = c-nat-operacao NO-ERROR.
       IF AVAIL b2-natur-oper  THEN DO:
    
           IF b2-natur-oper.baixa-estoq = NO THEN DO:
                ASSIGN  tt-it-docto.baixa-estoq = NO.
           END.
       END.
   END.
   /*  KML - Alteraáao para Feira gestante */
   ...
   assign tt-it-imposto.seq-tt-it-docto   = i-nr-sequencia
          ...
          tt-it-imposto.vl-icms-outras    = if i-cd-trib-icm = 3 then TrataNuloDec(int_dp_nota_saida_item.nsi_baseicms_n) else
                                            ...
                                            then TrataNuloDec(int_dp_nota_saida_item.nsi_valorliquido_n - int_dp_nota_saida_item.nsi_baseicms_n)  //era o contrario
                                            else 0                                                                 
    ...
    if int_dp_nota_saida.ped_codigo <> ? and
       int_dp_nota_saida.ped_codigo <> 0  then do:
        ...
        FIND first int_ds_pedido no-lock where 
                  int_ds_pedido.ped_codigo_n = int_dp_nota_saida.ped_codigo_n NO-ERROR.
                  
        if not avail int_ds_pedido then do:
           FIND first int_ds_pedido_subs no-lock where 
                     int_ds_pedido_subs.ped_codigo_n = int_dp_nota_saida.ped_codigo_n NO-ERROR.
...
/*         END.                                                                                            */
    ...
    c-uf-origem = "".
    FOR first ems2mult.emitente fields (cod-emitente nome-abrev estado identific) no-LOCK WHERE
    ...
    for each nota-fiscal no-lock where 
        ...
        for each bint_dp_nota_saida where
            ...
            for each int_ds_pedido exclusive-lock where 
                ...
                ASSIGN int_ds_pedido.envio_status      = 8
                       int_ds_pedido.retorno_validacao = "".
        ...
        log-manager:write-message("KML - 3 " + string(l-erro)) no-error.
        return.
    end.
    ...
    FIND first ems2mult.emitente no-lock 
        WHERE  emitente.cgc = int_dp_nota_saida.nsa_cnpj_destino_s NO-ERROR. 
    ...
    for first param-estoq no-lock : 
        ...
        then do:
         ...
         /* Descomentar quando precisar alterar data do cupom fiscal 
           FIND FIRST bint_dp_nota_saida EXCLUSIVE-LOCK
                WHERE ROWID(bint_dp_nota_saida) = ROWID(int_dp_nota_saida) NO-ERROR.

            ASSIGN bint_dp_nota_saida.nsa_dataemissao_d = 03/01/2019.
            ...
            */
            ...
            log-manager:write-message("KML - 4 " + string(l-erro)) no-error.                 
            return.
            //l-erro = YES.
        end.
    end.
   ...
   if not avail ser-estab then do:
    ...
    if i-cod-rep <> 0 then do:
        FIND first ems2mult.repres no-lock where 
    ...
    FIND FIRST ems2mult.emitente NO-LOCK 
        WHERE emitente.cgc = trim(int_dp_nota_saida.nsa_cnpj_transportadora_s) NO-ERROR.
    ...
    if not avail emitente then do:
        ...
        FIND FIRST bint_dp_nota_saida EXCLUSIVE-LOCK
            WHERE ROWID(bint_dp_nota_saida) = ROWID(int_dp_nota_saida) NO-ERROR.
        IF AVAIL bint_dp_nota_saida THEN
            ASSIGN bint_dp_nota_saida.nsa_cnpj_transportadora_s = ?.
        RELEASE bint_dp_nota_saida.
    ...
    */
    ...
    then do:
        for first item-doc-est no-lock where 
            ...
            item-doc-est.it-codigo    = TrataNuloChar(c-it-codigo) :
        ...
        if not avail item-doc-est then do:
            run pi-gera-log (input "Documento origem a devolver n∆o localizado - " 
                             ...
                             + " Pedido: " + TrataNuloChar(string(int_dp_nota_saida.ped_codigo_n))
                             + " Tipo pedido: " + STRING(ped_tipopedido_n),
                             input 1).
            next.
        end.
    end.
    ...
    MESSAGE  "PORCARIA DE INT115" SKIP
             int_ds_tipo_pedido.tp_pedido  SKIP
             c-uf-destino                  SKIP
             c-uf-origem                   SKIP
             c-nat-origem                  SKIP
             i-cod-emitente                SKIP
...
// comentada esta parte pois anteriormente a procfit realizava as criaá‰es de tabela e agora Ç realizado pela API (N∆o consigo Diferenciar) Lohan KML 18/12/2024 PROD  
            ...
            for each nota-fiscal exclusive-lock where 
                ...
                for each bint_dp_nota_saida exclusive-lock where
                    ...
                    if nota-fiscal.esp-docto = 23 /* NFT */ then do:
                        FIND first ems2mult.estabelec no-lock where estabelec.cod-estabel = nota-fiscal.cod-estabel NO-ERROR.
...
/*     END.                                                             */
    ...
    /* INICIO - Gravaªío LOG Tabela para a PROCFIT */
    FIND CURRENT int_dp_nota_saida EXCLUSIVE-LOCK NO-ERROR.
    IF AVAIL int_dp_nota_saida THEN DO:
       IF int_dp_nota_saida.situacao = 2 THEN DO:
           ASSIGN int_dp_nota_saida.envio_status      = 8
                  int_dp_nota_saida.retorno_validacao = "".
       END.
       ELSE IF int_dp_nota_saida.situacao = 1 THEN DO:
           ASSIGN int_dp_nota_saida.envio_status      = 9
                  int_dp_nota_saida.retorno_validacao = substring(c-informacao,1,500).
       END.
    END.
    /* FIM    - Gravaªío LOG Tabela para a PROCFIT */
    
                              
    
    if i-situacao = 1 then l-erro = yes.
    
    log-manager:write-message("KML - 5 " + string(l-erro)) no-error.
    ...
    ASSIGN tt-erros.CNPJDEV    = int_dp_nota_saida.nsa_cnpj_origem_s 
           tt-erros.NumeroNota = STRING(int_dp_nota_saida.nsa_notafiscal_n)
           //tt-erros.numeroDev  = int(c-num-cup)
           //tt-erros.DATADEV    = int_dp_nota_saida.nsa_cnpj_destino_s
           tt-erros.desc-erro  = int_dp_nota_saida.nsa_cnpj_origem_s + "/" + trim(int_dp_nota_saida.nsa_serie_s) + "/" + trim(string(int_dp_nota_saida.nsa_notafiscal_n)) + "/" + c-informacao  
           tt-erros.situacao   = i-situacao 
           tt-erros.l-erro     = l-erro. 

end.

{intprg/int011rp-Hub.i} /* pi-saidas */
