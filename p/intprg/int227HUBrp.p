...
DEF INPUT PARAMETER TABLE FOR tt-nota-devolucao.
...
/* ASSIGN tt-param.arquivo = "int227HUB.txt"    */
/*        tt-param.destino = 3                  */
...
FOR EACH tt-nota-devolucao:
    ...
    for each int_dp_nota_devolucao NO-LOCK
        WHERE int_dp_nota_devolucao.ndv_cnpjfilialdevolucao_s = tt-nota-devolucao.destinatario_cnpj
        ...
        AND   int_dp_nota_devolucao.ndv_notafiscal_n          = tt-nota-devolucao.numero :
            ...
            IF int_dp_nota_devolucao.ndv_cnpjfilialorigem_s = tt-nota-devolucao.emitente_cnpj   THEN DO:
            
                
                log-manager:write-message("KML - AP‡S FOR EACH") no-error.
                log-manager:write-message("KML - DENTRO INT227 - TEMP-TABLE - " + tt-nota-devolucao.emitente_cnpj) no-error.
                log-manager:write-message("KML - DENTRO INT227 - TEMP-TABLE - " + tt-nota-devolucao.serie) no-error.
                log-manager:write-message("KML - DENTRO INT227 - TEMP-TABLE - " + string(tt-nota-devolucao.numero)) no-error.
    ...
    */
    for each it-nota-fisc no-lock where 
        ...
        it-nota-fisc.nr-seq-fat    = (if int_dp_nota_devolucao_item.ndp_sequenciaorigem_n > 0 
                                      ...
                                      else it-nota-fisc.nr-seq-fat)          and
        it-nota-fisc.dt-emis-nota <= int_dp_nota_devolucao.ndv_dataemissao_d
        query-tuning(no-lookahead): 
...
procedure pi-gera-log:
    ...
    log-manager:write-message("KML - Entrou GERA-LOG") no-error.
...
/*             skip.                                                                  */
/*     end.                                                                           */
    if i-situacao = 1 then l-erro = yes. //KML

    RUN intprg/int999.p ("DEV PROC", 
                         int_dp_nota_devolucao.ndv_cnpjfilialdevolucao_s + "/" +       // era int_dp_nota_devolucao.ndv_cnpjfilialorigem_s
                         c-num-nota + "/" + 
                         c-num-cup + "/" + 
                         string(int_dp_nota_devolucao.ndv_dataemissao_d,"99/99/9999"),
                         c-informacao,
                         ...
                         "int227HUBrp.p").
    CREATE tt-erros.
    ASSIGN tt-erros.CNPJDEV    = int_dp_nota_devolucao.ndv_cnpjfilialdevolucao_s //era int_dp_nota_devolucao.ndv_cnpjfilialorigem_s 
           tt-erros.NumeroNota = c-num-nota
           tt-erros.numeroDev  = int(c-num-cup)
           tt-erros.DATADEV    = int_dp_nota_devolucao.ndv_dataemissao_d
           tt-erros.desc-erro  = int_dp_nota_devolucao.ndv_cnpjfilialdevolucao_s + "/" + c-num-nota + "/" + STRING(c-num-cup) + "/" +  c-informacao
...
procedure pi-gera-tt-fat-devol:
    ...
    for each int_dp_nota_devolucao_cond no-lock of int_dp_nota_devolucao
        ...
        IF int_dp_nota_devolucao.NDV_SERIEORIGEM_S <> "402" THEN
        DO:
        ...
        if not l-aux AND int_dp_nota_devolucao.NDV_SERIEORIGEM_S <> "402" then do:
