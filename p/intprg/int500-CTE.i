    ...
    def temp-table ttInfCte no-undo xml-node-name 'infCte'
    field ttInfCte as int xml-node-type "hidden"
    FIELD ttVPrest AS int xml-node-type "hidden"
    FIELD ttImp    AS int xml-node-type "hidden"
    FIELD ttIcms as int xml-node-type "hidden"
    field Id as char format "x(44)" xml-node-type "attribute"
    field versao as char format "x(04)" xml-node-type "attribute"
    field nome as char initial "Informação cte".
    ...
    field cCT as dec format "999999999"
    FIELD CFOP  AS CHAR FORMAT "x(04)"
    ...
    field nCT as dec format "999999999"
    ...
    FIELD tpCTe AS DEC FORMAT "9"
    field finNFe as dec format "9"
    field procEmi as dec format "9"
    field verProc as char format "x(20)"
    FIELD cMunEnv AS CHAR FORMAT "x(20)"
    FIELD xMunEnv AS CHAR FORMAT "x(20)"
    FIELD UFEnv   AS CHAR FORMAT "x(02)"
    FIELD modal   AS DEC FORMAT  "99"
    FIELD tpServ  AS DEC FORMAT   "9"
    FIELD cMunIni AS CHAR FORMAT "x(20)"
    FIELD xMunIni AS CHAR FORMAT "x(20)"
    FIELD UFIni   AS CHAR FORMAT "x(02)"
    FIELD cMunFim AS CHAR FORMAT "x(20)"
    FIELD xMunFim AS CHAR FORMAT "x(20)"
    FIELD UFFim   AS CHAR FORMAT "x(02)"
    FIELD retira  AS DEC  FORMAT "9"
    FIELD indIEToma AS DEC  FORMAT "9"
    ...
    def temp-table ttRem no-undo xml-node-name 'rem'
    field ttInfNFe as int xml-node-type "hidden"
    field ttRem as int xml-node-type "hidden"
    field tag as char format "x(20)"
    field CNPJ as char format "x(14)"
    field CPF as char format "x(11)"
    field xNome as char format "x(60)"
    ...
    field nome as char initial "Remetente".
    ...
    field ttImp as int xml-node-type "hidden"
    field nItem as dec format "999" xml-node-type "attribute"   //CTE
    field ttImposto as int xml-node-type "hidden"
    field ttIcms as int xml-node-type "hidden"
    ...
    field ChCTe as char format "x(60)" xml-node-type "hidden".
       
    def temp-table ttVPrest no-undo xml-node-name 'vPrest'
    field ttVPrest AS int xml-node-type "hidden"
    FIELD vTPrest AS dec format "zzzz,zzz,zzz,zz9.99".
    
    def temp-table ttComp no-undo xml-node-name 'Comp'
    field ttVPrest AS int xml-node-type "hidden"
    field xNome AS CHAR
    field vComp AS dec format "zzzz,zzz,zzz,zz9.99".
    
    def temp-table ttImp no-undo xml-node-name 'imp'
    field ttImp as int xml-node-type "hidden".
   
    def dataset dsCte xml-node-name 'CTe'
    for ttInfNFe, ttInfCte, ttIde, ttImp,  ttNFref, ttRefNF, ttEmit, ttEnderEmit, ttDest, ttRem, ttEnderDest, ttDet,
    ...
    ttCobr, ttFat, ttDup, ttpag, ttdetpag, ttInfAdic, ttCompra, ttrastro, ttmed, ttExporta , ttVPrest , ttComp    

    //data-relation aa for ttInfNFe ,ttIde relation-fields(ttInfNFe,ttInfNFe) NESTED
    
    data-relation aa for ttInfCte ,ttIde relation-fields(ttInfCte,ttInfCte) nested
    
    data-relation aaa for ttInfCte ,ttVPrest relation-fields(ttVPrest,ttVPrest) nested
    
    DATA-RELATION BBB for ttVPrest ,ttComp relation-fields(ttVPrest,ttVPrest) nested
     
    data-relation ccc for ttInfCte ,ttImp relation-fields(ttImp,ttImp) nested
    
    DATA-RELATION ddd for ttImp ,ttIcms relation-fields(ttImp,ttImp) nested
    
    DATA-RELATION eee for ttIcms ,ttICMS00 relation-fields(ttImp,ttImp) nested
    
    DATA-RELATION FFF for ttInfNFe ,ttRem relation-fields(ttInfNFe,ttInfNFe) NESTED
   ...
   def dataset dsChave xml-node-name 'protCTe'
   FOR ttInfProt.
