...
IF tt-param.i-nro-docto-ini = "0" OR tt-param.i-nro-docto-ini = ""  THEN 
    ASSIGN tt-param.destino = 3
           tt-param.arquivo = session:temp-directory + "INT002brp.txt"  //IF tt-param.destino = 3 THEN SESSION:TEMP-DIRECTORY + "INT002brp.txt" ELSE "INT002brp.txt" 
...
DEF BUFFER b-estab                 FOR ems2mult.estabelec.
    ...
    FOR EACH tt-docto-xml NO-LOCK
        ...
        FOR FIRST int_ds_docto_xml NO-LOCK  
            WHERE int_ds_docto_xml.tipo_nota = tt-docto-xml.tipo_nota  
              ...
              AND int_ds_docto_xml.serie     = tt-docto-xml.serie:
        END.
        IF AVAIL int_ds_docto_xml THEN DO:
...
/*         END.                                                                                                                 */
        ...
        FOR EACH ems2mult.emitente USE-INDEX cgc NO-LOCK 
...
/*         END. /* each tt-it-docto-xml */                                                                                                                                                                  */
        ...
        MESSAGE "FINAL DE TUDO "
                int_ds_docto_xml.situacao
            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    ...
    FOR EACH tt-docto-xml:
        ...
        FIND FIRST int_ds_nota_entrada 
         ...
         ELSE DO:
            ...
            FIND FIRST int_ds_docto_xml 
                  WHERE int_ds_docto_xml.CNPJ   = tt-docto-xml.CNPJ
                  AND   int_ds_docto_xml.serie  = tt-docto-xml.serie
                  AND   int_ds_docto_xml.nNF    = tt-docto-xml.NNF NO-ERROR.
             ...
             DO:
                ...
                DO:
                    ...
                    ASSIGN  int_ds_docto_xml.valor_mercad = ttVPrest.vTPrest.    
                ...
                ELSE DO:
                    ...
                    DO:
                        ...
                        ASSIGN  int_ds_docto_xml.valor_mercad = ttComp.vComp.
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION USE-INDEX idx_status NO-LOCK  
      ...
      //    AND NDD_ENTRYINTEGRATION.CNPJEMIT        = 73856593000166
		...
		/* KML - Guilherme Nichele 28/11/2024 */
		
		
        if can-find (first ems2mult.estabelec no-lock where estabelec.cgc = STRING(NDD_ENTRYINTEGRATION.CNPJEMIT) and
                     ( estabelec.cod-estabel = "10001"  OR estabelec.cod-estabel = "10004") )  then do transaction:
        ...
        DO:
            ...
            THEN do:
                ...
                log-manager:write-message("KML - pi-IntegracaoNDD 9.1 " + "TESTE LOHAN MERCO") no-error.
...
PROCEDURE pi-abre-xml:
    ...
    for first ttIde ON ERROR UNDO, RETURN ERROR:
        ...
        END.
        LOG-MANAGER:WRITE-MESSAGE("KML NFE - entrou for each int002g - nnfe: " + STRING(ttIde.nCT,">>>9999999")) NO-ERROR.
        
        
        ASSIGN tt-docto-xml.nNF = trim(STRING(ttIde.nCT,">>>9999999")).
        ...
        for first ttEmit:
           ...
           FIND FIRST ems2mult.estabelec NO-LOCK WHERE
           ...
           FOR EACH ems2mult.emitente NO-LOCK WHERE 
        ...
        for first ttRem:
            ...
            MESSAGE "DENTRO DESTINATARIO"
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
             ASSIGN tt-docto-xml.CNPJ_dest   = STRING(ttRem.CNPJ)
                    tt-docto-xml.tipo_estab  = 1.
             ...
             IF AVAIL estabelec THEN DO:
                 ...
                 IF estabelec.cod-estabel = estabelec.cod-estabel THEN
                    ASSIGN tt-docto-xml.tipo_estab = 1. /* Principal/CD */
                 ELSE 
                    ASSIGN tt-docto-xml.tipo_estab = 2. /* Frente de Loja */  //ALTERADO TESTE KML
        ...
        FOR EACH ttprod 
           ...
           then do:
            ...
            // KML - 05/11/2024 - Alterado leitura do pis/cofins para nÆo validar item fornecedor e ler pis/cofins do XML
		...
		/* KML - Guilherme Nichele 28/11/2024 */
		FIND FIRST 	centro_distribuicao NO-LOCK
			WHERE 	centro_distribuicao.deposito_padrao = estabelec.cod-estabel
			and 	centro_distribuicao.flag-CD = yes NO-ERROR.
        ...
        if  not can-find (first b-estab no-lock where b-estab.cgc = tt-docto-xml.CNPJ) and
            can-find (first estabelec no-lock where estabelec.cgc = tt-docto-xml.CNPJ_dest and
                      estabelec.cod-estabel <> centro_distribuicao.deposito_padrao) and 
            c-doc <> ? and c-doc <> ""
        then do:
