...
DEF BUFFER b-preco-item      FOR ems2dis.preco-item.
...
PROCEDURE calculaValorPautaItem:
    ...
    then
        for FIRST ems2dis.tb-preco
            fields ()
            where tb-preco.nr-tabpre = substr(emitente.char-1,1,5) NO-LOCK:
    ...
    THEN DO:
        ...
        for first item-cli
            ...
            where item-cli.it-codigo  = ITEM.it-codigo
            and   item-cli.nome-abrev = emitente.nome-abrev NO-LOCK:
        end.
        ...
        then
           ...
           FIND FIRST b-preco-item NO-LOCK
               WHERE b-preco-item.it-codigo   = ITEM.it-codigo
                 AND b-preco-item.nr-tabpre   = "PR-FP"
                 and b-preco-item.situacao   = 1 /* Ativo */
                 and b-preco-item.quant-min <= de-qtd-item NO-ERROR.

           IF AVAIL b-preco-item THEN DO:

                /* Valor da pauta pelo preco do item na tabela de pauta */
                assign de-valor-pauta = b-preco-item.preco-venda
                       c-tabela-pauta = "PR-FP".

           END.
           ELSE DO:
           
                for first ems2dis.preco-item
                    ...
                    and   preco-item.quant-min <= de-qtd-item NO-LOCK:
