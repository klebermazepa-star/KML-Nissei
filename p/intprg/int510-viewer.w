...
DEFINE VARIABLE i-id-seq        AS INTEGER     NO-UNDO.
DEFINE VARIABLE i-sit-docto     AS INTEGER     NO-UNDO.
DEFINE VARIABLE da-dt-movto     AS DATE        NO-UNDO.
DEFINE VARIABLE r-rowid-wms     AS ROWID       NO-UNDO.
...
DEF BUFFER b_int_ds_docto_xml FOR int_ds_docto_xml.
...
int_ds_docto_xml.nat_operacao int_ds_docto_xml.num_pedido ~
...
int_ds_docto_xml.dEmi int_ds_docto_xml.nat_operacao ~
int_ds_docto_xml.num_pedido int_ds_docto_xml.vbc ~
...
DEFINE FRAME f-main
     ...
     int_ds_docto_xml.cod_estab AT ROW 3 COL 11.29 COLON-ALIGNED WIDGET-ID 6
          LABEL "Filial" FORMAT "x(5)"
          VIEW-AS FILL-IN 
          SIZE 7.72 BY .88
     cEstabelec AT ROW 3 COL 19 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     ...
     int_ds_docto_xml.num_pedido AT ROW 4.79 COL 68 COLON-ALIGNED HELP
          "Numero do pedido de compra que est† sendo recebido" NO-LABEL WIDGET-ID 214 FORMAT ">>>>>,>>>>>>>>9"
          VIEW-AS FILL-IN 
          SIZE 12 BY .88
...
DO:
  /* Code placed here will execute AFTER standard behavior.    */
                                                 //FORMAT ">>>>>,>>>,>>9.99999"
  DEFINE VARIABLE vValor_Pis        AS DEC  FORMAT ">>>>,>>>,>>9.99"   NO-UNDO.
  DEFINE VARIABLE vValor_cofins    AS DEC FORMAT ">>>>,>>>,>>9.99"  NO-UNDO.
  ...
  if avail int_ds_docto_xml then do:
    ...
    IF v_cod_empres_usuar = "10" THEN DO:    
        ...
        FOR EACH  iNT_ds_it_docto_xml EXCLUSIVE-LOCK
            ...
            IF int_ds_it_docto_xml.vbc_pis <> 0 THEN
            DO:
                
                ASSIGN int_ds_it_docto_xml.vbc_pis =  (int_ds_it_docto_xml.vProd - int_ds_it_docto_xml.vicms)
                   int_ds_it_docto_xml.vbc_cofins = (int_ds_it_docto_xml.vProd - int_ds_it_docto_xml.vicms).
                   
                   
                //ASSIGN int_ds_it_docto_xml.vpis      = (int_ds_it_docto_xml.vProd * int_ds_it_docto_xml.ppis)
                //       int_ds_it_docto_xml.vcofins   = (int_ds_it_docto_xml.vProd * int_ds_it_docto_xml.pcofins) .
                
                ASSIGN int_ds_it_docto_xml.vpis =
                            ROUND(int_ds_it_docto_xml.vbc_pis
                                  * (int_ds_it_docto_xml.ppis / 100), 2)

                        int_ds_it_docto_xml.vcofins =
                            ROUND(int_ds_it_docto_xml.vbc_cofins
                                  * (int_ds_it_docto_xml.pcofins / 100), 2).

                
                
                
                ASSIGN vValor_Pis     = vValor_Pis +  int_ds_it_docto_xml.vpis
                        vValor_cofins = vValor_cofins + int_ds_it_docto_xml.vcofins .
                
                .MESSAGE  "aa "  int_ds_it_docto_xml.vpis
        ...
        .MESSAGE  "pis " vValor_Pis SKIP "cofins " vValor_cofins 
            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
        
        ASSIGN  int_ds_docto_xml.valor_pis    = vValor_Pis 
                int_ds_docto_xml.valor_cofins = vValor_cofins.
...
DO:
    ...
    for first ems2mult.emitente fields (nome-emit estado)
        no-lock where 
...
DO:
    ...
    for FIRST ems2mult.estabelec fields (nome cod-emitente)
        no-lock where 
        estabelec.cod-estabel = input frame f-Main int_ds_docto_xml.cod_estab:
        for first ems2mult.emitente fields (nome-abrev) no-lock where 
...
DO:
    ...
    for first natur-oper 
        ...
        if avail int_ds_docto_xml and
           ...
           &else
                   and natur-oper.log-contrib-st-antec = YES
           &endif
        then assign bt-gnre:sensitive in frame {&FRAME-NAME} = yes.
    end.  
    ...
    if avail b-int_ds_docto_xml then do:
        if trim(b-int_ds_docto_xml.cnpj_dest) = "" or 
           b-int_ds_docto_xml.cnpj_dest = ? then do:
            for first ems2mult.estabelec fields (cgc)
                no-lock where estabelec.cod-estabel = "973" OR estabelec.cod-estabel = "977"  OR estabelec.cod-estabel = "10001" OR estabelec.cod-estabel = "10004" OR estabelec.cod-estabel = "10008":
        ...
        if b-int_ds_docto_xml.cnpj   = "" or 
           ...
           b-int_ds_docto_xml.xNome  = ?  then do:
            for first ems2mult.emitente fields (cod-emitente nome-emit cgc) no-lock where 
...
------------------------------------------------------------------------------*/
  ...
  if avail int_ds_docto_xml then do:
      ...
      IF  int_ds_docto_xml.tipo_nota <> 3                AND 
          (int_ds_docto_xml.cnpj_dest  = "79430682025540" OR 
           int_ds_docto_xml.cnpj_dest  = "79430682057400" OR 
           int_ds_docto_xml.cnpj_dest  = "05912018000183"  OR 
           int_ds_docto_xml.cnpj_dest  = "05912018000426") AND 
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    for first ems2mult.emitente no-lock where
        ...
        first ems2mult.dist-emitente no-lock of emitente where dist-emitente.idi-sit-fornec = 1
        query-tuning(no-lookahead): 
        if avail int_ds_docto_xml and 
    ...
    .message "estabel " estabelec.cod-estabel VIEW-AS ALERT-BOX INFORMATION button ok. 
    
    if avail estabelec and 
       estabelec.cod-estabel <> "973" and
       estabelec.cod-estabel <> "977" and
       estabelec.cod-estabel <> "10001" and
       estabelec.cod-estabel <> "10004" AND
       estabelec.cod-estabel <> "10008" THEN DO: 
       
       .message "entrou valida erro estabelec" VIEW-AS ALERT-BOX INFORMATION button ok.
...
------------------------------------------------------------------------------*/
    ...
    FIND LAST int_ds_docto_wms USE-INDEX sequencial WHERE 
              int_ds_docto_wms.id_sequencial <> ? NO-LOCK NO-ERROR.
    IF AVAIL int_ds_docto_wms THEN
       ASSIGN i-id-seq = next-value(seq-int-ds-docto-wms).

    for each int_ds_docto_wms USE-INDEX documento where     
             int_ds_docto_wms.doc_numero_n  = int_ds_docto_xml.nNF and
             int_ds_docto_wms.doc_serie_s   = int_ds_docto_xml.serie and
             int_ds_docto_wms.doc_origem_n  = int_ds_docto_xml.cod_emitente 
        BY int_ds_docto_wms.situacao
        BY int_ds_docto_wms.envio_data_hora:

        ASSIGN int_ds_docto_wms.id_sequencial = i-id-seq
               i-id-seq                       = next-value(seq-int-ds-docto-wms).
    END.                                                     

    ASSIGN i-sit-docto = 0
           da-dt-movto = ?
           r-rowid-wms = ?.

    for each int_ds_docto_wms NO-LOCK USE-INDEX documento where     
             int_ds_docto_wms.doc_numero_n  = int_ds_docto_xml.nNF and
             int_ds_docto_wms.doc_serie_s   = int_ds_docto_xml.serie and
             int_ds_docto_wms.doc_origem_n  = int_ds_docto_xml.cod_emitente 
        BY int_ds_docto_wms.situacao
        ...
        ASSIGN i-sit-docto = int_ds_docto_wms.situacao
               da-dt-movto = int_ds_docto_wms.datamovimentacao_d
               r-rowid-wms = ROWID(int_ds_docto_wms).
        
    END.                                                     

    IF  i-sit-docto = 40
    AND da-dt-movto = ? THEN DO:
       ...
       for each int_ds_docto_wms NO-LOCK USE-INDEX documento where     
                int_ds_docto_wms.doc_numero_n  = int_ds_docto_xml.nNF and
                int_ds_docto_wms.doc_serie_s   = int_ds_docto_xml.serie and
                int_ds_docto_wms.doc_origem_n  = int_ds_docto_xml.cod_emitente AND
                int_ds_docto_wms.situacao      = 40 AND
                int_ds_docto_wms.datamovimentacao_d <> ?
            BY int_ds_docto_wms.envio_data_hora:

            ASSIGN da-dt-movto = int_ds_docto_wms.datamovimentacao_d.
            
       END.

       IF da-dt-movto <> ? THEN DO:
          FIND FIRST int_ds_docto_wms WHERE
                     ROWID(int_ds_docto_wms) = r-rowid-wms NO-ERROR.
          IF AVAIL int_ds_docto_wms THEN
             ASSIGN int_ds_docto_wms.datamovimentacao_d = da-dt-movto.
          RELEASE int_ds_docto_wms. 
       END.
    END.
    ...
    for each int_ds_docto_wms no-lock where
        ...
        if  int_ds_docto_wms.situacao = 55 and
            ...
            for each b-int_ds_docto_wms no-lock where 
                b-int_ds_docto_wms.doc_numero_n  = int_ds_docto_xml.nNF and
...
procedure pi-gera-tt-docto:
    ...
    create tt-docto.
    assign tt-docto.estado          = ems2mult.estabelec.estado
    ...
    assign tt-docto.seq-tt-docto    = i-seq-docto
           ...
           tt-docto.nome-abrev      = ems2mult.emitente.nome-abrev
...
------------------------------------------------------------------------------*/
   ...
   for each int_ds_it_docto_xml no-lock where
       ...
       if int_ds_it_docto_xml.tipo_nota <> 3 /* transferencia */ and
          not can-find (first ems2mult.estabelec no-lock where estabelec.cod-emitente = int_ds_docto_xml.cod_emitente) then do:
    ...
    /*
       ...
       if int_ds_docto_xml.situacao <> 7 /* j† aprovado compras */ and
          not can-find(first int_ds_docto_xml_compras no-lock of int_ds_docto_xml) then do:
           ...
           then do:
               ...
               if not avail ordem-compra and int_ds_param_re.vld-pedido 
                   ...
                   run pi-gera-erro(INPUT 14,    
                                    INPUT /*"Ordem de Compra " + string(int_ds_it_docto_xml.numero_ordem) + " N∆o cadastrada"*/
                                          "Produto n∆o consta no pedido de compra: " + trim(int_ds_it_docto_xml.it_codigo)).
               end.
               */
               ...
               if not avail item-mat then do:
                   run pi-gera-erro(INPUT 35,
                                    INPUT "Item materiais n∆o cadastrado: " + int_ds_it_docto_xml.it_codigo).   
               end.
               */
               /* AVB retirado 30/10/2017 - Projeto Procfit
    ...
    if avail int_ds_docto_xml then do:
        ...
        if i-situacao >= 10 and
           i-situacao <> 60 then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† est† em conferància no WMS." + 
                                     " Cancelamento/recusa somente ap¢s cancelamento da movimentaá∆o anterior.")).
            return "ADM-ERROR".
        end.
        */
        run utp/ut-msgs.p(input "show",
        ...
        do transaction:
            ...
            THEN DO:
                ...
                if avail b-int_ds_docto_xml then do:
                    ...
                    for first ems2mult.emitente fields (emitente.natureza) no-lock where 
                        emitente.cgc = int_ds_docto_xml.CNPJ: 
...
------------------------------------------------------------------------------*/
    ...
    if avail int_ds_docto_xml then do:
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input "Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† est† em processo de cancelamento.").
            return "ADM-ERROR".
        end.
        */
        if i-sit_re < 10 
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input "Reenvio bloqueado!" + "~~" + "A nota fiscal j† est† conferida.").
            return "ADM-ERROR".
        end.
        run utp/ut-msgs.p(input "show",
                          input 701,
        ...
        if return-value = "yes" then do:
            ...
            assign int_ds_docto_wms.doc_numero_n    = int_ds_docto_xml.nNF 
                   ...
                   int_ds_docto_wms.tipo_nota       = int_ds_docto_xml.tipo_nota
                   int_ds_docto_wms.ID_SEQUENCIAL   = NEXT-VALUE(seq-int-ds-docto-wms) .
            for first ems2mult.emitente no-lock where emitente.cgc = int_ds_docto_xml.CNPJ: 
...
------------------------------------------------------------------------------*/
    ...
    if RETURN-VALUE = "ADM-ERROR":U then do:
        run utp/ut-msgs.p(input "show",
                          input 17006,
                          input ("Atualizaá∆o inv†lida!" + "~~" + "Nota fiscal de estorno j† est∆o no RE1001.")).
        return "ADM-ERROR".
    end.
    
        //KML - 28/05/2024 - Operaá∆o Triangular
        
        IF int_ds_docto_xml.chave-ref-oper-triangular <> "" THEN
        DO:
                                                                                                                      
        FIND FIRST b_int_ds_docto_xml EXCLUSIVE-LOCK
            WHERE  b_int_ds_docto_xml.tipo_docto = int_ds_docto_xml.tipo_docto 
            AND    b_int_ds_docto_xml.tipo_estab = int_ds_docto_xml.tipo_estab
            AND    b_int_ds_docto_xml.chnfe      = int_ds_docto_xml.chave-ref-oper-triangular NO-ERROR. 
  
                  FIND FIRST natur-oper NO-LOCK
                    WHERE natur-oper.nat-operacao = b_int_ds_docto_xml.nat_operacao NO-ERROR.
                                     
                    IF natur-oper.nat-vinculada <> ""  AND natur-oper.baixa-estoq = NO AND natur-oper.emite-duplic = YES  THEN
                    DO:
                    
                    ASSIGN b_int_ds_docto_xml.situacao = 2.
                        
                        IF NOT AVAIL INT_ds_docto_wms THEN
                        DO:
                            
                       
                                          
                        CREATE int_ds_docto_wms.
                        ASSIGN int_ds_docto_wms.doc_numero_n        = b_int_ds_docto_xml.nNF
                               int_ds_docto_wms.doc_serie_s         = b_int_Ds_docto_xml.serie
                               int_ds_docto_wms.doc_origem_n        = b_int_ds_docto_xml.cod_emitente
                               int_ds_docto_wms.cnpj_cpf            = b_int_ds_docto_xml.CNPJ
                               int_ds_docto_wms.tipo_fornecedor     = "2"
                               int_ds_docto_wms.situacao            = 40
                               int_ds_docto_wms.datamovimentacao_d  = TODAY
                               int_ds_docto_wms.horamovimentacao_s  = string(DATETIME(TODAY, TIME))
                               int_ds_docto_wms.ID_SEQUENCIAL       = NEXT-VALUE (seq-int-ds-docto-wms)
                               int_ds_docto_wms.ENVIO_STATUS        = 8 
                               int_ds_docto_wms.ENVIO_DATA_HORA     = TODAY 
                               int_ds_docto_wms.ENVIO_ERRO          = ""
                               int_ds_docto_wms.retorno_validacao   = "" 
                               int_ds_docto_wms.tipo_nota           = b_int_ds_docto_xml.tipo_nota
                               int_ds_docto_xml.situacao            = 3.
                               
                               MESSAGE "Nota Financeira Atualizada para RE1001" SKIP
                                        int_ds_docto_wms.doc_numero_n           SKIP 
                                        int_ds_docto_wms.doc_serie_s
                                   VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
                        END.     
                    END.     
        END.       
    ...
    if avail int_ds_docto_xml then do:
        ...
        if  i-sit_re <> 40 /* movimentada WMS */ and
            ...
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Atualizaá∆o inv†lida!" + "~~" + "A nota fiscal ainda n∆o est† dispon°vel para importaá∆o no Totvs. Verifique a fase de recebimento no WMS.")).
            return "ADM-ERROR".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-guia-st V-table-Win 
PROCEDURE pi-guia-st :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    THEN DO:
        ...
        for first ems2mult.emitente fields (nome-abrev estado cidade pais)
            no-lock where 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-libera-agendamento V-table-Win 
PROCEDURE pi-libera-agendamento :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   do:
      ...
      find first int_ds_doc_erro no-lock where 
                ...
                int_ds_doc_erro.tipo_nota    = int_ds_docto_xml.tipo_nota    and
                int_ds_doc_erro.CNPJ         = int_ds_docto_xml.CNPJ         and     
                int_ds_doc_erro.tipo_erro   <> "Aviso" no-error.
   
   
      .message "entrou PI" VIEW-AS ALERT-BOX INFORMATION button ok.
   
      if not avail int_ds_doc_erro then do:
   
         .message "entrou IF" VIEW-AS ALERT-BOX INFORMATION button ok.
   
         
         RUN custom\eswms004.r (INPUT "Entrada" ,
                           INPUT "ENTRADA - FORNECEDOR" ,
                           INPUT ROWID(int_Ds_docto_xml)).
      end.
      else do:
      
         .message "ENTROU ELSE" VIEW-AS ALERT-BOX INFORMATION button ok.
      
         run utp/ut-msgs.p(input "show",
                            input 17006,
                            input ("Nota fiscal com ERROS!" + "~~" + "A nota fiscal possui erros. ê necess†rio acert†-los primeiro.")).
          return "ADM-ERROR".
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
 .message avail int_ds_docto_xml VIEW-AS ALERT-BOX INFORMATION button ok.
    ...
    if avail int_ds_docto_xml then do:
        ...
        then do:
            ...
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† est† em conferància no WMS." + 
                                     " Reenvio de conferància somente ap¢s cancelamento da movimentaá∆o anterior.")).
                                     */
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† est† em agendamento no WMS." + 
        ...
        then do:
            ...
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† est† conferida no WMS." + 
                                     " Reenvio de conferància somente ap¢s cancelamento da movimentaá∆o anterior.")).
                                     */
            run utp/ut-msgs.p(input "show",
                              input 17006,
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A movimentaá∆o da nota fiscal j† foi enviada p/o PRS. Movimentaá∆o n∆o ser† permitida.")).
            return "ADM-ERROR".
        end.
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal j† foi movimentada no PRS. Movimentaá∆o n∆o ser† permitida.")).
            return "ADM-ERROR".
        end.
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal ainda n∆o foi conferida no WMS. Movimentaá∆o n∆o ser† permitida.")).
            return "ADM-ERROR".
        end.
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Movimentaá‰es bloqueadas!" + "~~" + "A nota fiscal est† aguardando aprovaá∆o de compras. Conferància e movimentaá∆o n∆o ser∆o permitidas.")).
            return "ADM-ERROR".
        end.
        ...
        if i-sit_re <= /*26*/ 10 or
           i-sit_re  = 60 then do:
            ...
            run utp/ut-msgs.p(input "show",
                              input 701,
                              input ("liberaá∆o da nota fiscal para " +
                                     if p-situacao = 10 then /*'conferància no WMS' */ 'agendamento no WMS'
                                     else 'movimentaá∆o no PRS')).
            ...
            do transaction:
                ...
                if not avail int_ds_docto_wms then*/ do:
                    create int_ds_docto_wms.
                    assign int_ds_docto_wms.doc_numero_n  = TRIM(int_ds_docto_xml.nNF) 
                ...
                for first ems2mult.emitente no-lock where emitente.cgc = int_ds_docto_xml.CNPJ: end.
                assign  int_ds_docto_wms.tipo_fornecedor = if emitente.natureza = 1 then "F" else "J"
                ...
                // KML - 07/07/2025 - Alterado para enviar nota VIA API HUB
                
                RUN intprg/int510-procfit(INPUT ROWID(int_ds_docto_xml)).
                ASSIGN int_ds_docto_wms.ENVIO_STATUS    = 2.
                release int_ds_docto_wms.
            end.
            ...
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Modificaá‰es bloqueadas!" + "~~" + "A nota fiscal j† foi conferida ou movimentada. Alteraá‰es n∆o ser∆o permitidas.")).
            return "ADM-ERROR".
        end.
    end.
    return "OK":U.    
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-refleshdata V-table-Win 
PROCEDURE pi-refleshdata :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  /*  if valid-handle (h_int510-viewer) then
        RUN pi-avaliar-erros in h_int510-viewer.    
    if valid-handle (h_int510-viewer) then
        RUN dispatch IN h_int510-viewer ('row-available':U).
    if valid-handle (h_int510-viewer-totais) then
        RUN dispatch IN h_int510-viewer-totais ('row-available':U).
    if valid-handle (h_int510-browser-itens-2) then
        RUN dispatch IN h_int510-browser-itens-2 ('row-available':U).
    if valid-handle (h_int510-browser-erros) then
        RUN dispatch IN h_int510-browser-erros ('row-available':U).
    if valid-handle (h_int510-browser-itens-2) then
        RUN pi-refresh IN h_int510-browser-itens-2.

    /* geazi */
    if valid-handle (h_int510-browser-itens-2) then
        RUN pi-refresh IN h_int510-viewer-triangular.   */
...
------------------------------------------------------------------------------*/
    ...
    define input  param pi-emitente     like ems2mult.emitente.cod-emitente   no-undo.
...
------------------------------------------------------------------------------*/
    ...
    if avail int_ds_docto_xml then do:
        ...
        for first ems2mult.cond-pagto no-lock where 
        ...
        for first ordem-compra fields (tp-despesa)
            ...
            query-tuning(no-lookahead): 
            for first ems2mult.emitente fields (tp-desp-padrao) no-lock where 
        ...
        for each tt-int_ds_docto_xml_dup no-lock 
            ...
            if avail int_ds_param_re and int_ds_param_re.vld-pedido and 
               int_ds_docto_xml.situacao <> 7 /* j† aprovada compras */ and 
               not can-find (first int_ds_docto_xml_compras no-lock of int_ds_docto_xml)
            then do:
                for first tt-dupli no-lock where /*
                    ...
                    if tt-int_ds_docto_xml_dup.dt-vencto < tt-dupli.dt-vencim then do:
                        run pi-gera-erro(INPUT 21,    
                                         ...
                                         " n∆o confere com condiá∆o de pagto: " +  string(tt-dupli.dt-vencim,"99/99/9999")).
                        return.
                    end.
                end.
