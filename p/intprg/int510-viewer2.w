...
DEFINE VARIABLE tot-base-guia-st        AS DECIMAL INITIAL 0 NO-UNDO.
DEFINE VARIABLE tot-valor-guia-st       AS DECIMAL INITIAL 0 NO-UNDO.
...
DEF BUFFER b-int_ds_docto_xml_guia_st  FOR int_ds_docto_xml.
...
DO:
  ...
  if avail int_ds_docto_xml and
     int_ds_docto_xml.situacao <> 3 then do:
      ...
      /*
      if int_ds_docto_xml.sit-re < 25 or 
         int_ds_docto_xml.sit-re = 60 then*/ do:
          RUN pi-calcula-totais.

          for first b-int_ds_docto_xml where rowid(b-int_ds_docto_xml) = rowid(int_ds_docto_xml):
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    for last int_ds_docto_wms 
        use-index sequencial EXCLUSIVE where 
    ...
    then do:
        ...
        IF  input FRAME {&FRAME-NAME} int_ds_docto_xml.base_guia_st  <> tot-base-guia-st  OR 
            input FRAME {&FRAME-NAME} int_ds_docto_xml.valor_guia_st <> tot-valor-guia-st
        THEN DO:
            ...
            ASSIGN c-msg = "Valor Guia ST divergente! Confirma?" + "~~" + "O valor da Guia ST ou de sua base est† divergente com o calculado pelo sistema." + CHR(13) + 
                           ...
                           "Valor Calculado: " + STRING(tot-valor-guia-st, ">,>>>,>>9.99")                                        + CHR(13)  +
                           "Base Informada:  " + STRING(input FRAME {&FRAME-NAME} int_ds_docto_xml.base_guia_st, ">,>>>,>>9.99")  + CHR(13)  + 
                           "Base Calculada:  " + STRING(tot-base-guia-st, ">,>>>,>>9.99").
    ...
    for first b-int_ds_docto_xml exclusive-lock where 
        ...
        if trim(b-int_ds_docto_xml.cnpj_dest) = "" or 
           b-int_ds_docto_xml.cnpj_dest = ? then do:
            for first estabelec no-lock where estabelec.cod-estabel = "973" OR estabelec.cod-estabel = "977"
                query-tuning(no-lookahead):
        ...
        if b-int_ds_docto_xml.cnpj   = "" or 
           ...
           b-int_ds_docto_xml.xNome  = ?  then do:
            for first emitente no-lock where emitente.cod-emitente = b-int_ds_docto_xml.cod_emitente
                query-tuning(no-lookahead):
   ...
   for each int_ds_it_docto_xml exclusive-lock where
       ...
       if int_ds_it_docto_xml.num_pedido <> int_ds_docto_xml.num_pedido then do:
            assign int_ds_it_docto_xml.num_pedido = int_ds_docto_xml.num_pedido.
...
------------------------------------------------------------------------------*/
    ...
    if can-find (first usuar_grp_usuar no-lock where 
        ...
        assign custom.int_ds_docto_xml.valor_cofins :sensitive in FRAME {&FRAME-NAME} = yes
               custom.int_ds_docto_xml.valor_pis    :sensitive in FRAME {&FRAME-NAME} = yes.
    else 
        assign custom.int_ds_docto_xml.valor_cofins :sensitive in FRAME {&FRAME-NAME} = no
               custom.int_ds_docto_xml.valor_pis    :sensitive in FRAME {&FRAME-NAME} = no.
...
------------------------------------------------------------------------------*/
  ...
  if avail int_ds_docto_xml then do:
      ...
      IF  int_ds_docto_xml.tipo_nota <> 3                AND 
		  ...
		  int_ds_docto_xml.CNPJ_dest  = "79430682057400" ) AND 
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    if avail estabelec and estabelec.cod-estabel <> "973" AND estabelec.cod-estabel <> "977" then do:
       run pi-gera-erro(INPUT 3,    
...
------------------------------------------------------------------------------*/
    ...
    define variable dt-aux as date no-undo.
      ...
      i-sit-re = 0.
      for last int_ds_docto_wms 
          use-index sequencial no-lock where 
          int_ds_docto_wms.doc_numero_n = int_ds_docto_xml.nNF and
          int_ds_docto_wms.doc_serie_s  = int_ds_docto_xml.serie and
          int_ds_docto_wms.cnpj_cpf     = int_ds_docto_xml.CNPJ
          query-tuning(no-lookahead):
              ...
              if dt-aux <> ? or i-sit-re <> ? then do:
                  ...
                  buffer-copy int_ds_docto_wms 
                      ...
                      assign b-int_ds_docto_wms.situacao = i-sit-re
                             ...
                             b-int_ds_docto_wms.ID_SEQUENCIAL = /*next-value(seq-int_ds_docto_wms).*/ ?.
          ...
          if  int_ds_docto_wms.situacao = 20 and
              ...
              for each b-int_ds_docto_wms exclusive where 
                  rowid(b-int_ds_docto_wms) = rowid (int_ds_docto_wms)
                  query-tuning(no-lookahead):
          ...
          if  int_ds_docto_wms.situacao = 10 and
              ...
              buffer-copy int_ds_docto_wms 
                  ...
                  assign b-int_ds_docto_wms.situacao = 15
                         ...
                         b-int_ds_docto_wms.ENVIO_DATA_HORA = datetime(today)
                         b-int_ds_docto_wms.ID_SEQUENCIAL = /*next-value(seq-int_ds_docto_wms).*/ ?.
              assign i-sit-re = b-int_ds_docto_wms.situacao.
              release b-int_ds_docto_wms.
          end.
          ...
          if i-sit-re = ? then assign i-sit-re = int_ds_docto_wms.situacao.
...
------------------------------------------------------------------------------*/
   ...
   if i-situacao:screen-value in FRAME {&FRAME-NAME} = "5" and
      not can-find (first int_ds_docto_xml_compras no-lock of int_ds_docto_xml) then
       for first b-int_ds_docto_xml exclusive-lock where 
           rowid (b-int_ds_docto_xml) = rowid (int_ds_docto_xml)
           query-tuning(no-lookahead):
   ...
   for each int_ds_it_docto_xml exclusive-lock where
       ...
       if int_ds_it_docto_xml.tipo_nota <> 3 /* transferencia */ and
          not can-find (first estabelec no-lock where estabelec.cod-emitente = int_ds_docto_xml.cod_emitente) then do:
           ...
           if c-un-forn <> "" /*and 
              (int_ds_it_docto_xml.uCom-forn = "" or 
               int_ds_it_docto_xml.uCom-forn = ? or 
               trim(int_ds_it_docto_xml.uCom-forn) = "?")*/ then do:
           ...
           if avail item-fornec then do:
               ...
               run retornaIndiceConversao in h-boin176 (input int_ds_it_docto_xml.it_codigo,
                                                        input int_ds_it_docto_xml.cod-emitente,
            ...
            THEN DO:
                ...
                for first b-int_ds_docto_xml exclusive-lock where 
                    ...
                    assign  int_ds_docto_wms.doc_numero_n = int_ds_docto_xml.nNF
                            ...
                            int_ds_docto_wms.ID_SEQUENCIAL   = /*next-value(seq-int_ds_docto_wms).*/ ?.
...
------------------------------------------------------------------------------*/
    ...
    if avail int_ds_docto_xml then do:
        ...
        */
        ...
        if i-sit-re  >= 30 and i-sit-re < 55 and
        ...
        if return-value = "yes" then do:
            ...
            for first b-int_ds_docto_xml exclusive-lock
                ...
                assign i-sit-re = b-int_ds_docto_xml.sit_re.
                assign i-sit-re:screen-value in frame f-Main = string(i-sit-re).
                apply "leave":U to i-sit-re in frame f-Main.
            end.
            release int_ds_docto_wms.
...
------------------------------------------------------------------------------*/
    ...
    if RETURN-VALUE = "ADM-ERROR":U then do:
        run utp/ut-msgs.p(input "show",
                          input 17006,
                          input ("Atualizaá∆o inv†lida!" + "~~" + "Nota fiscail de estorno j† est∆o no RE1001.")).
        return "ADM-ERROR".
    end.
    ...
    if avail int_ds_docto_xml then do:
        ...
        if  i-sit-re <> 40 /* movimentada WMS */ and
            i-sit-re <> 3 /* recebimento antigo */ then do:
       ...
       if trim(string(int_ds_it_docto_xml.cfop,"9999")) begins "1" or
        ...
        ELSE
            RUN intprg/int013a.p( input int_ds_it_docto_xml.cfop,
                                  ...
                                  input /*int_ds_it_docto_xml.nep_cstb_ipi_n*/ int(item.fm-codigo),
            ...
            if avail item then do: 
                ...
                if i-pis = 1 /* tributado */ then do:
                    ...
                    if de-aliq-pis <> 0 then do:
                        ...
                        IF natur-oper.log-icms-substto-base-contrib = NO 
                            AND fnTrataNuloDec(int_ds_it_docto_xml.vOutro) <> 0 THEN DO:
                            ASSIGN de-base-pis = de-base-pis - int_ds_docto_xml.valor_st.
                        END.
                ...
                if i-cofins = 1 /* tributado */ then do:
                    ...
                    if de-aliq-cofins <> 0 then do:
                        ...
                        IF natur-oper.log-icms-substto-base-contrib = NO 
                            ...
                            ASSIGN de-base-cofins = de-base-cofins - int_ds_docto_xml.valor_st.
...
------------------------------------------------------------------------------*/
    ...
    find b-int_ds_docto_xml_guia_st exclusive where 
        rowid(b-int_ds_docto_xml_guia_st) = rowid(int_ds_docto_xml) no-error.

    assign tot-base-guia-st  = 0
           tot-valor-guia-st = 0.

    for each  int_ds_it_docto_xml EXCLUSIVE-LOCK 
        where int_ds_it_docto_xml.tipo_nota  = b-int_ds_docto_xml_guia_st.tipo_nota 
          and int_ds_it_docto_xml.CNPJ       = b-int_ds_docto_xml_guia_st.CNPJ      
          and int_ds_it_docto_xml.nNF        = b-int_ds_docto_xml_guia_st.nNF       
          and int_ds_it_docto_xml.serie      = b-int_ds_docto_xml_guia_st.serie
        ...
        if  first(int_ds_it_docto_xml.sequencia) and
            p-log-atualiza
        then
            assign b-int_ds_docto_xml_guia_st.base_guia_st  = 0
                   b-int_ds_docto_xml_guia_st.valor_guia_st = 0.
        ...
        FOR FIRST natur-oper NO-LOCK 
            ...
            THEN DO:
                ...
                run intprg/int510-icms-st.p (INPUT int_ds_it_docto_xml.cod_emitente,
                                             ...
                                             INPUT b-int_ds_docto_xml_guia_st.cod_estab,        
                ...
                if  p-log-atualiza
                then 
                    assign b-int_ds_docto_xml_guia_st.base_guia_st  = int_ds_docto_xml.base_guia_st  + de-base-st-calc
                           b-int_ds_docto_xml_guia_st.valor_guia_st = int_ds_docto_xml.valor_guia_st + de-valor-st-calc - int_ds_it_docto_xml.vicms
                           b-int_ds_docto_xml_guia_st.vbc_cst       = b-int_ds_docto_xml_guia_st.base_guia_st 
                           b-int_ds_docto_xml_guia_st.valor_st      = b-int_ds_docto_xml_guia_st.valor_guia_st
                           int_ds_it_docto_xml.vicmsst              = de-valor-st-calc - int_ds_it_docto_xml.vicms
                           int_ds_it_docto_xml.vbcst                = de-base-st-calc.
                else
                    assign tot-base-guia-st  = tot-base-guia-st  + de-base-st-calc
                           tot-valor-guia-st = tot-valor-guia-st + de-valor-st-calc - int_ds_it_docto_xml.vicms.
            end.
        end.
    end.
    ...
    IF  p-log-atualiza
    THEN DO:
        FOR FIRST int_ds_docto_xml_dup OF custom.int_ds_docto_xml EXCLUSIVE-LOCK 
        ...
        ASSIGN int_ds_docto_xml_dup.dVenc = INPUT FRAME {&FRAME-NAME} int_ds_docto_xml.dt_guia_st.
               int_ds_docto_xml_dup.vDup  = b-int_ds_docto_xml_guia_st.valor_guia_st.
            
        RUN new-state("RefreshData").
    END.

    RELEASE b-int_ds_docto_xml_guia_st.
...
------------------------------------------------------------------------------*/
        ...
        */
        ...
        if i-sit-re  = 90 or i-sit-re = 95 then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
        ...
        /*
        if (i-sit-re >= 20 /* agendada wms */ and
