...
DEFINE FRAME f-main
     ...
     int_ds_docto_xml.cod_estab AT ROW 3.04 COL 11.29 COLON-ALIGNED WIDGET-ID 6
          ...
          VIEW-AS FILL-IN 
          SIZE 7.72 BY .88
     cEstabelec AT ROW 3.04 COL 19.29 COLON-ALIGNED NO-LABEL WIDGET-ID 14
    ...
    */
    ...
    if trim(int_ds_docto_xml.cod_estab:screen-value in frame f-Main) = "" or
       trim(int_ds_docto_xml.cod_estab:screen-value in frame f-Main) = ? then do:
        run utp/ut-msgs.p(input "show",
                          input 17006,
                          input ("Estabelecimento inv†lido!" + "~~" + "O c¢digo do estabelcimento deve ser informado (973 977).")).
        return "ADM-ERROR".
    end.
        ...
        if trim(b-int_ds_docto_xml.cnpj_dest) = "" or 
           b-int_ds_docto_xml.cnpj_dest = ? then do:
                ...
                for first ems2mult.estabelec fields (cgc)
                   no-lock where estabelec.cod-estabel = "973" OR 
                                 estabelec.cod-estabel = "977" OR 
                                 estabelec.cod-estabel = "10001" OR 
                                 estabelec.cod-estabel = "10004" :
    ...
    if not avail estabelec then do:
       run pi-gera-erro(INPUT 3,    
        ...
        FIND FIRST      centro_distribuicao NO-LOCK
                WHERE   centro_distribuicao.deposito_padrao = int_ds_docto_xml.cod_estab
                and     centro_distribuicao.flag-CD = yes NO-ERROR.
        
    if avail estabelec and estabelec.cod-estabel <> centro_distribuicao.deposito_padrao then do:
       run pi-gera-erro(INPUT 3,    
...
------------------------------------------------------------------------------*/
    ...
    for each  int_ds_it_docto_xml no-lock
        ...
        FOR FIRST natur-oper NO-LOCK 
            ...
            &if "{&bf_dis_versao_ems}" >= "2.07" &THEN
            IF  natur-oper.log-contrib-st-antec THEN DO:
