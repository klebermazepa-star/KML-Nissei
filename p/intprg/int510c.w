...
*******************************************************************************/
{include/i-prgvrs.i INT510c 1.12.01.AVB}
...
DEFINE VARIABLE h_INT510c-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_INT510c-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     ...
     bt-cancela AT ROW 9.33 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 9.33 COL 70.72
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Duplicatas"
         HEIGHT             = 9.75
         WIDTH              = 80.72
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Duplicatas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Duplicatas */
DO:
...
DO:
{include/okfil.i h_INT510c-viewer}
END.
...
DO:
{include/salvafil.i h_INT510c-viewer}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 7.17 , 79.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT510c-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT510c-viewer ).
       RUN set-position IN h_INT510c-viewer ( 3.13 , 14.57 ) NO-ERROR.
       /* Size in UIB:  ( 4.54 , 49.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT510c-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_INT510c-query ).
       RUN set-position IN h_INT510c-query ( 1.00 , 74.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.42 , 6.72 ) */

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_INT510c-viewer. */
       RUN add-link IN adm-broker-hdl ( h_INT510c-query , 'Record':U , h_INT510c-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_INT510c-viewer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-ok:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT510c-viewer ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT510c" "1.12.01.AVB"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_INT510c-viewer(input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_INT510c-query (input v-row-table).
