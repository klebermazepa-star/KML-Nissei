...
&Scoped-Define ENABLED-OBJECTS rt-button btRecusa btIntegraXML btReavliar ~
btIntegra 
...
DEFINE BUTTON btIntegraXML 
     LABEL "Reprocessa XML" 
     SIZE 25 BY 1.13.

DEFINE BUTTON btReavliar 
     LABEL "Reavaliar Erros" 
     SIZE 27 BY 1.13.
...
DEFINE FRAME f-cad
     btRecusa AT ROW 1.33 COL 42.57 HELP
          "Recusa recebimento da nota fiscal" WIDGET-ID 16
     btIntegraXML AT ROW 1.38 COL 47 WIDGET-ID 4
     btReavliar AT ROW 1.38 COL 72.86 WIDGET-ID 6
     btIntegra AT ROW 1.38 COL 100.57 WIDGET-ID 2
...
&Scoped-define SELF-NAME btIntegraXML
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btIntegraXML w-cadco2
ON CHOOSE OF btIntegraXML IN FRAME f-cad /* Reprocessa XML */
DO:
    RUN pi-processa-xml IN h_int520-viewer.
    RUN dispatch IN h_int520-viewer ('row-available':U).
    RUN dispatch IN h_int520-viewer-totais ('row-available':U).
    RUN dispatch IN h_int520-browser-itens ('row-available':U).
    run pi_refresh in h_int520-browser-itens.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btReavliar w-cadco2
ON CHOOSE OF btReavliar IN FRAME f-cad /* Reavaliar Erros */
DO:
    RUN dispatch IN h_int520-viewer ('row-available':U).
    RUN dispatch IN h_int520-viewer-totais ('row-available':U).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btRecusa w-cadco2
ON CHOOSE OF btRecusa IN FRAME f-cad /* Recusa */
do:    
    RUN pi-cancela-recebimento in h_int520-viewer.
    /* verificar situacao do documento ....*/
    RUN dispatch IN h_int520-query ('open-query-cases':U).
    RUN dispatch IN h_int520-query ('GET-FIRST':U).
end.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-cadsi2 ).
       RUN set-position IN h_p-cadsi2 ( 1.33 , 26.29 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ENABLE rt-button btRecusa btIntegraXML btReavliar btIntegra 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT520" "1.12.01.AVB"}
