...
*******************************************************************************/
{include/i-prgvrs.i INT540-browser-duplicatas 1.12.01.AVB}
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 9.27 ROW-HEIGHT-CHARS .58.
...
DEFINE FRAME F-Main
     ...
     bt-incluir AT ROW 10.27 COL 1
     bt-modificar AT ROW 10.27 COL 11
     bt-eliminar AT ROW 10.27 COL 21
...
"int_ds_docto_xml_dup.nDup" "Duplicata" ? "character" ? ? ? ? ? ? no ? no no "22.38" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > custom.int_ds_docto_xml_dup.dVenc
"int_ds_docto_xml_dup.dVenc" "Vencimento" ? "character" ? ? ? ? ? ? no ? no no "13.38" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
    ...
    if can-find (first usuar_grp_usuar no-lock where 
                 ...
                 (usuar_grp_usuar.cod_grp_usuar = "CCT" or usuar_grp_usuar.cod_grp_usuar = "IBB") and
