...
*******************************************************************************/
{include/i-prgvrs.i INT540-vapara 1.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_chaveacesso c_cnpj c_serie ~
i_notafiscal bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_chaveacesso c_cnpj c_serie i_notafiscal 
...
DEFINE VARIABLE c_chaveacesso AS CHARACTER FORMAT "x(44)":U 
     LABEL "Chave NFe" 
     VIEW-AS FILL-IN 
     SIZE 47 BY .88 NO-UNDO.

DEFINE VARIABLE c_cnpj AS CHARACTER FORMAT "x(14)":U 
     LABEL "CNPJ Origem" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE c_serie AS CHARACTER FORMAT "x(3)":U 
     LABEL "Serie" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.

DEFINE VARIABLE i_notafiscal AS INTEGER FORMAT ">>>,>>>,>>9":U INITIAL 0 
     LABEL "Nr. Nota" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_chaveacesso AT ROW 1.25 COL 13.57 COLON-ALIGNED WIDGET-ID 2
     c_cnpj AT ROW 2.33 COL 13.57 COLON-ALIGNED WIDGET-ID 4
     c_serie AT ROW 3.42 COL 13.57 COLON-ALIGNED WIDGET-ID 6
     i_notafiscal AT ROW 4.46 COL 13.57 COLON-ALIGNED WIDGET-ID 8
     bt-ok AT ROW 6.58 COL 2.14
     bt-cancela AT ROW 6.58 COL 13
     bt-ajuda AT ROW 6.58 COL 58
     rt-button AT ROW 6.33 COL 1
     SPACE(0.12) SKIP(0.34)
...
DO:
    if c_chaveacesso:screen-value in frame {&FRAME-NAME}  <> "" then do:
        for first int_ds_docto_xml no-lock where 
            int_ds_docto_xml.chnfe = INPUT FRAME {&FRAME-NAME} c_chaveacesso and
            int_ds_docto_xml.situacao <> 3 and 
            int_ds_docto_xml.situacao <> 9 and 
            int_ds_docto_xml.tipo_nota <> 3 and
            int_ds_docto_xml.CNPJ_dest <> "79430682025540" /* 973 */: end.
        IF NOT AVAILABLE int_ds_docto_xml THEN DO:
            {utp/ut-table.i custom int_ds_docto_xml 1}
        ...
        for first int_ds_docto_xml no-lock where 
            int_ds_docto_xml.CNPJ EQ INPUT FRAME {&FRAME-NAME} c_cnpj AND 
            int_ds_docto_xml.serie EQ INPUT FRAME {&FRAME-NAME} c_serie AND 
            integer(int_ds_docto_xml.nNF) EQ INPUT FRAME {&FRAME-NAME} i_notafiscal and
            int_ds_docto_xml.situacao <> 3 and 
            int_ds_docto_xml.situacao <> 9 and 
            int_ds_docto_xml.tipo_nota <> 3 and
            int_ds_docto_xml.CNPJ_dest <> "79430682025540" /* 973 */: end.
    ...
    ASSIGN p-row-tabela = ROWID(int_ds_docto_xml).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_chaveacesso c_cnpj c_serie i_notafiscal 
      WITH FRAME d-vapara.
  ENABLE rt-button c_chaveacesso c_cnpj c_serie i_notafiscal bt-ok bt-cancela 
         bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT540-vapara" "1.12.01.AVB"}
