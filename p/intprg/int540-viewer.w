...
*******************************************************************************/
{include/i-prgvrs.i INT540-viewer 1.12.00.AVB}
...
DEFINE TEMP-TABLE tt-nota NO-UNDO  // Cabeáalho nota pepsico
    FIELD cnpj              AS CHAR
    FIELD cfop              AS CHAR
    FIELD serie             AS CHAR
...
DEFINE TEMP-TABLE tt-nota-item NO-UNDO  // Cabeáalho nota pepsico
    FIELD cnpj              AS CHAR
    FIELD cfop              AS CHAR
    FIELD serie             AS CHAR
    FIELD nr-nota-fis       AS CHAR
    ...
    FIELD item-fornec       AS CHAR
    FIELD desc-item         AS CHAR
    FIELD ncm-item          AS CHAR
    FIELD cfop-item         AS CHAR
    ...
    FIELD base-st           AS DEC
    FIELD mva               AS DEC
    FIELD valor-icmsST      AS DEC
    FIELD valor-IPI         AS DEC
    FIELD valor-PIS         AS DEC
    FIELD valor-cofins      AS DEC
    FIELD valor-liquido     AS DEC.
...
c-Mensagem bt-totais bt-un bt-natureza bt-importarExcel bt-ncm bt-icms 
...
DEFINE BUTTON bt-icms 
     LABEL "Calcular Tributos" 
     SIZE 17.86 BY 1.

DEFINE BUTTON bt-importarExcel 
     LABEL "Importar Espelho (Excel)" 
     SIZE 18 BY 2.

DEFINE BUTTON bt-natureza 
     LABEL "Buscar Naturezas" 
     SIZE 17.86 BY 1.

DEFINE BUTTON bt-ncm 
     LABEL "Buscar NCM" 
     SIZE 12.72 BY 1.
...
DEFINE FRAME f-main
     ...
     int_ds_docto_xml.dt_trans AT ROW 1.21 COL 128.72 COLON-ALIGNED WIDGET-ID 176
          VIEW-AS FILL-IN 
          SIZE 11.29 BY .88
     ...
     int_ds_docto_xml.nNF AT ROW 3.04 COL 82.86 COLON-ALIGNED WIDGET-ID 8
          ...
          VIEW-AS FILL-IN 
          SIZE 14.57 BY .88
     int_ds_docto_xml.serie AT ROW 3.04 COL 104 COLON-ALIGNED WIDGET-ID 10
          VIEW-AS FILL-IN 
          SIZE 7 BY .88
     int_ds_docto_xml.dEmi AT ROW 3.04 COL 128.72 COLON-ALIGNED WIDGET-ID 108
          LABEL "Emiss∆o"
          VIEW-AS FILL-IN 
          SIZE 11.29 BY .88
...
DEFINE FRAME f-main
     ...
     int_ds_docto_xml.valor_icms_des AT ROW 10.88 COL 81.29 COLON-ALIGNED NO-LABEL WIDGET-ID 114
          VIEW-AS FILL-IN 
          SIZE 17.29 BY .88
     int_ds_docto_xml.modFrete AT ROW 10.88 COL 100.43 COLON-ALIGNED NO-LABEL WIDGET-ID 210
          VIEW-AS FILL-IN 
          SIZE 2.14 BY .88
     cFrete AT ROW 10.88 COL 102.43 COLON-ALIGNED NO-LABEL WIDGET-ID 216
     bt-totais AT ROW 11.92 COL 61.86 WIDGET-ID 154
     bt-un AT ROW 11.96 COL 80.29 WIDGET-ID 206
     bt-natureza AT ROW 11.96 COL 93 WIDGET-ID 200
     bt-importarExcel AT ROW 12 COL 111 WIDGET-ID 218
     bt-red-icms AT ROW 12.67 COL 56.57 HELP
          "Atualiza % de Reduá∆o ICMS Itens" WIDGET-ID 188
     int_ds_docto_xml.valor_guia_st AT ROW 12.75 COL 3 NO-LABEL WIDGET-ID 170
          VIEW-AS FILL-IN 
          SIZE 12 BY .88
     int_ds_docto_xml.base_guia_st AT ROW 12.75 COL 14 COLON-ALIGNED NO-LABEL WIDGET-ID 194
          VIEW-AS FILL-IN 
          SIZE 15 BY .88
     int_ds_docto_xml.dt_guia_st AT ROW 12.75 COL 30 COLON-ALIGNED NO-LABEL WIDGET-ID 196
          VIEW-AS FILL-IN 
          SIZE 11 BY .88
     bt-guia-st AT ROW 12.75 COL 43.29 HELP
          "Confirma valor da guia ST e calcula ST itens" WIDGET-ID 174
     int_ds_docto_xml.perc_red_icms AT ROW 12.75 COL 48 COLON-ALIGNED NO-LABEL WIDGET-ID 198
          VIEW-AS FILL-IN 
          SIZE 5.86 BY .88
     bt-ncm AT ROW 12.96 COL 80.29 WIDGET-ID 204
     bt-icms AT ROW 12.96 COL 93 WIDGET-ID 202
     ...
     "%Red ICMS" VIEW-AS TEXT
          SIZE 10.86 BY .67 AT ROW 11.92 COL 49.57 WIDGET-ID 186
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE  WIDGET-ID 100.
...
DEFINE FRAME f-main
     ...
     "Mod Frete" VIEW-AS TEXT
          SIZE 9 BY .67 AT ROW 10 COL 103.14 WIDGET-ID 214
     "Base Guia ST" VIEW-AS TEXT
          SIZE 13 BY .67 AT ROW 11.92 COL 16.43 WIDGET-ID 180
     ...
     "Vencto ST" VIEW-AS TEXT
          SIZE 10 BY .67 AT ROW 11.92 COL 31.72 WIDGET-ID 182
     "Vl PIS" VIEW-AS TEXT
     ...
     RECT-23 AT ROW 12.17 COL 48.86 WIDGET-ID 184
...
&Scoped-define SELF-NAME bt-icms
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-icms V-table-Win
ON CHOOSE OF bt-icms IN FRAME f-main /* Calcular Tributos */
DO:
  /* Code placed here will execute AFTER standard behavior.    */
  if avail int_ds_docto_xml then do:

      RUN pi-calcula-tributos.
  end.
END.
...
&Scoped-define SELF-NAME bt-importarExcel
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-importarExcel V-table-Win
ON CHOOSE OF bt-importarExcel IN FRAME f-main /* Importar Espelho (Excel) */
DO:
    ...
    if  l-ok = yes THEN DO:
        ...
        RUN pi-import-excel(INPUT cfile).
...
ON CHOOSE OF bt-natureza IN FRAME f-main /* Buscar Naturezas */
DO:
  /* Code placed here will execute AFTER standard behavior.    */
  if avail int_ds_docto_xml then do:
...
&Scoped-define SELF-NAME bt-ncm
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-ncm V-table-Win
ON CHOOSE OF bt-ncm IN FRAME f-main /* Buscar NCM */
DO:
  /* Code placed here will execute AFTER standard behavior.    */
  if avail int_ds_docto_xml then do:
...
ON CHOOSE OF bt-totais IN FRAME f-main /* Copiar Totais Itens */
DO:
  /* Code placed here will execute AFTER standard behavior.    */
  if avail int_ds_docto_xml then do:
    ...
    if RETURN-VALUE = "ADM-ERROR":U then return "ADM-ERROR":U.
          ...
          for each b-int_ds_docto_xml exclusive-lock
              where rowid(b-int_ds_docto_xml) = rowid(int_ds_docto_xml):
              assign  b-int_ds_docto_xml.vbc            = tot-base-icms-it       
                      b-int_ds_docto_xml.vbc_cst        = tot-base-icms-st-it    
                      b-int_ds_docto_xml.valor_cofins   = tot-cofins-it          
                      b-int_ds_docto_xml.tot_desconto   = tot-desconto-it        
                      b-int_ds_docto_xml.valor_icms     = tot-icms-it            
                      b-int_ds_docto_xml.valor_st       = tot-icms-st-it         
                      b-int_ds_docto_xml.valor_ipi      = tot-ipi-it             
                      b-int_ds_docto_xml.vNF            = tot-nf-it              
                      b-int_ds_docto_xml.valor_pis      = tot-tot-pis-it         
                      b-int_ds_docto_xml.valor_mercad   = tot-valor-it
                      b-int_ds_docto_xml.valor_icms_des = tot-icms-des.
...
&Scoped-define SELF-NAME bt-un
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-un V-table-Win
ON CHOOSE OF bt-un IN FRAME f-main /* Buscar UN */
DO:
  /* Code placed here will execute AFTER standard behavior.    */
  if avail int_ds_docto_xml then do:
...
------------------------------------------------------------------------------*/
    ...
    if int_ds_docto_xml.situacao <> 3 and 
       int_ds_docto_xml.situacao <> 9 then do transaction:
        find b-int_ds_docto_xml exclusive-lock where 
            rowid (b-int_ds_docto_xml) = rowid (int_ds_docto_xml) no-error no-wait.
        if avail b-int_ds_docto_xml then do:
            if c-mensagem:screen-value in frame f-Main <> "" then 
                assign int_ds_docto_xml.situacao = 1. /* Pendente */
            else do:
                ...
                for each int_ds_it_docto_xml exclusive-lock where
                    ...
                    int_ds_it_docto_xml.nNF          = int_ds_docto_xml.nNF         and 
                    int_ds_it_docto_xml.serie        = int_ds_docto_xml.serie:
                    assign int_ds_it_docto_xml.situacao = 2 /* liberado */.
                    release int_ds_it_docto_xml.
                end.
            end.
            release b-int_ds_docto_xml.
...
------------------------------------------------------------------------------*/
   ...
   for each int_ds_it_docto_xml exclusive-lock where
       ...
       if (trim(int_ds_it_docto_xml.item_do_forn) = "" or
           trim(int_ds_it_docto_xml.item_do_forn) = ?) and
          int_ds_it_docto_xml.it_codigo <> "" then do:
           ...
           for first item no-lock where item.it-codigo = int_ds_it_docto_xml.it_codigo:
               for first item-fornec no-lock where 
                   ...
                   item-fornec.it-codigo = item.it-codigo:
                    assign int_ds_it_docto_xml.item_do_forn = item-fornec.item-do-forn.
               end.
           end.
       end.
       if int_ds_it_docto_xml.tipo_nota <> 3 /* transferencia */ and
          not can-find (first estabelec no-lock where estabelec.cod-emitente = int_ds_it_docto_xml.cod_emitente) then do:
                   ...
                   if not avail item-fornec then do:
                      run pi-gera-erro(INPUT 5,
                                       INPUT "Relacionamento item x fornecedor n∆o cadastrado. Item: " + 
                                             string(if int_ds_it_docto_xml.item_do_forn <> ? then int_ds_it_docto_xml.item_do_forn else "?") + 
           ...
           if avail item-fornec then do:
               if int_ds_it_docto_xml.it_codigo = "" then
               ...
               assign int_ds_it_docto_xml.qCom = round(int_ds_it_docto_xml.qCom_Forn / de-fator,0).
           ...
           */
           assign int_ds_it_docto_xml.qCom = int_ds_it_docto_xml.qCom_Forn.
       end. /* tipo_nota <> 3 */
       ...
       for first item no-lock where  
           item.it-codigo = int_ds_it_docto_xml.it_codigo: 
           assign int_ds_it_docto_xml.uCom   = if item.un <> int_ds_it_docto_xml.uCom then item.un else int_ds_it_docto_xml.uCom
                  int_ds_it_docto_xml.xprod  = substring(ITEM.desc-item,1,60)
                  int_ds_it_docto_xml.tipo_contr = 4.
       ...
       if not avail item then do:
           run pi-gera-erro(INPUT 7,    
                            INPUT "Item N∆o cadastrado " + string(int_ds_it_docto_xml.it_codigo)). 
           assign int_ds_it_docto_xml.situacao = 1 /* Pendente */.
       end.
       ...
       then do:
          run pi-gera-erro(INPUT 8,
                           INPUT "Natureza de Operaá∆o " + 
                                 if trim(string(int_ds_it_docto_xml.nat_operacao)) <> ? 
                                 then trim(string(int_ds_it_docto_xml.nat_operacao)) 
                                 else "?"
                                 + " do item seq: " + trim(string(int_ds_it_docto_xml.sequencia,">>>>>>9"))
                                 + " n∆o cadastrada!"). 
       end. 
       if not can-find(first classif-fisc no-lock where 
           classif-fisc.class-fiscal <> "00000000" and 
           classif-fisc.class-fiscal = trim(string(int_ds_it_docto_xml.ncm,"99999999"))) then do:
           run pi-gera-erro(INPUT 33,
                            INPUT "NCM " + (if int_ds_it_docto_xml.ncm <> ? then trim(string(int_ds_it_docto_xml.ncm,"99999999")) else "?") + " do item seq: " + trim(string(int_ds_it_docto_xml.sequencia,">>>>>>9")) + " n∆o cadastrada!"). 
           assign int_ds_it_docto_xml.situacao = 1 /* Pendente */.
       end.
...
------------------------------------------------------------------------------*/
    ...
    if avail int_ds_docto_xml then do:
        ...
        if  int_ds_docto_xml.situacao = 5 or /* falta aprovar compras */
            int_ds_docto_xml.situacao = 3 /* j† importada */ or
        ...
        then do:
            run utp/ut-msgs.p(input "show",
                              input 17006,
                              input ("Atualizaá∆o inv†lida!" + "~~" + "A nota fiscal n∆o est† dispon°vel para importaá∆o no Totvs. Verifique a situaá∆o da nota e os erros apontados.")).
            return "ADM-ERROR".
        end.

        run utp/ut-msgs.p(input "show",
                          input 701,
                          input ("atualizaá∆o da nota fiscal no RE1001?")).
        ...
        if return-value = "yes" then do:
            ...
            assign tt-param.arquivo = session:temp-directory + "INT540" + ".tmp":U.

            /* Executar do programa RP.P que ir† criar o relat¢rio */
            SESSION:SET-WAIT-STATE("GENERAL":U).
        
            os-command silent del value(session:temp-directory + "int073-re1005.txt") no-error.

            raw-transfer tt-param to raw-param.
            run intprg/int073rp.p (input raw-param,
                                   input table tt-raw-digita).
            
            if search ("int073-re1005.txt") <> ? then
                RUN winexec (INPUT "Notepad.exe" + CHR(32) + session:temp-directory + "int073-re1005.txt", input 1).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-import-excel V-table-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEF INPUT PARAM cFile    AS CHAR NO-UNDO.
   ...
   assign i-linha = 6.  // Linha do cabeáalho

   def var i-ind as integer no-undo.

   // Importando cabeáalho da nota fiscal
   ...
   CREATE tt-nota.
   ASSIGN tt-nota.cnpj          = string(int64(CHEXCEL:range("B" + string(i-linha)):value))
          tt-nota.cfop          = string(int64(chExcel:range("I" + string(i-linha)):value))
          tt-nota.serie         = string(int64(chExcel:range("K" + string(i-linha)):value))
          tt-nota.nr-nota-fis   = string(int64(chExcel:range("M" + string(i-linha)):value))
          tt-nota.dt-emissao    = DATE(chExcel:range("O" + string(i-linha)):value).


    RELEASE tt-nota.

    run utp/ut-acomp.p persistent set h-acomp.
    run pi-inicializar in h-acomp (input "Processando Notas Pepsico").

   // Validar se a nota do excel Ç igual a nota que est† apontada na tela int540

   FOR FIRST tt-nota:
       if avail int_ds_docto_xml then do:

           IF  int_ds_docto_xml.CNPJ_dest = tt-nota.cnpj  AND        
               int_ds_docto_xml.cfop      = int(tt-nota.cfop)  AND        
               int_ds_docto_xml.serie     = tt-nota.serie AND        
               int_ds_docto_xml.nNF       = tt-nota.nr-nota-fis AND
               int_ds_docto_xml.dt_trans  = tt-nota.dt-emissao   
               THEN DO:           // Se entrar nesse IF a nota est† certa

               // Deixar todos os itens como amarelo para quando validar se foi encontrado todos os itens deixar branco novamente
             //   RUN pi-trataItens IN h_INT540-browser-itens-2.

             // Importando itens da nota fiscal
            
                EMPTY TEMP-TABLE tt-nota-item.
                assign i-linha =  i-linha + 4. // Apontando o primeiro item da nota-fiscal
            
                blk-repeat:
                REPEAT: 

                    RUN pi-acompanhar IN h-acomp (INPUT "Importando item: " + string(i-linha - 9) + " da nota fiscal.").
            
                    IF string(CHEXCEL:range("A" + string(i-linha)):value) = "total:"  THEN LEAVE blk-repeat.
            
                    CREATE tt-nota-item.
                    ASSIGN tt-nota-item.cnpj            = tt-nota.cnpj       
                           tt-nota-item.cfop            = tt-nota.cfop       
                           tt-nota-item.serie           = tt-nota.serie      
                           tt-nota-item.nr-nota-fis     = tt-nota.nr-nota-fis
                           tt-nota-item.dt-emissao      = tt-nota.dt-emissao 
                           tt-nota-item.seq-item        = INT(CHEXCEL:range("A" + string(i-linha)):value)
                           tt-nota-item.item-fornec     = STRING(int64(CHEXCEL:range("B" + string(i-linha)):value))
                           tt-nota-item.desc-item       = string(CHEXCEL:range("C" + string(i-linha)):value)
                           tt-nota-item.ncm-item        = string(int64(CHEXCEL:range("D" + string(i-linha)):value))
                           tt-nota-item.cfop-item       = string(int64(CHEXCEL:range("E" + string(i-linha)):value))
                           tt-nota-item.qtd-item        = ROUND(DEC(CHEXCEL:range("F" + string(i-linha)):value), 2)
                           tt-nota-item.valor-unit      = ROUND(DEC(CHEXCEL:range("G" + string(i-linha)):value), 2)
                           tt-nota-item.valor-total     = ROUND(DEC(CHEXCEL:range("H" + string(i-linha)):value), 2)
                           tt-nota-item.valor-desc      = ROUND(DEC(CHEXCEL:range("I" + string(i-linha)):value), 2)
                           tt-nota-item.base-icms       = ROUND(DEC(CHEXCEL:range("J" + string(i-linha)):value), 2) 
                           tt-nota-item.aliq-icms       = ROUND(DEC(CHEXCEL:range("K" + string(i-linha)):value) * 100, 2)
                           tt-nota-item.valor-icms      = ROUND(DEC(CHEXCEL:range("L" + string(i-linha)):value), 2)
                           tt-nota-item.base-st         = ROUND(DEC(CHEXCEL:range("M" + string(i-linha)):value), 2)
                           tt-nota-item.mva             = ROUND(DEC(CHEXCEL:range("N" + string(i-linha)):value) * 100, 2)
                           tt-nota-item.valor-icmsST    = ROUND(DEC(CHEXCEL:range("O" + string(i-linha)):value), 2)
                           tt-nota-item.valor-IPI       = ROUND(DEC(CHEXCEL:range("P" + string(i-linha)):value), 2)
                           tt-nota-item.valor-PIS       = ROUND(DEC(CHEXCEL:range("R" + string(i-linha)):value), 2)
                           tt-nota-item.valor-cofins    = ROUND(DEC(CHEXCEL:range("S" + string(i-linha)):value), 2)
                           tt-nota-item.valor-liquido   = ROUND(DEC(CHEXCEL:range("T" + string(i-linha)):value), 2).
            
                    RELEASE tt-nota-item.
            
                    assign i-linha =  i-linha + 1. 
            
                END. // Fim repeat

                FOR EACH tt-nota-item:

                    RUN pi-acompanhar IN h-acomp (INPUT "Alterando item: " + string(i-linha - 6) + " do INT540.").
                    FIND FIRST int_ds_it_docto_xml   EXCLUSIVE-LOCK 
                        WHERE int_ds_it_docto_xml.tipo_nota     = int_ds_docto_xml.tipo_nota 
                          ...
                          AND int_ds_it_docto_xml.item_do_forn  = tt-nota-item.item-fornec NO-ERROR.
                    ...
                    IF AVAIL int_ds_it_docto_xml   THEN DO:
                        ...
                        ASSIGN  int_ds_it_docto_xml.ncm             = INT(tt-nota-item.ncm-item)      
                                int_ds_it_docto_xml.cfop            = INT(tt-nota-item.cfop-item)     
                                int_ds_it_docto_xml.qCom            = tt-nota-item.qtd-item      
                                int_ds_it_docto_xml.vUnCom          = tt-nota-item.valor-unit    
                                int_ds_it_docto_xml.vProd           = tt-nota-item.valor-total   
                                int_ds_it_docto_xml.vDesc           = tt-nota-item.valor-desc    
                                int_ds_it_docto_xml.vbc_icms        = tt-nota-item.base-icms     
                                int_ds_it_docto_xml.picms           = tt-nota-item.aliq-icms     
                                int_ds_it_docto_xml.vicms           = tt-nota-item.valor-icms    
                                int_ds_it_docto_xml.vbcst           = tt-nota-item.base-st       
                                int_ds_it_docto_xml.pmvast          = tt-nota-item.mva           
                                int_ds_it_docto_xml.vicmsst         = tt-nota-item.valor-icmsST  
                                int_ds_it_docto_xml.vipi            = tt-nota-item.valor-IPI     
                                int_ds_it_docto_xml.vpis            = tt-nota-item.valor-PIS     
                                int_ds_it_docto_xml.vcofins         = tt-nota-item.valor-cofins  .
                               // int_ds_it_docto_xml.vProd           = tt-nota-item.valor-liquido .

                    END.
                    ELSE DO:
                        MESSAGE "Item: " tt-nota-item.item-fornec " N∆o encontrado no int540" 
                            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
                    END.
                    RELEASE int_ds_it_docto_xml.
                END.

                RUN pi-acompanhar IN h-acomp (INPUT "Atualizando dados na tela int540").
                APPLY "CHOOSE" TO bt-totais IN FRAME f-main.
                RUN new-state ('RefreshData').

           END. // FIM do IF 
           ELSE DO:
               MESSAGE "Arquivo excel n∆o Ç equivalente a nota fiscal selecionada" SKIP
                       "CNPJ Espelho        - " tt-nota.cnpj           SKIP   
                       "CFOP Espelho        - " tt-nota.cfop           SKIP   
                       "SÇrie Espelho       - " tt-nota.serie          SKIP   
                       "Num Nota Espelho    - " tt-nota.nr-nota-fis   SKIP   
                       "Dt Emiss∆o Espelho  - " tt-nota.dt-emissao            
                   VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
           END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-ncm V-table-Win 
PROCEDURE pi-ncm :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
    if avail int_ds_docto_xml then do:
        ...
        for each int_ds_it_docto_xml exclusive-lock where
            ...
            if int_ds_it_docto_xml.ncm = 0 or
               int_ds_it_docto_xml.ncm = ? then do:
                for first item fields (class-fiscal)
                    no-lock where 
                    item.it-codigo = int_ds_it_docto_xml.it_codigo:

                    assign int_ds_it_docto_xml.ncm = int(item.class-fiscal).
                end.
            end.
            release int_ds_it_docto_xml.
        end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-un V-table-Win 
PROCEDURE pi-un :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
    ...
    if avail int_ds_docto_xml then do:
        ...
        for each int_ds_it_docto_xml exclusive-lock where
            ...
            if int_ds_it_docto_xml.uCom_forn = "" or
               ...
               int_ds_it_docto_xml.uCom = ? or
               int_ds_it_docto_xml.uCom_forn = "?" or 
               int_ds_it_docto_xml.uCom = "?" then do:
                ...
                if not avail item-fornec then do:
                    assign c-item-do-forn = int_ds_it_docto_xml.item_do_forn.
                    for first item-fornec fields (item-do-forn unid-med-for) no-lock where
                              item-fornec.cod-emitente = int_ds_docto_xml.cod_emitente and
                              item-fornec.item-do-forn = c-item-do-forn and 
                              item-fornec.ativo = yes
                        query-tuning(no-lookahead): 
                    ...
                    if not avail item-fornec then do:
                        ...
                        for first item-fornec-umd fields (it-codigo unid-med-for) where
                ...
                */
                if c-un-forn <> "" then 
                    assign int_ds_it_docto_xml.uCom_forn = c-un-forn.
                if int_ds_it_docto_xml.uCom = "" or int_ds_it_docto_xml.uCom = ? then
                    for first item no-lock where item.it-codigo = int_ds_it_docto_xml.it_codigo:
                        assign int_ds_it_docto_xml.uCom = item.un.
                    end.
            end.
            release int_ds_it_docto_xml.
        end.
