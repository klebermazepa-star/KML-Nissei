...
*******************************************************************************/
{include/i-prgvrs.i INT540A 1.12.00.AVB}
...
DEFINE VARIABLE h_INT540a-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_INT540a-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     ...
     bt-cancela AT ROW 17 COL 25 HELP
          "Cancela"
     bt-ajuda AT ROW 17 COL 136.88
     rt-button AT ROW 16.77 COL 2
...
DO:
{include/okpai.i h_INT540a-viewer}
END.
...
DO:
{include/salvapai.i h_INT540a-viewer}
 END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 14.93 , 147.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT540a-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT540a-viewer ).
       RUN set-position IN h_INT540a-viewer ( 2.77 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.90 , 143.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT540a-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_INT540a-query ).
       RUN set-position IN h_INT540a-query ( 12.77 , 141.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.00 , 7.00 ) */

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_INT540a-viewer. */
       RUN add-link IN adm-broker-hdl ( h_INT540a-query , 'Record':U , h_INT540a-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_INT540a-viewer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-ok:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT540a-viewer ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT540A" "1.12.00.AVB"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_INT540a-query (INPUT v-row-table). 
