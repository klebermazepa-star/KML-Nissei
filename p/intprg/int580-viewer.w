...
*******************************************************************************/
{include/i-prgvrs.i int580-viewer 1.12.00.AVB}
{cdp/cdcfgdis.i}
...
define variable h_int580-browser-itens as handle.
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    if avail estabelec and estabelec.cod-estabel <> "973" then do:
       run pi-gera-erro(INPUT 3,    
                        INPUT "Estabelecimento n∆o pode ter recebimento pelo INT580!").
    end. 
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL int_ds_docto_wms THEN
       ASSIGN i-id-seq = int_ds_docto_wms.id_sequencial + 1.
    ...
    if avail int_ds_docto_xml then do:
        ...
        */
        ...
        do transaction:
            ...
            RUN intprg\int580d3.w (OUTPUT c-motivo-aceite).
...
------------------------------------------------------------------------------*/
    ...
    if avail int_ds_docto_xml then do:
        ...
        if return-value = "yes" then do:
            ...
            assign tt-param.arquivo = session:temp-directory + "INT580" + ".tmp":U.
  ...
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    THEN DO:
        ...
        THEN DO:
            ...
            IF  VALID-HANDLE(h_int580-browser-itens) 
            THEN 
                RUN pi-refresh in h_int580-browser-itens.
        END.
    END.
...
------------------------------------------------------------------------------*/
    ...
    then
        RUN intprg\int580d4.w (INPUT ROWID(int_ds_docto_xml)).
    ELSE
        RUN intprg\int580d4.w (INPUT ?).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
define input parameter p_int580-browser-itens as handle.
assign h_int580-browser-itens = p_int580-browser-itens.
END PROCEDURE.
