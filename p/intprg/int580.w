...
*******************************************************************************/
{include/i-prgvrs.i INT580 1.12.01.NOVA_VERSAO}
...
DEFINE VARIABLE h_int580-browser-duplicatas-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int580-browser-erros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int580-browser-itens-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int580-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int580-viewer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int580-viewer-totais AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int580-viewer ).
       RUN set-position IN h_int580-viewer ( 2.42 , 1.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int580-zoom.w,
                     ProgVaPara = intprg/int580-vapara.w,
                     ProgIncMod = intprg/int580a.w,
                     Implantar = no':U ,
             OUTPUT h_int580-query ).
       RUN set-position IN h_int580-query ( 13.25 , 137.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.42 , 6.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_int580-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int580-query , 'Record':U , h_int580-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_int580-viewer ).
       RUN add-link IN adm-broker-hdl ( h_int580-viewer , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_int580-query. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_int580-query ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int580-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int580-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int580-query ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int580-viewer ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_int580-viewer , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-browser-itens.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int580b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int580-browser-itens-2 ).
       RUN set-position IN h_int580-browser-itens-2 ( 16.75 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 142.57 ) */

       /* Links to BrowserCadastro2 h_int580-browser-itens-2. */
       RUN add-link IN adm-broker-hdl ( h_int580-query , 'Record':U , h_int580-browser-itens-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int580-browser-itens-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-browser-duplicatas.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int580c.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int580-browser-duplicatas-2 ).
       RUN set-position IN h_int580-browser-duplicatas-2 ( 17.04 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.25 , 66.00 ) */

       /* Links to BrowserCadastro2 h_int580-browser-duplicatas-2. */
       RUN add-link IN adm-broker-hdl ( h_int580-query , 'Record':U , h_int580-browser-duplicatas-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int580-browser-duplicatas-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-viewer-totais.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int580-viewer-totais ).
       RUN set-position IN h_int580-viewer-totais ( 16.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.21 , 141.57 ) */

       /* Links to SmartViewer h_int580-viewer-totais. */
       RUN add-link IN adm-broker-hdl ( h_int580-query , 'Record':U , h_int580-viewer-totais ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int580-viewer-totais ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int580-browser-erros.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int580-browser-erros ).
       RUN set-position IN h_int580-browser-erros ( 16.42 , 2.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.25 , 141.00 ) */

       /* Links to BrowserCadastro2 h_int580-browser-erros. */
       RUN add-link IN adm-broker-hdl ( h_int580-query , 'Record':U , h_int580-browser-erros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int580-browser-erros ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT580" "1.12.01.NOVA_VERSAO"}
 ...
 /* if can-find (first usuar_grp_usuar no-lock where 
  ...
  RUN pi-recebe-handle in h_int580-viewer(h_int580-browser-itens-2).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    RUN pi-avaliar-erros in h_int580-viewer.    
    if valid-handle (h_int580-viewer) then
        RUN dispatch IN h_int580-viewer ('row-available':U).
    if valid-handle (h_int580-viewer-totais) then
        RUN dispatch IN h_int580-viewer-totais ('row-available':U).
    if valid-handle (h_int580-browser-itens-2) then
        RUN dispatch IN h_int580-browser-itens-2 ('row-available':U).
    if valid-handle (h_int580-browser-erros) then
        RUN dispatch IN h_int580-browser-erros ('row-available':U).
    if valid-handle (h_int580-browser-itens-2) then
        RUN pi-refresh IN h_int580-browser-itens-2.
