...
IF p-origem = "CLI" THEN DO: 
    ...
    for each int_ds_cliente use-index cgc  NO-LOCK
        WHERE int_ds_cliente.cgc      = p-chave :


        /* INICIO - Grava‡Æo LOG Tabela para a PROCFIT */
        FIND CURRENT int_ds_cliente EXCLUSIVE-LOCK NO-ERROR.
        IF AVAIL int_ds_cliente THEN DO:
           IF int_ds_cliente.situacao = 2 THEN DO:
               ASSIGN int_ds_cliente.envio_status      = 8
                      int_ds_cliente.retorno_validacao = ""
                   .
           END.
           ELSE IF int_ds_cliente.situacao = 1 THEN DO:
               ASSIGN int_ds_cliente.envio_status      = 9
                      int_ds_cliente.retorno_validacao = substring(p-desc-ocorrencia,1,500)
                   .
           END.
        END.
    ...
    END.
END.
/* KML - 01/10/2024 - Desativado gravação de logs por falta de utilização desse metodo de logs
...
IF p-situacao = 1 THEN DO: /* Pendente */
   ...
   IF NOT AVAIL int_ds_log_aux THEN DO:   
      ...
      ASSIGN int_ds_log_aux.origem          = p-origem
             ...
             int_ds_log_aux.ENVIO_STATUS    = 1
             int_ds_log_aux.ENVIO_DATA_HORA = datetime(today).
             int_ds_log_aux.id_sequencial   = NEXT-VALUE(seq-int-ds-log).
