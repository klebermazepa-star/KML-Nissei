...
&GLOBAL-DEFINE DBOName intbo002c
&GLOBAL-DEFINE DBOVersion 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName int-ds-cenar-fam
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-cenario LIKE int-ds-cenar-fam.cod-cenario NO-UNDO.
    DEFINE OUTPUT PARAMETER pfm-codigo   LIKE int-ds-cenar-fam.fm-codigo   NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE input PARAMETER pcod-cenario LIKE int-ds-cenar-fam.cod-cenario NO-UNDO.
    DEFINE input PARAMETER pfm-codigo   LIKE int-ds-cenar-fam.fm-codigo   NO-UNDO.
    
    FIND FIRST bfint-ds-cenar-fam WHERE 
               bfint-ds-cenar-fam.cod-cenario = pcod-cenario AND
               bfint-ds-cenar-fam.fm-codigo   = pfm-codigo NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-ds-cenar-fam THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-ds-cenar-fam)).
  ...
  /*:T--- Utilize o parÉmetro pType para identificar quais as validaá‰es a serem
    ...
    IF pType = "create" THEN DO:
        FIND FIRST int-ds-cenar-fam NO-LOCK WHERE 
                   int-ds-cenar-fam.cod-cenario = RowObject.cod-cenario AND 
                   int-ds-cenar-fam.fm-codigo   = RowObject.fm-codigo NO-ERROR.
        IF AVAIL int-ds-cenar-fam THEN DO:
               {method/svc/errors/inserr.i
                        &ErrorNumber="17006"
                        &ErrorType="EMS"
                        &ErrorSubType="ERROR" 
                        &ErrorParameters="'Fam°lia j† cadastrada para o cen†rio '"}
        END.
