...
&GLOBAL-DEFINE TableName int-ds-cenar-grupo
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-cenario LIKE int-ds-cenar-grupo.cod-cenario NO-UNDO.
    DEFINE OUTPUT PARAMETER pge-codigo   LIKE int-ds-cenar-grupo.ge-codigo   NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE input PARAMETER pcod-cenario LIKE int-ds-cenar-grupo.cod-cenario NO-UNDO.
    DEFINE input PARAMETER pge-codigo   LIKE int-ds-cenar-grupo.ge-codigo   NO-UNDO.
    
    FIND FIRST bfint-ds-cenar-grupo WHERE 
               bfint-ds-cenar-grupo.cod-cenario = pcod-cenario AND
               bfint-ds-cenar-grupo.ge-codigo   = pge-codigo NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-ds-cenar-grupo THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-ds-cenar-grupo)).
  ...
  /*:T--- Utilize o parÉmetro pType para identificar quais as validaá‰es a serem
    ...
    IF pType = "create" THEN DO:
        FIND FIRST int-ds-cenar-grupo NO-LOCK WHERE 
                   int-ds-cenar-grupo.cod-cenario = RowObject.cod-cenario AND 
                   int-ds-cenar-grupo.ge-codigo   = RowObject.ge-codigo NO-ERROR.
        IF AVAIL int-ds-cenar-grupo THEN DO:
               {method/svc/errors/inserr.i
                        &ErrorNumber="17006"
                        &ErrorType="EMS"
                        &ErrorSubType="ERROR" 
                        &ErrorParameters="'Grupo j† cadastrada para o cen†rio '"}
        END.
