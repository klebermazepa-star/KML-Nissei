...
*******************************************************************************/
{include/i-prgvrs.i NI0110 2.00.00.000}  /*** 010000 ***/
...
DEFINE VARIABLE h_q01di293 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01di293 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     ...
     bt-ajuda AT ROW 15.29 COL 69.43
     rt-button AT ROW 15.08 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Faixa Estabelecimentos Gera‡Æo Cupons"
         HEIGHT             = 15.54
         WIDTH              = 80
         MAX-HEIGHT         = 24.38
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Faixa Estabelecimentos Gera‡Æo Cupons */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Faixa Estabelecimentos Gera‡Æo Cupons */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Gera‡Æo Cupom' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.29 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 13.71 , 78.86 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/ni0110-v1.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01di293 ).
       RUN set-position IN h_v01di293 ( 3.00 , 7.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.13 , 62.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/ni0110-q1.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q01di293 ).
       RUN set-position IN h_q01di293 ( 14.88 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.79 , 8.29 ) */

       /* Links to SmartViewer h_v01di293. */
       RUN add-link IN adm-broker-hdl ( h_q01di293 , 'Record':U , h_v01di293 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_v01di293 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01di293 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
{utp/ut9000.i "NI0110" "2.00.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  find first int_ds_ger_cupom no-lock no-error. 
  if not avail int_ds_ger_cupom 
...
-------------------------------------------------------------*/
  RUN intprg/ni0110-v1.w.
  RUN adm\objects\folder.w.
  RUN intprg/ni0110-q1.w.
