...
DEFINE VARIABLE i_cod-emitente AS INTEGER FORMAT ">>>>>>>>9":U 
     LABEL "Emitente" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-emitente AT ROW 1.5 COL 23.43 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 3.04 COL 2.14
     bt-cancela AT ROW 3.04 COL 13
     bt-ajuda AT ROW 3.04 COL 58
     rt-button AT ROW 2.79 COL 1
     SPACE(0.13) SKIP(0.03)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "V  Para Emitente Agrupador"
...
ON GO OF FRAME d-vapara /* V  Para Emitente Agrupador */
DO:
FIND emitente_agrupador NO-LOCK
    WHERE emitente_agrupador.cod_emitente EQ INPUT FRAME {&FRAME-NAME} i_cod-emitente NO-ERROR.

IF NOT AVAILABLE emitente_agrupador THEN DO:
    {utp/ut-table.i emsesp emitente_agrupador 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(emitente_agrupador).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Emitente Agrupador */
DO:  
