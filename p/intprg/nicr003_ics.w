...
&Scoped-define FIELDS-IN-QUERY-br-tit-acr tt-tit-acr.marca tt-tit-acr.cod_estab tt-tit-acr.cdn_cliente tt-tit-acr.razao-social tt-tit-acr.cod_espec_docto tt-tit-acr.cod_ser_docto tt-tit-acr.cod_tit_acr tt-tit-acr.cod_parcela tt-tit-acr.dat_emis_docto tt-tit-acr.dat_vencto_origin_tit_acr tt-tit-acr.val_sdo_tit_acr tt-tit-acr.receita   
...
&Scoped-define FIELDS-IN-QUERY-br-tit-acr-sel tt-tit-acr-sel.marca tt-tit-acr-sel.cod_estab tt-tit-acr-sel.cdn_cliente tt-tit-acr-sel.razao-social tt-tit-acr-sel.cod_espec_docto tt-tit-acr-sel.cod_ser_docto tt-tit-acr-sel.cod_tit_acr tt-tit-acr-sel.cod_parcela tt-tit-acr-sel.dat_emis_docto tt-tit-acr-sel.dat_vencto_origin_tit_acr tt-tit-acr-sel.val_sdo_tit_acr tt-tit-acr-sel.receita DROP-DOWN-LIST   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-tit-acr-sel tt-tit-acr-sel.receita   
...
fi-dt-emis fi-dt-venc br-tit-acr br-tit-acr-sel bt-executar bt-cancelar 
&Scoped-Define DISPLAYED-OBJECTS fi-estab fi-cliente fi-nome-cliente ~
cb-convenio fi-dt-emis fi-dt-venc 
...
DEFINE VARIABLE fi-dt-venc AS DATE FORMAT "99/99/9999":U 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 117.72 BY 10.5 ROW-HEIGHT-CHARS .7 FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     bt-carrega-tit AT ROW 4.67 COL 119 WIDGET-ID 34
     fi-dt-venc AT ROW 4.88 COL 15.72 COLON-ALIGNED WIDGET-ID 58
     br-tit-acr AT ROW 6.25 COL 2.29 WIDGET-ID 200
     bt-filtro AT ROW 6.29 COL 120.29 WIDGET-ID 28
     bt-marca AT ROW 7.29 COL 120.29 WIDGET-ID 36
     bt-desmarca AT ROW 8.29 COL 120.29 WIDGET-ID 38
     bt-incluir AT ROW 9.25 COL 120.29 WIDGET-ID 40
     br-tit-acr-sel AT ROW 16.25 COL 2.29 WIDGET-ID 300
     bt-filtro-sel AT ROW 16.25 COL 120.29 WIDGET-ID 54
     bt-marca-sel AT ROW 17.25 COL 120.29 WIDGET-ID 44
     bt-desmarca-sel AT ROW 18.25 COL 120.29 WIDGET-ID 46
     bt-excluir AT ROW 19.25 COL 120.29 WIDGET-ID 48
     ...
     RECT-2 AT ROW 6.04 COL 1.43 WIDGET-ID 4
     rt-button AT ROW 27.04 COL 1.29 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 124.57 BY 27.54 WIDGET-ID 100.
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB br-tit-acr fi-dt-venc F-Main */
...
DO:
  DEFINE VARIABLE clista AS CHARACTER NO-UNDO.
    ...
    ASSIGN cLista = "Com Receita" + "," + "Sem Receita" + "," + "Ambos".
...
/*     END.                                       */
    ...
    ASSIGN
    tt-tit-acr-sel.receita:LIST-ITEMS IN BROWSE {&BROWSE-NAME} = clista.
END.
...
DO:
  DEFINE BUFFER b-cst-nota-fiscal FOR cst-nota-fiscal.

  ASSIGN tt-tit-acr-sel.receita = tt-tit-acr-sel.receita:SCREEN-VALUE IN BROWSE br-tit-acr-sel.

  FIND FIRST b-cst-nota-fiscal
       WHERE b-cst-nota-fiscal.cod-estabel = tt-tit-acr-sel.cod_estab           
         AND b-cst-nota-fiscal.serie       = tt-tit-acr-sel.cod_ser_docto 
         AND b-cst-nota-fiscal.nr-nota-fis = tt-tit-acr-sel.cod_tit_acr
         AND b-cst-nota-fiscal.convenio    = INPUT FRAME {&FRAME-NAME} cb-convenio EXCLUSIVE-LOCK NO-ERROR.
  IF AVAIL b-cst-nota-fiscal THEN DO:
      IF tt-tit-acr-sel.receita = "Com Receita" THEN DO:
          ASSIGN b-cst-nota-fiscal.tipo-venda = 1.
      END.
      ELSE IF tt-tit-acr-sel.receita = "Sem Receita" THEN DO:
          ASSIGN b-cst-nota-fiscal.tipo-venda = 2.
      END.
      ELSE IF tt-tit-acr-sel.receita = "Ambos" THEN DO:
          ASSIGN b-cst-nota-fiscal.tipo-venda = 3.
      END.
      
  END.
  RELEASE b-cst-nota-fiscal.
...
/*           ELSE DO:                                                                                                                                                                                        */
              ...
              FOR EACH int-ds-convenio NO-LOCK
                 WHERE int-ds-convenio.cnpj = cliente.cod_id_feder
/*                    AND int-ds-convenio.log-envia-cupom = YES */
              BREAK BY int-ds-convenio.cod-convenio :
                  IF c-convenio = "" THEN
                      ASSIGN c-convenio = STRING(int-ds-convenio.cod-convenio,"999999").

                  cb-convenio:ADD-LAST(STRING(int-ds-convenio.cod-convenio,"999999"),STRING(int-ds-convenio.cod-convenio,"999999")).
              END.
/*           END. */
      END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-estab fi-cliente fi-nome-cliente cb-convenio fi-dt-emis fi-dt-venc 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-1 RECT-2 rt-button fi-estab fi-cliente fi-dt-emis fi-dt-venc 
         br-tit-acr br-tit-acr-sel bt-executar bt-cancelar 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FOR EACH tit_acr NO-LOCK USE-INDEX titacr_cliente
   ...
   FIRST cst-nota-fiscal NO-LOCK
   WHERE cst-nota-fiscal.cod-estabel = tit_acr.cod_estab           
     AND cst-nota-fiscal.serie       = tit_acr.cod_ser_docto 
     AND cst-nota-fiscal.nr-nota-fis = tit_acr.cod_tit_acr
     AND cst-nota-fiscal.convenio    = INPUT FRAME {&FRAME-NAME} cb-convenio:
    ...
    IF NOT AVAIL tt-tit-acr THEN DO:
       ...
       /* INICIO - Alteraá∆o SM... para inclus∆o da informaá∆o de Venda com Receita para o Convànio */
        IF cst-nota-fiscal.tipo-venda = 1 THEN DO:
            ASSIGN tt-tit-acr.receita = "Com Receita".
        END.
        ELSE IF cst-nota-fiscal.tipo-venda = 2 THEN DO:
            ASSIGN tt-tit-acr.receita = "Sem Receita".
        END.
        ELSE IF cst-nota-fiscal.tipo-venda = 3 OR cst-nota-fiscal.tipo-venda = 0 OR cst-nota-fiscal.tipo-venda = ? THEN DO:
            ASSIGN tt-tit-acr.receita = "Ambos".
        END.
        /* FIM - Alteraá∆o SM... para inclus∆o da informaá∆o de Venda com Receita para o Convànio */
