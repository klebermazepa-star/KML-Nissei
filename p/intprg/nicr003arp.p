...
DEFINE TEMP-TABLE tt_int_ds_convenio  LIKE int_ds_convenio 
    FIELD cdn_cliente LIKE ems5.cliente.cdn_cliente
    .
...
PROCEDURE pi-processa-individual:
    ...
    FIND LAST int_ds_convenio NO-LOCK
            ...
            find first cliente no-lock
               ...
               IF NOT avail cliente THEN DO:
                ...
                RUN pi-cria-tt-erro-aux(INPUT tt_integr_acr_item_lote_impl.tta_num_seq_refer,
                                    INPUT 17006, 
                                    INPUT "Cliente nÆo cadastrado no cliente financeiro",
...
PROCEDURE pi-processa-coletivo:
    ...
    FOR EACH int_ds_convenio
        ...
        CREATE tt_int_ds_convenio.
        BUFFER-COPY int_ds_convenio TO tt_int_ds_convenio.
    
        FIND FIRST cliente USE-INDEX cliente_idfedercli
             WHERE cliente.cod_pais     = "BRA"
               AND cliente.cod_id_feder = int_ds_convenio.cnpj NO-LOCK NO-ERROR.
        IF AVAIL cliente THEN DO:
            ASSIGN tt_int_ds_convenio.cdn_cliente = cliente.cdn_cliente.
        END.
    END.
...
/*          AND int_ds_convenio.log_envia_cupom = NO NO-LOCK                           */
        LAST tt_int_ds_convenio
       WHERE tt_int_ds_convenio.cod_convenio     = INT(cst_nota_fiscal.convenio)
/*          AND tt_int_ds_convenio.log_envia_cupom  = NO */
         AND tt_int_ds_convenio.cdn_cliente     >= tt-param.c-cliente-ini               
         AND tt_int_ds_convenio.cdn_cliente     <= tt-param.c-cliente-fim NO-LOCK
       :  

       IF tt_int_ds_convenio.log_envia_cupom = YES THEN NEXT.
...
/*         IF NOT AVAIL cliente THEN NEXT.                                             */
       ...
       IF (DAY(DATE(dt_geracao_convenio)) = 1 AND MONTH(DATE(dt_geracao_convenio)) = 3) THEN DO:
           IF DAY(DATE(DATE(dt_geracao_convenio) - 1)) = 28 THEN DO:
               IF tt_int_ds_convenio.dia_fechamento <> 1  AND 
                  tt_int_ds_convenio.dia_fechamento <> 29 AND
                  tt_int_ds_convenio.dia_fechamento <> 30 THEN NEXT.
           END.
       ...
       END.
       ELSE DO:
           IF tt_int_ds_convenio.dia_fechamento <> DAY(DATE(dt_geracao_convenio)) THEN NEXT.
       END.
       ...
       ASSIGN tit_acr_convenio.cod_convenio     = tt_int_ds_convenio.cod_convenio
              tit_acr_convenio.cdn_cli_convenio = tt_int_ds_convenio.cdn_cliente.
...
FUNCTION retorna_vencimento RETURNS DATE (INPUT corte AS DATE ,INPUT dia AS INTEGER):
    ...
    IF (MONTH(corte) = 4 OR
       MONTH(corte) = 6 OR 
       MONTH(corte) = 9 OR 
       MONTH(corte) = 11) AND dia > 30 THEN ASSIGN dt = DATE(STRING(01) + "/" + STRING(03) + "/" + STRING(YEAR(corte))).
    ELSE
