...
define temp-table tt-movto-cheque no-undo
   ...
   FIELD cod-agencia   as character format "x(10)" label "Agància Banc†ria" column-label "Agància Banc†ria"
   FIELD dig-agencia   as character format "x(10)" label "Digito Agància Banc†ria" column-label "Digito Agància Banc†ria" 
   FIELD cod-conta     as character format "x(10)" label "Agància Banc†ria" column-label "Agància Banc†ria"
   FIELD dig-conta     as character format "x(10)" label "Digito Agància Banc†ria" column-label "Digito Agància Banc†ria" 
   FIELD cod-comp      AS INTEGER FORMAT "999" LABEL "C¢digo CÉmara Compensaá∆o"
...
DEFINE TEMP-TABLE tt_tit_acr_baixa LIKE tit_acr
    FIELD val_tit_acr_a_baixa LIKE tit_acr.val_sdo_tit_acr
    FIELD seq_reg             AS INT
    INDEX idx_seq_reg seq_reg ASC .
...
{include/i-rpout.i}
{utp/ut-liter.i Importaá∆o_Coleta_Lojas * L}
assign c-titulo-relat = trim(return-value).
{utp/ut-liter.i Importaá∆o * L}
...
RUN pi-inicializar IN h-acomp (INPUT "Importaá∆o Arquivos").

IF tt-param.tipo = 1 THEN DO:
    RUN pi-seta-titulo IN h-acomp (INPUT "Importaá∆o - Coleta":U).
...
ELSE IF tt-param.tipo = 2 THEN DO:
    RUN pi-seta-titulo IN h-acomp (INPUT "Importaá∆o - Dep¢sito":U).
       ...
       WHERE tt-movto-cheque.tipo = "CHQ":
        ...
        IF NOT AVAIL cheq_acr THEN DO:
            RUN pi-cria-tt-erro(INPUT 1,
                                INPUT 17006,
                                INPUT "N∆o encontrado nenhum t°tulo relacionado ao cheque.",
        ...
        FOR EACH tt-titulo
           BREAK BY tt-titulo.cod-estab:
                ...
                IF NOT AVAIL tit_acr THEN DO:                
                     RUN pi-cria-tt-erro(INPUT 1,
                                         INPUT 17006,
                                         INPUT "Estab/Titulo Nr.: " + string(tt-titulo.cod-estab) + string(tt-titulo.cd-tit) + " , n∆o foi encontrado.",
                                         INPUT "Estab/Titulo Nr.: " + string(tt-titulo.cod-estab)+ string(tt-titulo.cd-tit) + " , n∆o foi encontrado.").                
                ...
                assign tt_integr_acr_liquidac_lote.tta_cod_empresa                 = tit_acr.cod_empresa
                       ...
                       tt_integr_acr_liquidac_lote.tta_ind_sit_lote_liquidac_acr   = "Em digitaá∆o":U
...
PROCEDURE pi-cria-liquidacao-dinheiro:
    DEFINE VARIABLE v_hdl_programa AS HANDLE      NO-UNDO.
    DEFINE VARIABLE iSeq_reg       AS INTEGER     NO-UNDO.

    DEFINE VARIABLE dValorTotMovto LIKE tt-titulo.vl-movto.
           ...
           BREAK BY tt-titulo.cod-estab:
                ...
                IF AVAIL tit_acr AND log_sdo_tit_acr = NO THEN DO:
                    ...
                    IF NOT AVAIL tit_acr THEN DO:                
                         RUN pi-cria-tt-erro(INPUT 1,
                                             INPUT 17006,
                                             INPUT "Estab/Titulo Nr.: " + string(tt-titulo.cod-estab) + "/" + string(tt-titulo.cd-tit) + " , n∆o foi encontrado com saldo pra realizar a Liquidaá∆o.",
                                             INPUT "Estab/Titulo Nr.: " + string(tt-titulo.cod-estab) + "/" + string(tt-titulo.cd-tit) + " , n∆o foi encontrado com saldo pra realizar a Liquidaá∆o.").                
                         UNDO bloco_trans, LEAVE bloco_trans.                   
                    END. 
                END.

                EMPTY TEMP-TABLE tt_tit_acr_baixa.
                
                ASSIGN dValorTotMovto = tt-titulo.vl-movto
                       iSeq_reg       = 1.
                FOR EACH bf_tit_acr NO-LOCK
                   WHERE bf_tit_acr.cod_estab        = tt-titulo.cod-estab          
                     AND bf_tit_acr.cod_espec_docto  = "DI"                         
                     AND bf_tit_acr.cod_tit_acr      = tt-titulo.cd-tit
                     AND bf_tit_acr.log_sdo_tit_acr  = YES   :


                     CREATE tt_tit_acr_baixa.
                     BUFFER-COPY bf_tit_acr TO tt_tit_acr_baixa.
                     ASSIGN tt_tit_acr_baixa.seq_reg  = iSeq_reg 
                            iSeq_reg                  = iSeq_reg + 1.

                     IF dValorTotMovto <= bf_tit_acr.val_sdo_tit_acr THEN DO:
                         ASSIGN tt_tit_acr_baixa.val_tit_acr_a_baixa = dValorTotMovto.
                         ASSIGN dValorTotMovto = dValorTotMovto - tt_tit_acr_baixa.val_tit_acr_a_baixa.
                     END.
                     ELSE DO:
                         ASSIGN tt_tit_acr_baixa.val_tit_acr_a_baixa = bf_tit_acr.val_sdo_tit_acr.
                         ASSIGN dValorTotMovto = dValorTotMovto - bf_tit_acr.val_sdo_tit_acr.
                     END.

                     IF dValorTotMovto <= 0 THEN LEAVE.
                END.

                IF dValorTotMovto > 0 THEN DO:
                    FIND LAST tt_tit_acr_baixa USE-INDEX idx_seq_reg EXCLUSIVE-LOCK NO-ERROR.
                    IF AVAIL tt_tit_acr_baixa THEN DO:
                        ASSIGN tt_tit_acr_baixa.val_tit_acr_a_baixa = tt_tit_acr_baixa.val_tit_acr_a_baixa + dValorTotMovto.
                        
                    END.
                END.

                /* Inicio da Cargas das Temp-Tables da API para a Geraá∆o da Liquidaá∆o */
                empty temp-table tt_integr_acr_liquidac_lote    no-error.
                empty temp-table tt_integr_acr_liq_item_lote_3  no-error.
                empty temp-table tt_integr_acr_abat_antecip     no-error.
                empty temp-table tt_integr_acr_abat_prev        no-error.
                ...
                FOR EACH tt_tit_acr_baixa :
                    
                    CREATE tt_integr_acr_liq_item_lote_3.
                    ASSIGN tt_integr_acr_liq_item_lote_3.tta_cod_empresa              = tt_tit_acr_baixa.cod_empresa
                           tt_integr_acr_liq_item_lote_3.tta_cod_estab                = tt_tit_acr_baixa.cod_estab
                           tt_integr_acr_liq_item_lote_3.tta_cod_espec_docto          = tt_tit_acr_baixa.cod_espec_docto
                           tt_integr_acr_liq_item_lote_3.tta_cod_ser_docto            = tt_tit_acr_baixa.cod_ser_docto
                           tt_integr_acr_liq_item_lote_3.tta_cod_tit_acr              = tt_tit_acr_baixa.cod_tit_acr
                           tt_integr_acr_liq_item_lote_3.tta_cod_parcela              = tt_tit_acr_baixa.cod_parcela
                           tt_integr_acr_liq_item_lote_3.tta_cdn_cliente              = tt_tit_acr_baixa.cdn_cliente
                           ...
                           tt_integr_acr_liq_item_lote_3.tta_cod_indic_econ           = tt_tit_acr_baixa.cod_indic_econ
                           tt_integr_acr_liq_item_lote_3.tta_val_tit_acr              = tt_tit_acr_baixa.val_sdo_tit_acr     /* tt-titulo.vl-movto */
                           tt_integr_acr_liq_item_lote_3.tta_val_liquidac_tit_acr     = tt_tit_acr_baixa.val_tit_acr_a_baixa /* tt-titulo.vl-movto */ 
                           ...
                           tt_integr_acr_liq_item_lote_3.tta_des_text_histor          = "Liquidaá∆o realizada atravÇs do arquivo:  " + tt-titulo.arquivo + " linha: " + STRING(tt-titulo.linha) + "."
                    ...
                    IF tt_integr_acr_liq_item_lote_3.tta_val_liquidac_tit_acr > tt_tit_acr_baixa.val_sdo_tit_acr THEN DO:
                    ...
                    IF tt_tit_acr_baixa.val_tit_acr_a_baixa - tt_tit_acr_baixa.val_sdo_tit_acr = 0.01 THEN DO:
                        ASSIGN tt_integr_acr_liq_item_lote_3.tta_log_gera_antecip         = NO
                               tt_integr_acr_liq_item_lote_3.tta_val_liquidac_tit_acr     = tt_tit_acr_baixa.val_tit_acr_a_baixa - 0.01
        ...
        FOR EACH tt-titulo-cheque
           BREAK BY tt-titulo-cheque.cod_estab:
                ...
                IF NOT AVAIL tit_acr THEN DO:                
                     RUN pi-cria-tt-erro(INPUT 1,
                                         INPUT 17006,
                                         INPUT "Estab/Serie/Titulo: ":U + STRING(tit_acr.cod_estab) + "/" + STRING(tit_acr.cod_ser_docto) + "/" + STRING(tit_acr.cod_tit_acr) + " , n∆o foi encontrado.",
                                         INPUT "Estab/Serie/Titulo: ":U + STRING(tit_acr.cod_estab) + "/" + STRING(tit_acr.cod_ser_docto) + "/" + STRING(tit_acr.cod_tit_acr) + " , n∆o foi encontrado.").                
                     UNDO bloco_trans, LEAVE bloco_trans.                   
                END. 
