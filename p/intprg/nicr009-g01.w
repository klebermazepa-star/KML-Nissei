...
&Scoped-Define ENABLED-OBJECTS rt-button i_num_vale bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_num_vale 
...
DEFINE VARIABLE i_num_vale AS INTEGER FORMAT ">>>>>>>>>9":U 
     LABEL "Vale" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_num_vale AT ROW 1.75 COL 27 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND vale_manual NO-LOCK
    WHERE vale_manual.num_vale EQ INPUT FRAME {&FRAME-NAME} i_num_vale NO-ERROR.

IF NOT AVAILABLE vale_manual THEN DO:
    {utp/ut-table.i mgesp vale_manual 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(vale_manual).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_num_vale 
      WITH FRAME d-vapara.
  ENABLE rt-button i_num_vale bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
