...
&Scoped-Define ENABLED-OBJECTS bt-atualiza bt-historico bt-imprimi bt-movto ~
rt-button 
...
DEFINE BUTTON bt-atualiza 
     ...
     IMAGE-INSENSITIVE FILE "//192.168.200.52/datasul/ems2/image/toolbar/ii-chck1.bmp":U NO-FOCUS FLAT-BUTTON NO-CONVERT-3D-COLORS
     LABEL "Atualiza" 
     SIZE 3.86 BY 1.13 TOOLTIP "Atualiza Vale(Gera Movtimentaá‰es)".

DEFINE BUTTON bt-historico 
     IMAGE-UP FILE "image/toolbar/im-hist.bmp":U
     IMAGE-INSENSITIVE FILE "image/toolbar/ii-hist.bmp":U NO-FOCUS FLAT-BUTTON NO-CONVERT-3D-COLORS
     LABEL "Historico" 
     SIZE 3.86 BY 1.13 TOOLTIP "Hist¢rico do Vale".

DEFINE BUTTON bt-imprimi 
     IMAGE-UP FILE "image/toolbar/im-pri.bmp":U
     IMAGE-INSENSITIVE FILE "image/toolbar/ii-prigr.bmp":U NO-FOCUS FLAT-BUTTON NO-CONVERT-3D-COLORS
     LABEL "Imprime Vale" 
     SIZE 5.57 BY 1.13 TOOLTIP "Imprime o Vale Funcion†rio".

DEFINE BUTTON bt-movto 
     IMAGE-UP FILE "image/toolbar/im-brows.bmp":U
     IMAGE-INSENSITIVE FILE "image/toolbar/gr-tab.bmp":U NO-FOCUS FLAT-BUTTON NO-CONVERT-3D-COLORS
     LABEL "Button 1" 
...
DEFINE FRAME f-cad
     ...
     bt-imprimi AT ROW 1.17 COL 72 WIDGET-ID 6
...
DO:
    ...
    IF AVAIL vale_manual AND vale_manual.situacao = 2 THEN DO:
        run utp/ut-msgs.p('show',27100,substitute('Deseja confirmar o Vale &1, retornado assinado e gerar as movimentaá‰es nos T°tulos?~~O Vale Funcion†rio Nr.: &1 pode ser atualizado no contas a Receber, e realizar a movimentaá∆o dos t°tulos', vale_manual.num_vale)).

        IF RETURN-VALUE = "YES" THEN DO:

            RUN intprg/nicr009g.w.

            IF vale_manual.dat_movto <> ? THEN DO:
                 RUN pi-gera-movto-baixa.
            END.
        END.
    END.
    ELSE IF vale_manual.situacao = 3 THEN DO:
        run utp/ut-msgs.p('show',15825,substitute('O Vale &1 j† foi gerada as movimentaá‰es nos t°tulos~~O Vale Funcion†rio Nr.: &1 j† realizada a movimentaá∆o nos t°tulos!', vale_manual.num_vale)).
    END.
    ELSE IF  vale_manual.situacao = 1 THEN DO:
        run utp/ut-msgs.p('show',15825,substitute('O Vale &1 ainda n∆o foi impresso~~O Vale Funcion†rio Nr.: &1 ainda n∆o foi impresso para a geraá∆o da movimentaá∆o Ç necess†rio imprimir o vale e o mesmo voltar assinado!', vale_manual.num_vale)).
    END.
...
DO:
    ...
    END.
/*     ELSE DO:                                                                                                                                                                                                                                               */
/*         run utp/ut-msgs.p('show',15825,substitute('O Vale &1 n∆o pode ser Impresso!~~O Vale Funcion†rio Nr.: &1 n∆o foi realizada a movimentaá∆o nos t°tulos, s¢ Ç poss°vel Imprimir o vale, depois de realizada a Movimentaá∆o', vale_manual.num_vale)).  */
/*     END.                                                                                                                                                                                                                                                   */
...
DO:
    FIND FIRST vale_manual NO-LOCK
         WHERE ROWID(vale_manual) = gr-vale_manual NO-ERROR.
    IF AVAIL vale_manual AND (vale_manual.situacao = 1 OR vale_manual.situacao = 2) THEN DO:
        RUN intprg/nicr009c.w.

        IF gr-vale_manual <> ? THEN DO:
            RUN pi-posicao-query IN h_nicr009-q01 (OUTPUT gr-vale_manual). 
            RUN pi-reposiciona-query IN h_nicr009-q01 (INPUT gr-vale_manual).
            RUN pi-reposiciona-browse IN h_nicr009-b01. 
        END.
    END.
    ELSE DO:
        run utp/ut-msgs.p('show',15825,substitute('O Vale &1 j† foi gerada as movimentaá‰es nos t°tulos~~O Vale Funcion†rio Nr.: &1 j† foi realizada a movimentaá∆o, vocà deve gerar um novo vale para incluir novas movimentaá‰es', vale_manual.num_vale)).
    END.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_nicr009-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_nicr009-v01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_nicr009-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE bt-atualiza bt-historico bt-imprimi bt-movto rt-button 
...
DEFINE BUFFER bf-vale_manual FOR vale_manual.
    ...
    IF AVAIL vale_manual  THEN DO:
        ...
        FOR EACH movto_vale_manual 
           WHERE movto_vale_manual.num_vale = vale_manual.num_vale NO-LOCK:
            ASSIGN dValTot = dValTot + movto_vale_manual.val_movto.
        END.

        IF dValTot <> vale_manual.val_vale THEN DO:
            run utp/ut-msgs.p('show',17006,substitute('O valor do Vale &1, n∆o confere com Total dos Movimentos! ~~O valor informado no Vale Funcion†rio Nr.: &1 n∆o confere com o valoo total dos movimentos informado!', vale_manual.num_vale)).
            RETURN "NOK".
        END.
        ...
        FOR EACH movto_vale_manual 
           ...
           WHERE tit_acr.num_id_tit_acr = movto_vale_manual.num_id_tit_acr:
            ...
            RUN pi-gera-movto-menor(INPUT tit_acr.cod_estab,   /* Estabelecimento    */
                                    ...
                                    INPUT movto_vale_manual.val_movto,     /* Valor Movto        */
                                    INPUT vale_manual.obs_historico,
                                    INPUT vale_manual.dat_movto,
        ...
        IF lErro = YES THEN DO:
            ...
            FIND FIRST bf-vale_manual EXCLUSIVE-LOCK
                 WHERE ROWID(bf-vale_manual) = gr-vale_manual NO-ERROR.
            IF AVAIL bf-vale_manual THEN DO:
                ASSIGN bf-vale_manual.dat_movto = ?.
            END.
            RELEASE bf-vale_manual.
        ...
        ELSE DO:
            ...
            FIND FIRST bf-vale_manual EXCLUSIVE-LOCK
                 WHERE ROWID(bf-vale_manual) = gr-vale_manual NO-ERROR.
            IF AVAIL bf-vale_manual THEN DO:
                ASSIGN bf-vale_manual.situacao = 3.
            END.
            RELEASE bf-vale_manual.
