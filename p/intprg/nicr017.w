...
DEF VARIABLE raw-param AS RAW NO-UNDO.
...
DEFINE VARIABLE rs-execucao-rpw AS INTEGER INITIAL 1 
...
DEFINE FRAME F-Main
     ...
     bt-executar AT ROW 26.88 COL 19.14 HELP
          "Dispara a execuá∆o do relat¢rio" WIDGET-ID 32
     bt-cancelar AT ROW 26.88 COL 30.14 HELP
          "Fechar" WIDGET-ID 30
     rs-execucao-rpw AT ROW 27 COL 2.29 NO-LABEL WIDGET-ID 98
...
------------------------------------------------------------------------------*/
  DISPLAY c-cod-estab c-cod-espec-docto c-cod-ser-docto c-cod-tit-acr 
          ...
          c-saldo-tit-acr fi-data-emis fi-data-vencto rs-execucao-rpw 
  ...
  ENABLE RECT-1 RECT-2 rt-button RECT-3 c-cod-estab bt-zoom-estab 
         ...
         bt-executar bt-cancelar rs-execucao-rpw 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FOR EACH tt-tit-acr
   WHERE tt-tit-acr.cod_estab          >= c-estab-ini  
    ...
    FIND FIRST tt-tit-acr-sel 
         WHERE tt-tit-acr-sel.r-rowid = ROWID(tt-tit-acr) NO-LOCK NO-ERROR.
    IF NOT AVAIL tt-tit-acr-sel THEN
    ...
    IF RETURN-VALUE <> "NOK" THEN DO:
        ...
        IF RETURN-VALUE <> "NOK" THEN DO:
            ...
            ASSIGN tt-param.destino       = 3
                   tt-param.arquivo       = SESSION:TEMP-DIRECTORY + "nicr017rp.txt"
                   tt-param.usuario       = c-seg-usuario
                   tt-param.data-exec     = TODAY
                   tt-param.hora-exec     = TIME
                   tt-param.CodCliente    = cCodCliente
                   tt-param.NumRenegoc    = cNumRenegoc
                   tt-param.DatEmissao    = dDatEmissao
                   tt-param.DatVencto     = dDatVencto 
                   tt-param.CodEspec      = cCodEspec  
                   tt-param.SerDocto      = cSerDocto
                   tt-param.data-emis     = INPUT FRAME {&FRAME-NAME} fi-data-emis
                   tt-param.cod-tit-acr   = INPUT FRAME {&FRAME-NAME} c-cod-tit-acr
                   tt-param.convenio      = INPUT FRAME {&FRAME-NAME} fi-convenio
                   tt-param.cod-estab     = INPUT FRAME {&FRAME-NAME} c-cod-estab
                   tt-param.cod-cliente   = INPUT FRAME {&FRAME-NAME} c-cod-cliente
                   tt-param.cod-ser-docto = INPUT FRAME {&FRAME-NAME} c-cod-ser-docto
                   tt-param.data-vencto   = INPUT FRAME {&FRAME-NAME} fi-data-vencto.
            ...
            FOR EACH tt-tit-acr
                ...
                RAW-TRANSFER tt-tit-acr TO tt-raw-digita.raw-digita.
            END.
            
            RAW-TRANSFER tt-param TO raw-param.

            IF INPUT FRAME {&FRAME-NAME} rs-execucao-rpw = 1 THEN DO:
                RUN intprg/nicr017rp.p (INPUT raw-param,
                                        INPUT TABLE tt-raw-digita).
                OS-COMMAND SILENT VALUE(tt-param.arquivo) NO-ERROR.
            END.
            ELSE DO:
                run btb/btb911zb.p (input "nicr017rp",
                                    input "intprg/nicr017rp.p",
                                    input "2.00.00.000",
                                    input 97,
                                    input "nicr017rp.LST",
                                    input 2,
...
/*                                            int_ds_fat_conv_site.val_parcela        = bf_tit_acr.val_origin_tit_acr                                */
