...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 144.14 BY 26.83 WIDGET-ID 100.
...
DO:
    ...
    FOR EACH  tt-int_ds_it_docto_xml NO-LOCK 
        WHERE tt-int_ds_it_docto_xml.arquivo    = tt-int_ds_docto_xml.arquivo
          AND tt-int_ds_it_docto_xml.cnpj      = tt-int_ds_docto_xml.cnpj
   ...
   ASSIGN dt-trans-ini   = INPUT FRAME {&FRAME-NAME} dt-trans-ini
    ...
    THEN DO:
        ...
        FOR EACH  int_ds_docto_xml NO-LOCK 
            ...
            LAST  int_ds_docto_wms USE-INDEX documento NO-LOCK
            ...
            BY  int_ds_docto_wms.id_sequencial:

              RUN pi-cria-tt-int_ds_docto_xml.
        END.
        FOR EACH  int_ds_docto_xml USE-INDEX data_trans NO-LOCK 
            WHERE int_ds_docto_xml.situacao  <> 9
              AND int_ds_docto_xml.tipo_nota <> 3
              AND int_ds_docto_xml.CNPJ_dest  = "79430682025540"
              AND int_ds_docto_xml.chnfe      = c-chave
    ...
    ELSE DO:
        IF  c-nro-docto   <> "" AND
            c-serie-docto <> "" 
        THEN DO:
            FOR EACH   int_ds_docto_xml USE-INDEX documento NO-LOCK 
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
      FOR FIRST int_ds_docto_xml NO-LOCK
          ...
          FOR LAST  int_ds_docto_wms USE-INDEX documento NO-LOCK 
              WHERE int_ds_docto_wms.doc_numero_n        = int_ds_docto_xml.nNF
                AND int_ds_docto_wms.doc_serie_s         = int_ds_docto_xml.serie 
                AND int_ds_docto_wms.cnpj_cpf            = int_ds_docto_xml.CNPJ
              BY  int_ds_docto_wms.id_sequencial:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-busca-natureza w-cadpaifilho-ambos 
...
------------------------------------------------------------------------------*/
    ...
    if can-find (first tt-int_ds_docto_xml USE-INDEX ser_nf no-lock
   ...
   FIND FIRST int_ds_ext_emitente NO-LOCK 
    ...
    FOR EACH  int_ds_it_docto_xml USE-INDEX idx_cnpj NO-LOCK 
        ...
        if can-find (first tt-int_ds_it_docto_xml USE-INDEX idx_cnpj no-lock 
                     WHERE tt-int_ds_it_docto_xml.arquivo   = int_ds_docto_xml.arquivo
                       AND tt-int_ds_it_docto_xml.cnpj      = int_ds_docto_xml.cnpj
       ...
       IF AVAIL ITEM THEN
        ...
        RUN intprg/int013a.p( input int_ds_it_docto_xml.cfop,
                              ...
                              INPUT item.class-fiscal,
                              input int_ds_docto_xml.dt_trans,
                              output c-nat-operacao ).

       ASSIGN tt-int_ds_it_docto_xml.nat-oper-cad = c-nat-operacao.

       IF tt-int_ds_it_docto_xml.nat-oper-cad <> tt-int_ds_it_docto_xml.nat_operacao THEN
          ASSIGN tt-int_ds_it_docto_xml.l-divergente = YES.
...
------------------------------------------------------------------------------*/
    ...
    PUT UNFORMATTED "Emiss∆o;Transaá∆o;NF;SÇrie;Fornec;Nome Fornecedor;UF;Seq;ITEM;ITEM Fornec;Descriá∆o;Divergente;Nat Oper NF;Nat Oper Cad;NCM NF;NCM Cad;Qtd;Qtd Fornec;Un;Un Fornec;Vlr Unit;Vlr Desconto;Vlr Despesas;Vlr Total Prod;Base ICMS NF;Base ICMS Cad;% Red BC NF;% Red BC Cad;%ICMS NF;% ICMS Cad;Valor ICMS NF;Valor ICMS CAD;Vlr ICMS Deson NF;Base ST NF;Base ST Cad;% MVA NF;% MVA Cad;Vl PMC NF;Vl PMC Cad;% ST NF;% ST Cad;% Red Cad;Valor ST NF;Valor ST Cad;Base PIS NF;Base PIS Cad;% PIS NF;% PIS Cad;Valor PIS NF;Valor PIS Cad;Base Cofins NF;Base Cofins Cad;% Cofins NF;% Cofins Cad;Valor Cofins NF;Valor Cofins Cad;Base IPI NF;Base IPI Cad;% IPI NF;% IPI Cad;Valor IPI NF;Valor IPI Cad;Total NF;Total Cad;Chave NFE" SKIP.
    ...
    FOR EACH  tt-int_ds_docto_xml,
        ...
        PUT UNFORMATTED tt-int_ds_docto_xml.dEmi                   ";"
                        ...
                        tt-int_ds_it_docto_xml.nat-oper-cad        ";"
   ...
   ASSIGN ChExcel = NEW office.libre.excel.ExcelWrapper().
    ...
    assign chArquivo  = ? NO-ERROR.
    assign chPlanilha = ? NO-ERROR.
