...
ASSIGN ChExcelApplication = NEW office.libre.excel.ExcelWrapper().
...
FOR EACH tt-excel USE-INDEX data-docto BREAK BY tt-excel.dt-emissao
    ...
    ch-planilha:range("A" + STRING(i-linha)):SetValue(tt-excel.dt-emissao). 
    ch-planilha:range("B" + STRING(i-linha)):SetValue(tt-excel.dt-transacao).
    ch-planilha:range("C" + STRING(i-linha)):SetValue(tt-excel.cod-estabel).                          
    ch-planilha:range("D" + STRING(i-linha)):SetValue(tt-excel.cod-emitente).         
    ch-planilha:range("E" + STRING(i-linha)):NumberFormat = "@".
    ch-planilha:range("E" + STRING(i-linha)):SetValue(tt-excel.cnpj).
    ch-planilha:range("F" + STRING(i-linha)):SetValue(tt-excel.natur-oper).                             
    ch-planilha:range("G" + STRING(i-linha)):SetValue(tt-excel.nro-docto). 
    ch-planilha:range("H" + STRING(i-linha)):SetValue(tt-excel.serie).
    ch-planilha:range("I" + STRING(i-linha)):SetValue(tt-excel.situacao).
    ch-planilha:range("J" + STRING(i-linha)):SetValue(tt-excel.nr-contrato). 
    ch-planilha:range("K" + STRING(i-linha)):SetValue(tt-excel.conta-trans).  
    ch-planilha:range("L" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("L" + STRING(i-linha)):SetValue(tt-excel.vlr-desc).    
    ch-planilha:range("M" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("M" + STRING(i-linha)):SetValue(tt-excel.vlr-merc).                 
    ch-planilha:range("N" + STRING(i-linha)):SetValue(tt-excel.sit-nfe).                        
    ch-planilha:range("O" + STRING(i-linha)):SetValue(tt-excel.usuario).                  
    ch-planilha:range("P" + STRING(i-linha)):SetValue(tt-excel.doc-import).    
    ch-planilha:range("Q" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("Q" + STRING(i-linha)):SetValue(tt-excel.vlr-total).
    ch-planilha:range("R" + STRING(i-linha)):SetValue(tt-excel.sequencia).
    ch-planilha:range("S" + STRING(i-linha)):SetValue(tt-excel.it-codigo).
    ch-planilha:range("T" + STRING(i-linha)):SetValue(tt-excel.descricao).
    ch-planilha:range("U" + STRING(i-linha)):SetValue(tt-excel.un).   
    ch-planilha:range("V" + STRING(i-linha)):SetValue(tt-excel.numero-ordem).
    ch-planilha:range("W" + STRING(i-linha)):SetValue(tt-excel.nr-ord-produ).
    ch-planilha:range("X" + STRING(i-linha)):NumberFormat = "@".
    ch-planilha:range("X" + STRING(i-linha)):SetValue(tt-excel.conta-contabil).
    ch-planilha:range("Y" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("Y" + STRING(i-linha)):SetValue(tt-excel.qtde-item-doc).
    ch-planilha:range("Z" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("Z" + STRING(i-linha)):SetValue(tt-excel.vlr-item-doc).
    ch-planilha:range("AA" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("AA" + STRING(i-linha)):SetValue(tt-excel.desconto).
    ch-planilha:range("AB" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("AB" + STRING(i-linha)):SetValue(tt-excel.vlr-tot-item).
    
    IF FIRST-OF(tt-excel.numero-ordem) THEN DO:    
       ch-planilha:range("AC" + STRING(i-linha)):SetValue(tt-excel.data-pedido).
       ch-planilha:range("AD" + STRING(i-linha)):SetValue(tt-excel.num-pedido).
       ch-planilha:range("AE" + STRING(i-linha)):SetValue(tt-excel.cond-pagto).
       ch-planilha:range("AF" + STRING(i-linha)):NumberFormat = "@".
       ch-planilha:range("AF" + STRING(i-linha)):SetValue(tt-excel.vencimento).
       ch-planilha:range("AG" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
       ch-planilha:range("AG" + STRING(i-linha)):SetValue(tt-excel.qtde-pedido).
       ch-planilha:range("AH" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
       ch-planilha:range("AH" + STRING(i-linha)):SetValue(tt-excel.valor-pedido).
       ch-planilha:range("AI" + STRING(i-linha)):NumberFormat = "@".
       ch-planilha:range("AI" + STRING(i-linha)):SetValue(tt-excel.mensagem).
    END.
END.                                                                                                                
...
/*    RELEASE OBJECT ch-planilha.            */
    ...
    ASSIGN chExcelApplication = ? NO-ERROR.
