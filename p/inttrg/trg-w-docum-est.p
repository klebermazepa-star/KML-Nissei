...
DEFINE VAR      de-cod-depos     AS CHAR        NO-UNDO.
            ...
            THEN DO:
                ...
                DO TRANS ON ERROR UNDO, LEAVE: 
                    FOR EACH item-doc-est OF b-docum-est NO-LOCK,
                        ...
                        IF b-docum-est.cod-estabel = "977" 
                        THEN 
                            ASSIGN tt-movto.cod-depos = b-docum-est.cod-estabel.
                        ...
                        END.
                        ELSE DO:
                            /* Atualizar valor do /COFINS na DIV, temp-table n?o trata este campo */
            ...
            for each item-doc-est of b-docum-est no-lock:
                ...
                FOR FIRST emitente NO-LOCK
                    ...
                    IF int_ds_it_docto_xml.cst_icms = 60 THEN DO:
                        ...
                        /*REALIZADA ALTERAۂO DA LOGICA DE DEPOSITO PARA QUE INTEGRE SEMPRE O DEPOSITO DE ACORDO COM A FILIAL 04/03/2026*/
                        
                        IF b-docum-est.cod-estabel = "973" THEN
                        DO:
                        
                            ASSIGN de-cod-depos = "973".
                            
                        END.
                        
                        ELSE DO:
                        
                            ASSIGN de-cod-depos = "LOJ".
                        END.
                        
                        IF b-docum-est.cod-estabel = "977" THEN
                        DO:
                        
                            ASSIGN de-cod-depos = "977".
                            
                        END.
                        
                        /*FIM ALTERAۂO 04/03/2026*/
                    ...
                    */
                            ...
                            assign tt-movto.cod-versao-integ  = 1
                                   ...
                                   tt-movto.cod-depos         = de-cod-depos //IF b-docum-est.cod-estabel = "973" THEN "973" ELSE "LOJ" IF b-docum-est.cod-estabel   = "973" THEN "973" ELSE IF b-docum-est.cod-estabel = "977" THEN "977" ELSE "LOJ"     //REALIZADA ALTERAۂO DA LOGICA DE DEPOSITO PARA QUE INTEGRE SEMPRE O DEPOSITO DE ACORDO COM A FILIAL 04/03/2026
    ...
    */
    ...
    /*                             VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.               */
                        ...
                        assign tt-movto.cod-versao-integ  = 1                              
                               ...
                               tt-movto.sc-db             = if item.tipo-contr = 1 then item.sc-codigo else ""
                               tt-movto.cod-prog-orig     = "trnfesm135"
                               tt-movto.cod-depos         = IF b-docum-est.cod-estabel   = "973" THEN "973" ELSE IF b-docum-est.cod-estabel = "977" THEN "977" ELSE "LOJ"
    ...
    /*                         VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.               */
            ...
            if  b-docum-est.cod-emitente = 9356 OR b-docum-est.cod-emitente = 11434961 and 
            ...
            then do:
                ...
                FIND FIRST b-estabelec NO-LOCK
                    WHERE b-estabelec.cgc = emitente.cgc NO-ERROR.
                    
    
                FIND FIRST item-uni-estab NO-LOCK
                    WHERE item-uni-estab.cod-estabel    = b-estabelec.cod-estabel
                      AND item-uni-estab.it-codigo      = item-doc-est.it-codigo NO-ERROR.
                 
                 
                 // SUBSTRING(item-uni-estab.char-2,60,1) = "S"

                IF NOT AVAIL tt-bonificados AND AVAIL item-uni-estab THEN DO:
                    /* Retirar PIS */
                 
                    FIND FIRST b-emitente NO-LOCK
                        WHERE b-emitente.cod-emitente = b-docum-est.cod-emitente NO-ERROR.
                     
                    FIND FIRST estabelec NO-LOCK
                        WHERE estabelec.cod-estabel = b-docum-est.cod-estabel NO-ERROR.
                        
                     .MESSAGE "Antes movimento v2"  SKIP
                                SUBSTRING(item-uni-estab.char-2,60,1) SKIP
                                b-docum-est.cod-estabel SKIP
                                estabelec.estado SKIP
                                b-emitente.nome-abrev SKIP
                                b-emitente.estado  
                            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.   
                        
                         //DESTINO           //ORIGEM
                    IF estabelec.estado <> b-emitente.estado  OR
                       SUBSTRING(item-uni-estab.char-2,60,1) = "S" AND (estabelec.estado = b-emitente.estado) THEN
                    DO: 
                    
                    //KML 16/12
                  
                    
                        .MESSAGE "criando movto"  SKIP
                                b-docum-est.cod-estabel SKIP
                                SUBSTRING(item-uni-estab.char-2,60,1) SKIP
                                estabelec.estado SKIP
        ...
        IF CAN-FIND (FIRST tt-movto) THEN DO:
            ...
            for each tt-movto:
                ...
                ELSE IF b-docum-est.cod-estabel = "977" THEN
    ...
    THEN DO:
        ...
        /* Estorno PIS/COFINS */
        if b-docum-est.cod-emitente = 9356 OR b-docum-est.cod-emitente = 11434961 and b-docum-est.esp-docto = 23 then
        FOR EACH   item-doc-est OF b-docum-est NO-LOCK
