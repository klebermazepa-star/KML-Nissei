...
/* KML Consultoria Guilherme Nichele - 13/06/2024 - Criar tabela de alteraá∆o para leitura do BI */



FIND FIRST b-item  NO-ERROR.
...
IF AVAIL b-item THEN DO:
    ...
    IF v_cdn_empres_usuar = "10" THEN DO:
        ...
        assign tt-param.usuario         = "RPW"
               ...
               tt-param.codItemInicial  = b-item.it-codigo
               tt-param.codItemFinal    = b-item.it-codigo
               tt-param.situacao        = 0
               tt-param.arquivo        = "U:/teste.txt".
             
        raw-transfer tt-param TO raw-param. 
    
        RUN custom/eswms001rp.p (INPUT raw-param , 
    ...
    ASSIGN esp-alteracao-bi.tabela = "item "
           esp-alteracao-bi.dt-alteracao = TODAY
           esp-alteracao-bi.it-codigo = b-item.it-codigo.
...
/*IF PROGRAM-NAME(5) = "dispatch invwr/v34in172.w" OR PROGRAM-NAME(5) =  "dispatch invwr/v42in172.w" THEN DO: */
   ...
   IF  b-item.fm-codigo < "900" THEN DO:
       ...
       OR  l-difer = YES THEN DO:
           ...
           ASSIGN int_ds_item.envio_status               = 0
                  int_ds_item.tipo_movto                 = i-tipo-movto
                  ...
                  int_ds_item.pro_grupocomercial_n       = IF v_cdn_empres_usuar = "10" THEN 0 ELSE INT(b-item.fm-cod-com)
