...
define new global shared variable h-axsep006 as handle no-undo.
...
/*
OUTPUT TO \\192.168.200.52\datasul\_custom_homol\axsep006.LOG APPEND.
FOR EACH btt-epc:
    DISPLAY 
...
OUTPUT CLOSE.
*/
if valid-handle(h-axsep006) then return "OK".
for first tt-epc where 
    ...
    for first it-nota-fisc no-lock where rowid(it-nota-fisc) = r-rowid:
        ...
        for first nota-fiscal no-lock of it-nota-fisc:
            ...
            if /*not can-find (first int-ndd-envio where
                          int-ndd-envio.JOB begins (nota-fiscal.cod-estabel + nota-fiscal.serie + nota-fiscal.nr-nota-fis) and
                          int-ndd-envio.DOCUMENTUSER = c-seg-usuario and
                          int-ndd-envio.KIND         = 1 /* XML */)*/ TRUE then do:
                ...
                RUN pi-retornaXMLNFe   IN h-axsep006 (OUTPUT cXML).
            end.
            if cXML <> "" then do:
...
if valid-handle (h-axsep006) then delete procedure h-axsep006.
