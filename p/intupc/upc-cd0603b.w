...
*******************************************************************************/
{include/i-prgvrs.i CD0603B 1.12.00.AVB}
...
DEFINE VARIABLE h_upc-cd0603b-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_upc-cd0603b-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 9.33 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 9.33 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 9.33 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 9.33 COL 66.29
     rt-button AT ROW 9.17 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 76.72 BY 9.88 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica NCM X UF X ICMS"
         HEIGHT             = 9.63
         WIDTH              = 76.57
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica NCM X UF X ICMS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica NCM X UF X ICMS */
DO:
...
DO:
    {include/okfil.i h_upc-cd0603b-viewer}
END.
...
DO:
{include/salvafil.i h_upc-cd0603b-viewer}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intupc/upc-cd0603b-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_upc-cd0603b-viewer ).
       RUN set-position IN h_upc-cd0603b-viewer ( 1.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.00 , 75.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intupc/upc-cd0603b-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_upc-cd0603b-query ).
       RUN set-position IN h_upc-cd0603b-query ( 7.50 , 68.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_upc-cd0603b-viewer. */
       RUN add-link IN adm-broker-hdl ( h_upc-cd0603b-query , 'Record':U , h_upc-cd0603b-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_upc-cd0603b-viewer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_upc-cd0603b-viewer ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "CD0603B" "1.12.00.AVB"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_upc-cd0603b-viewer (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_upc-cd0603b-query (input v-row-table).
