...
define new global shared var h-btDelete   as handle no-undo.
define new global shared var h-dt-saldo   as handle no-undo.
define new global shared var h-btInventarioGeral   as handle no-undo.
...
IF p-ind-event = "After-display"  THEN DO:
    ...
    RUN utils/findWidget.p (INPUT  'btDelete',                                                               
                            INPUT  'button',       
                            INPUT  p-wgh-frame,
                            output h-btDelete).
                            
    RUN utils/findWidget.p (INPUT  'dt-saldo ',                                                               
                            INPUT  'fill-in',       
                            INPUT  p-wgh-frame,
                            output h-dt-saldo).
...
/*         WHERE  inventario.dt-saldo = date(h-dt-saldo:SCREEN-VALUE): */
/*                                                                     */
/*         ASSIGN c-dt-saldo       = DATE(h-dt-saldo:SCREEN-VALUE)     */
/*                c-it-codigo      = inventario.it-codigo              */
/*                c-cod-estabel    = inventario.cod-estabel            */
/*                c-cod-depos      = inventario.cod-depos.             */
/*                                                                     */
/*     END.                                                            */
    
              
    //ASSIGN r-rowid-upc-ce0712 = p-row-table.          
 
    ASSIGN c-dt-saldo   =   ?.
    
    ASSIGN c-dt-saldo   = DATE(h-dt-saldo:SCREEN-VALUE).  
    IF VALID-HANDLE(h-btDelete) THEN
    DO:
        CREATE BUTTON h-btInventarioGeral
         ASSIGN NAME      = "h-btInventarioGeral"
                FRAME     = h-btDelete:FRAME
                ROW       = h-btDelete:ROW
                COL       = h-btDelete:COL + 5
                width     = 15.00
                height    = 1.25
                VISIBLE   = YES
                SENSITIVE = YES
                LABEL     = "Inventario Geral"
                TRIGGERS:                                          
                    ON CHOOSE PERSISTENT RUN intupc/upc-ce0712-btInventario.p . 
                END TRIGGERS.                                        
