    ...
    AND p-row-table <> ? THEN DO:
        ...
        IF AVAIL b-docum-est THEN DO:
           FIND FIRST int_ds_docto_xml NO-LOCK
               WHERE int_ds_docto_xml.serie        = b-docum-est.serie-docto    AND
                     int_ds_docto_xml.nNF          = b-docum-est.nro-docto      AND
                     int_ds_docto_xml.cod_emitente = b-docum-est.cod-emitente   AND 
                     int_ds_docto_xml.tipo_nota    = b-docum-est.tipo-nota NO-ERROR.
...
IF  p-ind-event  = "EventDelete2"
AND p-ind-object = "EventDelete2" THEN DO:

    FOR FIRST b-docum-est NO-LOCK
        WHERE ROWID(b-docum-est) = p-row-table,
        ...
        WHERE esp-item-entr-st.cod-estab        = b-docum-est.cod-estabel
        AND   esp-item-entr-st.cod-serie        = b-docum-est.serie-docto
        AND   esp-item-entr-st.cod-docto        = b-docum-est.nro-docto
        AND   esp-item-entr-st.cod-natur-operac = b-docum-est.nat-operacao
        AND   esp-item-entr-st.cod-emitente     = string(b-docum-est.cod-emitente)
