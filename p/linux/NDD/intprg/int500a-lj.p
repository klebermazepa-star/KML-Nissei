/********************************************************************************
** Programa: int500a-lj - ConexÆo E leitura WebServer NDD - Entradas - Loja
**
...
if l-log then output stream str-log to value(cCaminhoTMP + "int500a_LJ" + "_" + replace(string(time,"HH:MM:SS"),':','_') + ".LOG").
...
for each estabelec NO-LOCK WHERE estabelec.cod-estabel <> "973"
    ...
    if last-of (estabelec.cgc) then do:
        ...
        if l-log then do:
            cFile = cCaminhoTMP + "eformsConsultarColdDFeHeader_" + estabelec.cod-estabel + ".xml".
        ...
        if l-connected then do:
            ...
            cFile = cCaminhoTMP + "eformsConsultarColdDFeRetorno" + "_" + 
                estabelec.cod-estabel + "_" + 
                c-seg-usuario + "_" + string(today,"99-99-9999") + 
