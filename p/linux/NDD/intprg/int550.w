...
DEFINE BUTTON btBuscaXML 
     ...
     LABEL "Busca XML" 
     SIZE 4 BY 1.27 TOOLTIP "Buscar XML na NDD"
     FONT 4.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 130.5 BY 8.8
         FONT 1 ROW-HEIGHT-CHARS .53.
...
DEFINE FRAME fpage0
     ...
     btHelp AT ROW 1.13 COL 131.38 HELP
          "Ajuda"
     btAtualiza AT ROW 2.6 COL 131.5 HELP
          "Atualiza consulta" WIDGET-ID 4
     cCaminhoTela AT ROW 2.87 COL 8.5 COLON-ALIGNED WIDGET-ID 20
     btBuscaXML AT ROW 3.93 COL 131.5 HELP
          "Busca XML da nota na NDD" WIDGET-ID 24
     cChaveNFe AT ROW 4.2 COL 77 COLON-ALIGNED WIDGET-ID 22
...
DO:
    if avail tt-NDD_ENTRYINTEGRATION then do:
        ...
        cFileXML =  cCaminhoNDD + "tmp\NF" + "-" + c-seg-usuario + "-" +
                    string(today,"99-99-9999") + replace(string(time,"HH:MM:SS"),':','-') + "-" + string(iId) + '.xml'.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    if l-log then output stream str-log to value(cCaminhoTMP + c-seg-usuario + "-int550.LOG").
