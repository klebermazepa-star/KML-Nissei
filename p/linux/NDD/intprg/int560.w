...
*******************************************************************************/
{include/i-prgvrs.i INT560 2.12.00.AVB}
...
DEFINE VARIABLE h_int560-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int560-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 99.5 BY 8.53 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Parƒmetros NDD"
         HEIGHT             = 8.73
         WIDTH              = 99.5
         MAX-HEIGHT         = 20.7
         MAX-WIDTH          = 114.25
...
ON END-ERROR OF w-cadsi2 /* Manuten‡Æo Parƒmetros NDD */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Manuten‡Æo Parƒmetros NDD */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 83.63 ) NO-ERROR.
       /* Size in UIB:  ( 1.27 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int560-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int560-viewer ).
       RUN set-position IN h_int560-viewer ( 2.83 , 1.50 ) NO-ERROR.
       /* Size in UIB:  ( 6.40 , 98.50 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int560-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_int560-query ).
       RUN set-position IN h_int560-query ( 7.13 , 95.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.40 , 5.25 ) */

       /* Links to SmartViewer h_int560-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int560-query , 'Record':U , h_int560-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_int560-viewer ).

       /* Links to SmartQuery h_int560-query. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_int560-query ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int560-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int560-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int560-query ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int560-viewer ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT560" "2.12.00.AVB"}
