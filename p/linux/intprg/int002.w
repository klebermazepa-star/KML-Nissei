...
DO:
  ...
  THEN DO:
     ...
     RUN pi-atualiza-seq (INPUT tt-int-ds-docto-xml.tipo-nota,  
                          INPUT tt-int-ds-docto-xml.cod-emitente,
                          INPUT tt-int-ds-docto-xml.nNF,
                          INPUT tt-int-ds-docto-xml.serie ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-atualiza-seq wMasterDetail 
PROCEDURE pi-atualiza-seq :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
define input  parameter ptipo-nota     LIKE int-ds-docto-xml.tipo-nota   .            
define input  parameter pcod-emitente  LIKE int-ds-docto-xml.cod-emitente  .              
define input  parameter pnnfe          LIKE int-ds-docto-xml.nNF   .    
define input  parameter pserie         LIKE int-ds-docto-xml.serie  .        


DEF VAR lgTroca AS LOGICAL INIT NO.
def var i-sequencia as INTEGER INIT 10.

        FOR EACH int-ds-docto-xml NO-LOCK
           WHERE int(int-ds-docto-xml.nNF)     = int(pnnfe)
            AND  int-ds-docto-xml.serie        = pserie   
             AND int-ds-docto-xml.cod-emitente = pcod-emitente :

                 FOR EACH int-ds-it-docto-xml EXCLUSIVE-LOCK 
                    WHERE int(int-ds-it-docto-xml.nNF)     = int(int-ds-docto-xml.nNF)
                      AND int-ds-it-docto-xml.serie        = int-ds-docto-xml.serie
                      AND int-ds-it-docto-xml.cod-emitente = int-ds-docto-xml.cod-emitente:
 
                        ASSIGN int-ds-it-docto-xml.sequencia = int-ds-it-docto-xml.sequencia * (-1).
                 END.
        END.
           ...
           WHERE int(int-ds-docto-xml.nNF)     = int(pnnfe)                   
            ...
            FOR EACH int-ds-it-docto-xml EXCLUSIVE-LOCK 
               WHERE int(int-ds-it-docto-xml.nNF)     = int(int-ds-docto-xml.nNF)
                 AND int-ds-it-docto-xml.serie        = int-ds-docto-xml.serie
                 AND int-ds-it-docto-xml.cod-emitente = int-ds-docto-xml.cod-emitente
               BREAK BY int-ds-it-docto-xml.sequencia :
      
               ASSIGN int-ds-it-docto-xml.sequencia = i-sequencia.
...
------------------------------------------------------------------------------*/
   ...
   /* if c-seg-usuario = "super" */
/*    then do: */
/*     message tt-int-ds-it-docto-xml.sequencia view-as alert-box. */
/*    end. */
                   
    IF AVAIL tt-int-ds-it-docto-xml THEN do:
        
      FOR FIRST bf-tt-int-ds-it-docto-xml NO-LOCK WHERE
               bf-tt-int-ds-it-docto-xml.nnf       = tt-int-ds-it-docto-xml.nnf       AND 
               bf-tt-int-ds-it-docto-xml.serie     = tt-int-ds-it-docto-xml.serie     AND
               bf-tt-int-ds-it-docto-xml.cnpj      = tt-int-ds-it-docto-xml.cnpj      AND 
               bf-tt-int-ds-it-docto-xml.tipo-nota = tt-int-ds-it-docto-xml.tipo-nota AND
               bf-tt-int-ds-it-docto-xml.sequencia = tt-int-ds-it-docto-xml.sequencia :
