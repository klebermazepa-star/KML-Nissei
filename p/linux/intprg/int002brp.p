...
DEF VAR i-cod-emitente AS INTEGER.
...
procedure pi-grava-nota:       
    ...
    FOR EACH tt-docto-xml USE-INDEX tp-estab-emis WHERE    
        ...
        ASSIGN int-ds-docto-xml.chnfe      = IF tt-docto-xml.chnfe      = ? then "" else tt-docto-xml.chnfe  
               ...
               int-ds-docto-xml.num-pedido = IF tt-docto-xml.num-pedido = ? then 0  else 0 /*tt-docto-xml.num-pedido*/.     
        ...
        END.
        i-cod-emitente = ?.
        IF tt-docto-xml.cod-emitente = ? OR 
           tt-docto-xml.cod-emitente = 0 THEN DO:
            FOR EACH emitente NO-LOCK WHERE 
                emitente.cgc = tt-docto-xml.CNPJ,
                EACH dist-emitente NO-LOCK WHERE
                dist-emitente.cod-emitente = emitente.cod-emitente AND
                dist-emitente.idi-sit-fornec = 1 /* ATIVO */: 
                i-cod-emitente = emitente.cod-emitente.
            END.
            IF i-cod-emitente <> ? THEN 
                FOR FIRST emitente NO-LOCK WHERE
                    emitente.cod-emitente = i-cod-emitente : END.
        END.
        ELSE 
            FOR FIRST emitente NO-LOCK WHERE
                emitente.cod-emitente = tt-docto-xml.cod-emitente : END.
        ...
        FOR EACH tt-it-docto-xml WHERE
            ...
            ASSIGN int-ds-it-docto-xml.lote          = if tt-it-docto-xml.lote            = ?   then "" else  tt-it-docto-xml.lote
                   ...
                   int-ds-it-docto-xml.modbcst       = if tt-it-docto-xml.modbcst         = ?   then 0  else  tt-it-docto-xml.modbcst     
                   int-ds-it-docto-xml.numero-ordem  = if tt-it-docto-xml.numero-ordem    = ?   then 0  else  0 /*tt-it-docto-xml.numero-ordem*/.
               ...
               IF AVAIL ITEM-fornec THEN DO: 
                 ...
                 ELSE 
                    ASSIGN tt-it-docto-xml.qCom-forn     = tt-it-docto-xml.qCom-forn 
                           ...
                           tt-it-docto-xml.uCom          = tt-it-docto-xml.ucom-forn
                           int-ds-it-docto-xml.qCom-forn = tt-it-docto-xml.qCom-forn
                           int-ds-it-docto-xml.uCom-forn = tt-it-docto-xml.ucom-forn
                           l-gera-nota = NO.
                  ...
                  THEN DO:
                      ...
                      THEN DO:
                           ...
                           IF AVAIL ordem-compra THEN
                              ASSIGN int-ds-it-docto-xml.numero-ordem = 0 /*ordem-compra.numero-ordem*/. 
                           /* ELSE DO:
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK WHERE       
             /*NDD_ENTRYINTEGRATION.emissiondate       >= datetime(08/03/2016) AND 
             NDD_ENTRYINTEGRATION.emissiondate       <= datetime(08/03/2016) AND*/
             NDD_ENTRYINTEGRATION.serie              <> 10                   AND  
        ...
        /*IF NDD_ENTRYINTEGRATION.emissiondate <> DATETIME(?) 
        THEN DO:
        
            ASSIGN l-estab = NO.

            FOR FIRST b-estab NO-LOCK WHERE 
                      b-estab.cgc = trim(string(NDD_ENTRYINTEGRATION.cnpjemit)) , 
                FIRST estabelec NO-LOCK WHERE
                      estabelec.cgc = trim(string(NDD_ENTRYINTEGRATION.cnpjdest)):
       ...
       /* Abertura para a leitura do arquivo XML */
        ...
        IF AVAIL tt-docto-xml AND 
                 tt-docto-xml.tipo-estab <> 2 /* Valida se registro j† existe na base */
        THEN DO:
    ...
    DO i-qtd-niveis = 1 TO p-h-raiz:NUM-CHILDREN:
        ...
        p-h-raiz:GET-CHILD(h-niveis,i-qtd-niveis).
            ...
            THEN DO:
                ...
                IF tt-digita.valor BEGINS "Transf" THEN ASSIGN l-transf = YES.
            ...
            THEN DO:
                ...
                IF tt-docto-xml.tipo-estab = 1
                THEN DO: 
                         
                   /*DISP tt-docto-xml.tipo-nota
                        tt-docto-xml.CNPJ     
                        int(tt-docto-xml.nNF) 
                        tt-docto-xml.serie
                        WITH WIDTH 333 STREAM-IO.*/
                ...
                IF tt-docto-xml.tipo-estab = 0 
                THEN DO: 
                    ASSIGN l-transf = YES.

                    FIND FIRST b-NDD_ENTRYINTEGRATION WHERE
                               rowid(b-NDD_ENTRYINTEGRATION) = rowid(NDD_ENTRYINTEGRATION) NO-ERROR.
                    IF AVAIL b-NDD_ENTRYINTEGRATION 
                    THEN DO:
...
DEF INPUT PARAMETER p-arquivo AS CHAR.
    ...
    FOR EACH tt-digita  WHERE
        ...
        BREAK BY tt-digita.arquivo
              BY tt-digita.linha query-tuning(no-lookahead):
           ...
           CASE tt-digita.campo: 
              ...
              WHEN "cProd" THEN DO:
                 ...
                 ASSIGN tt-it-docto-xml.arquivo      = tt-docto-xml.arquivo
                        ...
                        tt-it-docto-xml.item-do-forn = trim(STRING(tt-digita.valor))
              ...
              WHEN "xPed"   THEN ASSIGN tt-it-docto-xml.num-pedido = 0 /*INT(STRING(tt-digita.valor))*/.  
       ...
       ELSE IF tt-digita.node = "compra" THEN DO:
           ...
           CASE tt-digita.campo: 
              ...
              WHEN "xPed" THEN DO:
                 ...
                 ASSIGN tt-docto-xml.num-pedido = 0 /*INT(STRING(tt-digita.valor))*/. 
                 ...
                 FOR EACH tt-it-docto-xml WHERE
                          ...
                          int(tt-it-docto-xml.nNF)   = int(tt-docto-xml.nNF) AND 
                          tt-it-docto-xml.CNPJ       = tt-docto-xml.CNPJ     AND 
                          tt-it-docto-xml.num-pedido = 0 query-tuning(no-lookahead):

                     ASSIGN tt-it-docto-xml.num-pedido = 0 /*INT(STRING(tt-digita.valor))*/.
