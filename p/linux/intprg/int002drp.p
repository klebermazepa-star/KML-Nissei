/********************************************************************************
*******************************************************************************/
{include/i-prgvrs.i int002dRP 2.06.00.001}  
{include/i_fnctrad.i}
/******************************************************************************
**
**       PROGRAMA: int002dRP
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK WHERE     
             ...
             NDD_ENTRYINTEGRATION.emissiondate        >= datetime(08/01/2016) AND
             NDD_ENTRYINTEGRATION.kind                = 0 /* Envio */    AND   
             NDD_ENTRYINTEGRATION.status_             = 0 /* campo status pendente */ /*AND
             NDD_ENTRYINTEGRATION.emissiondate        = DATETIME(08/08/2016)*/
        BY NDD_ENTRYINTEGRATION.emissiondate:  
        ...
        p-h-raiz:GET-CHILD(h-niveis,i-qtd-niveis).
            ...
            THEN DO:
                ...
                /* DISP tt-digita.valor 
                     NDD_ENTRYINTEGRATION.ENTRYINTEGRATIONID 
            ...
            IF VALID-HANDLE(h-acomp) THEN
               RUN pi-acompanhar IN h-acomp(INPUT "Dia: " + string(datetime(NDD_ENTRYINTEGRATION.emissiondate))).
...
DEF INPUT PARAMETER p-arquivo AS CHAR.
    ...
    FOR EACH tt-digita  WHERE
       ...
       THEN DO:
           FIND FIRST tt-docto-xml NO-ERROR.
           FIND FIRST param-estoq no-lock /* AVB - trava rpw...*/  NO-ERROR.
           ...
           THEN DO:   
              ...
              THEN DO:
                  ...
                  FOR EACH it-nota-fisc OF nota-fiscal NO-LOCK ,
                      ...
                      IF VALID-HANDLE(h-acomp) THEN
                          RUN pi-acompanhar IN h-acomp(INPUT "Nota: " + string(datetime(nota-fiscal.nr-nota-fis))).
                     ...
                     if not avail int-ds-nota-saida-item then NEXT.
                      ...
                      ASSIGN tt-it-docto-xml.arquivo      = tt-docto-xml.arquivo
                             ...
                             tt-it-docto-xml.item-do-forn = it-nota-fisc.it-codigo                  
                             tt-it-docto-xml.lote         = int-ds-nota-saida-item.nsp-lote-s                                                            
