...
FOR EACH tt-docto-xml WHERE
    ...
    for first int_ds_nota_entrada where 
        ...
        int_ds_nota_entrada.nen_notafiscal_n  = int(tt-docto-xml.nnf)  query-tuning(no-lookahead): 
    end.
    ...
    */
    ...
    for each tt-it-docto-xml where 
        ...
        OBS: Alteraá∆o tempor†ria atÇ a vidada da OBLAK para o PROCFIT   */
        ...
        for first int_ds_nota_entrada_produt where 
                  ...
                  int_ds_nota_entrada_produt.nep_lote_s        = tt-it-docto-xml.lote query-tuning(no-lookahead):          
        end.
    ...
    if l-valida then
    for each tt-xml-dup query-tuning(no-lookahead):
        for first int_ds_nota_entrada_dup where 
            ...
            int_ds_nota_entrada_dup.nen_data_vencto_d = date(int(substring(tt-xml-dup.dVenc,6,2)),
                                                             int(substring(tt-xml-dup.dVenc,9,2)),
                                                             int(substring(tt-xml-dup.dVenc,1,4))) query-tuning(no-lookahead): 
        end.
    ...
    then do:
       ...
       assign int_ds_nota_entrada.nen_cnpj_origem_s  =  tt-docto-xml.CNPJ          /* CNPJ fornecedor/estabelecimento origem */
    ...
    assign int_ds_nota_entrada.situacao                 =  if not l-vendavel and not l-sem-item then 2 else 1 /* 1 - Pendente 2 - Integrado */     
           int_ds_nota_entrada.nen_conferida_n          =  if not l-vendavel and not l-sem-item then 1 else 0 /* 1 - Conferida, 0 - A conferir */
           int_ds_nota_entrada.nen_datamovimentacao_d   =  if not l-vendavel and not l-sem-item then tt-docto-xml.demi else ?
