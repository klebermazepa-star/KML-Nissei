    ...
    */
    ...
    for each tt-it-docto-xml where 
       ...
       if tt-docto-xml.tipo-nota = 3 then 
        ...
        for first int-ds-nota-entrada-produto where 
                  ...
                  int-ds-nota-entrada-produto.nep-lote-s                = tt-it-docto-xml.lote query-tuning(no-lookahead):          end.
        ...
        then do:
            for first int-ds-nota-entrada-produto where 
                      ...
                      int-ds-nota-entrada-produto.nep-sequencia-n           = tt-it-docto-xml.sequencia      and 
                      int-ds-nota-entrada-produto.nep-produto-n             = int(c-it-codigo)               and
                      int-ds-nota-entrada-produto.alternativo-fornecedor    = tt-it-docto-xml.item-do-forn query-tuning(no-lookahead):  end.
            ...
            if not avail int-ds-nota-entrada-produto THEN DO:
                for first int-ds-nota-entrada-produto where 
                          ...
                          int-ds-nota-entrada-produto.nen-serie-s               = tt-docto-xml.serie             and 
                          int-ds-nota-entrada-produto.nen-notafiscal-n          = int(tt-it-docto-xml.nNF)       and 
                          int-ds-nota-entrada-produto.nep-sequencia-n           = tt-it-docto-xml.sequencia query-tuning(no-lookahead):     end.
                
                if not avail int-ds-nota-entrada-produto THEN DO:
                    DISPLAY tt-it-docto-xml.CNPJ        
                            tt-docto-xml.serie          
                            int(tt-it-docto-xml.nNF)    
                            tt-it-docto-xml.sequencia   
                            tt-it-docto-xml.item-do-forn
                            tt-it-docto-xml.lote
                            de-vl-liquido
                            "NAO ENCONTRADO"
            ...
            assign int-ds-nota-entrada-produto.nen-cnpj-origem-s       = tt-it-docto-xml.CNPJ          
                   ...
                   int-ds-nota-entrada-produto.ped-codigo-n            = tt-it-docto-xml.num-pedido.
                   /* Acertar notas j  processadas !!!!
                   int-ds-nota-entrada-produto.nep-redutorbaseicms-n  = tt-it-docto-xml.vbcstret
                   */

            assign int-ds-nota-entrada-produto.nep-percentualicms-n   = tt-it-docto-xml.picmsst        
        ...
        DISPLAY int-ds-nota-entrada-produto.nep-valorliquido-n.

        if tt-it-docto-xml.cst-icms = 40 then
           assign de-tot-base-icms-isento = de-tot-base-icms-isento + tt-it-docto-xml.vbc-icms
                  int-ds-nota-entrada-produto.nep-baseisenta-n = int-ds-nota-entrada-produto.nep-baseisenta-n
                                                               + tt-it-docto-xml.vbc-icms.

        if tt-it-docto-xml.cst-icms = 51 then 
           assign de-tot-base-icms-diferido = de-tot-base-icms-diferido + tt-it-docto-xml.vbc-icms
                  int-ds-nota-entrada-produto.nep-basediferido-n = int-ds-nota-entrada-produto.nep-basediferido-n
                                                                 + tt-it-docto-xml.vbc-icms.
    ...
    then do:
       ...
       assign int-ds-nota-entrada.nen-cnpj-origem-s  =  tt-docto-xml.CNPJ          /* CNPJ fornecedor/estabelecimento origem */
              ...
              int-ds-nota-entrada.tipo-movto         =  1.                         /* 1 - InclusÆo 2 - Altera‡Æo 3 - ExclusÆo */ 
    ...
    assign int-ds-nota-entrada.situacao                 =  1                            /* 1 - Pendente 2 - Integrado */     
    ...
    ASSIGN 
           ...
           int-ds-nota-entrada.nen-valortotalprodutos-n =  tt-docto-xml.valor-mercad  /*  Valor total da mercadoria */.
