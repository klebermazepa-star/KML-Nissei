...
IF tt-param.arquivo = "" THEN do:
    ...
    &IF defined(faixa-ini) &THEN
          tt-param.arquivo = "INT010-" + {&faixa-ini} + "-" + {&faixa-fim} + ".txt".
    &ENDIF
end.
...
procedure importa:
    ...
    for first int-ds-ger-cupom no-lock
        query-tuning(no-lookahead):
        for-ped:
        for each bestabelec use-index codigo no-lock where 
            &IF defined(faixa-ini) &THEN
                 bestabelec.cod-estabel >= {&faixa-ini} and
                 bestabelec.cod-estabel <= {&faixa-fim}
            &else
                 true        
            &ENDIF
      ...
      if not l-pedido-com-erro then do:
         assign i-erro       = 1
                c-informacao = c-nr-pedcli
                             ...
                             + c-nome-abrev.
         do transaction:
              find current int-ds-pedido NO-ERROR.
