...
def var c-informacao         as char format "x(300)".
...
/*          WHERE bestabelec.estado = "SP"   */
/*          WHERE                             */
/*          bestabelec.cod-estabel = "042" or */
/*          bestabelec.cod-estabel = "068" or */
/*          bestabelec.cod-estabel = "070" or */
/*          bestabelec.cod-estabel = "071" or */
/*          bestabelec.cod-estabel = "072" or */
/*          bestabelec.cod-estabel = "077" or */
/*          bestabelec.cod-estabel = "079" or */
/*          bestabelec.cod-estabel = "080" or */
/*          bestabelec.cod-estabel = "081" or */
/*          bestabelec.cod-estabel = "082" or */
/*          bestabelec.cod-estabel = "083" or */
/*          bestabelec.cod-estabel = "084" or */
/*          bestabelec.cod-estabel = "085" or */
/*          bestabelec.cod-estabel = "086" or */
/*          bestabelec.cod-estabel = "097" or */
/*          bestabelec.cod-estabel = "101" or */
/*          bestabelec.cod-estabel = "144" or */
/*          bestabelec.cod-estabel = "149" or */
/*          bestabelec.cod-estabel = "152" or */
/*          bestabelec.cod-estabel = "179" or */
/*          bestabelec.cod-estabel = "198" or */
/*          bestabelec.cod-estabel = "200" or */
/*          bestabelec.cod-estabel = "208" or */
/*          bestabelec.cod-estabel = "217" or */
/*          bestabelec.cod-estabel = "221" or */
/*          bestabelec.cod-estabel = "236" or */
/*          bestabelec.cod-estabel = "237" or */
/*          bestabelec.cod-estabel = "245" or */
/*          bestabelec.cod-estabel = "248" or */
/*          bestabelec.cod-estabel = "263" or */
/*          bestabelec.cod-estabel = "286"    */
        ...
        ,each int-ds-pedido no-lock where 
            ...
            by int-ds-pedido.dt-geracao 
                by int-ds-pedido.hr-geracao
            on error undo, next:

            IF int-ds-pedido.ped-tipopedido-n <> 1 AND
               int-ds-pedido.ped-tipopedido-n <> 8 THEN NEXT.
            ...
            for each int-ds-pedido-produto no-lock of int-ds-pedido,
                ...
                c-aux = if string(int-ds-pedido-retorno.rpp-caixa-n) <> ? then string(int-ds-pedido-retorno.rpp-caixa-n) else "".
                assign  c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                        ...
                        c-numero-caixa      = (if c-numero-caixa = ""
                                               then string(c-aux)
                                               else c-numero-caixa + "," + string(c-aux))
            ...
            if not l-pedido-com-erro and not avail tt-ped-item-imp then do:
                assign i-erro = 37
                       c-informacao = "Pedido: " + c-nr-pedcli + "/" + "Nome Abrev.: " + c-nome-abrev + " - " + "CNPJ Origem: " + string(int-ds-pedido.ped-cnpj-origem)
                                    + " - " + "CNPJ Destino: " + string(int-ds-pedido.ped-cnpj-destino)
      ...
      if not l-pedido-com-erro then do:
         assign i-erro       = 1
                c-informacao = "Pedido: " + c-nr-pedcli
                               + " Nome Abrev.: " + c-nome-abrev.
         do transaction:
            find current int-ds-pedido exclusive-lock no-error.
...
procedure valida-registro-pedido.
    ...
    if c-nome-repres = "" then do:
        assign i-erro = 13
               c-informacao = "Pedido: " + c-nr-pedcli + "/" + "Nome Abrev.: " + c-nome-abrev + " - " + "Repres.:  " + string(i-cod-rep)
               l-pedido-com-erro = yes.
        run gera-log. 
        return.
    end.
...
procedure cria-tt-ped-venda-imp.
   ...
   find first b-loc-entr where 
              b-loc-entr.nome-abrev = emitente.nome-abrev NO-LOCK NO-ERROR.
   if avail b-loc-entr then do:
      ...
      FIND FIRST int-ds-rota-veic WHERE
                 int-ds-rota-veic.cod-rota = b-loc-entr.cod-rota NO-LOCK NO-ERROR.
      IF AVAIL int-ds-rota-veic THEN
         ASSIGN tt-ped-venda-imp.placa    = int-ds-rota-veic.placa
                tt-ped-venda-imp.uf-placa = int-ds-rota-veic.uf-placa.          
   END.
end.
      ...
      for each tt-ped-venda-imp no-lock:
         ...
         then do:
            ...
            for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                assign i-erro = 22
                       c-informacao = "Pedido: " + tt-ped-venda-imp.nr-pedcli +   
                                      " Emitente: " + string(tt-ped-venda-imp.cod-emitente) + " - " + "Natur. Oper.: " + tt-ped-venda-imp.nat-operacao + " - " +
                                      "Nr. Erro: " + string(RowErrors.ErrorNumber) + " - " + RowErrors.ErrorDescription.
                run gera-log.
                assign l-pedido-com-erro = yes.
            end.
            ...
            */
            ...
            run pi-acompanhar in h-acomp (input "Gerando Tabelas - Pedido").
            ...
            if not can-find(first RowErrors where RowErrors.ErrorSubType = "Error":u) then do:
               ...
               for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                   assign i-erro = 22
                          c-informacao = "di159 Create " + "Pedido: " + tt-ped-venda-imp.nr-pedcli + "/" +
                                         "Nome Abrev.: " + string(tt-ped-venda-imp.nome-abrev) + "/" + "Natur. Oper.: " + tt-ped-venda-imp.nat-operacao + " - " +  
                                         "Nr. Erro.: " + string(RowErrors.ErrorNumber) + " - " + RowErrors.ErrorDescription.
                   run gera-log.
                   assign l-pedido-com-erro = yes.
               end.
               ...
               for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                   assign i-erro = 22
                          c-informacao = "di159 Bring " + "Pedido: " + tt-ped-venda-imp.nr-pedcli + "/" +  
                                         "Nome Abrev.: " + string(tt-ped-venda-imp.nome-abrev) + "/" +  
                                         "Nr. Erro: " + string(RowErrors.ErrorNumber) + " - " + RowErrors.ErrorDescription.
                   run gera-log.
                   assign l-pedido-com-erro = yes.
               end.
               ...
               if not can-find(first RowErrors where RowErrors.ErrorSubType = "Error":u) then do:
                  ...
                  for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                     assign i-erro = 22
                            c-informacao = "di159 Get " + "Pedido: " +  tt-ped-venda-imp.nr-pedcli + "/" +  
                                           "Nome Abrev.: " + string(tt-ped-venda-imp.nome-abrev) + "/" +  
                                           "Nr. Erro: " + string(RowErrors.ErrorNumber) + " - " + RowErrors.ErrorDescription.
                     run gera-log.
                     assign l-pedido-com-erro = yes.
                  ...
                  for each tt-ped-item-imp no-lock of tt-ped-venda-imp:
                     ...
                     run pi-acompanhar in h-acomp (input "Gerando Tabelas - Pedido - Itens: " + tt-ped-item-imp.it-codigo).
                     ...
                     do:
                        ...
                        for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                            assign i-erro = 22
                                   c-informacao = "di154 Create Ped " + "Pedido: " + tt-ped-venda-imp.nr-pedcli +   
                                                  " Nome Abrev.: " + string(tt-ped-venda-imp.nome-abrev) + " Item: " +  
                                                  tt-ped-item.it-codigo + "/" + " Nr. Erro.: " +   
               ...
               run pi-acompanhar in h-acomp (input "Gerando Tabelas - Pedido - Repres").
               ...
               for each tt-ped-repre-imp
                  ...
                  for each RowErrors where RowErrors.ErrorSubType = "Error":u:
                     assign i-erro = 22
                            c-informacao = "di157 Create " + "Pedido: " + tt-ped-venda-imp.nr-pedcli + "/" +  
                                           "Nome Abrev.: " + string(tt-ped-venda-imp.nome-abrev) + "/" +  
                                           "Nome Repres.: " + tt-ped-repre-imp.nome-ab-rep + " - " +  
                                           "Nr. Erro: " + string(RowErrors.ErrorNumber) + " - " +
                                           RowErrors.ErrorDescription.
                     run gera-log.
                     assign l-pedido-com-erro = yes.
                  end.
               ...
               run pi-acompanhar in h-acomp (input "Gerando Tabelas - Pedido - CST").
               /* completa o pedido */
               ON WRITE OF ped-venda OVERRIDE
               DO:
                   RETURN.
               END.
               
               for first ped-venda EXCLUSIVE-LOCK where
                   ped-venda.nome-abrev = tt-ped-venda-imp.nome-abrev and
                   ped-venda.nr-pedcli  = tt-ped-venda-imp.nr-pedcli: 
