...
{include/i-prgvrs.i int011rp-l 2.12.01.AVB}
{cdp/cdcfgdis.i}
...
for each nota-fiscal no-lock use-index nfftrm-20 where 
    ...
    nota-fiscal.cod-estabel  >=  "001" AND
    nota-fiscal.cod-estabel  <=  "900"
    on stop undo, leave
    ...
    &else                                          
      for first sit-nf-eletro no-lock where 
          ...
          query-tuning(no-lookahead): end.
      if sit-nf-eletro.idi-sit-nf-eletro <> 3 and
         nota-fiscal.dt-cancel = ? then next.
    &endif
    ...
    for each cst_estabelec no-lock of estabelec where
        cst_estabelec.dt-fim-operacao >= nota-fiscal.dt-emis-nota
        query-tuning(no-lookahead):
        if  cst_estabelec.cod-estabel <> "973" and 
           (cst_estabelec.dt-inicio-oper = ? or
            cst_estabelec.dt-inicio-oper > nota-fiscal.dt-emis-nota) then c-origem = "OBLAK".
...
procedure pi-trata-entradas:
    ...
    if natur-oper.tipo = 1 /* entradas */ then do:
        ...
        /* emissoes de entrada para o CD */
        else if nota-fiscal.cod-estabel = "973" /* Notas de entrada do CD */ then do:
            &if "{&bf_dis_versao_ems}" >= "2.07" &then
            ...
            for first docum-est no-lock where
                ...
                query-tuning(no-lookahead):
                if index(docum-est.obs ,"Pedido Balan") > 0 
                then do:
                    /*
