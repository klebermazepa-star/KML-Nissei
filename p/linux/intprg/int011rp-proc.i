...
procedure pi-saidas:
    ...
    */
    ...
    then do transaction:
        ...
        assign  int_ds_nota_saida.nsa_cnpj_origem_s  = estabelec.cgc
    ...
    if  (nota-fiscal.nat-operacao begins "5" or
         ...
         int_ds_nota_saida.nsa_cnpj_destino_s <> int_ds_nota_saida.nsa_cnpj_origem_s then do: /* Balanáo os CNPJ s∆o iguais -> tratado em outro ponto */
         for FIRST b-estabelec fields (cgc cod-estabel cod-emitente) no-lock where 
            ...
            for each int_ds_nota_saida_item no-lock of int_ds_nota_saida
                ...
                if not avail int_ds_nota_entrada_produt then do:
                    ...
                    for first item fields (codigo-orig class-fiscal fm-codigo) no-lock where 
                        item.it-codigo = trim(string(int_ds_nota_saida_item.nsp_produto_n))
                        query-tuning(no-lookahead): end.
...
procedure pi-entradas-balanco:
    ...
    /* inclusío da nota */
    if not avail int_ds_nota_entrada then do transaction:
        ...
        create  int_ds_nota_entrada.
        assign  int_ds_nota_entrada.nen_cnpj_origem_s   = estabelec.cgc
        ...
        */
        ...
        for first emitente no-lock where 
            ...
            for first int_ds_docto_wms where
                int_ds_docto_wms.doc_numero_n   = int_ds_nota_entrada.nen_notafiscal_n and
            ...
            if not avail int_ds_docto_wms then do:
                ...
                assign int_ds_docto_wms.doc_numero_n    = int_ds_nota_entrada.nen_notafiscal_n
                       int_ds_docto_wms.doc_serie_s     = int_ds_nota_entrada.nen_serie_s
                       int_ds_docto_wms.doc_origem_n    = emitente.cod-emitente
                       int_ds_docto_wms.situacao        = 10 /* Inclus∆o */
...
procedure pi-entrada-cd:
    ...
    */
    ...
    assign  int_ds_docto_xml.CNPJ         = estabelec.cgc
    ...
    for first emitente no-lock where emitente.cod-emitente = estabelec.cod-emitente
        query-tuning(no-lookahead):
        CREATE int_ds_docto_wms.
        ASSIGN int_ds_docto_wms.doc_numero_n    = INT(nota-fiscal.nr-nota-fis)
        ...
        if int_ds_nota_saida.tipo_movto = 1 /* inclusao */ then do:
            ...
            if  int_ds_nota_saida.nsa_cnpj_destino_s <> int_ds_nota_saida.nsa_cnpj_origem_s and /* nao eh balanco */
                ...
                for first b-estabelec fields (cgc cod-estabel cod-emitente) no-lock where 
                    ...
                    create int_ds_docto_wms.
                    assign int_ds_docto_wms.doc_numero_n    = int_ds_nota_saida.nsa_notafiscal_n
                    ...
                    for first emitente no-lock where emitente.cgc = int_ds_nota_saida.nsa_cnpj_origem_s
                        query-tuning(no-lookahead): 
    ...
    for first estabelec no-lock where 
        ...
        /* Destino Loja */
            ...
            for each int_ds_nota_entrada where 
                ...
                int_ds_nota_entrada.nen_notafiscal_n   = int(nota-fiscal.nr-nota-fis)
                query-tuning(no-lookahead):
                assign int_ds_nota_entrada.tipo_movto        = 3 /* exclusío */
                       int_ds_nota_entrada.dt_geracao        = today
                       int_ds_nota_entrada.hr_geracao        = string(time,"HH:MM:SS")
                       int_ds_nota_entrada.situacao          = p-sit-oblak
                       int_ds_nota_entrada.ENVIO_STATUS      = p-sit-procfit.
