         ...
         for FIRST b-estabelec fields (cgc cod-estabel cod-emitente) no-lock where 
            ...
            if not avail int-ds-nota-entrada then do:
                ...
                for first ndd_entryintegration where
                    NDD_ENTRYINTEGRATION.DOCUMENTNUMBER = int-ds-nota-entrada.nen-notafiscal-n  and
                    NDD_ENTRYINTEGRATION.SERIE          = int(int-ds-nota-entrada.nen-serie-s)  and
                    NDD_ENTRYINTEGRATION.CNPJEMIT       = int(int-ds-nota-entrada.nen-cnpj-origem-s) and
                    NDD_ENTRYINTEGRATION.CNPJDEST       = int(int-ds-nota-entrada.nen-cnpj-origem-s)
                    query-tuning(no-lookahead):
                    assign status_ = 1.
                end.
                if tt-param.arquivo <> "" then DO:
