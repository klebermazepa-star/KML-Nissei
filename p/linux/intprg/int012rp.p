/********************************************************************************
** Programa: int012 - Importaá∆o de Notas de Entrada do Tutorial/PRS
**
** Versao : 12 - 09/02/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de vers∆o */
{include/i-prgvrs.i int012rp 2.12.06.AVB}
{cdp/cdcfgdis.i}
{utp/ut-glob.i}
...
for each int_ds_nota_entrada no-lock where
  ...
  /*int_ds_nota_entrada.nen_datamovimentacao_d <> ? and 
    ...
    /* retorno pela Procfit */
    for first int_dp_nota_entrada_ret NO-LOCK where
    ...
    if avail int_dp_nota_entrada_ret then next.

    /* assinalando notas duplicadas em int_ds_docto_xml como processadas */
    for each int_ds_docto_xml exclusive where 
        int_ds_docto_xml.serie    = int_ds_nota_entrada.nen_serie_s and
        int(int_ds_docto_xml.nNF) = int_ds_nota_entrada.nen_notafiscal_n and
        int_ds_docto_xml.CNPJ     = int_ds_nota_entrada.nen_cnpj_origem_s:
        assign int_ds_docto_xml.situacao = 9.
    end.
    ...
    assign d-data-movimento = int_ds_nota_entrada.nen_datamovimentacao_d.
    if d-data-movimento < 08/01/2016 then next.
        /*assign d-data-movimento = 11/01/2016.*/

    /* evitando importaá∆o da nota antes do termino da gravacao dos itens pelo PRS */
    if d-data-movimento = today and 
       substring(int_ds_nota_entrada.nen_horamovimentacao_s,1,2) = substring(string(time,"HH:MM:SS"),1,2) then next.

    assign de-tot-despesas = 0
           c-nat-principal = "".
    /*
    for first int-ds-retorno-nota-entrada fields (RNE-CODIGO-N) no-lock where
        int-ds-retorno-nota-entrada.RNE-CNPJ-ORIGEM-S = int_ds_nota_entrada.nen_cnpj_origem_s and
        int-ds-retorno-nota-entrada.RNE-NOTAFISCAL-N  = int_ds_nota_entrada.nen_notafiscal_n  and
        int-ds-retorno-nota-entrada.RNE-SERIE-S       = int_ds_nota_entrada.nen_serie_s:      end.
    if  avail int-ds-retorno-nota-entrada and
        can-find (first int-ds-retorno-nota-entrada-prod no-lock where
        ...
        run pi-gera-log (input "H† itens n∆o localizados na nota: " + string(int_ds_nota_entrada.nen_notafiscal_n) + " Liberada alteraá∆o manual.",
                         input 1).
        for first bint_ds_nota_entrada exclusive where rowid(bint_ds_nota_entrada) = rowid(int_ds_nota_entrada):
            assign bint_ds_nota_entrada.situacao = 5 /* atualizaá∆o manual */.
            next for-nota.
    ...
    */
    ...
    if d-data-procfit <> ? and d-data-procfit < d-data-movimento then do:
        run pi-gera-log (input "Estabelecimento Ç procfit. Processamento deve ser pelo int112: " 
                         + "Est: "     + c-cod-estabel,
                         input 1).
        next.
    end.
    ...
    if not can-find (first usuar_financ_estab_apb no-lock where 
        ...
        run pi-gera-log (input "Usu†rio Financeiro n∆o cadastrado para estabelecimento " + c-cod-estabel
                         ...
                         + " Forn: "     + string(emitente.cod-emitente)
                         + " Data: "     + string(int_ds_nota_entrada.nen_datamovimentacao_d,"99/99/9999"),
                         input 1).
        empty temp-table tt-movto.
        next .
    end.
    ...
    for each int_ds_nota_entrada_produt no-lock where
           ...
           trim(string(int_ds_nota_entrada_produt.nep_produto_n)) <> "0"   then do:
            for first item 
                fields (it-codigo class-fisc tipo-con-est peso-liquido fm-codigo tipo-contr ct-codigo sc-codigo
                        char-2)
                no-lock where 
                item.it-codigo = string(int_ds_nota_entrada_produt.nep_produto_n): end.
            if not avail item then do:
                run pi-gera-log (input "Item n∆o cadastrado: " + string(int_ds_nota_entrada_produt.nep_produto_n),
                                 input 1).
                DELETE tt-movto.    /* evitar erro 40 ou erro 14675 */         
                next.
            end.
        end.
        else do:
        ...
        /*
        if int_ds_nota_entrada_produt.nep-valorbruto-n < int_ds_nota_entrada_produt.nep-valordesconto-n then do:
            run pi-gera-log (input "Valor desconto maior que o valor bruto: " + string(int_ds_nota_entrada_produt.nep_produto_n) +
                                   " Vl Bruto: " + string(int_ds_nota_entrada_produt.nep-valorbruto-n,"->>>,>>>,>>9.99") +
                                   " Desconto: " + string(int_ds_nota_entrada_produt.nep-valordesconto-n,"->>>,>>>,>>9.99"),
                             input 1).
            next.
        end.
        */
        ...
        if i-cfop < 5000 then do:
            ...
            for each natur-oper fields (nat-operacao cod-cfop)
                ...
                for EACH int-ds-cfop-natur-oper fields (nat-operacao nen_cfop_n) no-lock where
                    int-ds-cfop-natur-oper.nat-operacao = natur-oper.nat-operacao and
                    int-ds-cfop-natur-oper.nep-cstb-icm-n = int_ds_nota_entrada_produt.nep-cstb-icm-n and
                    int-ds-cfop-natur-oper.nep-cstb-ipi-n = int(item.fm-codigo):
                    assign i-cfop = int-ds-cfop-natur-oper.nen_cfop_n.
                    leave for-nat.
                end.
                for EACH int-ds-cfop-natur-oper fields (nat-operacao nen_cfop_n) no-lock where
                    int-ds-cfop-natur-oper.nat-operacao = natur-oper.nat-operacao and
                    int-ds-cfop-natur-oper.nep-cstb-icm-n = int_ds_nota_entrada_produt.nep-cstb-icm-n:
                    assign i-cfop = int-ds-cfop-natur-oper.nen_cfop_n.
                    leave for-nat.
                end.
        ...
        if c-nat-operacao = "" then do:
            run pi-gera-log (input "N∆o encontrada natureza de operaá∆o para entrada. " + 
                                    ...
                                    /*" CSTB IPI:" + string(int_ds_nota_entrada_produt.nep-cstb-ipi-n)*/
            ...
            /*
            IF i-cfop < 5000 THEN
            for each ndd_entryintegration where 
                ...
                NDD_ENTRYINTEGRATION.CNPJEMIT = decimal(int_ds_nota_entrada.nen_cnpj_origem_s) AND
                kind = 0 AND
                STATUS_ = 1
                query-tuning(no-lookahead):  
                ASSIGN STATUS_ = 5.
            END.
            IF i-cfop < 5000 THEN
            for each ndd_enryintegration_new_leg where 
                ndd_enryintegration_new_leg.CNPJEMIT        = decimal(int_ds_nota_entrada.nen_cnpj_origem_s) AND
                ndd_enryintegration_new_leg.DOCUMENTNUMBER  = int_ds_nota_entrada.nen_notafiscal_n AND
                ndd_enryintegration_new_leg.SERIE           = int(int_ds_nota_entrada.nen_serie_s) AND
                kind = 0 AND
                STATUS_ = 1
                query-tuning(no-lookahead):  
                ASSIGN STATUS_ = 5.
    ...
    if can-find(first tt-movto) then do:
        ...
        /*
        tt-movto.tot-desconto 
        tt-movto.tot-frete    
        tt-movto.tot-seguro   
        tt-movto.tot-despesas 
        */
        ...
        if de-total-descto <> int_ds_nota_entrada.nen_desconto_n then do:
            ...
            /* verificar se desconto se est† com erro de multiplicaá∆o pela quantidade */
            de-total-bruto = 0.
            for each int_ds_nota_entrada_produt no-lock of int_ds_nota_entrada:
                de-total-bruto = de-total-bruto + (int_ds_nota_entrada_produt.nep_valordesconto_n / int_ds_nota_entrada_produt.nep_quantidade).
            end.
            /* acertar desconto se estiver com erro de multiplicaá∆o pela quantidade */
            if de-total-bruto = int_ds_nota_entrada.nen_desconto_n then do:
                for each int_ds_nota_entrada_produt of int_ds_nota_entrada:
                    assign int_ds_nota_entrada_produt.nep_valordesconto_n = (int_ds_nota_entrada_produt.nep_valordesconto_n / int_ds_nota_entrada_produt.nep_quantidade).
                end.
                /* ignorar esta integraá∆o para que possam ser recalculados pis/cofins na pr¢xima execuá∆o pois o desconto faz parte da base de c†lculo */
                next for-nota.
            end.
        ...
        if de-tot-despesas <> tt-movto.tot-despesas then do:
            run pi-gera-log (input "Total das despeas n∆o confere. Docto: " + string(int_ds_nota_entrada.nen_notafiscal_n) + " SÇrie: " + string(int_ds_nota_entrada.nen_serie_s) + " Estab.: " + c-cod-estabel +
...
for each int_ds_nota_entrada no-lock where
    ...
    int_ds_nota_entrada.nen_notafiscal_n <= nro-docto-fim and
    int_ds_nota_entrada.nen_datamovimentacao_d = ? and 
    int_ds_nota_entrada.nen_datamovimentacao_d >= 08/01/2016:

    /* pular notas a serem integradas manualmente pelo int520 */
    if int_ds_nota_entrada.situacao = 5 then next.
        ...
        if last-of (tt-movto.nr-nota-fis) then do:
            ...
            if avail tt-erro then do:
               ...
               for each tt-erro:
                   ...
                   END.
                   IF tt-erro.cd-erro = 15124 THEN DO:
                      ASSIGN i-sit-reg = 1.
                      for each bint_ds_nota_entrada where
                               bint_ds_nota_entrada.nen_cnpj_origem_s = tt-movto.nen_cnpj_origem_s and
                               bint_ds_nota_entrada.nen_serie_s       = tt-movto.serie and
            ...
            for first docum-est no-lock of tt-docum-est-nova:
                ...
                */
                ...
                assign 
                    ...
                    tt-param-re1005.arquivo            = "int012-re1005.txt"
...
procedure pi-gera-int_ds_doc:
    ...
    for each item-doc-est no-lock of docum-est:
        for first btt-movto no-lock where
            ...
            assign  int_ds_doc.nen_pedido_n = /*item-doc-est.num-pedido*/ btt-movto.num-pedido.
        end.
