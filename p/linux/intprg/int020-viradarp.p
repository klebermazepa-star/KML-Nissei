/********************************************************************************
** Programa: INT020-viradaRP - Geracao notas fiscais a partir de Cupom Fiscal PRS
**
** Versao : 12 - 20/03/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT020-viradaRP 2.12.01.AVB}
{cdp/cdcfgdis.i}
...
def temp-table tt-int-ds-nota-loja-virada like int-ds-nota-loja
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "INT020-virada.txt"
           tt-param.destino = 3
...
procedure pi-importa:
    ...
    EMPTY TEMP-TABLE tt-int-ds-nota-loja-virada.

    FOR EACH estabelec USE-INDEX codigo 
        WHERE estabelec.cod-estabel = "169" 
           OR estabelec.cod-estabel = "186" 
           OR estabelec.cod-estabel = "328" 
           OR estabelec.cod-estabel = "321" 
            NO-LOCK:
        ...
        for each int-ds-nota-loja no-lock USE-INDEX sit_cnpj_emiss where 
            ...
            run pi-acompanhar in h-acomp (input "Carregando informa‡äes: " + estabelec.cod-estabel + " - " + string(int-ds-nota-loja.emissao,"99/99/9999")).

            FOR FIRST tt-int-ds-nota-loja-virada WHERE 
                tt-int-ds-nota-loja-virada.rowid-nota-loja = ROWID(int-ds-nota-loja): END.
            IF NOT AVAIL tt-int-ds-nota-loja-virada THEN DO:
                CREATE tt-int-ds-nota-loja-virada.
                BUFFER-COPY int-ds-nota-loja TO tt-int-ds-nota-loja-virada.
                ASSIGN tt-int-ds-nota-loja-virada.rowid-nota-loja = ROWID(int-ds-nota-loja).
            END.

        end.
    END.

    FOR EACH tt-int-ds-nota-loja-virada USE-INDEX data:
        FOR FIRST int-ds-nota-loja WHERE ROWID(int-ds-nota-loja) = tt-int-ds-nota-loja-virada.rowid-nota-loja NO-LOCK:
        END.
