...
procedure pi-processa:
            ...
            for each nota-fiscal NO-LOCK where
                ...
                if nota-fiscal.idi-sit-nf-eletro <> 12 then do transaction:
                    FIND FIRST b-nota-fiscal EXCLUSIVE-LOCK
                        WHERE ROWID(b-nota-fiscal) = ROWID(nota-fiscal) NO-ERROR.
                    IF AVAIL b-nota-fiscal THEN
                        assign b-nota-fiscal.idi-sit-nf-eletro = 12.
                    RELEASE b-nota-fiscal.
                end.
                run pi-trata-ret-nfe (
                ...
                then do:
                    if not l-erro then do transaction:
                         ...
                         IF AVAIL b-nota-fiscal THEN DO:
                             ...
                             if nota-fiscal.idi-sit-nf-eletro <> 6 then
                                assign b-nota-fiscal.idi-sit-nf-eletro = 6.
            ...
            for each nota-fiscal NO-LOCK where 
                ...
                if nota-fiscal.idi-sit-nf-eletro <> 13 then do transaction:
                    FIND FIRST b-nota-fiscal EXCLUSIVE-LOCK
                        WHERE ROWID(b-nota-fiscal) = ROWID(nota-fiscal) NO-ERROR.
                    IF AVAIL b-nota-fiscal THEN
                        assign b-nota-fiscal.idi-sit-nf-eletro = 13.
                    RELEASE b-nota-fiscal.
                end.
    ...
    for each tt-retorno-substituicao
        ...
        if not l-ok then do:
            display tt-retorno-substituicao.chaveAtual format "X(44)" " -> Nota Fiscal nao encontrada!".
            run intprg/int999.p ("RETNFNDD", 
                                 tt-retorno-substituicao.chaveAtual,
                                 "Nota Fiscal " + tt-retorno-substituicao.chaveAtual + " n∆o encontrada!",
                                 1, /* 1 - Pendente, 2 - Processado */ 
                                 c-seg-usuario,
                                 "int022rp.p").
        end.
...
procedure pi-trata-ret-nfe:
    ...
    */
    ...
    if not nota-fiscal.obs-gerada matches "*ID NDD: " + string(pIdMovto) + "*" and
       ...
       length(trim(nota-fiscal.obs-gerada + " ID NDD: " + string(pIdMovto))) < 2000 THEN DO:
        FIND FIRST b-nota-fiscal EXCLUSIVE-LOCK
            WHERE ROWID(b-nota-fiscal) = ROWID(nota-fiscal) NO-ERROR.
        IF AVAIL b-nota-fiscal THEN        
