...
*******************************************************************************/
{include/i-prgvrs.i INT030B 2.09.00.001}
...
DEFINE VARIABLE h_int030b-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int030b-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int030b-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 90.14 BY 17.83 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Relacionamento Usu rio x Mensagens Integra‡Æo"
         HEIGHT             = 17.63
         WIDTH              = 90.29
         MAX-HEIGHT         = 30
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manuten‡Æo Relacionamento Usu rio x Mensagens Integra‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manuten‡Æo Relacionamento Usu rio x Mensagens Integra‡Æo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int030b-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int030b-v01 ).
       RUN set-position IN h_int030b-v01 ( 2.63 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 5.25 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Usuario x Msg' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.00 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int030b-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int030b_b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int030b-b01 ).
       RUN set-position IN h_int030b-b01 ( 9.29 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.04 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int030b-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = ,
                     ProgVaPara = ,
                     ProgIncMod = intprg/int030b_a.w,
                     Implantar = no':U ,
             OUTPUT h_int030b-q01 ).
       RUN set-position IN h_int030b-q01 ( 1.00 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 7.00 ) */

       /* Links to SmartViewer h_int030b-v01. */
       RUN add-link IN adm-broker-hdl ( h_int030b-q01 , 'Record':U , h_int030b-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_int030b-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_int030b-b01. */
       RUN add-link IN adm-broker-hdl ( h_int030b-q01 , 'Record':U , h_int030b-b01 ).

       /* Links to SmartQuery h_int030b-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_int030b-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int030b-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int030b-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int030b-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int030b-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_int030b-v01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int030b-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT030B" "2.09.00.001"}
