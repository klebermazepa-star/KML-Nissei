...
*******************************************************************************/
{include/i-prgvrs.i INT035-VaPara 1.00.00.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_ped-codigo-n bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_ped-codigo-n 
...
DEFINE VARIABLE i_ped-codigo-n AS INTEGER FORMAT ">>>>>>>>9":U 
     LABEL "Numero Pedido" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_ped-codigo-n AT ROW 1 COL 30 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.13
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     rt-button AT ROW 4.27 COL 1
     SPACE(0.13) SKIP(0.06)
...
ON GO OF FRAME d-vapara /* V  Para Pedido */
DO:
FIND int-ds-pedido NO-LOCK
    WHERE int-ds-pedido.ped-codigo-n EQ INPUT FRAME {&FRAME-NAME} i_ped-codigo-n NO-ERROR.

IF NOT AVAILABLE int-ds-pedido THEN DO:
    {utp/ut-table.i emsesp int-ds-pedido 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-ds-pedido).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Pedido */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_ped-codigo-n 
      WITH FRAME d-vapara.
  ENABLE rt-button i_ped-codigo-n bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT035-VaPara" "1.01.01.AVB"}
