...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i int039RP MFP}
&ENDIF
...
**  Autor...: ResultPro 
**
**  Objetivo: Lista Pedidos Balanáo 
**
******************************************************************************/
...
DEF TEMP-TABLE tt-pedido NO-UNDO LIKE int-ds-pedido
...
FIELD qtd-prs      LIKE int-ds-pedido-retorno.rpp-quantidade-n
FIELD qtd-inv      LIKE int-ds-pedido-retorno.rpp-qtd-inventario-n.
...
def var tb-erro              as char format "x(50)" extent 44 init
                             ["01. Faturado com Sucesso                          ",
                              ...
                              "34. Natureza de Operaá∆o deve ser Entrada         ",
                              ...
                              "39. Documento Pendente ou nío cadastrado no int002",
                              "40. Natureza de Operaá∆o deve ser de entrada NFE",
                              "41. Nota Deve ser autorizada pelo SEFAZ",
                              "42. Classificaáao Fiscal Do item n∆o cadastrada",
                              "43. Natureza de Operaá∆o deve estar marcada para gerar faturamento",
                              "44. Item Parametrizado como dÇbito direto."].
...
form   
    ...
    tt-param.rs-tipo        label "Modo Execuá∆o"     colon 34 skip
    skip(1)
...
run pi-inicializar in h-acomp (input "Int039 - Lista Pedidos Balanáo").
...
{utp/ut-liter.i Pedidos_Balanáo  * r}
...
DO i-cont = 1 TO 50:
    CASE i-cont:
        WHEN 15  THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "DEVOLUCAO FILIAL - FORNECEDOR").
        WHEN 16 THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "DEVOLUCAO DEPOSITO - FORNECEDOR").
        WHEN 49 THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "TRANSFERENCIA FILIAL - FORNECEDOR").
        WHEN 50 THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "TRANSFERENCIA DEPOSITO - FORNECEDOR (MED. CONTROLADO)").
        WHEN 33 THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "TRANSFERENCIA FILIAL - FORNECEDOR (MED. CONTROLADO)").  
        WHEN 32 THEN RUN pi-cria-balanco(INPUT i-cont, INPUT "DEVOLUCAO FILIAL - FORNECEDOR (MED. CONTROLADO)").  
    END CASE.
 END.
...
FOR EACH tt-pedido ,
    FIRST tt-balanco WHERE
          tt-balanco.ped-tipopedido-n = tt-pedido.ped-tipopedido-n
    BY tt-pedido.ped-tipopedido-n
    BY tt-pedido.cod-estab
    BY tt-pedido.dt-geracao :
   ...
   DISP  tt-pedido.ped-tipopedido-n COLUMN-LABEL "Tipo Ped"
         tt-balanco.desc-pedido     COLUMN-LABEL "Descriá∆o" FORMAT "X(40)"
         tt-pedido.cod-estab        COLUMN-LABEL "Estab"
         tt-pedido.dt-geracao
         tt-pedido.ped-codigo-n
         tt-pedido.situacao
         tt-pedido.desc-sit     COLUMN-LABEL "Descriá∆o" FORMAT "x(12)"
   ...
   FOR EACH tt-notas-geradas WHERE 
            tt-notas-geradas.ped-codigo-n = tt-pedido.ped-codigo-n:
   ...
   FOR EACH tt-nfe WHERE
            tt-nfe.ped-codigo-n = tt-pedido.ped-codigo-n :
       ...
       FOR EACH tt-it-nfe WHERE
           ...
           THEN DO:
               ...
               CASE ITEM.tipo-contr:
                 ...
                 WHEN 4 THEN ASSIGN c-tipo-contr  = "DÇbito Direto". 
           ...
           DISP  "Entrada"            COLUMN-LABEL "Tipo"    AT 05
                 ...
                 c-cod-obsoleto       COLUMN-LABEL "Situaá∆o" FORMAT "x(20)"
   ...
   FOR EACH tt-nfs WHERE
            tt-nfs.ped-codigo-n = tt-pedido.ped-codigo-n :

       PUT skip(1) "Itens Nota Sa°da :" AT 05 SKIP (1).

       FOR EACH tt-it-nfs WHERE
                tt-it-nfs.ped-codigo-n = tt-nfs.ped-codigo-n:
   ...
   THEN DO:
       ...
       FOR EACH tt-erro WHERE
                tt-erro.ped-codigo-n = tt-pedido.ped-codigo-n
...
PROCEDURE pi-leitura-pedidos:
        ...
        for EACH tt-balanco ,
            each int-ds-pedido no-lock WHERE
                 int-ds-pedido.ped-tipopedido-n  = tt-balanco.ped-tipopedido-n AND 
                 ...
                 date(int-ds-pedido.dt-geracao) <= tt-param.dt-periodo-fim ,
            FIRST tt-estab WHERE
                  (tt-estab.cgc = int-ds-pedido.ped-cnpj-origem-s OR tt-estab.cgc = int-ds-pedido.ped-cnpj-destino-s)  AND 
                  tt-estab.dt-fim-operacao >= int-ds-pedido.dt-geracao query-tuning(no-lookahead):
                
                 IF tt-param.rs-tipo-ped = 1 THEN 
                    IF int-ds-pedido.situacao = 2 THEN NEXT.
    
                IF tt-param.rs-tipo-ped = 2 THEN 
                    IF int-ds-pedido.situacao = 1 THEN NEXT.
    
                RUN pi-acompanhar IN h-acomp(INPUT "Pedido :" + string(int-ds-pedido.ped-codigo-n)).
    
               CREATE tt-pedido.
               BUFFER-COPY int-ds-pedido TO tt-pedido.
               ...
               FIND FIRST emitente NO-LOCK WHERE
                          emitente.cgc = int-ds-pedido.ped-cnpj-origem-s NO-ERROR.
               IF AVAIL emitente THEN
               ...
               THEN DO:
                   ...
                   FOR EACH nota-fiscal NO-LOCK WHERE
                     ...
                     ASSIGN tt-notas-geradas.ped-codigo-n  = int-ds-pedido.ped-codigo-n
...
PROCEDURE pi-cria-balanco:
  ...
  CREATE tt-balanco.
  ASSIGN tt-balanco.ped-tipopedido-n = p-tipo
...
PROCEDURE pi-item-divergente:
   ...
   ASSIGN d-dt-entrega        = int-ds-pedido.ped-dataentrega-d
          d-dt-emissao        = /*int-ds-pedido.ped-data-d*/ TODAY
          c-nr-pedcli         = trim(string(int-ds-pedido.ped-codigo-n))
   ...
   FOR each int-ds-pedido-produto no-lock of int-ds-pedido ,
       ...
       break by int-ds-pedido-retorno.ppr-produto-n
             ...
             RUN pi-acompanhar IN h-acomp(INPUT "Itens :" + string(int-ds-pedido-retorno.ppr-produto-n)).

             assign  l-movto-com-erro  = no.
                                
             assign de-vl-uni           = 0
                    c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                    de-quantidade       = int-ds-pedido-retorno.rpp-quantidade-n - int-ds-pedido-retorno.rpp-qtd-inventario-n 
                   ...
                   THEN DO:
                    ...
                    THEN DO:
                       ...
                       if de-vl-uni = 0 then do:                                        
                          for first item-uni-estab  no-lock where 
                              ...
                              assign de-vl-uni = item-uni-estab.preco-base.  /* Preáo base */
                          end.   
                       end.
                       ...
                       ASSIGN tt-it-nfe.ped-codigo-n = int-ds-pedido.ped-codigo-n
                              ...
                              tt-it-nfe.qtd-prs      = int-ds-pedido-retorno.rpp-quantidade-n
                              tt-it-nfe.qtd-inv      = int-ds-pedido-retorno.rpp-qtd-inventario-n  
                    ...
                    ELSE 
                       ASSIGN tt-it-nfe.quantidade   = tt-it-nfe.quantidade + de-quantidade
                              tt-it-nfe.qtd-prs      = tt-it-nfe.qtd-prs + int-ds-pedido-retorno.rpp-quantidade-n     
                              tt-it-nfe.qtd-inv      = tt-it-nfe.qtd-inv + int-ds-pedido-retorno.rpp-qtd-inventario-n.
            ...
            THEN DO:
                    ...
                    FOR FIRST tt-nfs NO-LOCK WHERE
                              tt-nfs.ped-codigo-n = int-ds-pedido.ped-codigo-n:
                    END.
                    IF NOT AVAIL tt-nfs 
                    THEN DO:
                   
                        CREATE tt-nfs.
                        ASSIGN tt-nfs.ped-codigo-n = int-ds-pedido.ped-codigo-n.
                    ...
                    FIND FIRST tt-it-nfs WHERE
                               tt-it-nfs.ped-codigo-n = int-ds-pedido.ped-codigo-n AND 
                    ...
                    THEN DO:
                       ...
                       CREATE tt-it-nfs.
                       ASSIGN tt-it-nfs.ped-codigo-n = int-ds-pedido.ped-codigo-n
                              tt-it-nfs.it-codigo    = c-it-codigo 
                              tt-it-nfs.quantidade   = de-quantidade * -1
                              tt-it-nfs.qtd-prs      = int-ds-pedido-retorno.rpp-quantidade-n
                              tt-it-nfs.qtd-inv      = int-ds-pedido-retorno.rpp-qtd-inventario-n  
                    ...
                    ELSE 
                       ASSIGN tt-it-nfs.quantidade   = tt-it-nfs.quantidade + (de-quantidade * -1)
                              tt-it-nfs.qtd-prs      = tt-it-nfs.qtd-prs    + int-ds-pedido-retorno.rpp-quantidade-n
                              tt-it-nfs.qtd-inv      = tt-it-nfs.qtd-inv    + int-ds-pedido-retorno.rpp-qtd-inventario-n.
        ...
        if nota-fiscal.dt-cancel = ? then do:
            assign i-erro = 36
                   c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Origem: " + string(int-ds-pedido.ped-cnpj-origem) + " Estab.: " + 
                                  c-cod-estabel + " SÇrie: " + nota-fiscal.serie + " NF: " + nota-fiscal.nr-nota-fis
                   l-movto-com-erro = yes.
...
procedure valida-item-entrada:
   ...
   IF AVAIL ITEM AND 
            ITEM.tipo-contr = 4 /* DÇbito Direto */
   THEN DO: 
...
procedure valida-item-saida:
  ...
  then do:
        assign i-erro-adv = 1
               i-erro = 38
               c-informacao = " Estabel : " + c-cod-estabel + " Tab. Preáo: " + 
                              c-nr-tabpre + " Item: " + 
                              c-it-codigo
               l-movto-com-erro = yes.
...
procedure pi-calcula-preco:
    ...
    then do:
       ...
       for first item-uni-estab  no-lock where 
           ...
           IF de-vl-uni = 0 THEN
              assign de-vl-uni = item-uni-estab.preco-ul-ent. /* Preáo Ult Entrada */ 
       end.      
...
procedure valida-pedido:
    ...
    do:
       assign i-erro-adv = 1
              i-erro = 30
              c-informacao = " CNPJ Origem: " + string(int-ds-pedido.ped-cnpj-origem)
              l-movto-com-erro = yes.
       run gera-log. 
...
procedure gera-log:
   ...
   CREATE tt-erro.
   ASSIGN tt-erro.ped-codigo-n = int-ds-pedido.ped-codigo-n
   ...
   ELSE
     ASSIGN tt-erro.desc-adv = "Advertància".                           
