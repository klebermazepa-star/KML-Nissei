...
{include/i-prgvrs.i INT042RP 2.12.06.AVB}
{cdp/cdcfgdis.i}
{utp/ut-glob.i}
...
FOR EACH bestabelec NO-LOCK WHERE 
    ...
    BY bestabelec.estado DESC QUERY-TUNING(NO-LOOKAHEAD):
    for-nota:
    for each int_ds_nota_entrada no-lock where
        ...
        AND int_ds_nota_entrada.nen_cnpj_destino = bestabelec.cgc QUERY-TUNING(NO-LOOKAHEAD):
        ...
        if d-data-procfit <> ? and d-data-procfit < int_ds_nota_entrada.nen_datamovimentacao_d then do:
            run pi-gera-log (input "Estabelecimento Ç procfit. Processamento deve ser pelo int142: " 
                             + "Est: "     + c-cod-estabel,
                             input 1).
            next.
        end.
        ...
        IF c-cod-estabel = "247" THEN NEXT.
        ...
        for each int_ds_nota_saida_item no-lock where
            ...
            RUN intprg/int013a.p( input i-cfop,
                                  input int_ds_nota_saida_item.nsp-cstb-n,
        ...
        if last-of (tt-movto.nr-nota-fis) then do:
            ...
            DO TRANSACTION:
                for first natur-oper exclusive where 
                    natur-oper.nat-operacao = tt-docum-est-nova.nat-operacao QUERY-TUNING(NO-LOOKAHEAD):
                    /*assign natur-oper.especie-doc = "NFE".*/
                    assign natur-oper.transf = no.
                end.
                for each btt-item-doc-est-nova QUERY-TUNING(NO-LOOKAHEAD):
                    for first natur-oper exclusive where 
                        natur-oper.nat-operacao = btt-item-doc-est-nova.nat-operacao QUERY-TUNING(NO-LOOKAHEAD):
                        /*assign natur-oper.especie-doc = "NFE".*/
                        assign natur-oper.transf = no.
                    end.
                    for first natur-oper exclusive where 
                        natur-oper.nat-operacao = btt-item-doc-est-nova.nat-of QUERY-TUNING(NO-LOOKAHEAD):
                        /*assign natur-oper.especie-doc = "NFE".*/
                        assign natur-oper.transf = no.
                    end.
                end.
