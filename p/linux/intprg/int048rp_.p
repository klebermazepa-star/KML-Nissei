...
procedure pi-importa-tudo:
    ...
    for each int-ds-mod-pedido no-lock where int-ds-mod-pedido.cod_prog_dtsul = "int048",
        each int-ds-tipo-pedido no-lock where int-ds-tipo-pedido.cod-mod-pedido = int-ds-mod-pedido.cod-mod-pedido
         and int-ds-tipo-pedido.log-ativo = yes:

        for each int-ds-pedido-subs no-lock where
            (int-ds-pedido-subs.situacao = 1 OR int-ds-pedido-subs.situacao = 3)
             /*and int-ds-pedido-subs.ped-tipopedido-n = 48 /* SUBSTITUICAO DE CUPOM */*/
             and int-ds-pedido-subs.ped-tipopedido-n = int(int-ds-tipo-pedido.tp-pedido)
            query-tuning(no-lookahead):
    
            run pi-acompanhar in h-acomp (input "Lendo Pedido: " + string(int-ds-pedido-subs.ped-codigo-n)).
            assign  l-movto-com-erro  = no
                    c-uf-destino      = ""
                    c-uf-origem       = "".
    
            assign  c-nr-pedcli         = trim(string(int-ds-pedido-subs.ped-codigo-n))
                    d-dt-implantacao    = today
                    d-dt-emissao        = /*int-ds-pedido-subs.ped-data-d*/ d-dt-implantacao
                    c-observacao        = IF int-ds-pedido-subs.ped-observacao-s <> ? THEN trim(int-ds-pedido-subs.ped-observacao-s) ELSE ""
                    c-tp-pedido         = trim(string(int-ds-pedido-subs.ped-tipopedido-n))
                    c-docto-orig        = trim(string(int-ds-pedido-subs.ped-codigo-n))
                    c-serie             = "".
            ...
            for each int-ds-pedido-produto-subs no-lock of int-ds-pedido-subs
                query-tuning(no-lookahead)
                break by int-ds-pedido-produto-subs.ped-codigo-n
                      by int-ds-pedido-produto-subs.ppr-produto-n:
    
                assign  c-it-codigo         = trim(string(int-ds-pedido-produto-subs.ppr-produto-n))
                        de-quantidade       = int-ds-pedido-produto-subs.ppr-quantidade-n
                        de-vl-uni           = int-ds-pedido-produto-subs.ppr-valorbruto-n
                        de-desconto         = if int-ds-pedido-produto-subs.ppr-percentualdesconto-n <> ? then int-ds-pedido-produto-subs.ppr-percentualdesconto-n else 0
                        c-nr-docum          = if int-ds-pedido-produto-subs.nen-notafiscal-n <> ? and
                                                 int-ds-pedido-produto-subs.nen-notafiscal-n <> 0 then trim(string(int-ds-pedido-produto-subs.nen-notafiscal-n,">>9999999")) else ""
                        c-serie-docum       = if int-ds-pedido-produto-subs.nen-serie-s <> ? then int-ds-pedido-produto-subs.nen-serie-s else ""
    
                        i-seq-docum         = if int-ds-pedido-produto-subs.nep-sequencia-n <> ? then int-ds-pedido-produto-subs.nep-sequencia-n else 0.
            ...
            if not l-movto-com-erro and not avail tt-ped-item-prs then do:
                assign i-erro = 37
                       c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Origem:" + string(int-ds-pedido-subs.ped-cnpj-origem)
...
procedure finaliza-pedido.
   ...
   do:
        ...
        if not l-movto-com-erro then do:
           ...
           find current int-ds-pedido-subs exclusive-lock no-wait no-error.
           if avail int-ds-pedido-subs then
               assign int-ds-pedido-subs.situacao = 2 /* integrado */.
           run gera-log.
           release int-ds-pedido-subs.
        end.
   end.
   run pi-elimina-tabelas.
end.
...
procedure valida-registro-pedido.
    ...
    for each estabelec 
        ...
        no-lock where
        estabelec.cgc = int-ds-pedido-subs.ped-cnpj-origem-s,
    ...
    for first nota-fiscal fields (cod-estabel serie nr-nota-fis dt-cancel) no-lock where
        ...
        if nota-fiscal.dt-cancel = ? then do:
            assign i-erro = 36
                   c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Origem: " + string(int-ds-pedido-subs.ped-cnpj-origem) + " Estab.: " + 
    ...
    for first emitente no-lock where
        emitente.cgc = trim(int-ds-pedido-subs.ped-cnpj-destino-s):
    ...
    if c-nome-abrev = "" then do:
        assign i-erro = 2
               c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Destino: " + string((int-ds-pedido-subs.ped-cnpj-destino))
               l-movto-com-erro = yes.
        run gera-log. 
        return.
    end.
    ...
    for each tt-notas-geradas:
        for first nota-fiscal exclusive-lock where 
            ...
            FOR FIRST int-ds-pedido-subs WHERE
                      int-ds-pedido-subs.ped-codigo-n = INT(tt-ped-venda-prs.nr-pedcli) NO-LOCK: 
            END.
            IF AVAIL int-ds-pedido-subs THEN DO:
               IF int-ds-pedido-subs.cod-model-ecf   <> "?" AND 
                  int-ds-pedido-subs.cod-fabricc-ecf <> "?" THEN DO:
...
procedure gera-log.
   ...
   RUN intprg/int999.p ("NF PED", 
                        ...
                        int-ds-pedido-subs.situacao, /* 1 - Pendente, 2 - Processado */ 
                        c-seg-usuario,
                        INPUT "INT048RP.P").
   assign c-informacao = " ".
end.
