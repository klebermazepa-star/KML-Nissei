    ...
    FOR EACH int-ds-pedido   NO-LOCK                   
       WHERE int-ds-pedido.ped-codigo-n = int(tt-param.nr-pedido):
        ...
        IF AVAIL emitente THEN DO:
             ASSIGN tt-nota.estado              = emitente.estado  
        ...
        ELSE DO:
                 ASSIGN tt-nota.nr-nota-fis         = ""         
                        ...
                        tt-nota.nr-pedcli           = "" 
                        tt-nota.estado              = ""
                        tt-nota.cod-emitente        = 0           
                        tt-nota.nome-abrev          = "".
        END.
    END. 


    FOR EACH int-ds-pedido-sub   NO-LOCK                   
       WHERE int-ds-pedido-sub.ped-codigo-n = int(tt-param.nr-pedido):

          ASSIGN c-sit = "".
          IF int-ds-pedido-sub.situacao <> 2 THEN
              ASSIGN c-sit = STRING(int-ds-pedido-sub.situacao) + " - " + "Pendente".
          ELSE 
              ASSIGN c-sit = STRING(int-ds-pedido-sub.situacao) + " - " + "Processado".

           ASSIGN c-tipo = "".
           CASE int-ds-pedido-sub.ped-tipopedido-n:
               WHEN 1 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA DEPOSITO - FILIAL".
               WHEN 2 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA FILIAL - DEPOSITO".
               WHEN 3 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO MANUAL FILIAL".
               WHEN 4 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "COMPRA FORNECEDOR - FILIAL".
               WHEN 5 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "COMPRA FORNECEDOR - DEPOSITO".
               WHEN 6 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA DEPOSITO - DEPOSITO".
               WHEN 7 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "ELETRONICO FORNECEDOR - FILIAL".
               WHEN 8 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "ELETRONICO DEPOSITO - FILIAL".
               WHEN 9 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "PBM FILIAL".
               WHEN 10 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "PBM DEPOSITO".
               WHEN 11 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO MANUAL DEPOSITO".
               WHEN 12 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO COLETOR FILIAL".
               WHEN 13 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO COLETOR DEPOSITO".
               WHEN 14 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO MANUAL FILIAL - PERMITE CONSOLIDA€ÇO".
               WHEN 15 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "DEVOLUCAO FILIAL - FORNECEDOR".                  
               WHEN 16 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "DEVOLUCAO DEPOSITO - FORNECEDOR".
               WHEN 17 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA DEPOSITO - DEPOSITO (COMERCIALIZAVEL)".
               WHEN 18 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA DEPOSITO - DEPOSITO (AVARIA)".
               WHEN 19 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA FILIAL - FILIAL".
               WHEN 31 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALANCO MANUAL FILIAL - ESPECIAL (MED. CONTROLADO)".                  
               WHEN 32 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "DEVOLUCAO FILIAL - FORNECEDOR (MED. CONTROLADO)".
               WHEN 33 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "TRANSFERENCIA FILIAL - FORNECEDOR (MED. CONTROLADO)".
               WHEN 35 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALAN€O GERAL CONTROLADOS FILIAL".
               WHEN 36 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "BALAN€O GERAL CONTROLADOS FILIAL".                  
               WHEN 37 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "ATIVO IMOBILIZADO DEPOSITO => FILIAL".
               WHEN 38 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "ESTORNO".
               WHEN 39 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "ATIVO IMOBILIZADO FILIAL => FILIAL".
               WHEN 46 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "RETIRADA FILIAL => PROPRIA FILIAL".
               WHEN 48 THEN ASSIGN c-tipo = string(int-ds-pedido-sub.ped-tipopedido-n) + " - " + "SUBSTITUI€ÇO DE CUPOM".                  
          END CASE.

           CREATE tt-nota.
           ASSIGN tt-nota.situacao            = c-sit
                  tt-nota.ped-tipopedido-n    = c-tipo
                  tt-nota.ped-codigo-n        = int-ds-pedido-sub.ped-codigo-n       
                  tt-nota.dt-geracao          = int-ds-pedido-sub.dt-geracao         
                  tt-nota.hr-geracao          = int-ds-pedido-sub.hr-geracao         
                  tt-nota.ped-cnpj-destino-s  = int-ds-pedido-sub.ped-cnpj-destino-s 
                  tt-nota.ped-cnpj-origem-s   = int-ds-pedido-sub.ped-cnpj-origem-s.


        FIND FIRST emitente NO-LOCK
             WHERE emitente.cgc = int-ds-pedido-sub.ped-cnpj-destino-s NO-ERROR.
        ...
        IF AVAIL emitente THEN DO:
             ...
             FOR FIRST nota-fiscal NO-LOCK
                 WHERE nota-fiscal.nr-pedcli = string(int-ds-pedido-sub.ped-codigo-n)
                   AND nota-fiscal.cod-emitente = emitente.cod-emitente :
