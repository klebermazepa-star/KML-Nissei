...
for each int_ds_docto_xml no-lock where
    ...
    int_ds_docto_wms.doc_numero_n = int64(int_ds_docto_xml.nNF) and
    ...
    int_ds_docto_wms.cnpj_cpf     = int_ds_docto_xml.CNPJ,
        FIRST bestabelec NO-LOCK WHERE 
            bestabelec.cgc = int_ds_docto_xml.CNPJ AND 
   ...
   if (int_ds_docto_wms.situacao <> 2 and int_ds_docto_wms.situacao <> 40) /* movimentada PRS */ or 
    ...
    for each estabelec 
        ...
        cst_estabelec.dt_fim_operacao >= int_ds_docto_xml.dEmi QUERY-TUNING(NO-LOOKAHEAD):
    ...
    for first nota-fiscal no-lock where 
        ...
        nota-fiscal.nr-nota-fis = trim(string(int64(int_ds_docto_xml.nNF),">>>9999999")) QUERY-TUNING(NO-LOOKAHEAD): end.
    ...
    for first docum-est no-lock where 
        ...
        for each bint_ds_docto_xml where
            ...
            Bint_ds_docto_xml.nNF   = int_ds_docto_xml.nNF QUERY-TUNING(NO-LOOKAHEAD):
            ...
            for first bint_ds_docto_wms exclusive-lock where 
                rowid(bint_ds_docto_wms) = rowid(int_ds_docto_wms) QUERY-TUNING(NO-LOOKAHEAD):
        ...
        if param-estoq.ult-fech-dia >= int_ds_docto_wms.datamovimentacao_d or
           param-estoq.mensal-ate >= int_ds_docto_wms.datamovimentacao_d then do:
            ...
            for each bint_ds_docto_xml where
                ...
                for each bint_ds_docto_wms exclusive use-index sequencial where 
                    bint_ds_docto_wms.doc_numero_n = int64(int_ds_docto_xml.nNF) and
    ...
    for each it-nota-fisc no-lock of nota-fiscal
        ...
        for first item 
            ...
            no-lock where 
            item.it-codigo = string(it-nota-fisc.it-codigo) QUERY-TUNING(NO-LOOKAHEAD): end.
        ...
        for first item-uni-estab no-lock where 
                  ...
                  item-uni-estab.it-codigo = string(it-nota-fisc.it-codigo) QUERY-TUNING(NO-LOOKAHEAD): 
        end.
        if not avail item-uni-estab THEN DO:
           for first item-estab NO-LOCK where 
                     item-estab.cod-estabel = tt-movto.cod-estabel and
                     item-estab.it-codigo = string(it-nota-fisc.it-codigo) QUERY-TUNING(NO-LOOKAHEAD): 
           end.
        END.
        ...
        */
        ...
        for first fat-ser-lote no-lock of it-nota-fisc QUERY-TUNING(NO-LOOKAHEAD): end.
               ...
               for each tt-erro QUERY-TUNING(NO-LOOKAHEAD):
                   ...
                   IF tt-erro.cd-erro = 1878 THEN DO:
                      ...
                      for each bint_ds_docto_xml where
                          ...
                          Bint_ds_docto_xml.nNF   = tt-movto.nNF QUERY-TUNING(NO-LOOKAHEAD):
                          assign  bint_ds_docto_xml.sit_re          = 40
                                  bint_ds_docto_xml.situacao        = 3 /* processada */.
                      end.
                   END.
