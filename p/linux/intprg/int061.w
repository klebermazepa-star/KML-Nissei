...
*******************************************************************************/
{include/i-prgvrs.i INT061 2.12.01.001}
...
DEFINE VARIABLE c-fim-estado AS CHARACTER FORMAT "X(2)":U INITIAL "ZZ" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE c-ini-estado AS CHARACTER FORMAT "X(2)":U 
     LABEL "Estado" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.
...
DEFINE VARIABLE l-atualiza-of0313 AS LOGICAL INITIAL no 
     LABEL "Atualiza OF0313?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-sel
     ...
     c-ini-estado AT ROW 3.17 COL 24.57 COLON-ALIGNED WIDGET-ID 12
     c-fim-estado AT ROW 3.17 COL 41.14 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     l-atualiza-of0313 AT ROW 4.75 COL 26.57 WIDGET-ID 18
...
ON END-ERROR OF w-relat /* Apura»’o de ICMS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Apura»’o de ICMS */
DO:
...
DO:
   do  on error undo, return no-apply:
       ...
       IF  MONTH(INPUT FRAME f-pg-sel c-ini-data) <> MONTH(INPUT FRAME f-pg-sel c-fim-data) OR
           YEAR (INPUT FRAME f-pg-sel c-ini-data) <> YEAR (INPUT FRAME f-pg-sel c-fim-data)
       THEN DO:
           RUN utp/ut-msgs.p (INPUT "show", 
                              INPUT 17006, 
                              INPUT "Data de Apura‡Æo Inv lida. ~~ A data de apura‡Æo deve estar no mesmo mˆs e ano informados.").
           APPLY "ENTRY" TO c-ini-data IN FRAME f-pg-sel.
           RETURN NO-APPLY.
       END.
...
/*    */
/*                    assign c-arquivo:screen-value in frame f-pg-imp = session:TEMP-directory + "INT060.xls". */
/*                    . */
/*    */
/*                    message "" */
/*                     view-as alert-box. */
...
{utp/ut9000.i "INT061" "2.12.01.001"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-ini-estab c-fim-estab c-ini-data c-fim-data c-ini-estado 
          c-fim-estado l-atualiza-of0313 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 c-ini-estab 
         c-fim-estab c-ini-data c-fim-data c-ini-estado c-fim-estado 
         l-atualiza-of0313 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    */
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           /*Alterado 14/02/2005 - tech1007 - Armazena a informa»’o se o RTF estÿ habilitado ou n’o*/
    ...
    assign tt-param.estab-ini         = input frame f-pg-sel c-ini-estab
           ...
           tt-param.estado-ini        = input frame f-pg-sel c-ini-estado
           tt-param.estado-fim        = input frame f-pg-sel c-fim-estado
           tt-param.l-atualiza-of0313 = input frame f-pg-sel l-atualiza-of0313.
    ...
    {include/i-rprun.i intprg/int061rp.p}
