...
&Scoped-Define ENABLED-OBJECTS rt-button bt-ajuda bt-ok bt-save bt-cancela 
...
DEFINE VARIABLE h_int115c-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int115c-viewer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ajuda AT ROW 10.38 COL 80
     bt-ok AT ROW 10.42 COL 3 HELP
          "Salva e sai"
     bt-save AT ROW 10.42 COL 13.43 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 10.42 COL 24 HELP
          "Cancela"
     rt-button AT ROW 10.25 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 93.86 BY 17 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Natureza Origem"
         HEIGHT             = 10.75
         WIDTH              = 90.43
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Natureza Origem */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Natureza Origem */
DO:
...
DO:
{include/okfil.i h_int115c-viewer}
END.
...
DO:
{include/salvafil.i h_int115c-viewer}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115c-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int115c-viewer ).
       RUN set-position IN h_int115c-viewer ( 1.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int115c-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_int115c-query ).
       RUN set-position IN h_int115c-query ( 10.00 , 72.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_int115c-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int115c-query , 'Record':U , h_int115c-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_int115c-viewer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int115c-viewer ,
             bt-ajuda:HANDLE IN FRAME f-cad , 'BEFORE':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE rt-button bt-ajuda bt-ok bt-save bt-cancela 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT115c" "1.12.01.AVB"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_int115c-viewer (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_int115c-query (input v-row-table).
