...
*******************************************************************************/
{include/i-prgvrs.i INT115A 2.12.01.AVB}
...
DEFINE FRAME f-cad
     bt-ok AT ROW 5.67 COL 2.14 HELP
          "Salva e sai"
     bt-save AT ROW 5.67 COL 13.14 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 5.67 COL 24.14 HELP
          "Cancela"
     bt-ajuda AT ROW 5.67 COL 84
     rt-button AT ROW 5.5 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 94.43 BY 6.08 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-incmdp ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Tipo de Pedido"
         HEIGHT             = 6.21
         WIDTH              = 95
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-incmdp /* Inclui/Modifica Tipo de Pedido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-incmdp w-incmdp
ON WINDOW-CLOSE OF w-incmdp /* Inclui/Modifica Tipo de Pedido */
DO:
...
DO:
    {include/okpai.i h_int115-viewer}
END.
...
DO:
    {include/salvapai.i h_int115-viewer}
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_int115-viewer ( 1.75 , 3.00 ) NO-ERROR.
       ...
       RUN set-position IN h_int115-query ( 1.00 , 87.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.00 , 8.00 ) */

       /* Links to SmartViewer h_int115-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int115-query , 'Record':U , h_int115-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_int115-viewer ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT115a" "2.12.01.AVB"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_int115-query (INPUT v-row-table). 
