...
*******************************************************************************/
{include/i-prgvrs.i INT015-Query.w 2.12.01.001}
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH int-ds-tp-natur-oper NO-LOCK
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH int-ds-tp-natur-oper NO-LOCK.
&Scoped-define TABLES-IN-QUERY-Query-Main int-ds-tp-natur-oper
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main int-ds-tp-natur-oper
...
<FOREIGN-KEYS>
cod-estabel||y|mgesp.int-ds-tp-natur-oper.cod-estabel
cod-emitente||y|mgesp.int-ds-tp-natur-oper.cod-emitente
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "emsesp.int-ds-tp-natur-oper"
     _Options          = "NO-LOCK"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-row-available q-tables 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-busca-natureza q-tables 
...
------------------------------------------------------------------------------*/
    ...
    define input parameter c-operacao as char no-undo.
    ...
    define variable i-cod-portador  as integer.
    define variable i-modalidade    as integer.
    define variable c-serie         as char.
    define variable c-natur-ent     as char no-undo.

    for first item fields (it-codigo fm-cod-com class-fiscal)
        no-lock where item.it-codigo = c-it-codigo: end.

    run intprg/int015a.p (  input  c-operacao       ,
                            input  if c-uf-destino <> "" then c-uf-destino else ?,
                            input  if c-uf-origem <> "" then c-uf-origem else ?,
                            input  if c-cod-estabel <> "" then c-cod-estabel else ?,
                            input  if i-cod-emitente <> 0 then i-cod-emitente else ?,
                            input  if avail item then item.class-fiscal else ?,
                            input  i-cst-icms,
                            output c-natureza,
    ...
    if r-rowid <> ? then do:
        reposition Query-Main to rowid r-rowid.
        for first int-ds-tp-natur-oper no-lock where
            rowid(int-ds-tp-natur-oper) = r-rowid:
            RUN notify IN THIS-PROCEDURE ( INPUT 'row-available':U ).
            return "OK".
        end.
    end.
    else return "NOK".
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "cod-estabel" "int-ds-tp-natur-oper" "cod-estabel"}
  {src/adm/template/sndkycas.i "cod-emitente" "int-ds-tp-natur-oper" "cod-emitente"}
