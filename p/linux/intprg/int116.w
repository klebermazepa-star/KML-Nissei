...
**
*******************************************************************************/
{include/i-prgvrs.i INT116 2.12.03.AVB}
/*{include/wms.i}*/
...
&GLOBAL-DEFINE Version        2.12.03.AVB

&GLOBAL-DEFINE WindowType     Master

&GLOBAL-DEFINE Folder         YES
&GLOBAL-DEFINE InitialPage    1
&GLOBAL-DEFINE FolderLabels   Parƒmetros ,Tributado, Isento, Cesta B sica, ST, Outros,  Nenhum, Nat Origem

&GLOBAL-DEFINE page0Widgets   btQueryJoins btReportsJoins btExit btHelp ~
                              btOK btCancel btHelp2 tp-pedido

&GLOBAL-DEFINE page1Widgets btLeitura class-fiscal-fim class-fiscal-ini cd-trib-icm nat-origem-ini nat-origem-fim
...
&GLOBAL-DEFINE page8Widgets   brOrigem
...
define new shared temp-table tt-origem like int-ds-tp-nat-origem
...
define temp-table tt-param
    ...
    field nat-origem-ini                like int-ds-tp-nat-origem.nat-origem
    field nat-origem-fim                like int-ds-tp-nat-origem.nat-origem
    .
...
define buffer btt-origem for tt-origem.
...
&Scoped-define INTERNAL-TABLES tt-movtos tt-origem
...
/* Definitions for BROWSE brOrigem                                      */
&Scoped-define FIELDS-IN-QUERY-brOrigem tt-origem.tp-pedido tt-origem.nat-origem tt-origem.descricao tt-origem.nat-saida tt-origem.nat-entrada tt-origem.nat-saida-nao-contrib tt-origem.nat-entrada-nao-contrib   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brOrigem   
&Scoped-define SELF-NAME brOrigem
&Scoped-define QUERY-STRING-brOrigem FOR EACH tt-origem
&Scoped-define OPEN-QUERY-brOrigem OPEN QUERY {&SELF-NAME} FOR EACH tt-origem.
&Scoped-define TABLES-IN-QUERY-brOrigem tt-origem
&Scoped-define FIRST-TABLE-IN-QUERY-brOrigem tt-origem
...
DEFINE MENU POPUP-MENU-brIsento-4 
       MENU-ITEM m_Apaga_Somente_Este_De-Para-4 LABEL "Apaga Somente Este De->Para" ACCELERATOR "CTRL-D".
...
DEFINE VARIABLE nat-origem-fim AS CHARACTER FORMAT "X(8)" INITIAL "ZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 10.57 BY .79.

DEFINE VARIABLE nat-origem-ini AS CHARACTER FORMAT "X(8)" 
     LABEL "Natureza Origem" 
     VIEW-AS FILL-IN 
     SIZE 10.57 BY .79.
...
DEFINE BROWSE brOrigem
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brOrigem wWindow _FREEFORM
  QUERY brOrigem DISPLAY
      tt-origem.tp-pedido                     column-label "Tp!Ped"
tt-origem.nat-origem                    column-label "Class. Fiscal"
tt-origem.descricao                     column-label "Descricao"
tt-origem.nat-saida                     column-label "Sa¡da!Contrib"
tt-origem.nat-entrada                   column-label "Entrada!Contrib"
tt-origem.nat-saida-nao-contrib         column-label "Sa¡da!NAO Cont"
tt-origem.nat-entrada-nao-contrib       column-label "Entr!NAO Cont"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 134 BY 16
         FONT 1 ROW-HEIGHT-CHARS .46.
...
DEFINE FRAME fPage8
     brOrigem AT ROW 1 COL 1
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.86 ROW 3.5
         SIZE 135.14 BY 16.25 WIDGET-ID 500.
...
DEFINE FRAME fPage1
     ...
     nat-origem-ini AT ROW 3.25 COL 16 COLON-ALIGNED HELP
          "Natureza de opera‡Æo origem" WIDGET-ID 186
     nat-origem-fim AT ROW 3.25 COL 41 COLON-ALIGNED HELP
          "Natureza de opera‡Æo origem" NO-LABEL WIDGET-ID 184
     cd-trib-icm AT ROW 5.5 COL 20 HELP
          "C¢digo de tributa‡Æo do ICMS" NO-LABEL WIDGET-ID 174
     btLeitura AT ROW 15 COL 109
     "Tributa‡Æo ICMS" VIEW-AS TEXT
          SIZE 13 BY .54 AT ROW 4.5 COL 19 WIDGET-ID 182
     IMAGE-42 AT ROW 2.25 COL 29 WIDGET-ID 166
     IMAGE-43 AT ROW 2.25 COL 40 WIDGET-ID 168
     RECT-1 AT ROW 4.75 COL 18 WIDGET-ID 180
     IMAGE-44 AT ROW 3.25 COL 29 WIDGET-ID 188
     IMAGE-45 AT ROW 3.25 COL 40 WIDGET-ID 190
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 3.5
         SIZE 135.43 BY 16.25
...
/* SETTINGS FOR FRAME fPage8
                                                                        */
/* BROWSE-TAB brOrigem 1 fPage8 */
ASSIGN 
       brOrigem:POPUP-MENU IN FRAME fPage8             = MENU POPUP-MENU-brIsento-4:HANDLE
       brOrigem:NUM-LOCKED-COLUMNS IN FRAME fPage8     = 7
       brOrigem:MAX-DATA-GUESS IN FRAME fPage8         = 1500
       brOrigem:COLUMN-RESIZABLE IN FRAME fPage8       = TRUE
       brOrigem:COLUMN-MOVABLE IN FRAME fPage8         = TRUE.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE brOrigem
/* Query rebuild information for BROWSE brOrigem
     _START_FREEFORM
...
&Scoped-define SELF-NAME m_Apaga_Somente_Este_De-Para-4
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_Apaga_Somente_Este_De-Para-4 wWindow
ON CHOOSE OF MENU-ITEM m_Apaga_Somente_Este_De-Para-4 /* Apaga Somente Este De->Para */
DO:
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
    ...
    assign  tt-movtos.r-rowid       = rowid(int-ds-tp-trib-natur-oper)
            tt-movtos.class-fiscal  = if avail int-ds-ncm-cd-trib-uf 
                                      then int-ds-ncm-cd-trib-uf.class-fiscal 
                                      else int-ds-classif-fisc.class-fiscal
            tt-movtos.uf-origem     = if avail int-ds-ncm-cd-trib-uf 
                                      then int-ds-ncm-cd-trib-uf.estado
                                      else tt-movtos.uf-origem
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE AddTTOrigem wWindow 
PROCEDURE AddTTOrigem :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    create  tt-origem.
    buffer-copy int-ds-tp-nat-origem to tt-origem
    assign  tt-origem.r-rowid       = rowid(int-ds-tp-nat-origem).

    for first natur-oper fields (denominacao)
        no-lock where natur-oper.nat-operacao = int-ds-tp-nat-origem.nat-origem:
        assign tt-origem.descricao = natur-oper.denominacao.
    end.
...
------------------------------------------------------------------------------*/
    ...
    run setEnabled in hFolder (input 8, input false).
...
assign  tt-param.tp-pedido        = tp-pedido:input-value in frame fPage0
        ...
        tt-param.class-fiscal-ini = class-fiscal-ini:input-value in frame fPage1
        tt-param.class-fiscal-fim = class-fiscal-fim:input-value in frame fPage1
        tt-param.nat-origem-ini   = nat-origem-ini:input-value in frame fPage1
        tt-param.nat-origem-fim   = nat-origem-fim:input-value in frame fPage1.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE querybrOrigemOpen wWindow 
PROCEDURE querybrOrigemOpen :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    open query brOrigem for each tt-origem no-lock.

    if num-results ("brOrigem") <> 0 then
    do:
        enable brOrigem with frame fPage8.
        run setEnabled in hFolder (input 8, input true).
    end.
    else do:
        disable brOrigem with frame fPage8.
        run setEnabled in hFolder (input 8, input false).
    end.
...
for first int-ds-tipo-pedido no-lock where 
    int-ds-tipo-pedido.tp-pedido = tt-param.tp-pedido: end.
for first int-ds-mod-pedido no-lock where 
    int-ds-mod-pedido.cod-mod-pedido = int-ds-tipo-pedido.cod-mod-pedido: end.

if avail int-ds-mod-pedido and int-ds-mod-pedido.log-nat-origem then do:
    for each int-ds-tp-nat-origem no-lock where 
        int-ds-tp-nat-origem.tp-pedido   = tt-param.tp-pedido and
        int-ds-tp-nat-origem.nat-origem >= tt-param.nat-origem-ini and
        int-ds-tp-nat-origem.nat-origem <= tt-param.nat-origem-fim:
    ...
    for each int-ds-tp-trib-natur-oper no-lock where
        ...
        each int-ds-ncm-cd-trib-uf no-lock where 
        int-ds-ncm-cd-trib-uf.cd-trib-icm   = int-ds-tp-trib-natur-oper.cd-trib-icm and
        int-ds-ncm-cd-trib-uf.class-fiscal >= tt-param.class-fiscal-ini and
        int-ds-ncm-cd-trib-uf.class-fiscal <= tt-param.class-fiscal-fim
    ...
    for each int-ds-tp-trib-natur-oper no-lock where
        ...
        /* se for por UF desconsidera cadatro inicial - j  foi lido anteriormente */
    ...
    for each int-ds-tp-trib-natur-oper no-lock where
        ...
        if not can-find (tt-movtos where tt-movtos.cd-trib-icm = int-ds-classif-fisc.cd-trib-icm and
                         tt-movtos.class-fiscal = int-ds-classif-fisc.class-fiscal and
                         tt-movtos.tp-pedido = int-ds-tp-trib-natur-oper.tp-pedido and
                         tt-movtos.uf-origem = int-ds-tp-trib-natur-oper.uf-origem) then
            RUN AddTTMovto.
    end.
end.    
...
else if num-results ("brOrigem") <> 0  then
    run SetFolder IN hFolder (INPUT 8).
