...
procedure pi-importa:
    ...
    for each int-ds-mod-pedido no-lock where 
        int-ds-mod-pedido.cod_prog_dtsul = "int117" OR
        int-ds-mod-pedido.cod_prog_dtsul = "int127" OR
        int-ds-mod-pedido.cod_prog_dtsul = "int137" OR
        int-ds-mod-pedido.cod_prog_dtsul = "int147",
        each int-ds-tipo-pedido no-lock where 
        int-ds-tipo-pedido.cod-mod-pedido = int-ds-mod-pedido.cod-mod-pedido AND
        int-ds-tipo-pedido.tp-pedido      = trim(string(tt-param.tipo-pedido)) and
        int-ds-tipo-pedido.log-ativo = yes
        query-tuning(no-lookahead):
    
        assign i-nr-registro    = 0.
        for each int-ds-pedido no-lock where
                 int-ds-pedido.situacao         = 1 and
                 int-ds-pedido.ped-codigo-n     = (if tt-param.tp-todos-ped = no 
                                                   then int(tt-param.nr-pedido) 
                                                   else int-ds-pedido.ped-codigo-n) and
                 int-ds-pedido.ped-tipopedido-n = int(int-ds-tipo-pedido.tp-pedido)
            query-tuning(no-lookahead):
        
            /*
            if  int-ds-pedido.ped-tipopedido-n = 1  or /* transf cd -> filial s¢ pedidos */
                int-ds-pedido.ped-tipopedido-n = 8  or /* ELETRONICO DEPOSITO - FILIAL */
                int-ds-pedido.ped-tipopedido-n = 20 or /* CD INT110 */
                int-ds-pedido.ped-tipopedido-n = 37 or /* CD INT110 */
                int-ds-pedido.ped-tipopedido-n = 2  or /* Retirada Lj->CD - Caixa */
                int-ds-pedido.ped-tipopedido-n = 48 OR   /* Substitiocao Cupom */
                int-ds-pedido.ped-tipopedido-n = 3  OR /* BALANCO MANUAL FILIAL */ 
                int-ds-pedido.ped-tipopedido-n = 11 OR /* BALANCO MANUAL DEPOSITO */
                int-ds-pedido.ped-tipopedido-n = 12 OR /* BALANCO COLETOR FILIAL */
                int-ds-pedido.ped-tipopedido-n = 13 OR /* BALANCO COLETOR DEPOSITO */
                int-ds-pedido.ped-tipopedido-n = 14 OR /* BALANCO MANUAL FILIAL - PERMITE CONSOLIDA€ÇO */
                int-ds-pedido.ped-tipopedido-n = 31 OR /* BALANCO MANUAL FILIAL - ESPECIAL (MED. CONTROLADO) */ 
                int-ds-pedido.ped-tipopedido-n = 35 OR /* BALAN€O GERAL CONTROLADOS DEPOSITO */
                int-ds-pedido.ped-tipopedido-n = 36 /* BALAN€O GERAL CONTROLADOS FILIAL */ 
                then next.
            */
            ...
            assign  c-nr-pedcli         = trim(string(int-ds-pedido.ped-codigo-n))
                    ...
                    c-placa             = if int-ds-pedido.ped-placaveiculo-s <> ? then int-ds-pedido.ped-placaveiculo-s else ""
                    c-uf-placa          = if int-ds-pedido.ped-estadoveiculo-s <> ? then int-ds-pedido.ped-estadoveiculo-s else ""
            ...
            if int-ds-pedido.envio_status <> ? and c-cod-estabel <> "973" then next.
            ...
            for each int-ds-pedido-produto no-lock of int-ds-pedido,
                ...
                assign  c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                        ...
                        de-vl-uni           = if int-ds-pedido.ped-tipopedido-n <> 10 
                                              then int-ds-pedido-produto.ppr-valorbruto-n
                                              else int-ds-pedido-produto.ppr-valorliquido-n
                ...
                if last-of(int-ds-pedido-retorno.rpp-lote-s) then run cria-tt-ped-item-prs.
            end.
...
procedure valida-registro-pedido.
    ...
    if  c-cod-estabel <>  "973" and d-dt-procfit <> ? and
    ...
    for first nota-fiscal fields (cod-estabel serie nr-nota-fis dt-cancel) no-lock where
        ...
        if nota-fiscal.dt-cancel = ? then do:
            assign i-erro = 36
                   c-informacao = "Pedido: " + c-nr-pedcli + " CNPJ Origem: " + string(int-ds-pedido.ped-cnpj-origem) + " Estab.: " + 
...
procedure valida-registro-item.
   ...
   i-cst-icms = 0.
   c-nat-docum = "".
   if trim(c-nr-docum) <> "" and c-tp-pedido <> "48"
   then do:  /* Nota de origem de uma devolu»’o no Sysfarma */
       ...
       IF AVAIL int-ds-ext-emitente AND 
                int-ds-ext-emitente.gera-nota = YES  /* Quando for Pepsico Verifica se a nota estÿ pendente de atualiza‡Æo */ 
       THEN DO:
                       
           FIND FIRST int-ds-docto-xml NO-LOCK WHERE
                      int-ds-docto-xml.nnf          = c-nr-docum        AND 
                      int-ds-docto-xml.serie        = c-serie-docum     AND 
                      int-ds-docto-xml.tipo-nota    = 1                 AND  
                      int-ds-docto-xml.cod-emitente = i-cod-emitente  NO-ERROR.
           IF AVAIL int-ds-docto-xml 
           THEN DO:

               IF int-ds-docto-xml.situacao = 0 AND /* Nunca */
               ...
               ELSE DO:   
                    ...
                    FIND FIRST tt-nota-origem WHERE
                               tt-nota-origem.nro-docto      = int-ds-docto-xml.nnf           AND 
                               tt-nota-origem.serie          = int-ds-docto-xml.serie         AND 
                               tt-nota-origem.tipo-nota      = int-ds-docto-xml.tipo-nota     AND 
                               tt-nota-origem.nat-operacao   = int-ds-docto-xml.nat-operacao  AND
                               tt-nota-origem.cod-emitente   = int-ds-docto-xml.cod-emitente NO-ERROR.
                    ...
                    THEN DO:
                        ...
                        ASSIGN tt-nota-origem.nome-abrev     = c-nome-abrev
                               ...
                               tt-nota-origem.nro-docto      = int-ds-docto-xml.nnf  
                               tt-nota-origem.serie          = int-ds-docto-xml.serie       
                               tt-nota-origem.tipo-nota      = int-ds-docto-xml.tipo-nota   
                               tt-nota-origem.nat-operacao   = int-ds-docto-xml.nat-operacao
                               tt-nota-origem.cod-emitente   = int-ds-docto-xml.cod-emitente.
                               
                        assign c-nat-docum = int-ds-docto-xml.nat-operacao.       
                    END.
               END.
                   ...
                   assign  tt-param-int012.destino          = 3
                    ...
                    if not avail item-doc-est then do:
                        assign i-erro = 18
                               ...
                               l-movto-com-erro = yes.
                        find current int-ds-pedido exclusive-lock no-wait no-error.
                        if avail int-ds-pedido AND int-ds-pedido.ped-tipopedido-n <> 48 then
                            assign int-ds-pedido.situacao = 3.
                        run gera-log. 
                        return.
                    end.
               end.
   ...
   if not avail natur-oper then do:
       assign i-erro = 21
              c-informacao = "Pedido: " + c-nr-pedcli + " Tipo Ped.: " + 
                             string(C-TP-PEDIDO) + " Natur. Oper.: " + c-natur + " UF Destino: " + 
...
procedure cria-tt-ped-item-prs.
   ...
   assign tt-ped-item-prs.numero-lote   = c-numero-lote
          ...
          tt-ped-item-prs.nr-docum      = if c-tp-pedido <> "48" then c-nr-docum else ""
    ...
    for each tt-notas-geradas:
        for first nota-fiscal exclusive-lock where 
            ...
            FOR FIRST int-ds-pedido WHERE
                      int-ds-pedido.ped-codigo-n = INT(tt-ped-venda-prs.nr-pedcli) NO-LOCK: 
            END.
            IF AVAIL int-ds-pedido THEN DO:

               IF int-ds-pedido.ped-tipopedido-n <> 33 THEN
                 ASSIGN nota-fiscal.docto-orig = c-docto-orig.

               IF int-ds-pedido.cod-model-ecf   <> "?" AND 
                  int-ds-pedido.cod-fabricc-ecf <> "?" THEN DO:
                  ...
                  IF NOT AVAIL nota-fisc-adc THEN DO:
                     ...
                     ASSIGN nota-fisc-adc.cod-estab      = nota-fiscal.cod-estabel  
                            ...
                            nota-fisc-adc.num-seq        = 1.
                  END.
                  ASSIGN nota-fisc-adc.cod-model-ecf          = int-ds-pedido.cod-model-ecf
                         nota-fisc-adc.cod-fabricc-ecf        = int-ds-pedido.cod-fabricc-ecf
                         nota-fisc-adc.cod-cx-ecf             = int-ds-pedido.cod-cx-ecf
                         nota-fisc-adc.cod-docto-referado-ecf = int-ds-pedido.cod-docto-referado-ecf
                         nota-fisc-adc.dat-docto-referado-ecf = int-ds-pedido.dat-docto-referado-ecf.
               END.
            END.
        end.
    end.
...
/* Atualiza as quantidades na nota de origem e gera uma nota de entrada com uma 
   serie espec­fica para devolu»’o cria um registro no int0002 e atualiza automaticamente no re1001 */
    ...
    FIND FIRST int-ds-docto-xml NO-LOCK WHERE
               int-ds-docto-xml.nnf           = tt-nota-origem.nro-docto     AND
               int-ds-docto-xml.serie         = tt-nota-origem.serie         AND 
               int-ds-docto-xml.tipo-nota     = tt-nota-origem.tipo-nota     AND 
               int-ds-docto-xml.nat-operacao  = tt-nota-origem.nat-operacao  AND 
               int-ds-docto-xml.cod-emitente  = tt-nota-origem.cod-emitente  NO-ERROR.
    IF AVAIL int-ds-docto-xml THEN DO:
        ...
        ASSIGN tt-docto-xml.serie    = c-serie-devol                           
               ...
               tt-docto-xml.vNF      = int-ds-pedido.ped-valortotalbruto-n
               tt-docto-xml.dt-trans = TODAY 
               tt-docto-xml.dEmi     = TODAY.

        FIND FIRST emitente NO-LOCK WHERE
                   emitente.cod-emitente = int-ds-docto-xml.cod-emitente NO-ERROR.

        FIND FIRST param-estoq NO-LOCK NO-ERROR. 
                          
        FOR EACH int-ds-pedido-produto NO-LOCK WHERE
                 int-ds-pedido-produto.ped-codigo-n = int-ds-pedido.ped-codigo-n,
            EACH  int-ds-pedido-retorno OF int-ds-pedido-produto NO-LOCK:

            ASSIGN c-numero-lote = if trim(int-ds-pedido-retorno.rpp-lote-s) <> "0" and 
                                      trim(int-ds-pedido-retorno.rpp-lote-s) <> "?" and
                                      trim(int-ds-pedido-retorno.rpp-lote-s) <> ? then trim(int-ds-pedido-retorno.rpp-lote-s) else "".
            ...
            FIND FIRST int-ds-it-docto-xml EXCLUSIVE-LOCK WHERE
                       ...
                       int-ds-it-docto-xml.it-codigo    = string(int-ds-pedido-produto.ppr-produto-n) NO-ERROR.
            IF AVAIL int-ds-it-docto-xml 
            THEN DO:
                IF int-ds-pedido-produto.ppr-quantidade-n >= int-ds-it-docto-xml.qcom 
                THEN ASSIGN int-ds-it-docto-xml.situacao = 4. /* Devolvida */
                ELSE ASSIGN int-ds-it-docto-xml.qcom = int-ds-it-docto-xml.qcom - int-ds-pedido-produto.ppr-quantidade-n.
                ...
                ASSIGN tt-it-docto-xml.serie    = c-serie-devol
                       ...
                       tt-it-docto-xml.qcom     = int-ds-pedido-produto.ppr-quantidade-n
            ...
            FIND FIRST tt-nota WHERE
                       tt-nota.nro-docto    = int-ds-docto-xml.nnf           AND 
                       tt-nota.serie-nota   = c-serie-devol /* serie devolucao */    AND 
                       tt-nota.nat-operacao = int-ds-docto-xml.nat-operacao  AND 
                       tt-nota.cod-emitente = emitente.cod-emitente NO-ERROR.
            IF NOT AVAIL tt-nota 
            THEN DO:
        
                CREATE tt-nota.
                ASSIGN tt-nota.nro-docto    = int-ds-docto-xml.nnf 
                       tt-nota.serie-nota   = c-serie-devol
                       tt-nota.serie-docum  = c-serie-devol
                       tt-nota.nat-operacao = int-ds-docto-xml.nat-operacao                                    
                ...
                assign tt-docum-est.nro-docto    = tt-nota.nro-docto
                       ...
                       tt-docum-est.nat-operacao = int-ds-docto-xml.nat-operacao                              
                       tt-docum-est.cod-observa  = if natur-oper.log-2 then 2 /* Comercio */ else 1  /* Industria*/
                       tt-docum-est.cod-estabel  = int-ds-docto-xml.cod-estab
                       tt-docum-est.estab-fisc   = int-ds-docto-xml.cod-estab
                       tt-docum-est.estab-de-or  = int-ds-docto-xml.estab-de-or  
                       ...
                       tt-docum-est.observacao   = "Devolu»’o nota : " + int-ds-docto-xml.nnf + "/" + int-ds-docto-xml.serie 
                       tt-docum-est.valor-mercad = 0 
                       tt-docum-est.tot-valor    = int-ds-pedido.ped-valortotalbruto-n
                       tt-docum-est.conta-transit = param-estoq.conta-fornec
                       OVERLAY(tt-docum-est.char-1,93,60)  = IF int-ds-docto-xml.chNfe = ? THEN "" ELSE int-ds-docto-xml.chNfe
            ...
            IF AVAIL tt-nota THEN DO:
                 ...
                 IF AVAIL int-ds-it-docto-xml THEN DO:
                     ...
                     FIND FIRST item-uni-estab NO-LOCK WHERE
                                item-uni-estab.cod-estabel = int-ds-docto-xml.cod-estab AND
                     ...
                     assign tt-item-doc-est.encerra-pa     = no
                            ...
                            tt-item-doc-est.quantidade     = int-ds-pedido-produto.ppr-quantidade-n
                            tt-item-doc-est.pre-uni[1]     = int-ds-pedido-produto.ppr-valorbruto-n 
        ...
        FOR EACH int-ds-doc-erro WHERE
                 ...
                 int-ds-doc-erro.nro-docto    = tt-nota.nro-docto    AND
                 int-ds-doc-erro.tipo-nota    = tt-nota.tipo-nota   :
            ...
            FOR EACH int-ds-doc-erro WHERE
                 ...
                 assign i-erro = 22 
                        c-informacao = "Estab.: " + c-cod-estabel + " - " + "Pedido: " + c-nr-pedcli + " - " +
                                       ...
                                       "Cod. Erro: " + string(int-ds-doc-erro.cod-erro) + " - " +  
                                       int-ds-doc-erro.descricao
                        l-movto-com-erro = yes.
                 run gera-log. 
                 return.    
            END.
               ...
               THEN DO:
                   ...
                   THEN DO:
                        ...
                        FIND FIRST int-ds-docto-xml NO-LOCK WHERE
                                   ...
                                   int-ds-docto-xml.cod-emitente = tt-docto-xml.cod-emitente AND 
                                   int-ds-docto-xml.nat-operacao = tt-docto-xml.nat-operacao AND 
                                   int-ds-docto-xml.tipo-nota    = tt-docto-xml.tipo-nota NO-ERROR.
                        ...
                        THEN DO:
                            ...
                            FOR EACH tt-docto-xml:
                                ...
                                BUFFER-COPY tt-docto-xml TO int-ds-docto-xml
                                    ASSIGN int-ds-docto-xml.id_sequencial = ?.
                                /*ASSIGN int-ds-docto-xml.id_sequencial = NEXT-VALUE(seq-int-ds-docto-xml) /* Prepara‡Æo para integra‡Æo com Procfit */*/
                                .
