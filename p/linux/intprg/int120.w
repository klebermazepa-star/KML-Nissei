...
*******************************************************************************/
{include/i-prgvrs.i INT120 1.12.00.AVB}
...
DEFINE VARIABLE h_int120-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int120-viewer AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Modalidade Pedido"
         HEIGHT             = 5.92
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Modalidade Pedido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Modalidade Pedido */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int120-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int120-viewer ).
       RUN set-position IN h_int120-viewer ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.83 , 86.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int120-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int120-zoom.w,
                     ProgVaPara = intprg/int120-vapara.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_int120-query ).
       RUN set-position IN h_int120-query ( 2.50 , 83.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_int120-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int120-query , 'Record':U , h_int120-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_int120-viewer ).

       /* Links to SmartQuery h_int120-query. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int120-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int120-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int120-query ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int120-viewer ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT120" "1.12.00.AVB"}
