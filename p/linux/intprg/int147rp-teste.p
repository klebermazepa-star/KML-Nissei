/********************************************************************************
** Programa: INT147 - Gera‡Æo notas fiscais 1 NF Caixa de Pedidos do PRS
**
** Versao : 12 - 10/09/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT147RP 2.12.04.AVB}
    ...
    for each int-ds-pedido no-lock where
         ...
         /*    int-ds-pedido.situacao     = 1 and*/
            (int-ds-pedido.ped-tipopedido-n = 2 /* Retirada LJ->CD */ or
             ...
             int-ds-pedido.ped-tipopedido-n = 16 or /* DEVOLUCAO CD - FORNECEDOR */
             ...
             int-ds-pedido.ped-tipopedido-n = 46 or /* RETIRADA FILIAL - PROPRIA FILIAL */
             /* procfit */
             int-ds-pedido.ped-tipopedido-n = 21 ) /*TRANSFERENCIA USO E CONSUMO FILIAL Î FILIAL*/

        and int-ds-pedido.ped-codigo-n =    59614908
        ...
        for each int-ds-pedido-produto no-lock of int-ds-pedido,
            ...
            assign  c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                    ...
                    c-nr-docum          = if int-ds-pedido-produto.nen-notafiscal-n <> ? and
                                             ...
                                             int-ds-pedido.ped-tipopedido-n <> 19 and
                                             int-ds-pedido.ped-tipopedido-n <> 21
...
procedure valida-registro-pedido.
    ...
    for each estabelec 
        ...
        DISPLAY int-ds-pedido.ped-cnpj-origem-s
                c-cod-estabel 
                c-uf-origem   
                c-serie       
   ...
   then do:  
       ...
       for last item-doc-est 
        ...
        /* item-doc-est.sequencia    = i-seq-docum and */
           ...
           FOR FIRST docum-est NO-LOCK OF item-doc-est:
               DISPLAY docum-est.cod-estabel " <<<-docto".
               FOR FIRST estabelec NO-LOCK WHERE estabelec.cod-estabel = docum-est.cod-estabel:
                   DISPLAY estabelec.cgc.
               END.
           END.
...
procedure emite-nota.
    ...
    for each tt-ped-venda-prs where 
        ...
        for first wt-docto where
            ...
            display wt-docto.cod-estabel
                    wt-docto.cod-emitente
                    wt-docto.nome-abrev
                    wt-docto.nome-transp
                    wt-docto.ind-tip-nota
...
procedure pi-devolucoes:
    ...
    display tt-ped-item-prs.it-codigo
            tt-ped-item-prs.serie-docum
            tt-ped-item-prs.nr-docum
            tt-ped-item-prs.nat-docum
            tt-ped-venda-prs.nome-abrev
            tt-ped-venda-prs.cod-emitente
            tt-ped-venda-prs.cod-estabel
        with width 300 stream-io.

    for first wt-docto no-lock where wt-docto.seq-wt-docto = i-seq-wt-docto:
        display wt-docto.cod-emitente 
                wt-docto.nome-abrev
...
procedure pi-calcula-nota:
   ...
   /*if tt-ped-item-prs.nr-docum <> "" then l-recal-apenas-totais = yes.*/
   run recebeVariavelTipoCalculoImpostos   in h-bodi317im1bra (input if l-recal-apenas-totais 
                                                                     then 1
                                                                     else 0,
                                                                     output l-proc-ok-aux).

    DISPLAY "tipo calcula: " l-recal-apenas-totais WITH STREAM-IO.
