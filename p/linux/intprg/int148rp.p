/********************************************************************************
** Programa: INT148 - Gera‡Æo notas fiscais devolu‡Æo Pedidos do PRS Tipo 16
**
** Versao : 12 - 22/11/2017 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT148RP 2.12.04.AVB}
...
procedure pi-importa:
    ...
    for each int-ds-mod-pedido no-lock where int-ds-mod-pedido.cod_prog_dtsul = "int148",
        ...
        for each int-ds-pedido no-lock where
            ...
            int-ds-pedido.ped-tipopedido-n = /*16 /* DEVOLUCAO CD - FORNECEDOR */ and*/
                                             int(int-ds-tipo-pedido.tp-pedido) and
            int-ds-pedido.ped-codigo = tt-param.pedido
            ...
            for each int-ds-pedido-produto no-lock of int-ds-pedido,
                ...
                if first-of(int-ds-pedido-retorno.ped-codigo-n) then do:
                    assign  c-observacao        = trim(c-observacao + " Pedido: " + string(int-ds-pedido.ped-codigo-n,">>>>>>>>999")) + " S‚rie/N£mero NF Origem:".
                end.
                c-numero-caixa = if int-ds-pedido-retorno.rpp-caixa-n <> ? then string(int-ds-pedido-retorno.rpp-caixa-n) else "".
                if first-of(int-ds-pedido-retorno.rpp-caixa-n) then do:
                    if c-caixa-grupo = "" then 
                        c-caixa-grupo = c-numero-caixa.
                    else 
                        c-caixa-grupo = c-caixa-grupo + "," + c-numero-caixa.
                end.
                ...
                assign  c-it-codigo         = trim(string(int-ds-pedido-produto.ppr-produto-n))
                        ...
                        c-nr-docum          = if int-ds-pedido-produto.nen-notafiscal-n <> ? and
                                                 int-ds-pedido-produto.nen-notafiscal-n <> 0 
                                              then trim(string(int-ds-pedido-produto.nen-notafiscal-n,">>9999999")) else ""
                        c-serie-docum       = if int-ds-pedido-produto.nen-serie-s <> ?
                                              then int-ds-pedido-produto.nen-serie-s else ""

                        i-seq-docum         = if int-ds-pedido-produto.nep-sequencia-n <> ?
                ...
                if last (int-ds-pedido-retorno.ped-codigo) then do:
            ...
            run finaliza-pedido.
        end. /* for each int-ds-pedido */
    end. /* for each int-ds-mod-pedido */
end. /* pi-importa */

procedure finaliza-pedido.
    run pi-acompanhar in h-acomp (input "Finalizando Pedido").
    if tt-param.acao = "Simula" then return.
    if not l-movto-com-erro then do:
        de-quantidade = 0.
        for each int-ds-pedido-produto no-lock of int-ds-pedido,
            each int-ds-pedido-retorno of int-ds-pedido-produto
...
procedure valida-registro-item.
   ...
   then do:  
       ...
       for first tt-item-doc-est where 
           ...
           c-nat-docum = tt-item-doc-est.nat-operacao.
...
procedure cria-tt-ped-venda-prs.
   ...
   assign tt-ped-venda-prs.cod-estabel    = c-cod-estabel
          ...
          tt-ped-venda-prs.numero-caixa   = c-caixa-grupo
...
procedure cria-tt-ped-item-prs.
   ...
   for first tt-ped-item-prs where
       ...
       tt-ped-item-prs.nat-docum    = c-nat-docum                 and
