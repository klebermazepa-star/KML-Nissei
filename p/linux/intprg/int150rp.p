...
{include/i-rpout.i}
{utp/ut-liter.i Exclusao_Contingencia_LF0203 * L}
...
RUN pi-inicializar IN h-acomp (INPUT "ExclusÆo Contingˆncia do LF0203").
...
PROCEDURE processamentoInformacoes:
    ...
    OUTPUT STREAM stream-a TO VALUE(diretorio + "dwf-docto-exc.d")                  CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-b TO VALUE(diretorio + "dwf-docto-compl-exc.d")            CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-c TO VALUE(diretorio + "dwf-docto-cupom-referado-exc.d")   CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-d TO VALUE(diretorio + "dwf-docto-fatur-exc.d")            CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-e TO VALUE(diretorio + "dwf-docto-fatur-parc-exc.d")       CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-f TO VALUE(diretorio + "dwf-docto-fisc-refer-exc.d")       CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-g TO VALUE(diretorio + "dwf-docto-item-exc.d")             CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-h TO VALUE(diretorio + "dwf-docto-item-imp-analit-exc.d")  CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-i TO VALUE(diretorio + "dwf-docto-item-impto-exc.d")       CONVERT TARGET "iso8859-1".
    OUTPUT STREAM stream-j TO VALUE(diretorio + "dwf-docto-transp-exc.d")           CONVERT TARGET "iso8859-1".
    ...
    FOR EACH dwf-docto 
       WHERE dwf-docto.cod-estab      >= tt-param.c-estab-ini
         ...
         AND dwf-docto.cod-serie      >= tt-param.c-serie-ini 
         AND dwf-docto.cod-serie      <= tt-param.c-serie-fim
         ...
         QUERY-TUNING(NO-LOOKAHEAD) :
           ...
           RUN pi-acompanhar IN h-acomp (INPUT STRING(dwf-docto.cod-estab)    + " | " +
                ...
                PUT UNFORMATTED
                     dwf-docto.cod-estab        space
                     dwf-docto.cod-serie        space
                     dwf-docto.cod-docto        space
                     dwf-docto.cod-emitente     space
                     dwf-docto.cod-natur-operac space
                     SUBSTR(dwf-docto.cod-chave-aces-nf-eletro,23,03) FORMAT "X(03)" 
                     SUBSTR(dwf-docto.cod-chave-aces-nf-eletro,28,07) FORMAT "X(09)" SKIP.
                ...
                FOR EACH  dwf-docto-cupom-referado
                    ...
                    DELETE dwf-docto-cupom-referado.
                END.
...
PROCEDURE criaDiretorioBackup:
    ...
    ASSIGN diretorio = diretorio
                     ...
                     + "-ate-" +  REPLACE(STRING(tt-param.c-data-fim,"99/99/9999"),"/","-") + "_Exclusao\".
