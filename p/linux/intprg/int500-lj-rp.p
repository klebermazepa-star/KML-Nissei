/******************************************************************************/
{include/i-prgvrs.i int500-lj-rp 2.06.00.001}  
{include/i_fnctrad.i}
/******************************************************************************
**
**       PROGRAMA: INT500-LJ-RP
**
**       DATA....: 11/2015
**
**       OBJETIVO: Integraá∆o NDD - XML Recebimento - LOJA 
**
**       VERSAO..: 2.06.001
** 
******************************************************************************/
...
assign c-programa = "int500-lj-rp"
       c-versao   = "2.06"
       c-revisao  = "001"
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK  WHERE    
        ...
        IF VALID-HANDLE(h-acomp) THEN
           RUN pi-acompanhar IN h-acomp(INPUT "Processando Loja: " + string(NDD_ENTRYINTEGRATION.ENTRYINTEGRATIONID)).
        ...
        */
        ...
        IF VALID-HANDLE(h-acomp) THEN
           RUN pi-acompanhar IN h-acomp(INPUT "Integracao Loja: " + STRING(NDD_ENTRYINTEGRATION.ENTRYINTEGRATIONID)).
...
PROCEDURE pi-gera-transf:
      ...
      for each it-nota-fisc of nota-fiscal no-lock query-tuning(no-lookahead):
          ...
          IF VALID-HANDLE(h-acomp) THEN
              RUN pi-acompanhar IN h-acomp(INPUT "Nota Loja: " + string(nota-fiscal.nr-nota-fis)).
