...
PROCEDURE pi-notas-pepsico:
   ...
   FOR EACH int-ds-docto-xml WHERE
         ...
         FOR EACH int-ds-it-docto-xml WHERE 
               ...
               CREATE tt-it-docto-xml.
               BUFFER-COPY int-ds-it-docto-xml TO tt-it-docto-xml.
               ASSIGN tt-it-docto-xml.qCom-forn  = int-ds-it-docto-xml.qCom.
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK  WHERE  
        ...
        AND DOCUMENTNUMBER =  1075 AND serie = 3       

        /*AND NDD_ENTRYINTEGRATION.CNPJEMIT = 79430682000122   
        AND NDD_ENTRYINTEGRATION.CNPJdest = 79430682025540 
        AND NDD_ENTRYINTEGRATION.SERIE    = 1
                  */
        ...
        /*IF NDD_ENTRYINTEGRATION.serie = 10 THEN NEXT. - e OS FORNECEDORES COM SRIE 10???*/
        ...
        THEN do:
            ...
            MESSAGE  tt-docto-xml.dEmi VIEW-AS ALERT-BOX.
            ...
            THEN do:
                ...
                MESSAGE tt-docto-xml.tipo-nota SKIP
                        tt-docto-xml.tipo-estab SKIP
                    tt-docto-xml.CNPJ   VIEW-AS ALERT-BOX.

                IF tt-docto-xml.tipo-nota  = 3  AND  /* NFT */
                   tt-docto-xml.tipo-estab = 2   /* CD */
                THEN DO:     
                    assign c-estab-orig = "".
                    for first b-estab no-lock where
                        ...
                        MESSAGE c-estab-orig VIEW-AS ALERT-BOX.
                    end.
                    for first b-estab no-lock where
    ...
    for first ttIde ON ERROR UNDO, RETURN ERROR:
        ...
        for first ttDest:
             ...
             IF AVAIL estabelec THEN DO:
                 ...
                 MESSAGE "estab" SKIP estabelec.cod-estabel VIEW-AS ALERT-BOX.
