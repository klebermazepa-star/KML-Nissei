...
procedure pi-grava-nota:       
    ...
    FOR EACH tt-it-docto-xml NO-LOCK:
        DISPLAY " 500 " tt-it-docto-xml.picms.
    END.

    /* Integrar notas direto com o PRS gerando int-ds-nota-entrada */
    RUN intprg/int002g_.p (INPUT TABLE tt-param,
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK  WHERE    
       ...
       AND NDD_ENTRYINTEGRATION.documentnumber = 89372
    ...
    for first ttIde ON ERROR UNDO, RETURN ERROR:
        ...
        FOR EACH ttprod 
           ...
           for first ttICMS00 WHERE
               ...
               DISPLAY 
                   tt-it-docto-xml.serie        
                   tt-it-docto-xml.nNF          
                   tt-it-docto-xml.CNPJ         
                   tt-it-docto-xml.sequencia
                   trim(tt-it-docto-xml.xProd) COLUMN-LABEL "xProd"
                   de-picms ttICMS00.picms.
           END. /* ttICMS00*/
