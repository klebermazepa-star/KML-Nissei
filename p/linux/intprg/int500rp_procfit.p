...
DEF VAR l-erro-xml     AS LOGICAL NO-UNDO.     
    ...
    FOR EACH tt-docto-xml USE-INDEX tp-estab-emis WHERE    
        ...
        IF l-altera = NO THEN DO:
          ...
          ASSIGN int-ds-docto-xml.id_sequencial = /*NEXT-VALUE(seq-int-ds-docto-xml)*/ ?
                 int-ds-docto-xml.envio_status  = 1
                 int-ds-docto-xml.envio_data    = datetime(today).
        END.
        ...
        FOR EACH tt-it-docto-xml WHERE
            ...
            FOR FIRST natur-oper NO-LOCK 
                WHERE natur-oper.nat-operacao = int-ds-it-docto-xml.nat-operacao: 

                IF  natur-oper.log-contrib-st-antec
                THEN
                    ASSIGN int-ds-it-docto-xml.vicmsst    = int-ds-it-docto-xml.vicmsst    - int-ds-it-docto-xml.vicms
                           int-ds-docto-xml.base-guia-st  = int-ds-docto-xml.base-guia-st  + int-ds-it-docto-xml.vbcst 
                           int-ds-docto-xml.valor-guia-st = int-ds-docto-xml.valor-guia-st + int-ds-it-docto-xml.vicmsst
                           int-ds-docto-xml.vbc-cst       = int-ds-docto-xml.base-guia-st 
            ...
            THEN DO:
               ...
               IF AVAIL ITEM-fornec THEN DO: 
                 ...
                 ASSIGN int-ds-it-docto-xml.qCom-forn = tt-it-docto-xml.qCom-forn
            ...
            IF int-ds-it-docto-xml.lote = "" 
            OR int-ds-it-docto-xml.lote = ? THEN DO:
               FOR FIRST rat-lote WHERE
                         rat-lote.serie-docto  = int-ds-it-docto-xml.serie        AND
                         rat-lote.nro-docto    = int-ds-it-docto-xml.nnf          AND
                         rat-lote.cod-emitente = int-ds-it-docto-xml.cod-emitente AND
                         rat-lote.sequencia    = int-ds-it-docto-xml.sequencia    NO-LOCK:
               END.
               IF AVAIL rat-lote THEN DO:
                  FOR FIRST natur-oper WHERE
                            natur-oper.nat-operacao = rat-lote.nat-operacao NO-LOCK:
                  END.
                  IF AVAIL natur-oper THEN DO:
                     IF natur-oper.imp-nota THEN DO:
                        ASSIGN int-ds-it-docto-xml.lote = rat-lote.lote.
                     END.
                  END.
               END.
            END.
        ...
        THEN DO:
           ...
           ASSIGN int-ds-docto-wms.doc_numero_n   = INT(tt-docto-xml.nnf)
                  ...
                  int-ds-docto-wms.tipo-nota      = tt-docto-xml.tipo-nota
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENTRYINTEGRATION NO-LOCK  WHERE    
        ...
        */
        ...
        IF l-erro-xml = YES THEN DO:
           RUN intprg/int999.p (INPUT "NFENDD", 
                                INPUT string(NDD_ENTRYINTEGRATION.DOCUMENTNUMBER) + " / " + string(NDD_ENTRYINTEGRATION.SERIE) + " / " + 
                                      STRING(NDD_ENTRYINTEGRATION.EMISSIONDATE,"99/99/9999") + " / " + STRING(NDD_ENTRYINTEGRATION.CNPJEMIT),
                                INPUT "Documento n∆o processado. Vers∆o do XML da nota diferente de 3.10",
        ...
        THEN do:
            ...
            THEN do:
                l-grava = no.
                /* c¢digo transferido para int011 - Baccin - Procfit
    ...
    for first ttIde ON ERROR UNDO, RETURN ERROR:
        ...
        FOR EACH ttprod 
           ...
           ASSIGN tt-it-docto-xml.cst-pis    = 0
            ...
            FOR FIRST natur-oper NO-LOCK 
                WHERE natur-oper.nat-operacao = tt-it-docto-xml.nat-operacao: 

                IF  natur-oper.log-contrib-st-antec
                THEN DO:
                    RUN intprg\int510-icms-st.p (INPUT tt-it-docto-xml.cod-emitente,
                                                 INPUT tt-it-docto-xml.it-codigo,   
                                                 INPUT tt-it-docto-xml.nat-operacao,
                                                 INPUT tt-docto-xml.cod-estab,        
                                                 INPUT tt-it-docto-xml.qCom,        
                                                 INPUT tt-it-docto-xml.vprod,    
                                                 INPUT tt-it-docto-xml.vdesc,
                                                 INPUT int-ds-it-docto-xml.vipi,
        ...
        for each ttDup:
            ...
            assign  tt-xml-dup.InfNFe    = ttDup.ttInfNFe
                    ...
                    tt-xml-dup.cnpj-cpf  = tt-docto-xml.CNPJ.
        end. /* ttDup */
        ...
        THEN DO:
            FOR FIRST b-tt-xml-dup 
                WHERE b-tt-xml-dup.serie        = tt-docto-xml.serie        
                  ...
                  AND b-tt-xml-dup.cod-emitente = tt-docto-xml.cod-emitente 
                  AND b-tt-xml-dup.tipo-nota    = tt-docto-xml.tipo-nota:
                ...
                ASSIGN tt-xml-dup.serie        = tt-docto-xml.serie        
                       ...
                       tt-xml-dup.cnpj-cpf     = b-tt-xml-dup.CNPJ .
            END.
        END.
