...
procedure pi-grava-nota:       
    ...
    /* grvar int-ds-docto-xml p/ notas destino = CD 
    FOR EACH tt-docto-xml USE-INDEX tp-estab-emis WHERE    
             tt-docto-xml.tipo-estab    = 1 /* CD */ AND  
        ...
        IF VALID-HANDLE(h-acomp) THEN
           RUN pi-acompanhar IN h-acomp(INPUT "Processando: " + string(NDD_ENRYINTEGRATION_NEW_LEG.ENTRYINTEGRATIONID)).
...
PROCEDURE pi-notas-pepsico:
   ...
   FOR EACH int-ds-docto-xml NO-LOCK WHERE
         ...
         END. 
   END.
   /* gravar intds-docto-xml e int-ds-nota-entrada */
   run pi-grava-nota. 
END.
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH NDD_ENRYINTEGRATION_NEW_LEG NO-LOCK  WHERE  
             ...
             NDD_ENRYINTEGRATION_NEW_LEG.status_ = 0 /* campo status pendente */
                /*
        AND DOCUMENTNUMBER = 75613         
        AND NDD_ENRYINTEGRATION_NEW_LEG.CNPJEMIT = 79430682000122   
        AND NDD_ENRYINTEGRATION_NEW_LEG.CNPJdest = 79430682025540 
        AND NDD_ENRYINTEGRATION_NEW_LEG.SERIE    = 1
                  */
                ...
                THEN DO:     
                    ...
                    for first b-estab no-lock where
                        ...
                        for first nota-fiscal no-lock where
                                  ...
                                  nota-fiscal.nr-nota-fis  = string(int(tt-docto-xml.nnf),"9999999") and
...
PROCEDURE pi-abre-xml:
    ...
    for first ttIde:
        ...
        then do:
            ...
            /*
            if not can-find(first int-ds-nota-entrada no-lock where 
                int-ds-nota-entrada.nen-cnpj-origem-s = tt-docto-xml.CNPJ and
                int-ds-nota-entrada.nen-notafiscal-n = int(tt-docto-xml.nnf) and
                int-ds-nota-entrada.nen-serie-s = trim(string(tt-docto-xml.serie)) and
                int-ds-nota-entrada.situacao = 2) then do:
...
PROCEDURE pi-gera-transf:
      ...
      ASSIGN tt-docto-xml.tot-desconto = nota-fiscal.vl-desconto  /*  Desconto total da nota fiscal  */          
             ...
             tt-docto-xml.observacao   = nota-fiscal.observ-nota + chr(13) + "NDDID: " + trim(substring(tt-docto-xml.arquivo,1,16))  /* Observacao da nota fiscal */
