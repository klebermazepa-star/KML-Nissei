...
**       DATA....: 11/2015
**
**       OBJETIVO: Integraá∆o NDD - XML Recebimento - Preencher pedido
**
**       VERSAO..: 2.06.001
** 
******************************************************************************/
...
DEF BUFFER b-int-ds-nota-entrada   FOR int-ds-nota-entrada.

find first param-global no-lock no-error.
find first mgadm.empresa where 
           empresa.ep-codigo = param-global.empresa-prin no-lock no-error.

assign c-programa = "INT500rp_sempedido"
       c-versao   = "2.06"
       c-revisao  = "001"
...
PROCEDURE pi-IntegracaoNDD:
    ...
    FOR EACH b-int-ds-nota-entrada WHERE 
        b-int-ds-nota-entrada.ped-codigo-n = 0 AND 
        b-int-ds-nota-entrada.situacao = 1,
        ...
        AND DOCUMENTNUMBER = b-int-ds-nota-entrada.nen-notafiscal-n
        AND NDD_ENTRYINTEGRATION.CNPJEMIT = decimal(b-int-ds-nota-entrada.nen-cnpj-origem-s)
        ...
        IF  AVAIL tt-docto-xml THEN do:
            display b-int-ds-nota-entrada.nen-cnpj-origem-s
                    b-int-ds-nota-entrada.nen-serie-s
                    b-int-ds-nota-entrada.nen-notafiscal-n
                    b-int-ds-nota-entrada.nen-dataemissao-d
                with width 300 stream-io down.
            for first ttCompra:
                IF ttCompra.xped <> "" THEN DO:
                    IF ttCompra.xped = "PBM" THEN
                        ASSIGN b-int-ds-nota-entrada.ped-codigo-n = ?.
                    ELSE
                        ASSIGN b-int-ds-nota-entrada.ped-codigo-n = decimal(ttCompra.xped) NO-ERROR.
                END.
            end.
            IF b-int-ds-nota-entrada.ped-codigo-n = 0 OR  
               b-int-ds-nota-entrada.ped-codigo-n = ? THEN
            FOR FIRST ttprod NO-LOCK WHERE ttProd.xPed <> "":
                IF ttProd.xped = "PBM" THEN
                    ASSIGN b-int-ds-nota-entrada.ped-codigo-n = ?.
                ELSE
                    ASSIGN b-int-ds-nota-entrada.ped-codigo-n = decimal(ttProd.xped) NO-ERROR.
            END.
            IF b-int-ds-nota-entrada.ped-codigo-n <> 0 THEN
            FOR EACH int-ds-nota-entrada-produto OF b-int-ds-nota-entrada:
                ASSIGN int-ds-nota-entrada-produto.ped-codigo-n = b-int-ds-nota-entrada.ped-codigo-n.
            END.
            DISPLAY b-int-ds-nota-entrada.ped-codigo-n.
            IF b-int-ds-nota-entrada.ped-codigo-n <> 0 THEN
                ASSIGN b-int-ds-nota-entrada.dt-geracao = TODAY
                       b-int-ds-nota-entrada.hr-geracao = string(TIME,"HH:MM:SS").
