...
*******************************************************************************/
{include/i-prgvrs.i INT510d3 1.12.01.AVB}
...
DEF OUTPUT PARAM p-des-motivo AS CHAR NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS rt-button RECT-24 ed-motivo bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS ed-motivo 
...
DEFINE VARIABLE ed-motivo AS CHARACTER 
     VIEW-AS EDITOR MAX-CHARS 100 SCROLLBAR-VERTICAL
     SIZE 66 BY 5 TOOLTIP "Justificativa para movimenta‡Æo" NO-UNDO.
...
DEFINE FRAME d-vapara
     ed-motivo AT ROW 2 COL 2 NO-LABEL WIDGET-ID 10
     bt-ok AT ROW 7.75 COL 2.14
     bt-cancela AT ROW 7.75 COL 13
     bt-ajuda AT ROW 7.75 COL 58
     " Motivo" VIEW-AS TEXT
          SIZE 8 BY .67 AT ROW 1 COL 2 WIDGET-ID 14
     rt-button AT ROW 7.5 COL 1
     RECT-24 AT ROW 1.5 COL 1 WIDGET-ID 12
...
ON GO OF FRAME d-vapara /* Motivo Aceite */
DO:
    IF ed-motivo:SCREEN-VALUE  IN FRAME {&FRAME-NAME} = "" then do:

        RUN utp/ut-msgs.p(INPUT "show",
                          INPUT 17006,
                          INPUT "Informe o motivo pelo qual a nota deve ser aceita.").
        RETURN NO-APPLY.
    end.

    ASSIGN p-des-motivo = ed-motivo:SCREEN-VALUE  IN FRAME {&FRAME-NAME}.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* Motivo Aceite */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY ed-motivo 
      WITH FRAME d-vapara.
  ENABLE rt-button RECT-24 ed-motivo bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT510d3" "1.12.01.AVB"}
