...
*******************************************************************************/
{include/i-prgvrs.i INT512-vapara 1.12.01.AVB}
...
DO:
    if c_chaveacesso:screen-value in frame {&FRAME-NAME}  <> "" then do:
        for first int-ds-docto-xml no-lock where 
            int-ds-docto-xml.chnfe = INPUT FRAME {&FRAME-NAME} c_chaveacesso and
            int-ds-docto-xml.situacao = 5 and 
            int-ds-docto-xml.tipo-nota <> 3: end.
        IF NOT AVAILABLE int-ds-docto-xml THEN DO:
            {utp/ut-table.i emsesp int-ds-docto-xml 1}
        ...
        for first int-ds-docto-xml no-lock where 
            int-ds-docto-xml.CNPJ EQ INPUT FRAME {&FRAME-NAME} c_cnpj AND 
            int-ds-docto-xml.serie EQ INPUT FRAME {&FRAME-NAME} c_serie AND 
            integer(int-ds-docto-xml.nNF) EQ INPUT FRAME {&FRAME-NAME} i_notafiscal and
            int-ds-docto-xml.situacao = 5 and 
            int-ds-docto-xml.tipo-nota <> 3: end.
    ...
    ASSIGN p-row-tabela = ROWID(int-ds-docto-xml).
END.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT512-vapara" "1.12.01.AVB"}
