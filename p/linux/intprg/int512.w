...
*******************************************************************************/
{include/i-prgvrs.i INT512 1.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button btAprovar 
...
DEFINE VARIABLE h_int512-browser-duplicatas-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int512-browser-erros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int512-browser-itens-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int512-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int512-viewer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int512-viewer-totais AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     btAprovar AT ROW 1.17 COL 61.43 WIDGET-ID 10
     rt-button AT ROW 1 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 144.29 BY 26.08 WIDGET-ID 100.
...
ON CHOOSE OF btAprovar IN FRAME f-cad /* Aprovar Compras */
DO:
    RUN pi-aprovar-compras in h_int512-viewer.
    if valid-handle (h_int512-viewer) then
        RUN dispatch IN h_int512-viewer ('row-available':U).
    if valid-handle (h_int512-viewer-totais) then
        RUN dispatch IN h_int512-viewer-totais ('row-available':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int512-viewer ).
       RUN set-position IN h_int512-viewer ( 2.46 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 143.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Itens|Duplicatas|Totais|Erros' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 13.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 13.25 , 143.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int512-zoom.w,
                     ProgVaPara = intprg/int512-vapara.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_int512-query ).
       RUN set-position IN h_int512-query ( 13.25 , 137.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.42 , 6.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_int512-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int512-query , 'Record':U , h_int512-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_int512-viewer ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_int512-query. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_int512-query ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int512-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int512-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int512-query ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             btAprovar:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadpai ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int512-viewer ,
             btAprovar:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_int512-viewer , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-browser-itens.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int512b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int512-browser-itens-2 ).
       RUN set-position IN h_int512-browser-itens-2 ( 14.83 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 142.57 ) */

       /* Links to BrowserCadastro2 h_int512-browser-itens-2. */
       RUN add-link IN adm-broker-hdl ( h_int512-query , 'Record':U , h_int512-browser-itens-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int512-browser-itens-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-browser-duplicatas.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int512-browser-duplicatas-2 ).
       RUN set-position IN h_int512-browser-duplicatas-2 ( 15.38 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.25 , 66.00 ) */

       /* Links to BrowserCadastro2 h_int512-browser-duplicatas-2. */
       RUN add-link IN adm-broker-hdl ( h_int512-query , 'Record':U , h_int512-browser-duplicatas-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int512-browser-duplicatas-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-viewer-totais.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int512-viewer-totais ).
       RUN set-position IN h_int512-viewer-totais ( 15.46 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.21 , 141.57 ) */

       /* Links to SmartViewer h_int512-viewer-totais. */
       RUN add-link IN adm-broker-hdl ( h_int512-query , 'Record':U , h_int512-viewer-totais ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int512-viewer-totais ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int512-browser-erros.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int512-browser-erros ).
       RUN set-position IN h_int512-browser-erros ( 14.79 , 2.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.25 , 141.00 ) */

       /* Links to BrowserCadastro2 h_int512-browser-erros. */
       RUN add-link IN adm-broker-hdl ( h_int512-query , 'Record':U , h_int512-browser-erros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int512-browser-erros ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT512" "1.12.01.AVB"}
  ...
  if can-find (first usuar_grp_usuar no-lock where 
      ...
      if can-find (first usuar_grp_usuar no-lock where 
                   usuar_grp_usuar.cod_usuario = c-seg-usuario and
                   usuar_grp_usuar.cod_grp_usuar = "X23") then do:
          assign btAprovar  :sensitive in FRAME {&FRAME-NAME}   = yes.
