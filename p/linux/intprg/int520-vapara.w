...
&Scoped-Define ENABLED-OBJECTS rt-button c_nen_chaveacesso_s ~
c_nen_cnpj_origem_s c_nen_serie_s i_nen_notafiscal_n bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_nen_chaveacesso_s c_nen_cnpj_origem_s ~
c_nen_serie_s i_nen_notafiscal_n 
...
DEFINE VARIABLE c_nen_chaveacesso_s AS CHARACTER FORMAT "x(44)":U 
     LABEL "Chave NFe" 
     VIEW-AS FILL-IN 
     SIZE 47 BY .87 NO-UNDO.

DEFINE VARIABLE c_nen_cnpj_origem_s AS CHARACTER FORMAT "x(14)":U 
     LABEL "CNPJ Origem" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .87 NO-UNDO.

DEFINE VARIABLE c_nen_serie_s AS CHARACTER FORMAT "x(3)":U 
     LABEL "Serie" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .87 NO-UNDO.

DEFINE VARIABLE i_nen_notafiscal_n AS INTEGER FORMAT ">>>,>>>,>>9":U 
     LABEL "Nr. Nota" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_nen_chaveacesso_s AT ROW 1.27 COL 13.5 COLON-ALIGNED WIDGET-ID 2
     c_nen_cnpj_origem_s AT ROW 2.33 COL 13.5 COLON-ALIGNED WIDGET-ID 4
     c_nen_serie_s AT ROW 3.4 COL 13.5 COLON-ALIGNED WIDGET-ID 6
     i_nen_notafiscal_n AT ROW 4.47 COL 13.5 COLON-ALIGNED WIDGET-ID 8
...
DO:
    if c_nen_chaveacesso_s:screen-value in frame {&FRAME-NAME}  <> "" then do:
        for first int_ds_nota_entrada no-lock where 
            int_ds_nota_entrada.nen_chaveacesso_s = INPUT FRAME {&FRAME-NAME} c_nen_chaveacesso_s: end.
        IF NOT AVAILABLE int_ds_nota_entrada THEN DO:
            {utp/ut-table.i custom int_ds_nota_entrada 1}
        ...
        for first int_ds_nota_entrada no-lock where 
            int_ds_nota_entrada.nen_cnpj_origem_s EQ INPUT FRAME {&FRAME-NAME} c_nen_cnpj_origem_s AND 
            int_ds_nota_entrada.nen_serie_s EQ INPUT FRAME {&FRAME-NAME} c_nen_serie_s AND 
            int_ds_nota_entrada.nen_notafiscal_n EQ INPUT FRAME {&FRAME-NAME} i_nen_notafiscal_n: end.
    ...
    ASSIGN p-row-tabela = ROWID(int_ds_nota_entrada).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_nen_chaveacesso_s c_nen_cnpj_origem_s c_nen_serie_s 
          i_nen_notafiscal_n 
      WITH FRAME d-vapara.
  ENABLE rt-button c_nen_chaveacesso_s c_nen_cnpj_origem_s c_nen_serie_s 
         i_nen_notafiscal_n bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
