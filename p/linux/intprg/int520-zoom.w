...
*******************************************************************************/
{include/i-prgvrs.i Z01Nota-Entrada 1.12.01.AVB}
...
DEFINE VARIABLE h_int520-browser-zoom-3 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-ok AT ROW 16.97 COL 3
     bt-cancela AT ROW 16.97 COL 14
     bt-ajuda AT ROW 17 COL 104.38
     rt-button AT ROW 16.73 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Nota de Entrada"
         HEIGHT             = 17.2
         WIDTH              = 117.25
         MAX-HEIGHT         = 18.53
         MAX-WIDTH          = 117.25
         VIRTUAL-HEIGHT     = 18.53
         VIRTUAL-WIDTH      = 117.25
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = yes
         BGCOLOR            = ?
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Nota de Entrada */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Nota de Entrada */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Nota' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.53 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 14.93 , 117.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int520-browser-zoom.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int520-browser-zoom-3 ).
       RUN set-position IN h_int520-browser-zoom-3 ( 3.40 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.53 , 114.00 ) */

       /* Links to SmartBrowser h_int520-browser-zoom-3. */
       RUN add-link IN adm-broker-hdl ( h_int520-browser-zoom-3 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int520-browser-zoom-3 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01Nota-Entrada" "1.12.01.AVB"}
