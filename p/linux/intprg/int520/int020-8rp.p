/********************************************************************************
** Programa: INT020-8RP - Geracao notas fiscais a partir de Cupom Fiscal PRS
**
** Versao : 12 - 20/03/2016 - Alessandro V Baccin
**
********************************************************************************/
/* include de controle de versao */
{include/i-prgvrs.i INT020-8RP 2.12.01.AVB}
{cdp/cdcfgdis.i}
...
def temp-table tt-int-ds-nota-loja-8 like int-ds-nota-loja
...
IF tt-param.arquivo = "" THEN 
    ASSIGN tt-param.arquivo = "INT020-8.txt"
           tt-param.destino = 3
...
procedure pi-importa:
    ...
    EMPTY TEMP-TABLE tt-int-ds-nota-loja-8.

    FOR EACH estabelec USE-INDEX codigo WHERE 
             estabelec.cod-estabel >= int-ds-ger-cupom.cod-estab-ini-8 AND
             estabelec.cod-estabel <= int-ds-ger-cupom.cod-estab-fim-8 NO-LOCK:
        ...
        for each int-ds-nota-loja no-lock USE-INDEX sit_cnpj_emiss where 
            ...
            FOR FIRST tt-int-ds-nota-loja-8 WHERE 
                tt-int-ds-nota-loja-8.rowid-nota-loja = ROWID(int-ds-nota-loja): END.
            IF NOT AVAIL tt-int-ds-nota-loja-8 THEN DO:
                CREATE tt-int-ds-nota-loja-8.
                BUFFER-COPY int-ds-nota-loja TO tt-int-ds-nota-loja-8.
                ASSIGN tt-int-ds-nota-loja-8.rowid-nota-loja = ROWID(int-ds-nota-loja).
            END.

        end.
    END.

    FOR EACH tt-int-ds-nota-loja-8 USE-INDEX data:
        FOR FIRST int-ds-nota-loja WHERE ROWID(int-ds-nota-loja) = tt-int-ds-nota-loja-8.rowid-nota-loja NO-LOCK:
        END.
