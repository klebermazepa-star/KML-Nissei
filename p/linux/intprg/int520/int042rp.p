...
def temp-table tt-movto no-undo
    ...
    field perc-red-icms     like int-ds-nota-saida-item.nsp-redutorbaseicms-n
    ...
    for each int-ds-nota-entrada no-lock where
        ...
        for each estabelec 
            fields (cod-estabel estado cidade cod-emitente
                    cep pais endereco bairro ep-codigo) 
            no-lock where
            estabelec.cgc = int-ds-nota-entrada.nen-cnpj-origem-s,
            first cst-estabelec no-lock where 
            cst-estabelec.cod-estabel = estabelec.cod-estabel and
            cst-estabelec.dt-fim-operacao >= int-ds-nota-entrada.nen-dataemissao-d:
        ...
        if c-cod-estabel-ori = "" then do:
            run pi-gera-log (input "Estabelecimento ORIGEM n∆o cadastrado ou fora de operaá∆o. CNPJ: " + int-ds-nota-entrada.nen-cnpj-origem-s,
                             input 1).
            next.
        end.
        ...
        if int-ds-nota-entrada.tipo-movto = 3 then do:
            run pi-gera-log (input "Nota Fiscal de sa°da est† cancelada " + string(int-ds-nota-entrada.nen-notafiscal-n) + " SÇrie: " + string(int-ds-nota-entrada.nen-serie-s) + " Estab.: " + c-cod-estabel,
                             input 1).
            next.
        end.
        for first docum-est no-lock where 
            ...
            run pi-gera-log (input "Documento j† cadastrado: " 
                             ...
                             input 2).
            for each bint-ds-nota-entrada where
                bint-ds-nota-entrada.nen-cnpj-origem-s  = int-ds-nota-entrada.nen-cnpj-origem-s and
                bint-ds-nota-entrada.nen-serie-s        = int-ds-nota-entrada.nen-serie-s       and
                bint-ds-nota-entrada.nen-notafiscal-n   = int-ds-nota-entrada.nen-notafiscal-n:
                assign  bint-ds-nota-entrada.nen-conferida-n = 2 
                        bint-ds-nota-entrada.situacao = 2 /* processada */.
                release bint-ds-nota-entrada.
            end.
            empty temp-table tt-movto.
            assign l-erro = yes.
            next for-nota.
        ...
        for first int-ds-pedido no-lock where
            int-ds-pedido.ped-codigo-n = int-ds-nota-entrada.ped-codigo-n:
            assign c-tp-pedido = trim(string(int-ds-pedido.ped-tipopedido-n)).
        end.

        for each int-ds-nota-saida-item no-lock where
            ...
            int-ds-nota-saida-item.nsa-notafiscal-n  = int-ds-nota-entrada.nen-notafiscal-n
            ...
            assign tt-movto.serie               = int-ds-nota-entrada.nen-serie-s
                   ...
                   tt-movto.tot-despesas        = IF (int-ds-nota-entrada.nen-despesas-n - int-ds-nota-entrada.nen-frete-n) > 0 
                                                  THEN (int-ds-nota-entrada.nen-despesas-n - int-ds-nota-entrada.nen-frete-n)
                                                  ELSE 0
                   ...
                   tt-movto.perc-red-icms       = int-ds-nota-saida-item.nsp-redutorbaseicms-n
                   tt-movto.cod-cond-pag        = emitente.cod-cond-pag
                   tt-movto.estab-de-or         = c-cod-estabel-ori.
    
            c-class-fiscal = int-ds-nota-saida-item.nsp-ncm-s.
            for first item 
                fields (it-codigo class-fisc tipo-con-est peso-liquido char-1 char-2
                        fm-codigo tipo-contr ct-codigo sc-codigo)
                no-lock where 
                item.it-codigo = string(int-ds-nota-saida-item.nsp-produto-n): 
            end.
            if not avail item then do:
                run pi-gera-log (input "Item n∆o cadastrado: " + string(int-ds-nota-saida-item.nsp-produto-n),
                                 input 1).
                next.
            end.
            if item.tipo-contr = 1 /* Fisico */ or item.tipo-contr = 4 /* DB Dir */ then do:
                if item.ct-codigo = "" then do:
                    run pi-gera-log (input "Item sem conta aplicacao: " + string(int-ds-nota-saida-item.nsp-produto-n),
                                     input 1).
                    next.
                end.
                else do:
            ...
            for first item-uni-estab no-lock where 
                      ...
                      item-uni-estab.it-codigo = string(int-ds-nota-saida-item.nsp-produto-n): 
            end.
            if not avail item-uni-estab THEN DO:
               for first item-estab NO-LOCK where 
                         item-estab.cod-estabel = tt-movto.cod-estabel and
                         item-estab.it-codigo = string(int-ds-nota-saida-item.nsp-produto-n): 
               end.
            END.
            if not avail item-estab and not avail item-uni-estab then do:
               run pi-gera-log (input "Item n∆o cadastrado no estabelecimento. Item: " + string(int-ds-nota-saida-item.nsp-produto-n) + " Estab.: " + tt-movto.cod-estabel,
                                input 1).
               next.
            end.

            for first nota-fiscal 
                fields (cod-estabel serie nr-nota-fis nat-operacao)
                no-lock where 
                ...
                for each it-nota-fisc no-lock of nota-fiscal where
                    it-nota-fisc.nr-seq-fat = int-ds-nota-saida-item.nsp-sequencia-n and
                    it-nota-fisc.it-codigo  = trim(string(int-ds-nota-saida-item.nsp-produto-n)):
            ...
            RUN intprg/int013a.p( input i-cfop,
                                  input int-ds-nota-saida-item.nsp-cstb-n,
                                  input /*int-ds-nota-saida-item.nsp-cstb-ipi-n*/ int(item.fm-codigo),
            ...
            */
            ...
            if c-tp-pedido = "" then do:
                run pi-gera-log (input "Pedido n∆o localizado: " + string(int-ds-nota-entrada.ped-codigo-n),
                                 input 1).
                next.
            end.
            ...
            if c-nat-operacao = "" then do:
                run pi-gera-log (input "N∆o encontrada natureza de operaá∆o para entrada. " + 
                                        "CFOP Nota: " + string(int-ds-nota-saida-item.nsp-cfop-n) + 
                                        " CFOP Calc: " + string(i-cfop) + 
                                        " CSTB ICMS: " + string(int-ds-nota-saida-item.nsp-cstb-n) + 
            ...
            assign tt-movto.it-codigo       = string(int-ds-nota-saida-item.nsp-produto-n)
                   tt-movto.nr-sequencia    = int-ds-nota-saida-item.nsp-sequencia-n
                   tt-movto.qt-recebida     = int-ds-nota-saida-item.nsp-quantidade-n
                   ...
                   tt-movto.lote            = if item.tipo-con-est = 3 then int-ds-nota-saida-item.nsp-lote-s else ""
                   tt-movto.dt-vali-lote    = if item.tipo-con-est = 3 then int-ds-nota-saida-item.nsp-datavalidade-d else ?
                   tt-movto.valor-mercad    = int-ds-nota-saida-item.nsp-valorbruto-n
                   tt-movto.vl-despesas     = int-ds-nota-saida-item.nsp-valordespesa-n
                   tt-movto.cd-trib-ipi     = i-trib-ipi
                   tt-movto.aliquota-ipi    = int-ds-nota-saida-item.nsp-percentualipi-n
                   ...
                   tt-movto.aliquota-icm    = int-ds-nota-saida-item.nsp-percentualicms-n
                   tt-movto.perc-red-icm    = int-ds-nota-saida-item.nsp-redutorbaseicms-n
                   tt-movto.vl-bicms-it     = int-ds-nota-saida-item.nsp-baseicms-n
                   tt-movto.vl-icms-it      = int-ds-nota-saida-item.nsp-valoricms-n
                   tt-movto.vl-bsubs-it     = int-ds-nota-saida-item.nsp-basest-n
                   tt-movto.vl-icmsub-it    = int-ds-nota-saida-item.nsp-icmsst-n
                   tt-movto.peso-liquido    = item.peso-liquido
                   tt-movto.class-fisc      = int-ds-nota-saida-item.nsp-ncm-s.
            ...
            if tt-movto.cd-trib-pis = 1 /* tributado */ then do:
                ...
                if tt-movto.aliquota-pis <> 0 then do:
                    tt-movto.base-pis           = int-ds-nota-saida-item.nsp-valorbruto-n
                                                - int-ds-nota-saida-item.nsp-desconto-n
                                                + int-ds-nota-saida-item.nsp-valordespesa-n.
            ...
            if tt-movto.cd-trib-cofins = 1 /* tributado */ then do:
                ...
                if tt-movto.aliquota-cofins <> 0 then do:
                    tt-movto.base-cofins        = int-ds-nota-saida-item.nsp-valorbruto-n 
                                                - int-ds-nota-saida-item.nsp-desconto-n
                                                + int-ds-nota-saida-item.nsp-valordespesa-n.
        ...
        if can-find(first tt-movto) then do:
            ...
            run pi-gera-log (input "Documento n∆o possui Itens. Docto: " + string(int-ds-nota-entrada.nen-notafiscal-n) + " SÇrie: " + string(int-ds-nota-entrada.nen-serie-s) + " Estab.: " + c-cod-estabel,
                             input 1).
        end.
                ...
                if avail tt-erro then do:
                   ...
                   for each tt-erro:
                       ...
                       IF tt-erro.cd-erro = 1878 THEN DO:
                          ASSIGN i-sit-reg = 2.
                          for each bint-ds-nota-entrada where
                                   bint-ds-nota-entrada.nen-cnpj-origem-s  = tt-movto.nen-cnpj-origem-s and
                                   bint-ds-nota-entrada.nen-serie-s        = tt-movto.serie and
                                   bint-ds-nota-entrada.nen-notafiscal-n   = integer(tt-movto.nr-nota-fis):
                              assign bint-ds-nota-entrada.nen-conferida-n = 2 
                                     bint-ds-nota-entrada.situacao        = 2 /* processada */.
                              release bint-ds-nota-entrada.
                          end.
