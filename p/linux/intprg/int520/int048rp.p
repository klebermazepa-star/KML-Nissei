...
procedure pi-importa-tudo:
    run pi-acompanhar in h-acomp (input "Lendo Todos os Pedidos tipo 48").
    
    assign i-nr-registro    = 0.

    for each int-ds-pedido-subs no-lock
       WHERE (int-ds-pedido-subs.situacao = 1 OR int-ds-pedido-subs.situacao = 3)
         and int-ds-pedido-subs.ped-tipopedido-n = 48 /* SUBSTITUICAO DE CUPOM */

        /*AND ped-codigo-n = 57499008*/
