...
*******************************************************************************/
{include/i-prgvrs.i INT510 1.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button btRecusa bt-hist btReavliar ~
btAprovar btWMS btWMSMovto btCancelar btIntegra 
...
DEFINE FRAME f-cad
     ...
     bt-hist AT ROW 1.13 COL 46.71 HELP
          "Hist¢rico Recusa/Aceite NF" WIDGET-ID 18
     btReavliar AT ROW 1.17 COL 54.29 WIDGET-ID 8
     btAprovar AT ROW 1.17 COL 66 WIDGET-ID 10
     btWMS AT ROW 1.17 COL 77.72 WIDGET-ID 6
     btWMSMovto AT ROW 1.17 COL 89.29 WIDGET-ID 12
     btCancelar AT ROW 1.17 COL 101 WIDGET-ID 14
     btIntegra AT ROW 1.17 COL 112.72 WIDGET-ID 2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btAprovar w-cadpaifilho-ambos
ON CHOOSE OF btAprovar IN FRAME f-cad /* Compras */
DO:
    RUN pi-aprovar-compras in h_int510-viewer.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btCancelar w-cadpaifilho-ambos
ON CHOOSE OF btCancelar IN FRAME f-cad /* Cancela Movto */
DO:
...
ON CHOOSE OF btWMS IN FRAME f-cad /* Conf. WMS */
DO:
...
&Scoped-define SELF-NAME btWMSMovto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btWMSMovto w-cadpaifilho-ambos
ON CHOOSE OF btWMSMovto IN FRAME f-cad /* Movto PRS */
DO:
    RUN pi-liberar-wms in h_int510-viewer (30).
    /*
    if valid-handle (h_int510-viewer) then
        RUN dispatch IN h_int510-viewer ('row-available':U).
    if valid-handle (h_int510-viewer-totais) then
...
------------------------------------------------------------------------------*/
  ENABLE rt-button btRecusa bt-hist btReavliar btAprovar btWMS btWMSMovto 
         btCancelar btIntegra 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT510" "1.12.01.AVB"}
  ...
  if can-find (first usuar_grp_usuar no-lock where 
      ...
      assign btCancelar :sensitive in FRAME {&FRAME-NAME} = yes.
      ...
      if can-find (first usuar_grp_usuar no-lock where 
          ...
          assign btAprovar  :sensitive in FRAME {&FRAME-NAME}   = yes
                 ...
                 btCancelar :sensitive in FRAME {&FRAME-NAME} = no
      ...
      /* demais pode alterar documento, n∆o pode aprovar */
         ...
         assign btCancelar :sensitive in FRAME {&FRAME-NAME} = yes /*can-find (first usuar_grp_usuar no-lock where 
                                                                 usuar_grp_usuar.cod_usuario = c-seg-usuario and
