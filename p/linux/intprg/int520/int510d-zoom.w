...
*******************************************************************************/
{include/i-prgvrs.i Z01Docto-XML 1.12.01.AVB}
...
DEFINE VARIABLE h_int510d-browser-zoom AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-ok AT ROW 16.96 COL 3
     bt-cancela AT ROW 16.96 COL 14
     bt-ajuda AT ROW 17 COL 104.43
     rt-button AT ROW 16.75 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 117.25 BY 17.2
         CANCEL-BUTTON bt-cancela WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Nota' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.54 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 14.92 , 117.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-browser-zoom.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int510d-browser-zoom ).
       RUN set-position IN h_int510d-browser-zoom ( 3.42 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 114.00 ) */

       /* Links to SmartBrowser h_int510d-browser-zoom. */
       RUN add-link IN adm-broker-hdl ( h_int510d-browser-zoom , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-browser-zoom ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01Docto-XML" "1.12.01.AVB"}
