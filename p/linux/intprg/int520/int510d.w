...
*******************************************************************************/
{include/i-prgvrs.i INT510d 1.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button btAceita bt-hist 
...
DEFINE VARIABLE h_int510d-browser-duplicatas-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int510d-browser-erros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int510d-browser-itens-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int510d-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int510d-viewer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_int510d-viewer-totais AS HANDLE NO-UNDO.
...
DEFINE BUTTON btAceita 
     IMAGE-UP FILE "image\im-undo":U
     IMAGE-INSENSITIVE FILE "image\ii-undo":U
     LABEL "Recusa" 
     SIZE 4 BY 1.25 TOOLTIP "Aceita recebimento da nota fiscal"
     FONT 4.
...
DEFINE FRAME f-cad
     btAceita AT ROW 1.13 COL 42.29 HELP
          "Aceita recebimento da nota fiscal" WIDGET-ID 16
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Notas Recebimento"
         HEIGHT             = 26.83
         WIDTH              = 144.29
         MAX-HEIGHT         = 27.5
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manuten‡Æo Notas Recebimento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manuten‡Æo Notas Recebimento */
DO:
...
do:    
    RUN pi-historico in h_int510d-viewer.
end.
...
&Scoped-define SELF-NAME btAceita
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btAceita w-cadpaifilho-ambos
ON CHOOSE OF btAceita IN FRAME f-cad /* Recusa */
do:    
    RUN pi-aceita-recebimento in h_int510d-viewer.

    /* verificar situacao do documento ....*/
    RUN dispatch IN h_int510d-query ('open-query-cases':U).
    RUN dispatch IN h_int510d-query ('GET-FIRST':U).
end.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int510d-viewer ).
       RUN set-position IN h_int510d-viewer ( 2.42 , 1.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/int510d-zoom.w,
                     ProgVaPara = intprg/int510d-vapara.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_int510d-query ).
       RUN set-position IN h_int510d-query ( 13.25 , 137.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.42 , 6.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_int510d-viewer. */
       RUN add-link IN adm-broker-hdl ( h_int510d-query , 'Record':U , h_int510d-viewer ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_int510d-viewer ).
       RUN add-link IN adm-broker-hdl ( h_int510d-viewer , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_int510d-query. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_int510d-query ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_int510d-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_int510d-query ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_int510d-query ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             btAceita:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadpai ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             bt-hist:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-viewer ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_int510d-viewer , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-browser-itens.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int510d1.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int510d-browser-itens-2 ).
       RUN set-position IN h_int510d-browser-itens-2 ( 16.75 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 142.57 ) */

       /* Links to BrowserCadastro2 h_int510d-browser-itens-2. */
       RUN add-link IN adm-broker-hdl ( h_int510d-query , 'Record':U , h_int510d-browser-itens-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-browser-itens-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-browser-duplicatas.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = intprg/int510d2.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_int510d-browser-duplicatas-2 ).
       RUN set-position IN h_int510d-browser-duplicatas-2 ( 17.04 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.25 , 66.00 ) */

       /* Links to BrowserCadastro2 h_int510d-browser-duplicatas-2. */
       RUN add-link IN adm-broker-hdl ( h_int510d-query , 'Record':U , h_int510d-browser-duplicatas-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-browser-duplicatas-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-viewer-totais.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int510d-viewer-totais ).
       RUN set-position IN h_int510d-viewer-totais ( 16.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.21 , 141.57 ) */

       /* Links to SmartViewer h_int510d-viewer-totais. */
       RUN add-link IN adm-broker-hdl ( h_int510d-query , 'Record':U , h_int510d-viewer-totais ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-viewer-totais ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/int510d-browser-erros.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_int510d-browser-erros ).
       RUN set-position IN h_int510d-browser-erros ( 16.42 , 2.72 ) NO-ERROR.
       /* Size in UIB:  ( 11.25 , 141.00 ) */

       /* Links to BrowserCadastro2 h_int510d-browser-erros. */
       RUN add-link IN adm-broker-hdl ( h_int510d-query , 'Record':U , h_int510d-browser-erros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_int510d-browser-erros ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
...
------------------------------------------------------------------------------*/
  ENABLE rt-button btAceita bt-hist 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT510d" "1.12.01.AVB"}
  ...
  ASSIGN btAceita:SENSITIVE IN FRAME {&FRAME-NAME} = YES.

  RUN enable-copia     in h_p-cadpai (no).
  RUN enable-inclui    in h_p-cadpai (no).
  RUN enable-modifica  in h_p-cadpai (no).
  RUN enable-elimina   in h_p-cadpai (no).
  RUN pi-recebe-handle in h_int510d-viewer(h_int510d-browser-itens-2).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    RUN pi-avaliar-erros in h_int510d-viewer.    
    if valid-handle (h_int510d-viewer) then
        RUN dispatch IN h_int510d-viewer ('row-available':U).
    if valid-handle (h_int510d-viewer-totais) then
        RUN dispatch IN h_int510d-viewer-totais ('row-available':U).
    if valid-handle (h_int510d-browser-itens-2) then
        RUN dispatch IN h_int510d-browser-itens-2 ('row-available':U).
    if valid-handle (h_int510d-browser-erros) then
        RUN dispatch IN h_int510d-browser-erros ('row-available':U).
    if valid-handle (h_int510d-browser-itens-2) then
        RUN pi-refresh IN h_int510d-browser-itens-2.
