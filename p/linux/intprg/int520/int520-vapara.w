...
*******************************************************************************/
{include/i-prgvrs.i INT520-vapara 1.12.01.AVB}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_nen-chaveacesso-s ~
c_nen-cnpj-origem-s c_nen-serie-s i_nen-notafiscal-n bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_nen-chaveacesso-s c_nen-cnpj-origem-s ~
c_nen-serie-s i_nen-notafiscal-n 
...
DEFINE VARIABLE c_nen-chaveacesso-s AS CHARACTER FORMAT "x(44)":U 
     LABEL "Chave NFe" 
     VIEW-AS FILL-IN 
     SIZE 47 BY .87 NO-UNDO.

DEFINE VARIABLE c_nen-cnpj-origem-s AS CHARACTER FORMAT "x(14)":U 
     LABEL "CNPJ Origem" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .87 NO-UNDO.

DEFINE VARIABLE c_nen-serie-s AS CHARACTER FORMAT "x(3)":U 
     LABEL "Serie" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .87 NO-UNDO.

DEFINE VARIABLE i_nen-notafiscal-n AS INTEGER FORMAT ">>>,>>>,>>9":U 
     LABEL "Nr. Nota" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_nen-chaveacesso-s AT ROW 1.27 COL 13.5 COLON-ALIGNED WIDGET-ID 2
     c_nen-cnpj-origem-s AT ROW 2.33 COL 13.5 COLON-ALIGNED WIDGET-ID 4
     c_nen-serie-s AT ROW 3.4 COL 13.5 COLON-ALIGNED WIDGET-ID 6
     i_nen-notafiscal-n AT ROW 4.47 COL 13.5 COLON-ALIGNED WIDGET-ID 8
     bt-ok AT ROW 6.57 COL 2.13
     bt-cancela AT ROW 6.57 COL 13
     bt-ajuda AT ROW 6.57 COL 58
     rt-button AT ROW 6.33 COL 1
     SPACE(0.12) SKIP(0.33)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "V  Para Nota Fiscal"
...
ON GO OF FRAME d-vapara /* V  Para Nota Fiscal */
DO:
    if c_nen-chaveacesso-s:screen-value in frame {&FRAME-NAME}  <> "" then do:
        for first int-ds-nota-entrada no-lock where 
            int-ds-nota-entrada.nen-chaveacesso-s = INPUT FRAME {&FRAME-NAME} c_nen-chaveacesso-s: end.
        IF NOT AVAILABLE int-ds-nota-entrada THEN DO:
            {utp/ut-table.i emsesp int-ds-nota-entrada 1}
        ...
        for first int-ds-nota-entrada no-lock where 
            int-ds-nota-entrada.nen-cnpj-origem-s EQ INPUT FRAME {&FRAME-NAME} c_nen-cnpj-origem-s AND 
            int-ds-nota-entrada.nen-serie-s EQ INPUT FRAME {&FRAME-NAME} c_nen-serie-s AND 
            int-ds-nota-entrada.nen-notafiscal-n EQ INPUT FRAME {&FRAME-NAME} i_nen-notafiscal-n: end.
    ...
    ASSIGN p-row-tabela = ROWID(int-ds-nota-entrada).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Nota Fiscal */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_nen-chaveacesso-s c_nen-cnpj-origem-s c_nen-serie-s 
          i_nen-notafiscal-n 
      WITH FRAME d-vapara.
  ENABLE rt-button c_nen-chaveacesso-s c_nen-cnpj-origem-s c_nen-serie-s 
         i_nen-notafiscal-n bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT520-vapara" "1.12.01.AVB"}
