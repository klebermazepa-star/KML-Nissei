...
c-Mensagem bt-totais bt-un bt-ncm bt-natureza bt-icms 
...
DEFINE FRAME f-main
     ...
     bt-totais AT ROW 12.13 COL 62.14 WIDGET-ID 154
     bt-un AT ROW 12.13 COL 82 WIDGET-ID 206
     bt-ncm AT ROW 12.13 COL 95.86 WIDGET-ID 204
     bt-natureza AT ROW 12.13 COL 108.86 WIDGET-ID 200
     bt-icms AT ROW 12.13 COL 127 WIDGET-ID 202
...
DO:
    for first modalid-frete no-lock where 
...
------------------------------------------------------------------------------*/
    ...
    if int_ds_docto_xml.situacao <> 3 and 
       int_ds_docto_xml.situacao <> 9 then
        for first b-int_ds_docto_xml exclusive-lock where 
            rowid (b-int_ds_docto_xml) = rowid (int_ds_docto_xml):
            if c-mensagem:screen-value in frame f-Main <> "" then 
...
------------------------------------------------------------------------------*/
   ...
   for each int_ds_it_docto_xml exclusive-lock where
       ...
       c-nat-operacao = "".
       for first item fields (it-codigo fm-codigo) no-lock where 
           item.it-codigo = int_ds_it_docto_xml.it_codigo: 
           /* tratar natur-oper */
