...
DEFINE VARIABLE h_INT540-browser-zoom AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT540-browser-zoom.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT540-browser-zoom ).
       RUN set-position IN h_INT540-browser-zoom ( 3.42 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 114.00 ) */

       /* Links to SmartBrowser h_INT540-browser-zoom. */
       RUN add-link IN adm-broker-hdl ( h_INT540-browser-zoom , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT540-browser-zoom ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
