...
*******************************************************************************/
{include/i-prgvrs.i INT540c 1.12.01.AVB}
...
DEFINE VARIABLE h_INT540c-query AS HANDLE NO-UNDO.
DEFINE VARIABLE h_INT540c-viewer AS HANDLE NO-UNDO.
...
DO:
{include/okfil.i h_INT540c-viewer}
END.
...
DO:
{include/salvafil.i h_INT540c-viewer}
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT540c-viewer.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_INT540c-viewer ).
       RUN set-position IN h_INT540c-viewer ( 3.13 , 14.50 ) NO-ERROR.
       /* Size in UIB:  ( 4.53 , 49.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/INT540c-query.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_INT540c-query ).
       RUN set-position IN h_INT540c-query ( 1.00 , 74.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.40 , 6.75 ) */

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_INT540c-viewer. */
       RUN add-link IN adm-broker-hdl ( h_INT540c-query , 'Record':U , h_INT540c-viewer ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_INT540c-viewer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-ok:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_INT540c-viewer ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "INT540c" "1.12.01.AVB"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_INT540c-viewer(input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_INT540c-query (input v-row-table).
