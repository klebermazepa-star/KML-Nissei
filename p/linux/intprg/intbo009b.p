...
&GLOBAL-DEFINE TableName int-ds-cenar-estab
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-cenario LIKE int-ds-cenar-estab.cod-cenario NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-estab   LIKE int-ds-cenar-estab.cod-estab   NO-UNDO.
    ...
    ASSIGN pcod-cenario = RowObject.cod-cenario
           pcod-estab   = RowObject.cod-estab.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE input PARAMETER pcod-cenario LIKE int-ds-cenar-estab.cod-cenario NO-UNDO.
    DEFINE input PARAMETER pcod-estab   LIKE int-ds-cenar-estab.cod-estab   NO-UNDO.
    
    FIND FIRST bfint-ds-cenar-estab WHERE 
               bfint-ds-cenar-estab.cod-cenario = pcod-cenario AND
               bfint-ds-cenar-estab.cod-estab   = pcod-estab NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-ds-cenar-estab THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-ds-cenar-estab)).
  ...
  /*:T--- Utilize o parÉmetro pType para identificar quais as validaá‰es a serem
    ...
    IF pType = "create" THEN DO:
        FIND FIRST int-ds-cenar-estab NO-LOCK WHERE 
                   int-ds-cenar-estab.cod-cenario = RowObject.cod-cenario AND 
                   int-ds-cenar-estab.cod-estab   = RowObject.cod-estab NO-ERROR.
        IF AVAIL int-ds-cenar-estab THEN DO:
               {method/svc/errors/inserr.i
                        &ErrorNumber="17006"
                        &ErrorType="EMS"
                        &ErrorSubType="ERROR" 
                        &ErrorParameters="'Estabelecimento j† cadastrado para o cen†rio '"}
        END.
