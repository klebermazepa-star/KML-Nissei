...
{include/i-prgvrs.i NI0105 2.00.00.000}
...
DEFINE VARIABLE h_Q01IN047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_V02IN047 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     rt-button AT ROW 1.17 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Integradores Executados via RPW"
         HEIGHT             = 6.08
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Integradores Executados via RPW */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Integradores Executados via RPW */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navega ).
       RUN set-position IN h_p-navega ( 1.29 , 2.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-cadsim ).
       RUN set-position IN h_p-cadsim ( 1.29 , 27.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.29 , 73.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/ni0105-v1.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_V02IN047 ).
       RUN set-position IN h_V02IN047 ( 2.71 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 4.17 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'intprg/ni0105-q1.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = intprg/ni0105-z1.w,
                     ProgVaPara = intprg/ni0105-g1.w,
                     ProgIncMod = ':U ,
             OUTPUT h_Q01IN047 ).
       RUN set-position IN h_Q01IN047 ( 1.00 , 64.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 8.29 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_V02IN047. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_V02IN047 ).
       RUN add-link IN adm-broker-hdl ( h_Q01IN047 , 'Record':U , h_V02IN047 ).

       /* Links to SmartQuery h_Q01IN047. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_Q01IN047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_Q01IN047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_Q01IN047 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_V02IN047 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  /* Dispatch standard ADM method.                             */
  {utp/ut9000.i "NI0105" "2.00.00.000"}
