...
OR tt-param.estab-fim >= "973" THEN DO:
   FOR EACH int_ds_docto_xml WHERE
            ...
            int_ds_docto_xml.demi     >= tt-param.emiss-ini AND
            int_ds_docto_xml.demi     <= tt-param.emiss-fim NO-LOCK QUERY-TUNING(NO-LOOKAHEAD):

       FOR FIRST emitente WHERE 
                 emitente.cgc = int_ds_docto_xml.cnpj NO-LOCK QUERY-TUNING(NO-LOOKAHEAD):
       END.
       ...
       ASSIGN l-re2001     = NO
              ...
              i-sit-re1001 = 1
              i-sit-re2001 = 1
              /*da-dt-trans  = int_ds_docto_xml.dt-trans*/
              da-dt-trans  = ?.
       ...
       FOR FIRST int_ds_docto_wms WHERE
           ...
           da-dt-conf   = int_ds_docto_wms.datamovimentacao_d.
       END.

       FOR FIRST doc-fisico WHERE 
                 int(doc-fisico.nro-docto)   = int(int_ds_docto_xml.nNF)  AND  
       ...
       FOR FIRST docum-est WHERE
                 int(docum-est.nro-docto)   = int(int_ds_docto_xml.nNF)     AND 
       ...
       IF AVAIL docum-est THEN DO:
          ...
          FOR EACH dupli-apagar OF docum-est WHERE 
              ...
              /*FOR FIRST tit_ap WHERE 
                        tit_ap.cod_estab       = dupli-apagar.cod-estabel  AND
                        tit_ap.cdn_fornecedor  = dupli-apagar.cod-emitente AND
                        tit_ap.cod_espec_docto = dupli-apagar.cod-esp      AND
                        tit_ap.cod_ser_docto   = dupli-apagar.serie-docto  AND
                        tit_ap.cod_tit_ap      = dupli-apagar.nro-docto    AND
                        tit_ap.cod_parcela     = dupli-apagar.parcela      NO-LOCK QUERY-TUNING(NO-LOOKAHEAD):
              END.
...
FOR EACH int_ds_nota_entrada WHERE
         int_ds_nota_entrada.nen_cnpj_destino_s <> "79430682025540"   AND 
         int_ds_nota_entrada.nen_dataemissao_d  >= tt-param.emiss-ini AND
         int_ds_nota_entrada.nen_dataemissao_d  <= tt-param.emiss-fim NO-LOCK QUERY-TUNING(NO-LOOKAHEAD):
    ...
    ASSIGN l-re2001     = NO
           ...
           i-sit-re1001 = 1
           i-sit-re2001 = 1
           /*da-dt-trans  = int_ds_nota_entrada.nen-datamovimentacao-d*/
           da-dt-trans  = ?
           da-dt-conf   = IF int_ds_nota_entrada.nen_conferida_n >= 1 
                          THEN int_ds_nota_entrada.nen_datamovimentacao_d ELSE ?.

    FOR FIRST doc-fisico WHERE 
              int(doc-fisico.nro-docto)   = int(int_ds_nota_entrada.nen_notafiscal_n) AND  
    ...
    FOR FIRST docum-est WHERE
              int(docum-est.nro-docto)   = int(int_ds_nota_entrada.nen_notafiscal_n) AND 
    ...
    IF AVAIL docum-est THEN DO:
       ...
       FOR EACH dupli-apagar OF docum-est WHERE 
           ...
           ASSIGN tt-notas.cod-estabel    = estabelec.cod-estabel
                  ...
                  tt-notas.sit-re1001     = IF i-sit-re1001 = 1 THEN "NÆo Atualizado" ELSE IF i-sit-re1001 = 2 THEN "Atualizado" ELSE ""
                  tt-notas.confer         = YES
                  tt-notas.int520         = IF int_ds_nota_entrada.situacao = 5 THEN YES ELSE NO
                  tt-notas.financeiro     = YES
                  tt-notas.dt-trans       = da-dt-trans
                  tt-notas.dt-conf        = da-dt-conf
                  l-duplic                = YES.
...
FOR EACH tt-notas USE-INDEX codigo NO-LOCK 
    BREAK BY tt-notas.cod-estabel 
          ...
          BY tt-notas.serie       
          BY tt-notas.nr-nota QUERY-TUNING(NO-LOOKAHEAD):
    ...
    DISP tt-notas.cod-estabel    COLUMN-LABEL "Estab"           WHEN FIRST-OF(tt-notas.nr-nota)
         ...
         tt-notas.financeiro     COLUMN-LABEL "Financ"          WHEN FIRST-OF(tt-notas.nr-nota)
