...
&Scoped-Define ENABLED-OBJECTS rt-button i_num-vale bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_num-vale 
...
DEFINE VARIABLE i_num-vale AS INTEGER FORMAT ">>>>>>>>>9":U 
     LABEL "Vale" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_num-vale AT ROW 1.75 COL 27 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Vale Manual */
DO:
FIND vale-manual NO-LOCK
    WHERE vale-manual.num-vale EQ INPUT FRAME {&FRAME-NAME} i_num-vale NO-ERROR.

IF NOT AVAILABLE vale-manual THEN DO:
    {utp/ut-table.i mgesp vale-manual 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(vale-manual).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Vale Manual */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_num-vale 
      WITH FRAME d-vapara.
  ENABLE rt-button i_num-vale bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
