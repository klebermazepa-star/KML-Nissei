...
fi-matricula fi-nome-func fi-num-vale 
...
DEFINE VARIABLE fi-num-vale AS INTEGER FORMAT ">>>>>>>>>9":U INITIAL 0 
     LABEL "Vale" 
     VIEW-AS FILL-IN 
     SIZE 16 BY .88 NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     fi-num-vale AT ROW 3.46 COL 28 COLON-ALIGNED WIDGET-ID 10
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME L-To-R                                                    */
/* BROWSE-TAB br-titulo fi-num-vale D-Dialog */
ASSIGN 
...
/* SETTINGS FOR FILL-IN fi-nome-func IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-num-vale IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
  ...
  IF AVAIL vale-manual THEN DO:
      ...
      FOR EACH movto-vale-manual
         WHERE movto-vale-manual.num-vale = vale-manual.num-vale:
          DELETE movto-vale-manual.
      END.

      ASSIGN iSeq = 1.
      FOR EACH tt-titulo-movto-aux:

          CREATE movto-vale-manual.
          ASSIGN movto-vale-manual.num-vale       = vale-manual.num-vale
                 movto-vale-manual.seq-movto      = iSeq
                 movto-vale-manual.val-movto      = tt-titulo-movto-aux.vl-movto
                 movto-vale-manual.num-id-tit-acr = tt-titulo-movto-aux.num_id_tit_acr
              .
          ASSIGN iSeq = iSeq + 1.
      END.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-cod-empresa fi-nome-empresa fi-matricula fi-nome-func fi-num-vale 
      WITH FRAME D-Dialog.
...
FIND FIRST vale-manual NO-LOCK
     WHERE ROWID(vale-manual) = gr-vale-manual NO-ERROR.
IF AVAIL vale-manual THEN DO:
    ASSIGN fi-matricula:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(vale-manual.mat-funcionario)
           fi-num-vale:SCREEN-VALUE  IN FRAME {&FRAME-NAME} = STRING(vale-manual.num-vale).
    ...
    FOR EACH movto-vale-manual
       WHERE movto-vale-manual.num-vale = vale-manual.num-vale,
       FIRST tit_acr
       WHERE tit_acr.num_id_tit_acr = movto-vale-manual.num-id-tit-acr NO-LOCK:

        CREATE tt-titulo-movto-aux.
        BUFFER-COPY tit_acr TO tt-titulo-movto-aux.

        ASSIGN tt-titulo-movto-aux.vl-movto = movto-vale-manual.val-movto
