...
DEF TEMP-TABLE tt-excel 
    ...
    FIELD medio-31-07  AS DEC FORMAT "->>>,>>9.9999"
    FIELD medio-31-08  AS DEC FORMAT "->>>,>>9.9999"
...
ASSIGN chExcelApplication:SheetsInNewWorkbook           = 1
       ...
       ch-planilha:Name                                 = "Custos dos Itens"
       ch-planilha:range("A1:P1"):MergeCells            = TRUE
       ch-planilha:range("A1"):VALUE                    = "Custos dos Itens - EmissÆo: " + STRING(TODAY,"99/99/9999")
       ch-planilha:range("A1:P1"):FONT:bold             = TRUE
       ch-planilha:range("A1:P1"):FONT:SIZE             = 22
       ch-planilha:range("A1:P1"):Interior:ColorIndex   = 15
       ch-planilha:range("A1:P1"):HorizontalAlignment   = 3
       ch-planilha:range("A1:P1"):Borders(7):LineStyle  = 1
       ch-planilha:range("A1:P1"):Borders(7):Weight     = 3
       ch-planilha:range("A1:P1"):Borders(8):LineStyle  = 1
       ch-planilha:range("A1:P1"):Borders(8):Weight     = 3
       ch-planilha:range("A1:P1"):Borders(9):LineStyle  = 1
       ch-planilha:range("A1:P1"):Borders(9):Weight     = 3
       ch-planilha:range("A1:P1"):Borders(10):LineStyle = 1
       ch-planilha:range("A1:P1"):Borders(10):Weight    = 3.
...
for each ITEM where
    ...
    for each pr-it-per where 
        ...
        IF pr-it-per.periodo = 07/31/2016 THEN
           ASSIGN de-medio-31-07 = pr-it-per.val-unit-mat-m[1] + pr-it-per.val-unit-mob-m[1] + pr-it-per.val-unit-ggf-m[1].
        
        IF pr-it-per.periodo = 08/31/2016 THEN
           ASSIGN de-medio-31-08 = pr-it-per.val-unit-mat-m[1] + pr-it-per.val-unit-mob-m[1] + pr-it-per.val-unit-ggf-m[1].
    end.
   ...
   ASSIGN tt-excel.medio-31-07 = de-medio-31-07
          tt-excel.medio-31-08 = de-medio-31-08
...
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):FONT:bold = TRUE.
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):Interior:ColorIndex  = 15.
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):Borders(8):LineStyle = 1.
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):Borders(8):Weight    = 3.
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):Borders(9):LineStyle = 1.
ch-planilha:range("A" + STRING(i-linha) + ":" + "P" + STRING(i-linha)):Borders(9):Weight    = 3.
...
FOR EACH tt-excel USE-INDEX ITEM: 
    ...
    ch-planilha:range("A" + STRING(i-linha)):VALUE = tt-excel.cod-estabel.
    /*ch-planilha:range("B" + STRING(i-linha)):NumberFormat = "@".*/
    ch-planilha:range("B" + STRING(i-linha)):VALUE = tt-excel.it-codigo.
    ch-planilha:range("C" + STRING(i-linha)):VALUE = tt-excel.desc-item.                          
    ch-planilha:range("D" + STRING(i-linha)):VALUE = tt-excel.un.         
    ch-planilha:range("E" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".  
    ch-planilha:range("E" + STRING(i-linha)):VALUE = tt-excel.preco-ul-ent.
    ch-planilha:range("F" + STRING(i-linha)):VALUE = tt-excel.data-ult-ent.                             
    ch-planilha:range("G" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".                               
    ch-planilha:range("G" + STRING(i-linha)):VALUE = tt-excel.preco-base. 
    ch-planilha:range("H" + STRING(i-linha)):VALUE = tt-excel.data-base.
    ch-planilha:range("I" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".                               
    ch-planilha:range("I" + STRING(i-linha)):VALUE = tt-excel.medio-31-07. 
    ch-planilha:range("J" + STRING(i-linha)):NumberFormat = "###.###.##0,0000".                               
    ch-planilha:range("J" + STRING(i-linha)):VALUE = tt-excel.medio-31-08.  
    ch-planilha:range("K" + STRING(i-linha)):VALUE = tt-excel.tp-controle.                
    ch-planilha:range("L" + STRING(i-linha)):VALUE = tt-excel.obsoleto.                 
    ch-planilha:range("M" + STRING(i-linha)):VALUE = tt-excel.ge-codigo.                        
    ch-planilha:range("N" + STRING(i-linha)):VALUE = tt-excel.desc-ge.                  
    ch-planilha:range("O" + STRING(i-linha)):VALUE = tt-excel.fm-codigo.                         
    ch-planilha:range("P" + STRING(i-linha)):VALUE = tt-excel.desc-fm.   
END.                                                                                                                
...
ch-planilha:SELECT().
chExcelApplication:COLUMNS("A:AD"):AutoFit.
