...
then do:
    ...
    do:
        ...
        */
        ...
        for each ems2log.portador no-lock where
            ems2log.portador.ep-codigo = i-ep-codigo-usuario:
            h-cod-portador:add-last(string(ems2log.portador.cod-portador,">>>>9") + " / " + 
                                    trim({adinc/i03ad209.i 04 ems2log.portador.modalidade}) + " / " +
                                    trim(ems2log.portador.nome)).
        end.
       ...
       valid-handle (h-cod-portador) then do:
        ...
        for first cst_cond_pagto where 
            ...
            if cst_cond_pagto.cod_portador <> 0 then do:
                for first ems2log.portador fields (nome) where 
                    ems2log.portador.ep-codigo = i-ep-codigo-usuario and
                    ems2log.portador.cod-portador = cst_cond_pagto.cod_portador and
                    ems2log.portador.modalidade = cst_cond_pagto.modalidade:
                    /*assign h-nom-portador:screen-value = ems2log.portador.nome.*/
                    assign h-cod-portador:screen-value = string(cst_cond_pagto.cod_portador) 
                                               + " / " + {adinc/i03ad209.i 04 cst_cond_pagto.modalidade} 
                                               + " / " + ems2log.portador.nome.
                end.
            end.
            else
