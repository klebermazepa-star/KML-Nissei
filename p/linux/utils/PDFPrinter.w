...
function getPathNameAcroRd32 return character():
    define variable acrobatReaderApplication_ as character no-undo.

    load "AcroExch.Document" base-key "HKEY_CLASSES_ROOT".
    use "AcroExch.Document".
    get-key-value section 'Shell\Open\Command' key default value acrobatReaderApplication_.
    unload "AcroExch.Document".

    acrobatReaderApplication_ = trim(entry(1,replace(acrobatReaderApplication_,'"',''),'%')).

    return acrobatReaderApplication_.
end function.
...
DEFINE FRAME DDEFrame
     "     Aguarde... Imprimindo arquivo PDF..." VIEW-AS TEXT
          SIZE 52 BY 1.25 AT ROW 1 COL 1 WIDGET-ID 2
...
ON END-ERROR OF C-Win /* PDFPrinter */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* PDFPrinter */
DO:
...
------------------------------------------------------------------------------*/
    ...
    if viSys = 0 then
        dde initiate viSys frame frame DDEFrame:handle 
            application "AcroRd32" topic "Control" no-error. 
