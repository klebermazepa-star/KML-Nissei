...
def image im_msg_ico     file "image/im-mqqst".
...
def button bt_cancel     label "Cancelar" size-char 10 by 1 auto-go.
...
def frame f_msg_help
    ...
    bt_cancel    at row 7.71 col 25
    skip(0.5)
    with Title "Mensagem de Confirma‡Æo" three-d no-label view-as dialog-box.
 
on  cursor-right of
 bt_ok, bt_no, bt_cancel    apply "TAB" to self.
on  cursor-left of
 bt_ok, bt_no, bt_cancel    apply "SHIFT-TAB" to self.
 
on  choose of bt_ok
    return "yes".
on  choose of bt_no
    return "no".
on  choose of bt_cancel
    return "cancel".

assign l-question = yes.
...
if  l-question then
    assign bt_no:hidden in frame f_msg_help = no
           ...
           bt_cancel:sensitive in frame f_msg_help = yes.
else
...
if  session:batch-mode = no then do:
   do  on endkey undo, retry with frame f_msg_help:
       ...
       update bt_ok bt_no bt_cancel when l-question with keep-tab-order.
   end.
end.
else do:
