...
def new Global shared var c-seg-usuario        as char format "x(12)" no-undo.
   ...
   if avail tt_param_program_formul then do:
       case INT(tt_param_program_formul.ttv_des_contdo) :
           when 2 /*l_doc*/  then
               run piRetornaValorTaxaOperacao.
...
PROCEDURE piRetornaValorTaxaOperacao:
    DEFINE VAR vValorTaxaOperacao AS CHAR NO-UNDO.
    ...
    if avail bord_ap then do:
        assign vValorTaxaOperacao = STRING(bord_ap.val_livre_1 * 10000000,"9999999999").

        return vValorTaxaOperacao.
    end.
    else do:
        return "0".
    end.
