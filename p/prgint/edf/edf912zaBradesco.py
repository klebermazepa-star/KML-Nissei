...
** Nome Externo..........: prgint/edf/edf912zaBradesco.py
...
** Altera‡Æo por ........: Gesto
** VersÆo ...............: 004
** Alterado em...........: 06/03/2020 09:44:17
** Altera‡Æo por ........: Gesto
** VersÆo ...............: 005
** Alterado em...........: 27/04/2020 14:00:00
*****************************************************************************/
...
PROCEDURE pi_extracao_detalhe_edf912za:
        ...
        def var i_num_docto                      as decimal         no-undo. /* altera‡Æo 004 */
        def var v_num_docto                      as character       no-undo. /* altera‡Æo 004 */
		def var v_serie_docto                    as character       no-undo. /* altera‡Æo 004 */
		def var v_dt_emissao                     as character       no-undo. /* altera‡Æo 004 */
...
/*            output stream s3 to value( SESSION:TEMP-DIRECTORY + "/pge_bradesco.txt" ) no-convert append.
...
/*            put stream s3 unformatted 
                "Elemento 3743" substring(tt_param_program_formul.ttv_des_contdo,1,20)
                skip. */
...
/*            put stream s3 unformatted 
                "Elemento 3928" substring(tt_param_program_formul.ttv_des_contdo,1,20)
                skip. */


            /* --- Numero do Titulo/Docto ---*/
            find tt_param_program_formul
                where tt_param_program_formul.tta_cdn_segment_edi = 289
                and   tt_param_program_formul.tta_cdn_element_edi = 75 no-error.
			assign i_num_docto = decimal( substring( tt_param_program_formul.ttv_des_contdo,1,10 ) ) no-error.
			if  i_num_docto = ? then assign i_num_docto = 0.
			assign v_num_docto = string( i_num_docto, "999999999999999" ).

/*            put stream s3 unformatted 
                "Elemento 75-" substring(tt_param_program_formul.ttv_des_contdo,1,10) skip
                i_num_docto skip
                v_num_docto skip.
            ...
            find tt_param_program_formul
                where tt_param_program_formul.tta_cdn_segment_edi = 289
                and   tt_param_program_formul.tta_cdn_element_edi = 3704 no-error.
			assign v_dt_emissao = substring( tt_param_program_formul.ttv_des_contdo,1,8 ).

            /* --- Numero da S‚rie ---*/
            find tt_param_program_formul
                where tt_param_program_formul.tta_cdn_segment_edi = 289
                and   tt_param_program_formul.tta_cdn_element_edi = 4689 no-error.
            assign v_serie_docto = trim( substring(tt_param_program_formul.ttv_des_contdo,1,20) ).
            ...
            /* 005 Fixando ocorrenica */
            assign v_cod_ocor_bcia = "09".
           ...
           /* Tratamento Tipo de Pagamento*/
            ...
            then do:        
				...
				/* ------- inicio Altera‡Æo 004 ------- */
				
                /* Segmento 5 - procura sequencia do registro */ 
                find first tt_segment_tot
                     where tt_segment_tot.tta_cdn_segment_edi = 371 no-error.
                ...
                assign  v_cod_det = caps(v_cod_det +
                                         ...
                                         '001' + 
										 string( v_num_docto, 'x(15)' ) + 
										 string( v_serie_docto, 'x(2)' ) + 
