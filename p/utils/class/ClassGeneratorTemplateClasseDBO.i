...
define variable dbo as handle      no-undo.
...
------------------------------------------------------------------------------*/
    do {&throws}:
        run validate('delete':u).

        run emptyRowErrors in dbo.
        run repositionRecord in dbo(tt{&tableName}.r-rowid).
        run deleteRecord in dbo.
...
------------------------------------------------------------------------------*/
    do {&throws}:
        ...
        run emptyRowErrors in dbo.
        run setRecord in dbo(table tt{&tableName}).
        run createRecord in dbo.

        run verifyError.

        run getRecord in dbo(output table tt{&tableName}).
        find first tt{&tableName}.
    end.
...
/*--gen_startup_begin--*/
    run createInstance in ghInstanceManager(this-procedure,
        '&1', output dbo).
    run openQueryStatic in dbo('main':u).
/*--gen_startup_end--*/
...
------------------------------------------------------------------------------*/
    ...
    do {&throws}:
        run validate('update':u).

        run repositionRecord in dbo(tt{&tableName}.r-rowid).
        run setRecord in dbo(table tt{&tableName}).
        run updateRecord in dbo.
...
&IF DEFINED(EXCLUDE-verifyError) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE verifyError Procedure 
PROCEDURE verifyError PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    do {&throws}:
        run getRowErrors in dbo(output table rowErrors).
...
&IF DEFINED(EXCLUDE-verifyRowErrors) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE verifyRowErrors Procedure 
PROCEDURE verifyRowErrors PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
